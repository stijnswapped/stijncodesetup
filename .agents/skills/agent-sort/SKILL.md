---
name: agent-sort
description: Build an evidence-backed SCS install plan for a specific repo by sorting skills, commands, rules, hooks, and extras into DAILY vs LIBRARY buckets using parallel repo-aware review passes. Use when SCS should be trimmed to what a project actually needs instead of loading the full bundle.
---

# Agent Sort

Use this skill when a repo needs a project-specific SCS surface instead of the default full install.

The goal is not to guess what "feels useful." The goal is to classify SCS components with evidence from the actual codebase.

## When to Use

- A project only needs a subset of SCS and full installs are too noisy
...
- Prefer SCS-native surfaces; do not introduce a second install system
