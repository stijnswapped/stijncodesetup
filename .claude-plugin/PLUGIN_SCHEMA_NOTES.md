...
SCS keeps `.mcp.json` at the repository root for Codex plugin installs and manual MCP setup.
...
The Claude plugin slug is intentionally short (`scs`), but this opt-out is still required because legacy installs and strict provider gateways have failed on generated names from longer plugin identifiers.
...
This explicit empty object prevents Claude plugin installs from auto-loading SCS's root MCP definitions.
Without the opt-out, strict OpenAI-compatible gateways can reject plugin MCP tool names such as `mcp__plugin_stijncodesetup_github__create_pull_request_review` because they exceed 64 characters.
