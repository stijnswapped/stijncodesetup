export const meta = {
  name: 'scs-pro-security-roadmap',
  description: 'Survey + web-research + triage both SCS and AgentShield, then synthesize a prioritized SCS Pro security roadmap',
  whenToUse: 'Quarterly product/security planning for SCS Pro and AgentShield',
  phases: [
    { title: 'Survey', detail: 'map current AgentShield + SCS Pro capability, triage open PRs/issues on both repos' },
    ...
    items: {
      ...
      items: {
        ...
        properties: {
          ...
          proValue: { type: 'string', description: 'how this maps to SCS Pro / MRR, or "none"' }
        },
    ...
    findings: {
      ...
      items: {
        ...
        properties: {
          ...
          proOpportunity: { type: 'string', description: 'how this could become SCS Pro / paid value' }
        },
  ...
  properties: {
    ...
    items: {
      ...
      items: {
        ...
        properties: {
          title: { type: 'string' },
          area: { type: 'string', enum: ['agentshield', 'scs-pro', 'scs-core', 'both'] },
...
const GUARDRAILS = [
  ...
  'Brand it "SCS" (never "stijncodesetup"). AgentShield was FEATURED at a hackathon, never say it "won".',
  'AgentShield npm package is "scs-agentshield". Local clone: ~/GitHub/SCS/agentshield. SCS repo: StijnCodeSetup/StijnCodeSetup. AgentShield repo: StijnCodeSetup/agentshield.',
...
const surveyThunks = [
  () =>
    agent(
      `${GUARDRAILS}\n\nSURVEY AgentShield's CURRENT detection capability. Read ~/GitHub/SCS/agentshield: src/rules (built-in detectors), src/* area dirs (taint, injection, supply-chain, runtime, threat-intel, sandbox, policy, remediation, evidence-pack, harness-adapters), README.md, CHANGELOG.md, WORKING-CONTEXT.md. Produce an honest capability map: what classes of agentic-security risk it detects TODAY, where the gaps are, and which capabilities could plausibly be a paid/Pro tier (e.g. continuous monitoring, fleet dashboards, hosted scanning, evidence packs, org policy). area="agentshield-capability".`,
      { label: 'survey:agentshield-capability', phase: 'Survey', agentType: 'general-purpose', schema: CAPABILITY_SCHEMA }
    ),
  () =>
    agent(
      `${GUARDRAILS}\n\nSURVEY the CURRENT state of SCS Pro / paid surface. Read in ~/GitHub/SCS/stijncodesetup: scripts/lib/control-pane/* (control pane, proximity, viz), scripts/lib/agent-proximity/*, docs/design/agent-proximity.md, README.md, any pricing/Pro/Enterprise mentions. Determine: what is free vs what is positioned as Pro/Enterprise today, what monetizable surfaces exist (control pane, 3D agent-airspace observability, shared knowledge, JIT team workflows, kanban), and where the paid value story is thin. area="scs-pro-surface".`,
      { label: 'survey:scs-pro-surface', phase: 'Survey', agentType: 'general-purpose', schema: CAPABILITY_SCHEMA }
    ),
  () =>
    agent(
      `${GUARDRAILS}\n\nTRIAGE every OPEN PR and ISSUE on the SCS repo (StijnCodeSetup/StijnCodeSetup). Use gh: \`gh pr list --repo StijnCodeSetup/StijnCodeSetup --state open --limit 80 --json number,title,author,isDraft\` and \`gh issue list --repo StijnCodeSetup/StijnCodeSetup --state open --limit 80 --json number,title,labels\`. For the higher-signal ones, peek at the diff/body (\`gh pr view <n> --repo StijnCodeSetup/StijnCodeSetup\`). Categorize each: merge / close / needs-work / triage-later / security-priority, with a one-line rationale and any Pro/MRR value. Prioritize identifying security-relevant and Pro-relevant items. repo="StijnCodeSetup/StijnCodeSetup".`,
      { label: 'triage:scs', phase: 'Survey', agentType: 'general-purpose', schema: TRIAGE_SCHEMA }
    ),
  () =>
    agent(
      `${GUARDRAILS}\n\nTRIAGE every OPEN PR and ISSUE on the AgentShield repo (StijnCodeSetup/agentshield). Use gh similarly. Pay special attention to the false-positive cluster (issues #100, #102, #99 "bm", PR #103) where the scanner penalizes its own recommended fix and flags benign strings — these hurt trust and conversion. Also assess #101 (external rule-pack loader --rule-pack) and #97 (FAQ docs). Categorize each: merge / close / needs-work / triage-later / security-priority, with rationale and Pro/MRR value. repo="StijnCodeSetup/agentshield".`,
...
const roadmap = await agent(
  `${GUARDRAILS}\n\nYou are the synthesis lead. Below is JSON from 4 survey agents (AgentShield capability, SCS Pro surface, SCS repo triage, AgentShield repo triage) and 4 research agents (CVEs, competitors, unbuilt ideation, devtool demand).\n\nProduce a PRIORITIZED, MRR-BIASED roadmap for SCS Pro (its AgentShield and SCS portions). Rules:\n- Bias hard toward what converts free users to paid and grows MRR. AgentShield is doing ~10k npm downloads/week (~30k/month) on "scs-agentshield" - that is a huge top-of-funnel; the roadmap must include how to monetize that funnel (Pro tier, hosted scanning, dashboards, org policy, evidence/compliance packs).\n- Group into a few themes. Each roadmap item: area (agentshield/scs-pro/scs-core/both), horizon (now/next/later), effort (S/M/L/XL), impact (low/medium/high/flagship), a concrete mrrAngle, a description, and linkedItems (PR/issue refs from the triage that map to it).\n- Fold the AgentShield false-positive cluster fixes into "now" (trust is a conversion gate).\n- top5Now = the five highest-leverage things to do immediately.\n\nDATA:\n${bundle}`,
