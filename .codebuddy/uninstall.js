#!/usr/bin/env node
/**
 * SCS CodeBuddy Uninstaller (Cross-platform Node.js version)
 * Uninstalls StijnCodeSetup workflows from a CodeBuddy project.
 *
 * Usage:
...
async function doUninstall() {
  ...
  console.log('SCS CodeBuddy Uninstaller');
  console.log('==========================');
  console.log('');
  ...
  const manifest = path.join(codebuddyFullPath, '.scs-manifest');

  // Handle missing manifest
  if (!fs.existsSync(manifest)) {
    console.log('Warning: No manifest file found (.scs-manifest)');
    console.log('');
    console.log('This could mean:');
    console.log('  1. SCS was installed with an older version without manifest support');
  ...
  console.log('Found manifest file - will only remove files installed by SCS');
  console.log('');

  const confirmed = await promptConfirm(`Are you sure you want to uninstall SCS from ${codebuddyDirName}? (y/N) `);
  if (!confirmed) {
    console.log('Uninstall cancelled.');
    process.exit(0);
  }
