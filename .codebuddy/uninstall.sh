...
do_uninstall() {
    ...
    echo "SCS CodeBuddy Uninstaller"
    echo "=========================="
    echo ""
    ...
    if [ ! -f "$MANIFEST" ]; then
        echo "Warning: No manifest file found (.scs-manifest)"
        echo ""
        echo "This could mean:"
        echo "  1. SCS was installed with an older version without manifest support"
    ...
    echo "Found manifest file - will only remove files installed by SCS"
    echo ""
    read -p "Are you sure you want to uninstall SCS from $CODEBUDDY_DIR? (y/N) " -n 1 -r
    echo
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
        echo "Uninstall cancelled."
        exit 0
