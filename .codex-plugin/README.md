# .codex-plugin — Codex Native Plugin for SCS

This directory contains the **Codex plugin manifest** for SCS.
...
After install, `codex plugin list` is only a registration check. From an SCS
...
repository shape: `plugins/scs/.codex-plugin/plugin.json`,
`plugins/scs/skills/`, and the supporting README/assets. Until that listing is
...
The installed plugin registers under the short slug `scs` so tool and command names
...
- SCS is moving to a skills-first workflow surface. Legacy `commands/` remain for
