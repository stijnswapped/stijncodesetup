  ...
  .then(raw => {
    try {
      ...
      if (hookEnabled('pre:bash:dev-server-block', ['standard', 'strict']) && process.platform !== 'win32') {
        ...
        if (hasBlockedDev) {
          console.error('[SCS] BLOCKED: Dev server must run in tmux for log access');
          console.error('[SCS] Use: tmux new-session -d -s dev "npm run dev"');
          process.exit(2);
        }
      }
      ...
      if (
        ...
        console.error('[SCS] Consider running in tmux for session persistence');
      }

      if (hookEnabled('pre:bash:git-push-reminder', ['strict']) && /\bgit\s+push\b/.test(cmd)) {
        console.error('[SCS] Review changes before push: git diff origin/main...HEAD');
      }
    } catch {
      // noop
    }
