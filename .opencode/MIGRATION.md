...
This guide helps you migrate from Claude Code to OpenCode while using the SCS configuration.
...
### SCS Plugin Hooks Included

The SCS OpenCode configuration includes translated hooks:
...
### 2. Use the SCS OpenCode Configuration
...
│   ├── scs-hooks.ts           # All SCS hooks as plugins
...
### Option 1: Use SCS Configuration Directly
...
This only loads the published SCS OpenCode plugin module (hooks/events and exported plugin tools).
It does **not** automatically inject SCS's full `agent`, `command`, or `instructions` config into your project.

If you want the full SCS OpenCode workflow surface, use the repository's bundled `.opencode/opencode.json` as your base config or copy these pieces into your project:
- `.opencode/commands/`
- `.opencode/prompts/`
...
4. If you installed only `plugin: ["stijncodesetup"]`, note that npm plugin install does not auto-add SCS commands or agents to your project config
...
- **SCS Configuration**: Report to [github.com/StijnCodeSetup/StijnCodeSetup](https://github.com/StijnCodeSetup/StijnCodeSetup)
