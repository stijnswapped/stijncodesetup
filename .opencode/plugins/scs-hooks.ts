...
export const SCSHooksPlugin: SCSHooksPluginFn = async ({
  client,
  $,
  directory,
  worktree,
  ...
  const log = (level: "debug" | "info" | "warn" | "error", message: string) =>
    client.app.log({ body: { service: "scs", level, message } })
  ...
  const disabledHooks = new Set(
    (process.env.SCS_DISABLED_HOOKS || "")
      .split(",")
      .map((item) => item.trim())
      .filter(Boolean)
  )
  ...
  return {
      ...
      if (hookEnabled("post:edit:console-warn", ["standard", "strict"]) && event.path.match(/\.(ts|tsx|js|jsx)$/)) {
        try {
          ...
          if (result.trim()) {
            ...
            log(
              "warn",
              `[SCS] console.log found in ${event.path} (${lines} occurrence${lines > 1 ? "s" : ""})`
            )
          }
        } catch {
    ...
    "tool.execute.before": async (
      ...
      if (
        ...
        log(
          "info",
          "[SCS] Remember to review changes before pushing: git diff origin/main...HEAD"
        )
      }
      ...
      if (
        ...
        if (
          ...
          log(
            "warn",
            `[SCS] Creating ${filePath} - consider if this documentation is necessary`
          )
        }
      }
      ...
      if (hookEnabled("pre:bash:tmux-reminder", "strict") && input.tool === "bash") {
        ...
        if (
          ...
          log(
            "info",
            "[SCS] Long-running command detected - consider using background execution"
          )
        }
      }
    },
    ...
    "session.idle": async () => {
      ...
      if (totalConsoleLogCount > 0) {
        log(
          "warn",
          `[SCS] Audit: ${totalConsoleLogCount} console.log statement(s) in ${filesWithConsoleLogs.length} file(s)`
        )
