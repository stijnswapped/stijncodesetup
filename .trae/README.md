# StijnCodeSetup for Trae

Bring StijnCodeSetup (SCS) workflows to Trae IDE. This repository provides custom commands, agents, skills, and rules that can be installed into any Trae project with a single command.
...
This creates `/path/to/your/project/.trae-cn/` with all SCS components.
...
This creates `~/.trae-cn/` with all SCS components. All Trae projects will use these global installations.

**Note**: Global installation is useful when you want to maintain a single copy of SCS across all your projects.
...
The uninstaller uses a manifest file (`.scs-manifest`) to track installed files, ensuring safe removal:
...
- **Safe removal**: Only removes files tracked in the manifest (installed by SCS)
...
- **Manifest-based**: Requires `.scs-manifest` file (created during install)
