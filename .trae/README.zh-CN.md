...
为 Trae IDE 带来 StijnCodeSetup (SCS) 工作流。此仓库提供自定义命令、智能体、技能和规则，可以通过单个命令安装到任何 Trae 项目中。
...
**注意**：全局安装适用于希望在所有项目之间维护单个 SCS 副本的场景。
...
卸载程序使用清单文件（`.scs-manifest`）跟踪已安装的文件，确保安全删除：
...
- **安全删除**：仅删除清单中跟踪的文件（由 SCS 安装的文件）
- **保留用户文件**：您手动添加的任何文件都会被保留
- **非空目录**：包含用户添加文件的目录会被跳过
- **基于清单**：需要 `.scs-manifest` 文件（在安装时创建）
