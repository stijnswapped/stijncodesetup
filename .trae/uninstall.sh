#!/bin/bash
#
# SCS Trae Uninstaller
# Uninstalls StijnCodeSetup workflows from a Trae project.
#
# Usage:
...
do_uninstall() {
    ...
    # Manifest file path
    MANIFEST="$trae_full_path/.scs-manifest"
    ...
    echo ""
    read -p "Are you sure you want to uninstall SCS from $trae_dir? (y/N) " -n 1 -r
    echo
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
        echo "Uninstall cancelled."
        exit 0
