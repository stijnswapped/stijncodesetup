...
- Discord community launch: server + GitHub PR/issue/release feed, `release-announce.yml` workflow (announce + pin + Discussions cross-post), and a dependency-free community bot (`scripts/discord/scs-bot.mjs`) with `/scs`, `/help`, `/skill`, `/docs`, `/release`.
- `orch-*` orchestrator skill family and dynamic workflow team orchestration.
- `kubernetes-patterns` skill, worktree-lifecycle service, MCP inventory (`scs.mcp.v1`), codex-worktree and opencode session adapters.
...
- Adds the public SCS 2.0 release-candidate surface for the Hermes operator story.
- Documents SCS as the reusable cross-harness substrate across Claude Code, Codex, Cursor, OpenCode, and Gemini.
...
- Added `docs/architecture/cross-harness.md` and regression coverage for the SCS/Hermes boundary.
- Kept `scs2/` versioning independent for now; it remains an alpha control-plane scaffold unless release engineering decides otherwise.

### Notes

- This is a release candidate, not a GA claim for the full SCS 2.0 control-plane roadmap.
...
- SCS 2.0 alpha control-plane binary now builds locally from `scs2/` and exposes the first usable CLI/TUI surface.
...
- `cargo build --manifest-path scs2/Cargo.toml` passes on the repository baseline.
- `scs-tui` currently exposes `dashboard`, `start`, `sessions`, `status`, `stop`, `resume`, and `daemon`.
...
- This release is a repo-surface correction and ecosystem sync, not a claim that the full SCS 2.0 roadmap is complete.
...
- `stijncodesetup` — Self-referential SCS skill (#335)
...
- Added `SCS_HOOK_PROFILE` and `SCS_DISABLED_HOOKS` controls.
