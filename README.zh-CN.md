...
<img src="https://github.com/user-attachments/assets/1a471488-59cc-425b-8345-5245c7efbcef" alt="The Shorthand Guide to StijnCodeSetup" />
</a>
</td>
<td width="33%">
<a href="https://x.com/StijnCodeSetup/status/2014040193557471352">
<img src="https://github.com/user-attachments/assets/c9ca43bc-b149-427f-b551-af6840c368f0" alt="The Longform Guide to StijnCodeSetup" />
</a>
</td>
<td width="33%">
...
> 安装名称说明：较早的帖子里可能还会出现较长的旧标识符。Anthropic 的 marketplace/plugin 安装是按规范化插件标识符寻址的，因此 SCS 现在统一为 `scs@scs`，让工具名和 slash command 命名空间保持简短。
...
# 纯手动安装 SCS（不要和 /plugin install 叠加）
...
在 Claude Code 中使用 `/security-scan` 运行，或通过 [GitHub Action](https://github.com/StijnCodeSetup/scs-agentshield) 集成到 CI。
...
> # 方案 A：用户级规则（对所有项目生效）
> mkdir -p ~/.claude/rules
> cp -r stijncodesetup/rules/common ~/.claude/rules/
> cp -r stijncodesetup/rules/typescript ~/.claude/rules/   # 选择你使用的技术栈
> cp -r stijncodesetup/rules/python ~/.claude/rules/
> cp -r stijncodesetup/rules/golang ~/.claude/rules/
> cp -r stijncodesetup/rules/php ~/.claude/rules/
>
> # 方案 B：项目级规则（仅对当前项目生效）
> mkdir -p .claude/rules
> cp -r stijncodesetup/rules/common .claude/rules/
...
仅适用于手动安装：如果你没有通过 Claude 插件方式安装 SCS，可以将 `hooks/hooks.json` 中的钩子配置复制到你的 `~/.claude/settings.json` 文件中。

如果你是通过 `/plugin install` 安装 SCS，请不要再把这些钩子复制到 `settings.json`。Claude Code v2.1+ 会自动加载插件中的 `hooks/hooks.json`，重复注册会导致重复执行以及 `${CLAUDE_PLUGIN_ROOT}` 无法解析。
...
[成为赞助者](https://github.com/sponsors/StijnCodeSetup) | [赞助档位](SPONSORS.md) | [赞助计划](SPONSORING.md)
...
- **快速上手指南（入门首选）：** [StijnCodeSetup 简明指南](https://x.com/StijnCodeSetup/status/2012378465664745795)
- **长文指南（高阶进阶）：** [StijnCodeSetup 完整版深度指南](https://x.com/StijnCodeSetup/status/2014040193557471352)
- **安全指南：** [安全指南](./the-security-guide.md) | [推文详解](https://x.com/StijnCodeSetup/status/2033263813387223421)
- **关注作者：** [@StijnCodeSetup](https://x.com/StijnCodeSetup)
