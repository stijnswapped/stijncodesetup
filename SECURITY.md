...
- the `StijnCodeSetup/StijnCodeSetup` repository
- the `stijncodesetup` npm package
- SCS plugin, install, repair, dashboard, hook, rule, skill, MCP, and command surfaces shipped from this repository
- GitHub Actions workflows and release automation in this repository
- the SCS Tools GitHub App integration points documented by this repository
- AgentShield usage docs when they are embedded here. AgentShield code issues belong in <https://github.com/StijnCodeSetup/scs-agentshield>

## Official Distribution Surfaces

Official SCS surfaces are:

- GitHub repo: <https://github.com/StijnCodeSetup/StijnCodeSetup>
- npm package: `stijncodesetup`
- GitHub App: <https://github.com/apps/scs-tools>
- marketplace/plugin slug: `scs@scs`
- website: <https://scs.tools>

Official AgentShield surface:

- npm package: `scs-agentshield`
- GitHub repo: <https://github.com/StijnCodeSetup/scs-agentshield>

The following packages have been observed using SCS repository metadata but are **not maintained by SCS**:

- `@chil_ntl/scs-cli`
- `scs-100xprompt-plugin`

Treat any package not listed under official surfaces as unofficial until verified. Do not install packages named `opencode-scs`, `stijncodesetup`, or other SCS-like aliases unless this repository explicitly documents them as official.

GitHub dependency graph may also show Go module aliases such as `github.com/StijnCodeSetup/StijnCodeSetup` or historical repository paths. SCS is not currently distributed as a supported Go module.
...
- screenshots, stale line numbers, or reports against `StijnCodeSetup/stijncodesetup` that do not reproduce on current `StijnCodeSetup/StijnCodeSetup`
- self-XSS or social engineering with no repository-controlled exploit path
- dependency graph/package metadata confusion without an install path to an official SCS package
- vulnerabilities in third-party packages unless SCS pins, installs, or executes them in a way that creates extra impact
...
SCS treats supply-chain exposure as a first-class security surface.
...
- Package metadata should point at `StijnCodeSetup/StijnCodeSetup`, not historical repo paths.
...
SCS runs inside agent harnesses that may inject ephemeral client-side system reminders into the model input on every turn. These blocks are not automatically repository-carried payloads.
...
- **AgentShield:** `npx scs-agentshield scan`
