---
description: Detect a project's stack and produce a dry-run SCS onboarding plan using the repository's install manifests and stack mappings.
---

# /project-init

Create a safe, reviewable SCS onboarding plan for the current project. This command should start in dry-run mode and only write files after explicit user approval.
...
/project-init --config scs-install.json
```
...
3. Use SCS's installer and manifest tooling. Do not hand-copy files or clone arbitrary remotes as an install shortcut.
...
- SCS config: `scs-install.json`
- optional stack map: `config/project-stack-mappings.json` in the SCS repo

When the SCS checkout is available, use `config/project-stack-mappings.json` as the stack-to-rules/skills reference. If the file is unavailable, fall back to the installed SCS manifests and explicit user choices.
...
3. Resolve the smallest useful SCS plan:
   - project has an `scs-install.json`: `node scripts/install-plan.js --config scs-install.json --json`
...
- `/scs-guide` for interactive feature discovery before installing
