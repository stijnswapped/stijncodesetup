---
description: Run the SCS formatter quality gate for a single file and report remediation steps.
---
...
- Behavior toggles are environment variables:
  - `SCS_QUALITY_GATE_FIX=true` - apply formatting fixes instead of check-only
  - `SCS_QUALITY_GATE_STRICT=true` - log formatter failures as gate failures
...
echo '{"tool_input":{"file_path":"src/example.ts"}}' \
  | SCS_QUALITY_GATE_FIX=true node scripts/hooks/quality-gate.js
```
