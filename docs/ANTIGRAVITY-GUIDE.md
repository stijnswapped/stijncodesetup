...
Google's [Antigravity](https://antigravity.dev) is an AI coding IDE that uses a `.agent/` directory convention for configuration. SCS provides first-class support for Antigravity through its selective install system.

## Quick Start

```bash
# Install SCS with Antigravity target
...
This installs SCS components into your project's `.agent/` directory, ready for Antigravity to pick up.

## How the Install Mapping Works

SCS remaps its component structure to match Antigravity's expected layout:

| SCS Source | Antigravity Destination | What It Contains |
|------------|------------------------|------------------|
...
- **Commands become workflows**: SCS's `/command` files land in `.agent/workflows/`, which is Antigravity's equivalent of slash commands.
- **Agents become skills**: SCS agent definitions map to `.agent/skills/`, where Antigravity looks for skill configurations.
...
│   │   ├── tdd-guide.md
│   │   └── ...
│   └── scs-install-state.json     # tracks what SCS installed
```
...
The installer writes `.agent/scs-install-state.json` to track which files SCS owns. This enables safe uninstall and repair — SCS will never touch files it didn't create.
...
| Install state | `scs-install-state.json` | `scs-install-state.json` | `scs-install-state.json` | `scs-install-state.json` |
...
- Check that `scs-install-state.json` was created — if missing, re-run the installer
...
- If you manually copied SCS commands, rename the directory
