...
This document captures architect-level improvements for the StijnCodeSetup (SCS) project. It is written from the perspective of a Claude Code coding architect aiming to improve maintainability, consistency, and long-term quality.
...
- Ensure `SCS_HOOK_PROFILE` and `SCS_DISABLED_HOOKS` are respected in all code paths (including shell) so behavior is consistent.
