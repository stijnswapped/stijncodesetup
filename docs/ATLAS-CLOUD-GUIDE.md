...
[Atlas Cloud](https://www.atlascloud.ai/?utm_source=github&utm_medium=link&utm_campaign=stijncodesetup) is a full-modal AI inference platform providing an OpenAI-compatible API for 59+ LLM models, image generation, and video generation.
...
SCS can install its managed surfaces into any OpenAI-compatible backend. To use Atlas Cloud with Claude Code (or any SCS-managed harness), set the base URL and API key:
...
response = client.chat.completions.create(
    ...
    messages=[{"role": "user", "content": "Hello from SCS + Atlas Cloud!"}],
)
