# Hermes / OpenClaw -> SCS Migration

This document is the public migration guide for moving a Hermes or OpenClaw-style operator setup into the current SCS model.
...
SCS is the durable public system:
...
- install surfaces
- session adapters
- SCS 2.0 control-plane work

Hermes and OpenClaw are useful inputs because they contain repeated operator workflows that can be distilled into SCS-native surfaces.

That means the shortest safe path is:

1. extract the reusable behavior
2. translate it into SCS-native skills, hooks, docs, or adapter work
...
- SCS hook / command automation for local repeatability
- SCS 2.0 scheduler work under issue `#1050`
...
- SCS 2.0 as the future long-lived scheduling/control plane
...
- SCS session adapter and control-plane work
- orchestration/session inspection commands
- SCS 2.0 control-plane backlog under:
  - `#1045`
  - `#1046`
  - `#1047`
  - `#1048`
...
- SCS-native top-level skills when the workflow is reusable
...
- SCS-native operator skills when the workflow logic is the real asset
- adapter/control-plane work when the missing piece is session/runtime coordination

Do not import opaque third-party runtimes into SCS just because a private workflow depended on them.

If a workflow is valuable:

1. understand the behavior
2. rebuild the minimum SCS-native version
...
- SCS 2.0 adapter/control-plane discovery docs
...
- continuing the SCS 2.0 operator/control-plane buildout

SCS 2.0 now ships a bounded migration audit entrypoint:

- `scs migrate audit --source ~/.hermes`
- `scs migrate plan --source ~/.hermes --output migration-plan.md`
- `scs migrate scaffold --source ~/.hermes --output-dir migration-artifacts`
- `scs migrate import-skills --source ~/.hermes --output-dir migration-artifacts/skills`
- `scs migrate import-tools --source ~/.hermes --output-dir migration-artifacts/tools`
- `scs migrate import-plugins --source ~/.hermes --output-dir migration-artifacts/plugins`
- `scs migrate import-schedules --source ~/.hermes --dry-run`
- `scs migrate import-remote --source ~/.hermes --dry-run`
- `scs migrate import-env --source ~/.hermes --dry-run`
- `scs migrate import-memory --source ~/.hermes`

Use that first to inventory the legacy workspace and map detected surfaces onto the current SCS2 scheduler, remote dispatch, memory graph, templates, and manual-translation lanes.
...
1. Keep the public SCS repo as the canonical reusable layer.
2. Port reusable Hermes/OpenClaw workflows into SCS-native skills one lane at a time.
3. Keep private auth and personal context outside the repo.
4. Use GitHub / Linear / KB systems as durable truth.
5. Treat SCS 2.0 as the path to a native operator shell, not as a finished product.
