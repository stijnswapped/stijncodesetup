...
Hermes is the operator shell. SCS is the reusable system behind it.
...
- SCS skills, agents, commands, hooks, and MCP configs from this repo
...
Use Hermes as the front door and SCS as the reusable workflow substrate.
...
Telegram / CLI / TUI
      ...
      Hermes
        ↓
 SCS skills + hooks + MCPs + generated workflow packs
        ↓
...
- `~/.hermes/config.yaml`
  ...
  - plugin loading
- `~/.hermes/skills/scs-imports/`
  - SCS skills copied in for Hermes-native use
...
- SCS for skills, rules, prompts, and cross-harness conventions
...
0. Run `scs migrate audit --source ~/.hermes` first to inventory the legacy workspace and see which parts already map onto SCS2.
0.5. Plan and scaffold migration artifacts before importing anything:
   - generate reviewable plans with `scs migrate plan` and `scs migrate scaffold`
   - scaffold reusable legacy skills with `scs migrate import-skills --output-dir migration-artifacts/skills`
   - scaffold tool translation templates with `scs migrate import-tools --output-dir migration-artifacts/tools`
   - scaffold bridge plugin templates with `scs migrate import-plugins --output-dir migration-artifacts/plugins`
   - preview recurring jobs with `scs migrate import-schedules --dry-run`
   - preview gateway dispatch with `scs migrate import-remote --dry-run`
   - preview safe env/service context with `scs migrate import-env --dry-run`
   - import sanitized workspace memory with `scs migrate import-memory`
1. Install SCS and verify the baseline harness setup with `node tests/run-all.js`; the expected result is a zero-failure test summary.
2. Install Hermes and point it at SCS-imported skills.
...
SCS v2.0.0-rc.1 documents the Hermes surface and ships launch collateral now.
