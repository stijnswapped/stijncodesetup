...
Use this guide when you want SCS behavior inside a harness that does not natively load `.claude/`, `.codex/`, `.opencode/`, `.cursor/`, or `.agent/` layouts.
...
Prefer a first-class SCS target whenever one exists:
...
Use this guide only when you need SCS behavior in a non-native harness.

## What You Are Reproducing

When you adapt SCS manually, you are trying to preserve four things:
...
```bash
cd /path/to/stijncodesetup

sed -n '1,220p' skills/tdd-workflow/SKILL.md > /tmp/scs-context.md
printf '\n\n---\n\n' >> /tmp/scs-context.md
sed -n '1,220p' skills/backend-patterns/SKILL.md >> /tmp/scs-context.md
printf '\n\n---\n\n' >> /tmp/scs-context.md
sed -n '1,220p' skills/security-review/SKILL.md >> /tmp/scs-context.md
```
...
Optional: if you already use a repo packer like `repomix`, it can help compress selected files into one handoff document. It is a convenience tool, not the canonical SCS path.

## Compression Rules

When manually packing SCS for another harness:
...
You are not implementing real commands. You are giving the harness explicit invocation handles that map to SCS behavior.
...
That does not recreate true automation, but it captures the operational discipline of SCS.

## Harness Capability Matrix

| Capability | First-Class SCS Targets | Manual-Adaptation Targets |
| --- | --- | --- |
...
2. Pack the selected SCS skill files into one upload or paste block.
...
```text
You are operating with a manually adapted SCS bundle.
...
- use manual adaptation to carry SCS behavior into non-native harnesses
- use native SCS targets whenever you want the full system
