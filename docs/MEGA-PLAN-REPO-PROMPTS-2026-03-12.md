...
- `stijncodesetup` has finished the orchestration, Codex baseline, and
  Windows CI recovery lane.
- The next open SCS Phase 1 items are:
  - review `#399`
  - convert recurring discussion pressure into tracked issues
  - define selective-install architecture
  - write the SCS 2.0 discovery doc
- `agentshield`, `SCS-website`, and `skill-creator-app` all have dirty
...
```text
Work in: <SCS_ROOT>/stijncodesetup
...
issues. Focus on high-signal roadmap items that unblock SCS 1.x and SCS 2.0.
...
5. SCS 2.0 discovery doc / adapter contracts
...
### Prompt C — SCS 2.0 Discovery and Adapter Spec

```text
Work in: <SCS_ROOT>/stijncodesetup

Goal:
Turn the existing SCS 2.0 vision into a first concrete discovery doc focused on
...
4. Define what stays in SCS 1.x versus what belongs in SCS 2.0.
...
- distinguish SCS 1.x toolkit from SCS 2.0 control plane clearly
...
```text
Work in: <SCS_ROOT>/skill-creator-app
...
1. Assess whether the app should support:
   ...
   - SCS convention adaptation workflows
...
- keep the focus on audited skill ingestion and SCS-compatible output
...
## Repo: `SCS` Workspace (`applications/`, `knowledge/`, `tasks/`)
...
Goal:
Use the parent SCS workspace to support the mega-plan hosted/workflow lanes.
...
- keep recommendations compatible with the current multi-repo SCS setup
