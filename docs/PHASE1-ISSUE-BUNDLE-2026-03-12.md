...
- `#423` Implement manifest-driven selective install profiles for SCS
- `#421` Add SCS install-state plus uninstall / doctor / repair lifecycle
- `#424` Define canonical session adapter contract for SCS 2.0 control plane
...
Implement manifest-driven selective install profiles for SCS
...
SCS still installs primarily by target and language. The repo now has first-pass
...
Implement manifest-driven install execution for current SCS targets:
...
- `scs-install --profile <name>`
- `scs-install --modules <id,id,...>`
...
Add SCS install-state plus uninstall / doctor / repair lifecycle
...
SCS has no canonical installed-state record. That makes uninstall, repair, and
...
- what paths SCS owns
- how to remove or repair only SCS-managed files
...
- Claude: `~/.claude/scs/install-state.json`
- Cursor: `./.cursor/scs-install-state.json`
- Antigravity: `./.agent/scs-install-state.json`
...
- `uninstall` removes only SCS-managed files and leaves unrelated local files
  alone
...
Define canonical session adapter contract for SCS 2.0 control plane
...
SCS now has real orchestration/session substrate, but it is still
...
Without that adapter contract, any future SCS 2.0 operator shell will be forced
...
- Full SCS 2.0 UI in the same issue
...
SCS now has a large and growing skill surface, but generated/imported/learned
...
As SCS grows, the repo needs explicit rules for where generated skill artifacts
