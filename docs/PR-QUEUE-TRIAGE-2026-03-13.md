...
`stijncodesetup` pull-request queue as of `2026-03-13T08:33:31Z`.
...
- targeted hook invocations for:
  ...
  - `SCS_SKIP_OBSERVE=1`
  - `agent_id` payload
  - trimmed `SCS_OBSERVE_SKIP_PATHS`
...
2. Low: the new guard matrix shipped without direct regression coverage.
   ...
   directly assert the new `CLAUDE_CODE_ENTRYPOINT`, `SCS_HOOK_PROFILE`,
   `SCS_SKIP_OBSERVE`, `agent_id`, or trimmed skip-path branches.
