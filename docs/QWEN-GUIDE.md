...
SCS can install its managed command, agent, skill, rule, and MCP surfaces into the Qwen CLI home directory.

## Install

From the SCS repository root:
...
The Qwen adapter writes into `~/.qwen/` and records managed file ownership in `~/.qwen/scs-install-state.json`.
...
Rerun the same install command after pulling SCS updates. The installer uses the install-state file to update SCS-managed files without claiming unrelated user files in `~/.qwen/`.
...
That removes files recorded in `~/.qwen/scs-install-state.json` and leaves unrelated Qwen configuration alone.
