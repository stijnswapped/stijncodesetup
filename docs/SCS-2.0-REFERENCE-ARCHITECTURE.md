# SCS 2.0 Reference Architecture

Current execution mirror:
[`SCS-2.0-GA-ROADMAP.md`](SCS-2.0-GA-ROADMAP.md).

This document turns the May 2026 reference sweep into concrete SCS backlog
...
| Reference | Primary pressure on SCS 2.0 | Concrete SCS delta |
| --- | --- | --- |
...
| [`jarrodwatts/claude-hud`](https://github.com/jarrodwatts/claude-hud) | Always-visible Claude Code statusline for context, tools, agents, todos, and transcript-backed activity. | Formalize the SCS HUD/status payload for context, cost, tool calls, active agents, todos, queue state, checks, and risk. |
| [`stanford-iris-lab/meta-harness`](https://github.com/stanford-iris-lab/meta-harness) | Automated search over task-specific harness design: what to store, retrieve, and show. | Split SCS improvement loops into scenario spec, proposer trace, verifier result, and promoted playbook. |
...
| [`NousResearch/hermes-agent`](https://github.com/NousResearch/hermes-agent) | Self-improving operator shell with memories, skills, scheduler, gateways, subagents, terminal backends, and migration tooling. | Keep SCS portable across local, SSH, container, and hosted terminal backends without hiding the underlying commands. |
...
SCS 2.0 should be a harness operating system, not only a catalog of commands,
agents, and skills.
...
│ AgentShield policies/SARIF, SCS Tools checks, billing,       │
...
- `stijncodesetup`: extend the adapter compliance matrix and public
  scorecard onramp.
- `scs2`: surface session/worktree state through a stable local payload before
  adding hosted telemetry.
- `SCS-Tools`: consume the same lifecycle events for PR checks, issue routing,
  and Linear sync.
...
- Keep `docs/architecture/observability-readiness.md` as the operator-facing
  readiness gate.
- Define a versioned HUD/status JSON contract that both SCS2 and SCS Tools can
  consume.
...
- Use RAG/reference sets for vetted SCS patterns, team history, CI failures,
  review outcomes, harness config quality, and security decisions.

Repo work:

- `stijncodesetup`: document scenario specs, verifier contracts, and
  playbook promotion rules.
- `SCS-Tools`: map analyzer findings to PR comments, check runs, and Linear
...
### SCS Tools Commercial And Review Platform

SCS Tools should become the GitHub-native layer for billing, deep analysis,
...
- Deep analyzer comparable in scope to the useful parts of GitGuardian,
  ...
  and harness drift.
- RAG/reference set over vetted SCS patterns, historical PR outcomes,
...
5. Audit SCS Tools billing and check-run surfaces.
