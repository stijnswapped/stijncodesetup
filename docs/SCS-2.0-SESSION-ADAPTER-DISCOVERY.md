# SCS 2.0 Session Adapter Discovery

## Purpose

This document turns the March 11 SCS 2.0 control-plane direction into a
...
- `scripts/lib/session-adapters/canonical-session.js`
  defines the canonical `scs.session.v1` normalization layer
...
In practice, SCS can already answer:
...
## Gap Between SCS 1.x And SCS 2.0

SCS 1.x currently has two different "session" surfaces:
...
The missing SCS 2.0 layer is a harness-neutral session adapter boundary that
can normalize:
...
SCS 2.0 should introduce a canonical session adapter contract.
...
This provides a non-orchestrated baseline for SCS 2.0.
...
3. Should event streaming be part of v1, or should SCS 2.0 ship snapshot-only
   first?
...
5. Should the adapter registry live inside this repo long-term, or move into the
   eventual SCS 2.0 control-plane app once the interface stabilizes?
...
The shortest path to SCS 2.0 is:
