# SCS Pro + AgentShield Security Roadmap

> Status: draft for review. Generated 2026-06-21 from a multi-agent survey + research pass
> (capability map of AgentShield and SCS Pro, triage of every open PR/issue on both repos,
> and web research on competitors, unbuilt ideas, and dev-tool demand). MRR-biased: every
> item is scored for how it converts the free funnel into paid SCS Pro / Enterprise.

## Why now

AgentShield (npm `scs-agentshield`) is doing roughly **30K downloads/month with no decay**
(~7.2K/week, ~78K year-to-date) and **903 GitHub stars** — a large, growing top-of-funnel.
Today there is almost no bridge from that free funnel to paid SCS Pro, and the single most
...
| now | Render the 3D agent-airspace observability dashboard (the moat made visible) | scs-pro | M | flagship |
| now | Add a Pro conversion CTA to free CLI output and GitHub App PR comments | both | S | high |
| now | Ship merge-ready correctness and Windows fixes that protect release velocity and core UX | scs-core | S | medium |
| now | Harden continuous-learning storage (path traversal + registry race) | scs-core | S | medium |
| next | Hosted continuous-scanning dashboard with fleet trend lines ('Sentry for agent security') | agentshield | L | flagship |
| next | Inline PR-comment review + autofix-PR via the scs-tools GitHub App | agentshield | M | high |
...
| next | Harness-neutral expansion: Kimi, Codex alias, OpenClaude/Codex compat | scs-core | L | medium |
| next | Batch-review and dedup the community skill/agent PR backlog | scs-core | M | low |
| later | Free runtime guard hook with Pro centralized telemetry + trust registry | agentshield | XL | flagship |
| later | Cross-machine team airspace + A2A topology security in the control pane | scs-pro | XL | high |
...
- **Linked:** issue #102
- **MRR angle:** Verified, trustworthy autofix is the activation moment that makes the free CLI feel magical and seeds confidence in the paid managed-remediation workflow (autofix-as-PR in SCS Tools).

src/fixer/index.ts applies string transforms but never re-scans to prove the finding is gone and no new finding was introduced — and issue #102 proved a naive permission tighten can be re-flagged by the scanner. Close the loop: after applying --fix, re-run the scanner, diff the findings set, auto-revert if the score regresses, and emit a verified-fix attestation. OSS gets verify-after-fix locally; Pro gets autofix-as-PR via the scs-tools GitHub App (open remediation PR, run verified re-scan in CI, attach before/after evidence pack, auto-merge on green).

### Render the 3D agent-airspace observability dashboard (the moat made visible)

- **Area:** scs-pro | **Effort:** M | **Impact:** flagship
- **Linked:** PR #2320
- **MRR angle:** This is the single most ownable, demo-able paid-looking surface SCS has and nothing else offers it. 'Watch N agents crawl toward each other in code-space and one steer away' converts on the demo alone — it justifies a Team/Enterprise seat that competitors (CodeRabbit/Greptile) cannot match.
...
- **Linked:** PR #97
- **MRR angle:** Directly monetizes the ~30K downloads/month (78,108 YTD, ~7,228/week, no decay) free funnel. There is currently no surfaced upgrade path from the free scanner to SCS Pro — adding a contextual CTA at the point of value is the lowest-effort, highest-leverage conversion lever available.

Surface a Pro CTA where free users already feel value: a footer in terminal/JSON/markdown reports ('hosted fleet posture + continuous monitoring at scs-tools Pro'), in the GitHub Action job summary, and in PR check-run comments. Lead with the privacy wedge ('scans never leave your machine' vs Snyk Agent Scan transmitting tool metadata to cloud) and the low-noise/runtimeConfidence accuracy story as the differentiators. Keep AgentShield free + zero-account as the moat against token-gated Snyk Agent Scan.

### Ship merge-ready correctness and Windows fixes that protect release velocity and core UX

- **Area:** scs-core | **Effort:** S | **Impact:** medium
...
Merge the clean, verified batch: PR #2133 (Claude provider model-ID + adaptive-thinking fix — replaces invalid IDs with claude-sonnet-4-6/haiku-4-5/opus-4-8, routes SYSTEM to top-level, omits temperature, adaptive thinking for Opus 4.7/4.8; previous default would 404/400 at the API), PR #2307 + #2063 (Windows test/UTF-8 fixes), PR #2273/#2246/#2312 (docs/workflow), PR #2293 (dependabot minor/patch). Schedule a fix for issue #2316 (plan-orchestrate still probes old paths after the scs@scs marketplace rename — broken install detection on a core workflow command).

### Harden continuous-learning storage (path traversal + registry race)

- **Area:** scs-core | **Effort:** S | **Impact:** medium
- **Linked:** issue #2297, issue #2294, issue #2300, issue #2296
- **MRR angle:** SCS sells security tooling; a path-traversal or registry-corruption bug in our own learning layer is a credibility liability that undercuts the entire security brand the Pro tier trades on.
...
- **MRR angle:** THE core SCS Tools Pro product and the clearest recurring-revenue moat: nobody unifies config-scan + runtime telemetry. Billed per seat/repo. Reuses operatorReadback/reviewItems as the API contract — lowest-effort-to-highest-leverage Pro upgrade because the data model already exists.
...
### Inline PR-comment review + autofix-PR via the scs-tools GitHub App
...
- **Area:** scs-core | **Effort:** L | **Impact:** medium
- **Linked:** PR #2154, PR #2254, issue #2076, issue #2073, issue #2074
- **MRR angle:** Broadens the addressable user base for the whole funnel and aligns with the SCS 2.0 harness-neutral control-pane vision — more harnesses scanned = more top-of-funnel feeding Pro.
...
- **Area:** scs-core | **Effort:** M | **Impact:** low
...
- **Area:** scs-pro | **Effort:** XL | **Impact:** high
...
- No hosted/SaaS scanning backend. Everything runs locally or in the user's CI. GitHub App (scs-tools) is referenced but the scanner core is fully local/offline.
...
- **IDE/editor integration — findings and fixes where agents actually write code** — Ship a VS Code/Cursor extension (and a Claude Code skill already exists via scs:security-scan) that lints agent configs on save, shows findings inline, and offers fixes — gated behind Pro for org policy sync. Builds on existing harness-adapters; meets developers in the editor where Snyk Agent Scan (CLI/MDM) does not.
...
| triage-later | PR #2319 | feat: add scs-recipes skill |
...
| merge | issue #2316 | plan-orchestrate: stale SCS install detection after marketplace rename to scs@scs |
...
| triage-later | issue #2112 | ctx — potential synergy between SCS and ctx |
