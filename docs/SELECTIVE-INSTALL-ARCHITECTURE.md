...
- what SCS owns and may safely remove or repair later

That is the missing contract between SCS 1.x installation and an SCS 2.0
control plane.
...
- unified `scs` CLI routing install, planning, and lifecycle commands
...
- legacy `scs-install` compatibility still points at `install.sh`
...
- the repo already has a unified Node entrypoint through `scs` and
  `install-apply.js`
...
SCS 2.0 should make operation planning a standalone phase so that:
...
- `uninstall`: remove only SCS-owned outputs
...
- `scripts/scs.js` as the unified CLI
...
If the goal is SCS 2.0 and not just “working enough,” the next modularization
steps should be:
...
Today SCS still behaves like a broad payload copier:
...
That means SCS 2.0 needs two contracts, not one:
...
1. Keep `stijncodesetup` as the canonical source repo.
...
Fields still needed for SCS 2.0:

- `installStrategy`
  for example `copy`, `flatten-rules`, `generate`, `merge-config`
- `ownership`
  whether SCS fully owns the target path or only generated files under it
- `pathMode`
...
That lets SCS 2.0 say things like:
...
SCS 2.0 needs the next layer: operation planning.
...
```json
{
  ...
  "destination": "/Users/example/.claude/rules/scs/common/coding-style.md",
  "ownership": "managed",
...
Install-state is the durable contract that SCS 1.x is missing.

Suggested path conventions:

- Claude target:
  `~/.claude/scs/install-state.json`
- Cursor target:
  `./.cursor/scs-install-state.json`
- Antigravity target:
  `./.agent/scs-install-state.json`
- future Codex target:
  `~/.codex/scs-install-state.json`
...
- enough detail for uninstall to remove only SCS-managed outputs
...
- `scs list-installed` routes to `node scripts/list-installed.js`
- `scs uninstall` routes to `node scripts/uninstall.js`
- `scs doctor` routes to `node scripts/doctor.js`
- `scs repair` routes to `node scripts/repair.js`
...
- load install-state
- remove only SCS-managed destinations recorded in state
...
SCS 2.0 should translate legacy language arguments into a compatibility request.
...
SCS 2.0 should improve this carefully.
...
3. define `scs.install.v1` state schema
...
4. decide when `scs-install` should become a thin alias for `scs install`
...
3. reduce package shipping surface to the module graph instead of broad folders
4. decide whether `scs-install` remains separate or becomes `scs install`
5. add tests that lock down:
   - target-specific merge/remove behavior
   - repair and uninstall safety for non-copy operations
   - unified `scs` CLI routing and compatibility guarantees
...
That is the shortest path from SCS 1.x installer sprawl to an SCS 2.0
