# SCS Selective Install Design

## Purpose

This document defines the user-facing selective-install design for SCS.
...
- how users choose SCS components
- what the CLI should feel like
- what config file should exist
- how installation should behave across harness targets
- how the design maps onto the current SCS codebase without requiring a rewrite

## Problem

Today SCS still feels like a large payload installer even though the repo now
...
The selective-install system should make SCS feel composable instead of
all-or-nothing.
...
1. Let users install a small default SCS footprint quickly.
...
5. Preserve backward compatibility with the current `scs-install typescript`
   style during rollout.

## Non-Goals

- packaging SCS into multiple npm packages in the first phase
...
A user should be able to get a useful SCS install with one command:

```bash
scs install --target claude --profile core
```
...
```bash
scs install --target cursor --profile developer --with lang:typescript --with framework:nextjs --dry-run
```
...
```bash
scs install --config scs-install.json
```
...
These are the default SCS building blocks:
...
Capability packs are cross-cutting SCS feature bundles.
...
- `core`
  minimal baseline, safe default for most users trying SCS
- `developer`
...
```bash
scs install --target claude --profile developer --with lang:typescript --with framework:nextjs --without capability:orchestration
```
...
```bash
scs install [--target <target>] [--profile <name>] [--with <component>]... [--without <component>]... [--config <path>] [--dry-run] [--json]
```
...
```bash
scs install --target claude --profile core
scs install --target cursor --profile developer --with lang:typescript --with framework:nextjs
scs install --target antigravity --with capability:security --with lang:python
scs install --config scs-install.json
```
...
```bash
scs plan [same selection flags as install]
```
...
```bash
scs-install typescript
scs-install --target cursor typescript
scs typescript
```
...
```json
{
  "$schema": "./schemas/scs-install-config.schema.json",
  "version": 1,
  "target": "cursor",
  "profile": "developer",
  "include": [
...
The commands differ in action, not in how SCS understands the selected install.
...
- `scs-install.json` config file parsing
...
## Mapping To Current SCS Manifests
...
That is the shortest path from the current SCS codebase to a real selective
install experience that feels like SCS 2.0 instead of a large legacy installer.
