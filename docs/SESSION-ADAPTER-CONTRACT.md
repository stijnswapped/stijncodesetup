...
This document defines the canonical SCS session snapshot contract for
`scs.session.v1`.
...
SCS has multiple session sources:
...
| Field | Type | Notes |
| --- | --- | --- |
| `schemaVersion` | string | MUST be exactly `scs.session.v1` for this contract |
...
Every SCS session adapter MUST:

1. Emit `schemaVersion: "scs.session.v1"` exactly.
...
- rely only on documented fields for `scs.session.v1`
