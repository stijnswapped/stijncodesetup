...
- Use SCS's `strategic-compact` guidance instead of forcing a lower auto-compact threshold.
...
- Advanced users sometimes script a local `/reload` command around `kill -HUP $PPID`, but SCS does not ship that because it is shell-dependent and not universally reliable.
