...
- SCS-Tools now consumes that fleet primitive in hosted security review:
...
default. SCS-Tools follow-up routing now consumes fleet summaries and surfaces
...
AgentShield is already connected conceptually to the SCS Tools GitHub App.
...
- Billing readiness checks shared with SCS-Tools so payment state never changes
...
- It aligns AgentShield with SCS's harness-agnostic positioning.
...
Connect AgentShield findings to SCS-Tools follow-up routing.

Artifacts:

- Finding fingerprints compatible with SCS-Tools issue caps.
...
- SCS-Tools can consume the finding fingerprints or backlog export without
