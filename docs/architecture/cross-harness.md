...
SCS is the reusable workflow layer. Harnesses are execution surfaces.
...
[SCS Platform Value Loop](platform-value-loop.md).
...
| Sessions | `scs2/`, session adapters, orchestration scripts | TUI/daemon, tmux/worktree orchestration, harness-specific runners | Alpha |

## What Travels Unchanged

`SKILL.md` is the most portable unit.

A good SCS skill should:
...
- OpenCode has a plugin/event system that can reuse SCS hook logic through an adapter layer.
- Cursor uses its own rule and hook layout, so SCS maintains translated surfaces under `.cursor/`.
...
Hermes is not the public SCS runtime.

Hermes is an operator shell that can consume SCS assets:

- import selected SCS skills into a Hermes skills directory
- use SCS MCP conventions for tool access
- route chat, CLI, cron, and handoff workflows through reusable SCS patterns
- distill repeated local operator work back into sanitized SCS skills
...
OpenCode gets the skill through the OpenCode package/plugin surface. Event handling can reuse SCS hook logic through the adapter layer, while the skill text stays unchanged.
...
- the full platform loop where external products contribute skill packs,
  gated APIs, evals, and case studies back into SCS

## Rule For New Work

When adding a workflow, put the durable behavior in SCS first.
