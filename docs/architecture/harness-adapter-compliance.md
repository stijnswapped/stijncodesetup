...
This matrix is the public onramp for teams that want to use SCS across more
...
SCS's durable units stay in shared sources:
...
| State | Meaning |
| --- | --- |
| Native | SCS can install or verify the surface directly for this harness. |
| Adapter-backed | SCS has a thin adapter, plugin, or package surface, but parity differs by harness. |
| Instruction-backed | SCS can provide the guidance and files, but the harness does not expose the runtime hook/session surface SCS needs for enforcement. |
| Reference-only | The tool is useful as a design pressure or external runtime, but SCS does not yet ship a direct installer or adapter for it. |
...
| Orca | Reference-only | worktree lifecycle; review state; notification; provider-identity design pressure | No SCS installer or direct adapter today | Use as a comparison target for worktree/session state requirements | `npm run observability:ready` | Do not import product-specific assumptions; convert lessons into SCS event fields. |
| Superset | Reference-only | workspace presets; parallel-agent review loops; worktree isolation design pressure | No SCS installer or direct adapter today | Use as a comparison target for workspace preset taxonomy | `npm run observability:ready` | Keep SCS portable; do not require a desktop workspace to get basic value. |
| Ghast | Reference-only | terminal-native pane grouping; cwd grouping; search; notifications | No SCS installer or direct adapter today | Use as a comparison target for terminal-first session grouping | `node scripts/session-inspect.js --list-adapters` | Preserve terminal ergonomics before adding visual UI assumptions. |
...
Use this sequence before asking SCS to make a team or repo setup more
autonomous:
...
node scripts/loop-status.js --json --write-dir .scs/loop-status
```
...
- Treat reference-only tools as design pressure until SCS has a direct adapter.
