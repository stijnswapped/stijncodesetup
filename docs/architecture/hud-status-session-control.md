...
This contract defines the portable status payload SCS uses for local operator
...
Every status payload uses `schema_version: "scs.hud-status.v1"` and keeps these
...
| `sessionControls` | Supported operator actions for the current target | SCS CLI, dmux, git/GitHub |
...
- `scs status --json` exposes readiness, active sessions, skill runs, install
  health, governance, and linked work items from the SQLite state store.
- `scs loop-status --json --write-dir <dir>` writes live transcript snapshots
  and attention signals for long-running loops.
- `scs session-inspect <target> --write <path>` emits canonical session
  snapshots from dmux and Claude-history adapters.
- `scripts/hooks/scs-statusline.js` renders compact model, task, cost, tool,
  file, duration, directory, and context pressure signals inside Claude Code.

The `scs.hud-status.v1` payload is the common outer contract these surfaces can
project into before SCS grows a dedicated full-screen HUD.
