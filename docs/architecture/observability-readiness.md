# SCS 2.0 Observability Readiness

SCS 2.0 should be observable before it becomes more autonomous. The local
...
- Tool activity: `scripts/hooks/session-activity-tracker.js` records local
  `tool-usage.jsonl` events that SCS2 can sync.
- Risk ledger: `scs2/src/observability/mod.rs` scores tool calls and stores a
...
SCS's answer is not a hosted analytics dependency by default. The first
...
2. Run `npm run harness:audit -- --format json` for the broader harness
   scorecard.
3. Run `node scripts/loop-status.js --json --write-dir .scs/loop-status`
...
7. Use SCS2 tool logs for risky operations, conflict analysis, and handoff
...
The end-state is practical: before asking SCS to run larger multi-agent loops,
