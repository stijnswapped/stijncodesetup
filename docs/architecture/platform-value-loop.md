# SCS Platform Value Loop

SCS 2.0 is moving from a portable harness layer toward a full operator
...
2. Dedicated SCS agent: an agent that directly operates over SCS assets instead
...
```text
SCS can be used full-stack as a meta-harness + agent + control pane, or
...
- product-specific integrations that can become reusable SCS skills.
...
External products can build on SCS without becoming SCS-branded products. The
contract is:

| Layer | Product contributes | SCS receives |
| --- | --- | --- |
...
| Case study | A real product workflow that works through SCS | Distribution, sponsors, Pro interest, consulting demand |
...
Ito is a separate prediction-market basket product. SCS can still distribute
Ito-shaped skills because the skill workflows are useful without making SCS
...
The boundary is strict: public SCS skills do not place trades, do not provide investment advice, do not expose private strategy, and do not merge SCS Tools billing with Ito billing.

## Value Loop

The platform loop should be explicit:

1. A product team builds a useful workflow as an SCS skill pack.
...
5. Sanitized patterns become better SCS skills, evals, gates, or docs.
6. SCS gains distribution, maintainers, sponsors, Pro interest, and consulting leads.
...
integration becomes another reason to install SCS, and every serious SCS user
