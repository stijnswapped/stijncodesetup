...
```bash
# Weekly downloads
curl -s https://api.npmjs.org/downloads/point/last-week/stijncodesetup
curl -s https://api.npmjs.org/downloads/point/last-week/scs-agentshield

# Last 30 days
curl -s https://api.npmjs.org/downloads/point/last-month/stijncodesetup
curl -s https://api.npmjs.org/downloads/point/last-month/scs-agentshield
```

### GitHub repository adoption

```bash
gh api repos/StijnCodeSetup/StijnCodeSetup \
...
```bash
gh api repos/StijnCodeSetup/StijnCodeSetup/traffic/views
gh api repos/StijnCodeSetup/StijnCodeSetup/traffic/clones
```
...
- [SCS Tools Marketplace](https://github.com/marketplace/scs-tools)
...
> SCS is now positioned as an agent harness performance system, not a config repo.
