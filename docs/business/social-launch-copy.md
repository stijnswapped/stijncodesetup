...
SCS 2.0 is the harness-native operator system for agentic work: skills, hooks,
...
```text
SCS v2.0.0-rc.1 keeps the public surface honest:
- reusable SCS substrate in repo
...
In SCS we turned this into production checks via:
- /harness-audit
- /quality-gate
...
SCS v2.0.0-rc.1 pushes that further: reusable skills, thin harness adapters, and Hermes as the operator shell on top.
```
...
SCS 2.0 is the harness-native operator system for agentic work. The same reusable layer now reaches Claude Code, Codex, OpenCode, Cursor, Gemini, Zed, GitHub Copilot workflows, and terminal-only operator lanes.

This release-candidate surface includes:
- sanitized Hermes setup documentation
- release notes and launch collateral
- cross-harness architecture notes
- Hermes import guidance for turning local operator patterns into public SCS skills
