...
SCS does not treat these surfaces as interchangeable. The goal is to put each capability in the narrowest surface that preserves correctness, keeps token cost under control, and does not create unnecessary runtime or supply-chain drag.
...
- prefer SCS-native packaging over introducing another third-party dependency

Do not normalize external plugin or package dependencies as first-class SCS surfaces unless the capability is clearly worth the maintenance, security, and install burden.
...
- repackage it as an SCS-native rule, skill, script, or MCP surface
- rename it if the functionality has been materially expanded or reshaped for SCS
