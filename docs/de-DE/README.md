...
![SCS - das Harness-native Operator-System für agentische Arbeit](../../assets/hero.png)
...
SCS v2.0.0-rc.1 ergänzt diese wiederverwendbare Schicht um die öffentliche Hermes-Operator-Story: Beginne mit dem [Hermes-Setup-Leitfaden](../../docs/HERMES-SETUP.md), prüfe anschließend die [rc.1-Release-Notes](../../docs/releases/2.0.0-rc.1/release-notes.md) und die [Cross-Harness-Architektur](../../docs/architecture/cross-harness.md).
...
<td width="25%" align="center">
  <a href="https://scs.tools/pricing">
    <strong> SCS Pro</strong><br />
    <sub>Private Repos · GitHub App · 19 $/Platz/Monat</sub>
  </a>
</td>
<td width="25%" align="center">
  <a href="https://github.com/sponsors/StijnCodeSetup">
...
<td width="25%" align="center">
  <a href="https://github.com/apps/scs-tools">
...
<sub>**OSS bleibt kostenlos.** Dieses Repo ist für immer MIT-lizenziert. SCS Pro ist die gehostete GitHub App für private Repos. <a href="https://github.com/sponsors/StijnCodeSetup">Sponsoren</a> und <a href="https://scs.tools/pricing">Pro-Abonnenten</a> finanzieren die Arbeit — deshalb liefert ein einzelner Maintainer wöchentlich über 7 Harnesses hinweg aus.</sub>
...
<tr>
<td width="33%">
<a href="https://x.com/StijnCodeSetup/status/2012378465664745795">
<img src="../../assets/images/guides/shorthand-guide.png" alt="The Shorthand Guide to StijnCodeSetup" />
</a>
</td>
<td width="33%">
<a href="https://x.com/StijnCodeSetup/status/2014040193557471352">
<img src="../../assets/images/guides/longform-guide.png" alt="The Longform Guide to StijnCodeSetup" />
</a>
</td>
<td width="33%">
<a href="https://x.com/StijnCodeSetup/status/2033263813387223421">
...
### v2.0.0-rc.1 — Oberflächen-Refresh, Operator-Workflows und SCS 2.0 Alpha (April 2026)

- **Dashboard-GUI** — Neue Tkinter-basierte Desktop-Anwendung (`scs_dashboard.py` oder `npm run dashboard`) mit Umschalter für dunkles/helles Theme, Schriftanpassung und Projektlogo in Kopfzeile und Taskleiste.
...
- **Erweiterung von Operator- und Outbound-Workflows** — `brand-voice`, `social-graph-ranker`, `connections-optimizer`, `customer-billing-ops`, `scs-tools-cost-audit`, `google-workspace-ops`, `project-flow-ops` und `workspace-surface-audit` runden die Operator-Spur ab.
...
- **SCS 2.0 Alpha ist im Tree** — der Rust-Control-Plane-Prototyp in `scs2/` baut jetzt lokal und stellt die Befehle `dashboard`, `start`, `sessions`, `status`, `stop`, `resume` und `daemon` bereit. Er ist als Alpha nutzbar, aber noch kein allgemeines Release.
- **Operator-Status-Snapshots** — `scs status --markdown --write status.md` verwandelt den lokalen State Store in eine portable Übergabe, die Bereitschaft, aktive Sessions, Skill-Run-Gesundheit, Install-Gesundheit, ausstehende Governance-Events und verknüpfte Arbeitselemente aus Linear/GitHub/Handovers abdeckt. Nutze `scs work-items upsert ...` für manuelle Einträge, `scs work-items sync-github --repo owner/repo` für den Queue-Status von PRs/Issues und `scs status --exit-code`, um Automatisierung scheitern zu lassen, wenn die Bereitschaft Aufmerksamkeit erfordert.
- **Ökosystem-Härtung** — AgentShield, SCS-Tools-Kostenkontrollen, Arbeiten am Billing-Portal und Website-Refreshes werden weiterhin rund um das Kern-Plugin ausgeliefert, statt in separate Silos abzudriften.
...
- **Harness-First-Release** — SCS ist nun ausdrücklich als Performance-System für Agent-Harnesses positioniert, nicht nur als Config-Paket.
...
- **Hook-Laufzeitsteuerung** — `SCS_HOOK_PROFILE=minimal|standard|strict` und `SCS_DISABLED_HOOKS=...` für Laufzeit-Gating ohne Bearbeitung von Hook-Dateien.
...
- **GitHub Marketplace** — SCS-Tools-GitHub-App live unter [github.com/marketplace/scs-tools](https://github.com/marketplace/scs-tools) mit Free-/Pro-/Enterprise-Stufen
...
- **Inhaltsverlust beim Instinct-Import behoben** — `parse_instinct_file()` verwarf während `/instinct-import` stillschweigend sämtlichen Inhalt nach dem Frontmatter (Abschnitte Action, Evidence, Examples). ([#148](https://github.com/StijnCodeSetup/StijnCodeSetup/issues/148), [#161](https://github.com/StijnCodeSetup/StijnCodeSetup/pull/161))

### v1.4.0 — Mehrsprachige Rules, Installationsassistent & PM2 (Februar 2026)

- **Interaktiver Installationsassistent** — Der neue `configure-scs`-Skill bietet ein geführtes Setup mit Merge-/Überschreiben-Erkennung
...
Den vollständigen Changelog findest du unter [Releases](https://github.com/StijnCodeSetup/StijnCodeSetup/releases).
...
- **Stapele Installationsmethoden nicht.** Das häufigste kaputte Setup ist: zuerst `/plugin install`, danach `install.sh --profile full` oder `npx scs-install --profile full`.

Falls du bereits mehrere Installationen übereinandergelegt hast und Dinge doppelt aussehen, springe direkt zu [SCS zurücksetzen / deinstallieren](#scs-zurücksetzen--deinstallieren).

### Low-Context-/No-Hooks-Pfad

Falls sich Hooks zu global anfühlen oder du nur SCS's Rules, Agents, Commands und Kern-Workflow-Skills willst, überspringe das Plugin und nutze das minimale manuelle Profil:
...
Falls du nicht sicher bist, welches SCS-Profil oder welche Komponente du installieren sollst, frage den mitgelieferten Advisor aus jedem beliebigen Projekt:
...
```bash
npx scs consult "mlops training model deployment" --target claude
npx scs install --profile minimal --target claude --with capability:machine-learning
```
...
SCS hat jetzt drei öffentliche Bezeichner, und sie sind nicht austauschbar:

- GitHub-Quell-Repo: `StijnCodeSetup/StijnCodeSetup`
- Claude-Marketplace-/Plugin-Bezeichner: `scs@scs`
- npm-Paket: `stijncodesetup`

Das ist beabsichtigt. Anthropic-Marketplace-/Plugin-Installationen werden über einen kanonischen Plugin-Bezeichner gekeyt, daher verwendet SCS `scs@scs`, um Tool-Namen und Slash-Command-Namespaces kurz genug für strenge Desktop-/API-Validatoren zu halten. Ältere Beiträge zeigen möglicherweise noch den früheren langen Marketplace-Bezeichner; behandle diesen lediglich als Legacy-Alias. Das npm-Paket blieb davon getrennt bei `stijncodesetup`, daher verwenden npm-Installationen und Marketplace-Installationen absichtlich unterschiedliche Namen.
...
>
> Falls du SCS bereits über `/plugin install` installiert hast, **führe danach nicht `./install.sh --profile full`, `.\install.ps1 --profile full` oder `npx scs-install --profile full` aus**. Das Plugin lädt SCS-Skills, -Commands und -Hooks bereits. Wird der vollständige Installer nach einer Plugin-Installation ausgeführt, kopiert er dieselben Oberflächen in deine Benutzerverzeichnisse und kann doppelte Skills sowie doppeltes Laufzeitverhalten erzeugen.
>
> Kopiere für Plugin-Installationen manuell nur die `rules/`-Verzeichnisse, die du willst, nach `~/.claude/rules/scs/`. Beginne mit `rules/common` plus einem Sprach- oder Framework-Paket, das du tatsächlich verwendest. Kopiere nicht jedes Rules-Verzeichnis, es sei denn, du willst diesen gesamten Kontext ausdrücklich in Claude haben.
>
> Verwende den vollständigen Installer nur dann, wenn du eine vollständig manuelle SCS-Installation statt des Plugin-Pfads durchführst.
>
> Falls dein lokales Claude-Setup gelöscht oder zurückgesetzt wurde, bedeutet das nicht, dass du SCS erneut kaufen musst. Beginne mit `node scripts/scs.js list-installed`, führe dann `node scripts/scs.js doctor` und `node scripts/scs.js repair` aus, bevor du irgendetwas neu installierst. Das stellt SCS-verwaltete Dateien üblicherweise wieder her, ohne dein Setup neu aufzubauen. Falls das Problem im Konto- oder Marketplace-Zugriff für SCS Tools liegt, behandle die Konto-/Abrechnungswiederherstellung separat.
...
# Plugin-Installationspfad: nur SCS-Rules in einen SCS-eigenen Namespace kopieren
mkdir -p ~/.claude/rules/scs
cp -R rules/common ~/.claude/rules/scs/
cp -R rules/typescript ~/.claude/rules/scs/

# Vollständig manueller SCS-Installationspfad (nutze diesen statt /plugin install)
...
### SCS zurücksetzen / deinstallieren

Falls sich SCS doppelt, aufdringlich oder kaputt anfühlt, installiere es nicht weiter über sich selbst.

- **Plugin-Pfad:** Entferne das Plugin aus Claude Code, lösche dann die konkreten Rule-Ordner, die du manuell unter `~/.claude/rules/scs/` kopiert hast.
...
Entferne anschließend SCS-verwaltete Dateien:
...
SCS entfernt nur Dateien, die in seinem Install-State erfasst sind. Es löscht keine fremden Dateien, die es nicht installiert hat.

Falls du Methoden gestapelt hast, räume in dieser Reihenfolge auf:

1. Entferne die Claude-Code-Plugin-Installation.
2. Führe den SCS-Uninstall-Befehl vom Repo-Root aus, um über den Install-State verwaltete Dateien zu entfernen.
...
# Bestehende Slash-artige Command-Namen funktionieren weiterhin, während SCS von commands/ wegmigriert.

# Die Plugin-Installation verwendet die kanonische Namespace-Form
/scs:plan "Benutzerauthentifizierung hinzufügen"
...
Starte das Desktop-Dashboard, um SCS-Komponenten visuell zu erkunden:

```bash
npm run dashboard
# oder
python3 ./scs_dashboard.py
```
...
export SCS_HOOK_PROFILE=standard

# Komma-getrennte Hook-IDs, die deaktiviert werden sollen
export SCS_DISABLED_HOOKS="pre:bash:tmux-reminder,post:edit:typecheck"

# Zusatzkontext bei SessionStart begrenzen (Standard: 8000 Zeichen)
export SCS_SESSION_START_MAX_CHARS=4000

# Zusatzkontext bei SessionStart für Low-Context-/lokale-Modell-Setups vollständig deaktivieren
export SCS_SESSION_START_CONTEXT=off

# Kontext-/Scope-/Loop-Warnungen behalten, aber API-Rate-Kostenschätzungen unterdrücken
export SCS_CONTEXT_MONITOR_COST_WARNINGS=off
```

Windows PowerShell:

```powershell
[Environment]::SetEnvironmentVariable('SCS_CONTEXT_MONITOR_COST_WARNINGS', 'off', 'User')
```
...
|   |-- configure-scs/              # Interaktiver Installationsassistent (NEU)
...
|
|-- rules/            # Stets zu befolgende Richtlinien (nach ~/.claude/rules/scs/ kopieren)
...
|
|-- scs_dashboard.py  # Desktop-GUI-Dashboard (Tkinter)
|
...
[GitHub App installieren](https://github.com/apps/scs-skill-creator) | [scs.tools](https://scs.tools)
...
Verwende `/security-scan` in Claude Code, um es auszuführen, oder füge es per [GitHub Action](https://github.com/StijnCodeSetup/scs-agentshield) zur CI hinzu.

[GitHub](https://github.com/StijnCodeSetup/scs-agentshield) | [npm](https://www.npmjs.com/package/scs-agentshield)
...
**Historie:** Dies hat in diesem Repo wiederholte Fix-/Revert-Zyklen verursacht ([#29](https://github.com/StijnCodeSetup/StijnCodeSetup/issues/29), [#52](https://github.com/StijnCodeSetup/StijnCodeSetup/issues/52), [#103](https://github.com/StijnCodeSetup/StijnCodeSetup/issues/103)). Das Verhalten änderte sich zwischen Claude-Code-Versionen, was zu Verwirrung führte. Wir haben jetzt einen Regressionstest, der verhindert, dass dies erneut eingeführt wird.
...
```json
{
  "extraKnownMarketplaces": {
    "scs": {
      "source": {
        "source": "github",
        "repo": "StijnCodeSetup/StijnCodeSetup"
      }
    }
  },
  "enabledPlugins": {
...
> mkdir -p ~/.claude/rules/scs
> cp -r rules/common ~/.claude/rules/scs/
> cp -r rules/typescript ~/.claude/rules/scs/   # wähle deinen Stack
> cp -r rules/python ~/.claude/rules/scs/
> cp -r rules/golang ~/.claude/rules/scs/
> cp -r rules/php ~/.claude/rules/scs/
>
...
cp -r rules/golang ~/.claude/rules/scs/
cp -r rules/php ~/.claude/rules/scs/
cp -r rules/arkts ~/.claude/rules/scs/
...
# Manuelle Installationen nicht unter ~/.claude/skills/scs/ verschachteln.
...
Falls du SCS über `/plugin install` installiert hast, kopiere diese Hooks nicht in `settings.json`. Claude Code v2.1+ lädt Plugin-`hooks/hooks.json` bereits automatisch, und sie in `settings.json` zu duplizieren, verursacht doppelte Ausführung und Cross-Platform-Hook-Konflikte.

Windows-Hinweis: Das Claude-Konfigurationsverzeichnis ist `%USERPROFILE%\\.claude`, nicht `~/claude`.

#### MCPs konfigurieren

Claude-Plugin-Installationen aktivieren die mitgelieferten MCP-Server-Definitionen von SCS absichtlich nicht automatisch. Das vermeidet überlange Plugin-MCP-Tool-Namen auf strengen Drittanbieter-Gateways und hält gleichzeitig das manuelle MCP-Setup verfügbar.
...
Falls du bereits eigene Kopien der von SCS mitgelieferten MCPs betreibst, setze:

```bash
export SCS_DISABLED_MCPS="github,context7,exa,playwright,sequential-thinking,memory"
```

SCS-verwaltete Install- und Codex-Sync-Flows überspringen oder entfernen diese mitgelieferten Server, statt Duplikate erneut hinzuzufügen. `SCS_DISABLED_MCPS` ist ein SCS-Install-/Sync-Filter, kein Live-Toggle für Claude Code.
...
|--------------|-----------------|------------|
| Ein neues Feature planen | `/scs:plan "Add auth"` | planner |
| Systemarchitektur entwerfen | `/scs:plan` + architect-Agent | architect |
...
```
/scs:plan "Add user authentication with OAuth"
...
Das ist das häufigste Problem. **Füge KEIN `"hooks"`-Feld zu `.claude-plugin/plugin.json` hinzu.** Claude Code v2.1+ lädt `hooks/hooks.json` aus installierten Plugins automatisch. Es explizit zu deklarieren, verursacht Fehler durch Duplikaterkennung. Siehe [#29](https://github.com/StijnCodeSetup/StijnCodeSetup/issues/29), [#52](https://github.com/StijnCodeSetup/StijnCodeSetup/issues/52), [#103](https://github.com/StijnCodeSetup/StijnCodeSetup/issues/103).
</details>

<details>
<summary><b>Kann ich SCS mit Claude Code an einem benutzerdefinierten API-Endpoint oder Modell-Gateway verwenden?</b></summary>

Ja. SCS hat keine Anthropic-gehosteten Transporteinstellungen fest verdrahtet. Es läuft lokal über die normale CLI-/Plugin-Oberfläche von Claude Code, daher funktioniert es mit:
...
Falls dein Gateway Modellnamen umbildet, konfiguriere das in Claude Code statt in SCS. SCS's Hooks, Skills, Commands und Rules sind modellanbieter-agnostisch, sobald die `claude`-CLI bereits funktioniert.
...
- **Codex**: Erstklassige Unterstützung sowohl für die macOS-App als auch die CLI, mit Adapter-Drift-Guards und SessionStart-Fallback. Siehe PR [#257](https://github.com/StijnCodeSetup/StijnCodeSetup/pull/257).
...
Diese werden nicht mit SCS mitgeliefert und nicht von diesem Repo auditiert, aber sie sind wissenswert, falls du das breitere Claude-Code-Skills-Ökosystem erkundest:
...
SCS bietet Cursor-IDE-Unterstützung mit Hooks, Rules, Agents, Skills, Commands und MCP-Konfigurationen, die an Cursors Projektlayout angepasst sind.
...
| Agents | 48 | `.cursor/agents/scs-*.md` bei Installation; präfixiert, um Kollisionen mit Benutzer- oder Marketplace-Agents zu vermeiden |
...
SCS installiert keine Root-`AGENTS.md` in `.cursor/`. Cursor behandelt verschachtelte `AGENTS.md`-Dateien als Verzeichniskontext, daher würde das Kopieren von SCS's Repo-Identität in ein Host-Projekt dieses Projekt verunreinigen.

Das Cursor-native Ladeverhalten kann je nach Cursor-Build variieren. SCS installiert Agents als `.cursor/agents/scs-*.md`; falls dein Cursor-Build keine Projekt-Agents bereitstellt, funktionieren diese Dateien weiterhin als explizite Referenzdefinitionen statt als versteckter globaler Prompt-Kontext.
...
SCS bietet **erstklassige Codex-Unterstützung** sowohl für die macOS-App als auch die CLI, mit einer Referenzkonfiguration, einem Codex-spezifischen AGENTS.md-Zusatz und gemeinsam genutzten Skills.
...
# Automatisches Setup: SCS-Assets (AGENTS.md, Skills, MCP-Server) nach ~/.codex synchronisieren
npm install && bash scripts/sync-scs-to-codex.sh
# oder: pnpm install && bash scripts/sync-scs-to-codex.sh
# oder: yarn install && bash scripts/sync-scs-to-codex.sh
# oder: bun install && bash scripts/sync-scs-to-codex.sh

# Oder manuell: die Referenzkonfiguration in dein Home-Verzeichnis kopieren
cp .codex/config.toml ~/.codex/config.toml
```

Das Sync-Skript merged SCS-MCP-Server sicher in deine bestehende `~/.codex/config.toml` mit einer **add-only**-Strategie — es entfernt oder verändert deine bestehenden Server nie. Führe es mit `--dry-run` aus, um Änderungen in der Vorschau zu sehen, oder mit `--update-mcp`, um ein erzwungenes Refresh der SCS-Server auf die neueste empfohlene Konfiguration zu erzwingen.

Für Context7 verwendet SCS den kanonischen Codex-Abschnittsnamen `[mcp_servers.context7]`, startet aber weiterhin das Paket `@upstash/context7-mcp`. Falls du bereits einen veralteten `[mcp_servers.context7-mcp]`-Eintrag hast, migriert `--update-mcp` ihn auf den kanonischen Abschnittsnamen.
...
| stijncodesetup | Entwicklungskonventionen und -Patterns für das Projekt |
...
Codex bietet **noch keine Claude-artige Parität bei der Hook-Ausführung**. Die SCS-Durchsetzung dort ist instruction-basiert über `AGENTS.md`, optionale `model_instructions_file`-Overrides sowie Sandbox-/Approval-Einstellungen.
...
SCS liefert drei Beispiel-Rollenkonfigurationen aus:
...
SCS bietet Zed-Projektunterstützung über einen konservativen `.zed`-Adapter für projektlokale Einstellungen, abgeflachte Rules, Agents, Commands und Skills.
...
Der Adapter schreibt SCS-verwaltete Dateien unter `.zed/` und hält BYOK-/OpenRouter-Credentials aus dem Repo heraus. Konfiguriere das Zed-Konto oder API-Keys über Zeds eigene Einstellungs-UI oder deine lokalen Benutzereinstellungen.

---

## OpenCode-Unterstützung

SCS bietet **vollständige OpenCode-Unterstützung** einschließlich Plugins und Hooks.
...
Dieser npm-Plugin-Eintrag aktiviert SCS's veröffentlichtes OpenCode-Plugin-Modul (Hooks/Events und Plugin-Tools).
Er fügt **nicht** automatisch SCS's vollständigen Command-/Agent-/Instruction-Katalog zu deiner Projektkonfiguration hinzu.

Für das vollständige SCS-OpenCode-Setup entweder:
...
SCS bietet **GitHub-Copilot-Unterstützung** für VS Code über das native Instruction- und Prompt-Datei-System von Copilot Chat — kein zusätzliches Tooling erforderlich.
...
- **`.github/copilot-instructions.md`** — Instructions auf Repository-Ebene, die in jede Copilot-Chat-Anfrage injiziert werden. Enthält SCS's Kern-Coding-Standards, Sicherheits-Checkliste, Testanforderungen und Git-Workflow.
- **`.github/prompts/*.prompt.md`** — wiederverwendbare Prompt-Dateien, die Nutzer bei Bedarf aufrufen. Jeder Prompt führt Copilot durch einen bestimmten SCS-Workflow wie Planung, TDD, Security-Review, Build-Fix oder Refactor.
...
| SCS-Feature | Copilot-Entsprechung |
|-------------|-------------------|
...
GitHub Copilot hat kein Hook-System und keine Subagent-API, daher sind SCS's Hook-Automatisierungen (Auto-Formatierung, TypeScript-Prüfung, Session-Persistenz, Dev-Server-Guard) sowie die Agent-Delegation nicht verfügbar. Die Instruction- und Prompt-Schicht bringt dennoch die vollständige SCS-Coding-Philosophie — Standards, Sicherheit, TDD und Workflow — in jede Copilot-Chat-Session.

---

## Cross-Tool-Feature-Parität

SCS ist das **erste Plugin, das jedes große KI-Coding-Tool ausreizt**. So vergleicht sich jeder Harness:
...
| `SCS_CONTEXT_MONITOR_COST_WARNINGS` | on | **off für Abonnement-Nutzer** | Unterdrückt agentenseitige API-Rate-Schätzwarnungen, behält aber Kontext-/Scope-/Loop-Warnungen |
...
Falls du ein Claude-Abonnement nutzt und die API-Rate-Schätzungen des Kontext-Monitors nicht nützlich sind, setze `SCS_CONTEXT_MONITOR_COST_WARNINGS=off`. Das unterdrückt nur die agentenseitigen Kostenwarnungen; es deaktiviert keine Warnungen zu Kontexterschöpfung, Scope oder Loops.
...
- Verwende `SCS_DISABLED_MCPS` nur, um SCS-generierte MCP-Konfigurationen während der Install-/Sync-Flows zu filtern
...
Projekte, die auf SCS aufbauen oder davon inspiriert sind:
...
Etwas mit SCS gebaut? Öffne einen PR, um es hier hinzuzufügen.
...
[**Sponsor werden**](https://github.com/sponsors/StijnCodeSetup) | [Sponsor-Stufen](../../SPONSORS.md) | [Sponsoring-Programm](../../SPONSORING.md)

---

## Star-Verlauf

[![Star History Chart](https://api.star-history.com/svg?repos=StijnCodeSetup/StijnCodeSetup&type=Date)](https://star-history.com/#StijnCodeSetup/StijnCodeSetup&Date)

---

## Links

- **Kurzleitfaden (Hier starten):** [The Shorthand Guide to StijnCodeSetup](https://x.com/StijnCodeSetup/status/2012378465664745795)
- **Langleitfaden (fortgeschritten):** [The Longform Guide to StijnCodeSetup](https://x.com/StijnCodeSetup/status/2014040193557471352)
- **Security-Leitfaden:** [Security-Leitfaden](../../the-security-guide.md) | [Thread](https://x.com/StijnCodeSetup/status/2033263813387223421)
- **Folgen:** [@StijnCodeSetup](https://x.com/StijnCodeSetup)
