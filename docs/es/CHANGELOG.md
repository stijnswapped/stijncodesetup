...
- Añade la superficie pública del release candidate de SCS 2.0 para la historia del operador Hermes.
- Documenta SCS como el sustrato reutilizable cross-harness para Claude Code, Codex, Cursor, OpenCode y Gemini.
...
- Añadido `docs/architecture/cross-harness.md` y cobertura de regresión para el límite SCS/Hermes.
- Mantenido el versionado de `scs2/` independiente por ahora; sigue siendo un scaffold alfa del plano de control a menos que ingeniería de releases decida lo contrario.

### Notas

- Este es un release candidate, no una declaración GA para el roadmap completo del plano de control de SCS 2.0.
...
- El binario del plano de control alfa de SCS 2.0 ya compila localmente desde `scs2/` y expone la primera superficie de CLI/TUI utilizable.
...
- `cargo build --manifest-path scs2/Cargo.toml` pasa en la línea base del repositorio.
- `scs-tui` actualmente expone `dashboard`, `start`, `sessions`, `status`, `stop`, `resume` y `daemon`.
...
- Este lanzamiento es una corrección de la superficie del repo y una sincronización del ecosistema, no una declaración de que el roadmap completo de SCS 2.0 está completo.
...
- `stijncodesetup` — skill autorreferencial de SCS (#335)
...
- Añadidos controles `SCS_HOOK_PROFILE` y `SCS_DISABLED_HOOKS`.
