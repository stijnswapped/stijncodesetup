...
SCS v2.0.0-rc.1 añade la historia pública del operador Hermes sobre esa capa reutilizable: comienza con la [guía de configuración de Hermes](../HERMES-SETUP.md), luego revisa las [notas de la versión rc.1](../releases/2.0.0-rc.1/release-notes.md) y la [arquitectura multi-harness](../architecture/cross-harness.md).
...
<sub>**El OSS es gratis para siempre.** Este repositorio tiene licencia MIT permanente. SCS Pro es la GitHub App alojada para repositorios privados. Los <a href="https://github.com/sponsors/StijnCodeSetup">patrocinadores</a> y los <a href="https://scs.tools/pricing">suscriptores Pro</a> financian el trabajo — por eso un solo mantenedor publica semanalmente en 7 harnesses.</sub>
...
<img src="../../assets/images/guides/shorthand-guide.png" alt="La Guía Resumida de SCS" />
</a>
</td>
<td width="33%">
<a href="https://x.com/StijnCodeSetup/status/2014040193557471352">
<img src="../../assets/images/guides/longform-guide.png" alt="La Guía Extensa de SCS" />
</a>
</td>
<td width="33%">
...
### v2.0.0-rc.1 — Actualización de Superficie, Flujos de Trabajo de Operador y Alpha de SCS 2.0 (Abr 2026)

- **Dashboard GUI** — Nueva aplicación de escritorio basada en Tkinter (`scs_dashboard.py` o `npm run dashboard`) con alternancia de tema oscuro/claro, personalización de fuente y logo del proyecto en el encabezado y la barra de tareas.
...
- **Expansión de flujos de trabajo de operador y salida** — `brand-voice`, `social-graph-ranker`, `connections-optimizer`, `customer-billing-ops`, `scs-tools-cost-audit`, `google-workspace-ops`, `project-flow-ops` y `workspace-surface-audit` completan el carril de operador.
...
- **SCS 2.0 alpha incluido en el árbol** — el prototipo del plano de control en Rust en `scs2/` ya compila localmente y expone los comandos `dashboard`, `start`, `sessions`, `status`, `stop`, `resume` y `daemon`. Está disponible como alpha, aún no como versión general.
- **Instantáneas de estado del operador** — `scs status --markdown --write status.md` convierte el almacén de estado local en un informe portátil de transferencia que cubre disponibilidad, sesiones activas, estado de ejecución de skills, estado de la instalación, eventos de gobernanza pendientes y elementos de trabajo vinculados de Linear/GitHub/transferencias. Usa `scs work-items upsert ...` para entradas manuales, `scs work-items sync-github --repo owner/repo` para el estado de la cola de PRs/issues, y `scs status --exit-code` para hacer fallar la automatización cuando la disponibilidad requiere atención.
- **Hardening del ecosistema** — AgentShield, controles de costos de SCS Tools, trabajo en el portal de facturación y actualizaciones del sitio web continúan publicándose junto al plugin principal en lugar de desviarse hacia silos separados.
...
- **Primera versión centrada en el harness** — SCS ahora se enmarca explícitamente como un sistema de rendimiento del harness de agentes, no solo un paquete de configuración.
...
- **Controles de ejecución de hooks** — `SCS_HOOK_PROFILE=minimal|standard|strict` y `SCS_DISABLED_HOOKS=...` para control en tiempo de ejecución sin editar los archivos de hooks.
...
- **GitHub Marketplace** — La GitHub App SCS Tools disponible en [github.com/marketplace/scs-tools](https://github.com/marketplace/scs-tools) con niveles gratuito/pro/empresarial
...
- **Asistente de instalación interactivo** — La nueva skill `configure-scs` proporciona configuración guiada con detección de fusión/sobrescritura
...
- **No combines métodos de instalación.** La configuración rota más común es: `/plugin install` primero, luego `install.sh --profile full` o `npx scs-install --profile full` después.

Si ya combinaste múltiples instalaciones y hay duplicados, salta directamente a [Restablecer / Desinstalar SCS](#restablecer--desinstalar-scs).

### Ruta sin contexto / sin hooks

Si los hooks te parecen demasiado globales o solo quieres las reglas, agentes, comandos y skills principales de SCS, omite el plugin y usa el perfil manual mínimo:
...
Si no estás seguro de qué perfil o componente de SCS instalar, consulta al asesor empaquetado desde cualquier proyecto:
...
SCS tiene tres identificadores públicos que no son intercambiables:

- Repositorio fuente de GitHub: `StijnCodeSetup/StijnCodeSetup`
- Identificador de marketplace/plugin de Claude: `scs@scs`
- Paquete npm: `stijncodesetup`

Esto es intencional. Las instalaciones del marketplace/plugin de Anthropic se identifican por un identificador de plugin canónico, por lo que SCS usa `scs@scs` para mantener los nombres de herramientas y los espacios de nombres de comandos slash lo suficientemente cortos para los validadores estrictos de Desktop/API. Las publicaciones antiguas pueden mostrar el anterior identificador largo del marketplace; trátalo solo como un alias heredado. Por su parte, el paquete npm se mantuvo en `stijncodesetup`, por lo que las instalaciones de npm y las del marketplace usan intencionalmente nombres diferentes.
...
>
> Si ya instalaste SCS mediante `/plugin install`, **no ejecutes `./install.sh --profile full`, `.\install.ps1 --profile full`, ni `npx scs-install --profile full` después**. El plugin ya carga las skills, comandos y hooks de SCS. Ejecutar el instalador completo tras una instalación del plugin copia esas mismas superficies en tus directorios de usuario y puede crear skills duplicadas más comportamiento duplicado en tiempo de ejecución.
>
> Para instalaciones de plugin, copia manualmente solo los directorios `rules/` que quieras bajo `~/.claude/rules/scs/`. Empieza con `rules/common` más un pack de lenguaje o framework que uses realmente. No copies todos los directorios de reglas a menos que quieras explícitamente todo ese contexto en Claude.
>
> Usa el instalador completo solo cuando hagas una instalación completamente manual de SCS en lugar de la ruta del plugin.
>
> Si tu configuración local de Claude fue eliminada o restablecida, eso no significa que necesites volver a comprar SCS. Empieza con `node scripts/scs.js list-installed`, luego ejecuta `node scripts/scs.js doctor` y `node scripts/scs.js repair` antes de reinstalar cualquier cosa. Eso generalmente restaura los archivos gestionados por SCS sin reconstruir tu configuración. Si el problema es el acceso a la cuenta o al marketplace para SCS Tools, gestiona la recuperación de la facturación/cuenta por separado.
...
# Ruta de plugin: copiar solo las reglas de SCS en un espacio de nombres propio
...
### Restablecer / Desinstalar SCS

Si SCS parece duplicado, intrusivo o roto, no lo reinstales encima de sí mismo.

- **Ruta del plugin:** elimina el plugin de Claude Code, luego borra las carpetas de reglas específicas que copiaste manualmente bajo `~/.claude/rules/scs/`.
...
Luego elimina los archivos gestionados por SCS:
...
SCS solo elimina los archivos registrados en su estado de instalación. No borrará archivos no relacionados que no haya instalado.

Si combinaste métodos, limpia en este orden:

1. Elimina la instalación del plugin de Claude Code.
2. Ejecuta el comando de desinstalación de SCS desde la raíz del repo para eliminar los archivos gestionados por el estado de instalación.
...
# Los nombres de comandos estilo slash existentes siguen funcionando mientras SCS migra fuera de commands/.

# La instalación por plugin usa la forma canónica con espacio de nombres
/scs:plan "Añadir autenticación de usuario"
...
Lanza el dashboard de escritorio para explorar visualmente los componentes de SCS:
...
|-- rules/            # Directrices de cumplimiento obligatorio (copiar a ~/.claude/rules/scs/)
...
|-- scs_dashboard.py  # Dashboard GUI de escritorio (Tkinter)
...
[Instalar GitHub App](https://github.com/apps/scs-skill-creator) | [scs.tools](https://scs.tools)
...
Usa `/security-scan` en Claude Code para ejecutarlo, o añádelo a CI con la [GitHub Action](https://github.com/StijnCodeSetup/scs-agentshield).
...
> mkdir -p ~/.claude/rules/scs
> cp -r rules/common ~/.claude/rules/scs/
> cp -r rules/typescript ~/.claude/rules/scs/   # elige tu stack
...
mkdir -p ~/.claude/skills/scs
cp -r .agents/skills/* ~/.claude/skills/scs/
cp -r skills/search-first ~/.claude/skills/scs/

# Opcional: añadir skills específicas de framework solo cuando las necesites
# for s in django-patterns django-tdd laravel-patterns springboot-patterns quarkus-patterns; do
# cp -r skills/$s ~/.claude/skills/scs/
# done
...
No copies el `hooks/hooks.json` del repo directamente en `~/.claude/settings.json` ni en `~/.claude/hooks/hooks.json`. Ese archivo está orientado al plugin/repo y está pensado para instalarse mediante el instalador de SCS o cargarse como plugin, por lo que la copia directa no es una ruta de instalación manual soportada.
...
Si instalaste SCS mediante `/plugin install`, no copies esos hooks en `settings.json`. Claude Code v2.1+ ya carga automáticamente el `hooks/hooks.json` del plugin, y duplicarlos en `settings.json` provoca ejecución duplicada y conflictos de hooks multiplataforma.

Nota para Windows: el directorio de configuración de Claude es `%USERPROFILE%\\.claude`, no `~/claude`.

#### Configurar MCPs

Las instalaciones de plugin de Claude intencionalmente no habilitan automáticamente las definiciones de servidores MCP empaquetadas en SCS. Esto evita nombres de herramientas MCP demasiado largos en puertas de acceso estrictas de terceros mientras mantiene la configuración manual de MCP disponible.
...
Si ya ejecutas tus propias copias de los MCPs empaquetados en SCS, establece:

```bash
export SCS_DISABLED_MCPS="github,context7,exa,playwright,sequential-thinking,memory"
```

Los flujos de instalación y sincronización de Codex gestionados por SCS omitirán o eliminarán esos servidores empaquetados en lugar de volver a añadir duplicados. `SCS_DISABLED_MCPS` es un filtro de instalación/sincronización de SCS, no un interruptor en tiempo de ejecución de Claude Code.
...
Las skills son la superficie principal de flujo de trabajo. Pueden invocarse directamente, sugerirse automáticamente y ser reutilizadas por agentes. SCS sigue enviando `commands/` mantenidas durante la migración, mientras que los shims de nombres cortos retirados están en `legacy-command-shims/` solo para opt-in explícito. El nuevo desarrollo de flujos de trabajo debe aterrizar primero en `skills/`.
...
|-----------|---------------------|--------------|
| Planificar una nueva feature | `/scs:plan "Añadir auth"` | planner |
| Diseñar arquitectura del sistema | `/scs:plan` + agente architect | architect |
...
```
/scs:plan "Añadir autenticación de usuario con OAuth"
...
<details>
<summary><b>¿Puedo usar SCS con Claude Code en un endpoint de API personalizado o un gateway de modelos?</b></summary>

Sí. SCS no tiene configuraciones de transporte alojadas en Anthropic. Se ejecuta localmente a través de la superficie CLI/plugin normal de Claude Code, por lo que funciona con:
...
Si tu gateway reasigna nombres de modelos, configúralo en Claude Code en lugar de en SCS. Los hooks, skills, comandos y reglas de SCS son agnósticos al proveedor de modelos una vez que el CLI `claude` ya funciona.
...
Demasiados servidores MCP consumen tu contexto. Cada descripción de herramienta MCP consume tokens de tu ventana de 200k, potencialmente reduciéndola a ~70k. El contexto de SessionStart está limitado a 8000 caracteres por defecto; redúcelo con `SCS_SESSION_START_MAX_CHARS=4000` o desactívalo con `SCS_SESSION_START_CONTEXT=off` para configuraciones de bajo contexto o modelo local.
...
Estos no están empaquetados con SCS y no son auditados por este repo, pero vale la pena conocerlos si estás explorando el ecosistema más amplio de skills de Claude Code:
...
SCS proporciona soporte para Cursor IDE con hooks, reglas, agentes, skills, comandos y configuraciones de MCP adaptados para el diseño de proyecto de Cursor.
...
| Agentes | 48 | `.cursor/agents/scs-*.md` cuando se instala; con prefijo para evitar colisiones con agentes de usuario o marketplace |
...
SCS no instala el `AGENTS.md` raíz en `.cursor/`. Cursor trata los archivos `AGENTS.md` anidados como contexto de directorio, por lo que copiar la identidad del repo de SCS en un proyecto host contaminaría ese proyecto.

El comportamiento de carga nativo de Cursor puede variar según la versión. SCS instala agentes como `.cursor/agents/scs-*.md`; si tu versión de Cursor no expone los agentes del proyecto, esos archivos siguen funcionando como definiciones de referencia explícitas en lugar de contexto de prompt global oculto.
...
SCS proporciona **soporte de primera clase para Codex** tanto para la app macOS como para el CLI, con una configuración de referencia, un suplemento AGENTS.md específico de Codex y skills compartidas.
...
# Configuración automática: sincronizar activos de SCS (AGENTS.md, skills, servidores MCP) en ~/.codex
...
El script de sincronización fusiona de forma segura los servidores MCP de SCS en tu `~/.codex/config.toml` existente usando una estrategia **solo de adición** — nunca elimina ni modifica tus servidores existentes. Ejecuta con `--dry-run` para previsualizar los cambios, o `--update-mcp` para forzar la actualización de los servidores SCS a la configuración recomendada más reciente.
...
SCS proporciona **soporte completo para OpenCode** incluyendo plugins y hooks.
...
SCS proporciona **soporte para GitHub Copilot** para VS Code mediante el sistema nativo de archivos de instrucciones y prompts de Copilot Chat — sin herramientas adicionales necesarias.
...
SCS es el **primer plugin que maximiza todas las principales herramientas de codificación con IA**. Así se compara cada harness:
...
| `SCS_CONTEXT_MONITOR_COST_WARNINGS` | on | **off para suscriptores** | Suprime las advertencias de estimación de tasa de API frente al agente manteniendo las advertencias de contexto/alcance/bucle |
...
Proyectos construidos sobre o inspirados en SCS:
...
¿Construiste algo con SCS? Abre un PR para añadirlo aquí.
...
[**Conviértete en Patrocinador**](https://github.com/sponsors/StijnCodeSetup) | [Niveles de Patrocinio](SPONSORS.md) | [Programa de Patrocinio](SPONSORING.md)
...
- **Guía Resumida (Empieza aquí):** [La Guía Resumida de StijnCodeSetup](https://x.com/StijnCodeSetup/status/2012378465664745795)
- **Guía Extensa (Avanzado):** [La Guía Extensa de StijnCodeSetup](https://x.com/StijnCodeSetup/status/2014040193557471352)
- **Guía de Seguridad:** [Guía de Seguridad](../../the-security-guide.md) | [Hilo](https://x.com/StijnCodeSetup/status/2033263813387223421)
- **Seguir:** [@StijnCodeSetup](https://x.com/StijnCodeSetup)
