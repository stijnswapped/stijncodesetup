...
Problemas comunes y soluciones para el plugin StijnCodeSetup (SCS).
...
**Soluciones:**
```bash
# Primero inspecciona qué sabe SCS sobre esta máquina
scs list-installed
scs doctor
scs repair
...
# Claude Code → Extensions → StijnCodeSetup → Uninstall
# Luego reinstalar desde el marketplace

# Si el problema es el acceso al marketplace/cuenta, usa la recuperación de cuenta/facturación de SCS Tools por separado; no uses la reinstalación como sustituto de la recuperación de cuenta
...
cp -r stijncodesetup ~/.claude/plugins/scs
```
...
cd ~/.claude/plugins/cache/scs
npm install

# O para instalación manual
cd ~/.claude/plugins/scs
npm install
```
...
1. **Revisa los Issues de GitHub**: [github.com/StijnCodeSetup/stijncodesetup/issues](https://github.com/StijnCodeSetup/stijncodesetup/issues)
