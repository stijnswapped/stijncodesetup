...
Community-tested workarounds for current Claude Code bugs that can affect SCS hook-heavy setups.

This page is intentionally narrow: it collects the highest-signal operational fixes from the longer troubleshooting surface without repeating speculative or unsupported configuration advice. These are upstream Claude Code behaviors, not SCS bugs.
...
For the fuller SCS troubleshooting surface, use [TROUBLESHOOTING.md](./TROUBLESHOOTING.md).
...
- Use SCS's `strategic-compact` guidance instead of forcing a lower threshold.
...
- Advanced users sometimes use shell-local reload helpers, but SCS does not ship one because those approaches are shell- and platform-dependent.
