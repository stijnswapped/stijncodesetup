...
- **インタラクティブインストールウィザード** — 新しい`configure-scs`スキルがマージ/上書き検出付きガイドセットアップを提供
...
|   |-- configure-scs/              # インタラクティブインストールウィザード（新規）
...
[GitHub アプリをインストール](https://github.com/apps/scs-skill-creator) | [scs.tools](https://scs.tools)
...
Claude Codeで`/security-scan`を実行、または[GitHub Action](https://github.com/StijnCodeSetup/scs-agentshield)でCIに追加できます。
...
> cp -r stijncodesetup/rules/common ~/.claude/rules/common
> cp -r stijncodesetup/rules/typescript ~/.claude/rules/typescript   # スタックを選択
> cp -r stijncodesetup/rules/python ~/.claude/rules/python
> cp -r stijncodesetup/rules/golang ~/.claude/rules/golang
>
...
`/plugin install` で SCS を導入した場合は、これらのフックを `settings.json` にコピーしないでください。Claude Code v2.1+ はプラグインの `hooks/hooks.json` を自動読み込みするため、二重登録すると重複実行や `${CLAUDE_PLUGIN_ROOT}` の解決失敗が発生します。
...
stijncodesetup は [Cursor IDE](https://cursor.com) の事前翻訳設定を含みます。`.cursor/` ディレクトリには、Cursor フォーマット向けに適応されたルール、エージェント、スキル、コマンド、MCP 設定が含まれています。
...
SCSは**フルOpenCodeサポート**をプラグインとフック含めて提供。
...
- **簡潔ガイド（まずはこれ）:** [StijnCodeSetup 簡潔ガイド](https://x.com/StijnCodeSetup/status/2012378465664745795)
- **詳細ガイド（高度）:** [StijnCodeSetup 詳細ガイド](https://x.com/StijnCodeSetup/status/2014040193557471352)
- **フォロー:** [@StijnCodeSetup](https://x.com/StijnCodeSetup)
