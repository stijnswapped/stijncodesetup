---
description: SCSの現在のエージェント、スキル、コマンド、フック、インストールプロファイル、ドキュメントをライブリポジトリサーフェスからナビゲートします。
---

# /scs-guide

このコマンドをStijnCodeSetupの会話型マップとして使用します。完全なREADMEや古いカタログ数をダンプすることなく、タスクに適したSCSサーフェスをユーザーが発見するのを支援します。
...
- `scripts/ci/catalog.js --json` — SCS内で実行時のライブカタログカウント
...
- `/project-init` — スタック対応のSCSオンボーディング
