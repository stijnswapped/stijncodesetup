---
name: agent-sort
description: 並行リポジトリ対応のレビューパスを使用して、スキル、コマンド、ルール、フック、エクストラを DAILY と LIBRARY のバケットに分類することで、特定のリポジトリ向けのエビデンスに基づいた SCS インストール計画を構築します。プロジェクトが完全なバンドルをロードする代わりに実際に必要なものに SCS をトリミングする必要がある場合に使用します。
origin: SCS
---

# エージェントソート

リポジトリにデフォルトのフルインストールではなく、プロジェクト固有の SCS サーフェスが必要な場合にこのスキルを使用します。

目標は「便利そうなもの」を推測することではありません。目標は実際のコードベースからのエビデンスで SCS コンポーネントを分類することです。

## 使用タイミング

- プロジェクトが SCS のサブセットのみを必要とし、フルインストールがノイズが多すぎる場合
...
- SCS ネイティブのサーフェスを優先；2 番目のインストールシステムを導入しない
