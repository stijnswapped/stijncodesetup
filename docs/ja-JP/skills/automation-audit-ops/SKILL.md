...
description: SCS用の証拠ベースの自動化インベントリとオーバーラップ監査ワークフロー。ユーザーがどのジョブ、フック、コネクタ、MCPサーバー、またはラッパーがライブか、壊れているか、冗長であるか、修正前に不足しているかを知りたい場合に使用します。
origin: SCS
---
...
関連するときにこれらのSCS固有のスキルをワークフローに取り込みます：
...
- `scs-tools-cost-audit` 実際の問題がWebhookファンアウト、キュー済みジョブ、または兄弟アプリリポの請求バーンである場合
...
- ユーザーが別のエージェントシステムからポートされたものを知りたい、そしてSCS内で何がまだ再構築される必要があるか
...
値はノイズの多い自動化を1つの正規SCSレーンに折りたたむことであり、すべての履歴パスを保存することではありません。
