...
description: GitHub、Linear、デスクトップアラート、フック、接続された通信インターフェースを網羅する、統合されたSCSネイティブワークフローとして通知を運用する。真の問題がアラートルーティング、重複排除、エスカレーション、またはインボックス崩壊である場合に使用する。
origin: SCS
---
...
* ユーザーが重複する通知ブランチや積み残しのプロポーザルを単一のSCSネイティブチャネルに統合したい
...
独立した通知製品をユーザーに勧めるより、SCSネイティブのオーケストレーションを優先する。
...
SCSがすでにプリミティブを持っている場合は優先して使用する：
...
* 「重複する通知PRのアイデアを1つの正規SCSチャネルに統合してほしい」
