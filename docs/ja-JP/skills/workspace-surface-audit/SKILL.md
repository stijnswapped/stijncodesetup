...
description: アクティブなリポジトリ、MCPサーバー、プラグイン、コネクター、環境サーフェス、ツールのセットアップを監査し、最も価値の高いSCSネイティブスキル、フック、エージェント、オペレーターワークフローを推奨する。ユーザーがClaude Codeのセットアップを支援してほしい場合や、環境で実際に何が使えるかを理解したい場合に使用する。
origin: SCS
---
...
* SCSがそのサーフェスを合理的に所有できる場合は、一般的な「別のプラグインをインストール」という推奨よりSCSネイティブワークフローを優先する。
* 外部プラグインをベースラインとインスピレーションとして扱い、権威ある製品境界としてではない。
* 3つのことを明確に区別する：
  * 現在利用可能なもの
  * 利用可能だがSCSのカプセル化が不十分なもの
  * 利用不可能で新しい統合が必要なもの
...
2. 環境サーフェス
   * アクティブなリポジトリと明らかに隣接するSCSワークスペース内の `.env*` ファイル
...
* 設定済みのLSPサーバー
* キー名で示唆される環境ベースのサービス
* ワークスペースに関連する既存のSCSスキル

サーフェスが生の形式でしか存在しない場合は指摘する。例：

* 「Stripeは接続されたアプリで利用可能だが、SCSには課金操作スキルがない」
* 「Google Driveは接続されているが、SCSにはGoogle Workspaceネイティブの操作ワークフローがない」
...
1. それらが実際に何をするか
2. SCSが同等の機能をすでに持っているか
3. SCSが生の形式のみを持っているか
4. SCSがそのワークフローを完全に欠いているか

### フェーズ3：ギャップをSCSの決定に変換する

各実際のギャップについて、適切なSCSネイティブの形式を推奨する：
...
1. **現在のサーフェス**
   * 現在利用可能なもの
2. **同等の機能**
   * SCSがベースラインに一致または超えている場所
3. **生の形式のみのギャップ**
   * ツールは存在するが、SCSには簡潔な操作スキルがない
4. **欠けている統合**
   * まだ利用できない機能
5. **上位3-5の次のステップ**
   * 影響度順の具体的なSCSネイティブな追加
...
* SCSがすでに強力な生の形式を持っている場合は、まったく新しいサブシステムを発明するのではなくカプセル化スキルを提案する。

## 良い結果

* ユーザーが接続されているもの、欠けているもの、SCSが次に持つべきものをすぐに確認できる。
