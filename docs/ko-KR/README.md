...
- **하네스 중심 릴리스** — SCS는 이제 단순 설정 모음이 아닌, 에이전트 하네스 성능 시스템으로 명시됩니다.
- **훅 안정성 개선** — SessionStart 루트 폴백, Stop 단계 세션 요약, 취약한 인라인 원라이너를 스크립트 기반 훅으로 교체.
- **훅 런타임 제어** — `SCS_HOOK_PROFILE=minimal|standard|strict`와 `SCS_DISABLED_HOOKS=...`로 훅 파일 수정 없이 런타임 제어.
...
- **GitHub 마켓플레이스** — [github.com/marketplace/scs-tools](https://github.com/marketplace/scs-tools)에서 무료/프로/엔터프라이즈 티어 제공
...
[GitHub 앱 설치](https://github.com/apps/scs-skill-creator) | [scs.tools](https://scs.tools)
...
Claude Code에서 `/security-scan`을 사용하거나, [GitHub Action](https://github.com/StijnCodeSetup/scs-agentshield)으로 CI에 추가하세요.
...
> cp -r stijncodesetup/rules/typescript ~/.claude/rules/typescript   # 사용하는 스택 선택
>
...
# 스킬 복사
cp -r stijncodesetup/skills/* ~/.claude/skills/
...
|-------------|-------------|-----------------|
| 새 기능 계획하기 | `/scs:plan "인증 추가"` | planner |
| 시스템 아키텍처 설계 | `/scs:plan` + architect 에이전트 | architect |
...
[**스폰서 되기**](https://github.com/sponsors/StijnCodeSetup) | [스폰서 티어](../../SPONSORS.md) | [스폰서십 프로그램](../../SPONSORING.md)
...
- **요약 가이드 (여기서 시작):** [The Shorthand Guide to StijnCodeSetup](https://x.com/StijnCodeSetup/status/2012378465664745795)
- **상세 가이드 (고급):** [The Longform Guide to StijnCodeSetup](https://x.com/StijnCodeSetup/status/2014040193557471352)
- **팔로우:** [@StijnCodeSetup](https://x.com/StijnCodeSetup)
