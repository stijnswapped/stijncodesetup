...
### v2.0.0-rc.1 — Sincronização de Superfície, Fluxos Operacionais e SCS 2.0 Alpha (Abr 2026)
...
- **SCS 2.0 alpha já está no repositório** — o plano de controle em Rust dentro de `scs2/` já compila localmente e expõe `dashboard`, `start`, `sessions`, `status`, `stop`, `resume` e `daemon`.
- **Fortalecimento do ecossistema** — AgentShield, controles de custo do SCS Tools, trabalho no portal de billing e a renovação do site continuam sendo entregues ao redor do plugin principal.
...
- **Lançamento focado em harness** — O SCS agora é explicitamente enquadrado como um sistema de desempenho de harness de agentes, não apenas um pacote de configurações.
...
- **Controles de runtime de hooks** — `SCS_HOOK_PROFILE=minimal|standard|strict` e `SCS_DISABLED_HOOKS=...` para controle em tempo de execução sem editar arquivos de hook.
...
/scs:plan "Adicionar autenticação de usuário"
...
> cp -r stijncodesetup/rules/typescript ~/.claude/rules/typescript   # escolha sua stack
>
...
cp -r stijncodesetup/rules/common ~/.claude/rules/common
cp -r stijncodesetup/rules/typescript ~/.claude/rules/typescript
...
|----------|-----------------|--------------|
| Planejar um novo recurso | `/scs:plan "Adicionar auth"` | planner |
| Projetar arquitetura de sistema | `/scs:plan` + agente architect | architect |
...
```
/scs:plan "Adicionar autenticação de usuário com OAuth"
...
<details>
<summary><b>Posso usar o SCS com Cursor / OpenCode / Codex / Antigravity?</b></summary>

Sim. O SCS é multiplataforma:
