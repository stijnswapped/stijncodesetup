# SCS v1.10.0 Release Notes

## Positioning

SCS v1.10.0 is a surface-sync and operator-lane release.
...
SCS 2.0 is **real and usable as an alpha**, but it is **not general-availability complete**.

What exists today:

- `scs2/` Rust control-plane codebase in the main repo
- `cargo build --manifest-path scs2/Cargo.toml` passes
- `scs-tui` commands currently available:
  - `dashboard`
  - `start`
  - `sessions`
  - `status`
...
- You should not describe the full SCS 2.0 roadmap as finished.
- The right framing today is **SCS 2.0 alpha / control-plane preview**, not GA.
...
- The selective install / OSS path is still the most reliable full install for teams that want the complete SCS surface.
...
4. Treat SCS 2.0 as an alpha control-plane surface until the open P0/P1 roadmap is materially burned down.
