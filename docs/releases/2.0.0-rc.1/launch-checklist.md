# SCS v2.0.0-rc.1 Launch Checklist

## Repo

- verify local `main` is synced to `origin/main`
- verify `docs/SCS-2.0-GA-ROADMAP.md` reflects the current Linear milestone
  plan and the latest `SCS Platform Roadmap` project snapshot under the Ito
  Markets workspace
...
- verify `scs2/Cargo.toml` stays at `0.1.0` for rc.1; `scs2/` remains an alpha control-plane scaffold
...
- validate the release video suite with `npm run release:video-suite -- --format json`
  after setting `SCS_VIDEO_SOURCE_ROOT` and `SCS_VIDEO_RELEASE_SUITE_ROOT`
...
- "SCS is the reusable substrate; Hermes is the operator shell"
