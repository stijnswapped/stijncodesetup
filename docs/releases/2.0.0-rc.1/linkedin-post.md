# LinkedIn Draft - SCS v2.0.0-rc.1

SCS v2.0.0-rc.1 is ready for final release review as the first release-candidate pass at the 2.0 direction.

The practical shift is simple: SCS is no longer framed as only a Claude Code plugin or config bundle.
...
- a gated Itô prediction-market skill pack for research, comparison, planning,
  and risk review, with Itô API access kept separate from SCS Tools and
  approval-based
...
There is still more to harden before GA, especially around packaging, installers, and the `scs2/` control plane. But rc.1 is enough to show the shape clearly.
