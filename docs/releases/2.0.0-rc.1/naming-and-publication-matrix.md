# SCS v2.0.0-rc.1 Naming And Publication Matrix
...
`StijnCodeSetup/StijnCodeSetup`. It is evidence for planning, not a publication action.

## Decision

For `v2.0.0-rc.1`, ship the public identity as **SCS**.

Use `StijnCodeSetup/StijnCodeSetup` as the canonical GitHub repo and `SCS` as the product name
in copy, plugin slugs, status surfaces, diagrams, and release collateral. Keep
the npm package and package entry points as `stijncodesetup` until a separate
post-rc migration plan exists.

Reason:

- the current install surface already works as `stijncodesetup` plus the `scs`
  plugin slug;
- the exact npm package name `scs` is already occupied by an unrelated elliptic
  curve cryptography package;
- `StijnCodeSetup/StijnCodeSetup` is the live public GitHub repo;
- Claude and Codex plugin surfaces are already short enough as `scs`;
...
| --- | --- | --- | --- | --- |
| Product display name | `SCS` | `rg -n "^# SCS\|displayName.*SCS\|StijnCodeSetup/StijnCodeSetup" README.md .codex-plugin/plugin.json docs/releases/2.0.0-rc.1` | Present across README, plugin manifests, release copy, and URL ledger | Keep for rc.1 and GA |
| GitHub repo | `StijnCodeSetup/StijnCodeSetup` | `git remote get-url origin` | `https://github.com/StijnCodeSetup/StijnCodeSetup.git` | Keep for rc.1 and GA |
| npm package | `stijncodesetup` | `node -p "require('./package.json').name"` | `stijncodesetup` | Keep for rc.1 |
| npm package version | `2.0.0-rc.1` local, `1.10.0` registry latest, `2.0.0-rc.1` registry next | `node -p "require('./package.json').version"` and `npm view stijncodesetup name version dist-tags --json` | Local rc.1 is ready; registry latest remains `1.10.0`; `next` points to `2.0.0-rc.1` | Keep rc on `next`, not `latest` |
| Exact npm short name | `scs` | `npm view scs name version description repository.url --json` | Occupied by `scs@0.0.2`, "Elliptic curve cryptography functions." | Do not use |
| Scoped npm short name | `@StijnCodeSetup/scs` | `npm view @StijnCodeSetup/scs name version --json` | Registry 404 | Possible future scoped package if npm scope policy permits |
| Former package name | `stijncodesetup` | `npm view stijncodesetup name version dist-tags --json` | Registry reports unpublished on 2026-02-07 | Do not revive for rc.1 |
| Claude plugin slug | `scs` | `node -p "require('./.claude-plugin/plugin.json').name"` | `scs` | Keep |
| Claude plugin version | `2.0.0-rc.1` | `claude plugin validate .claude-plugin/plugin.json`; `claude plugin tag .claude-plugin --dry-run` | Validation passed on Claude Code `2.1.143`; dry run would create `scs--v2.0.0-rc.1` | Ready for release-tag gate |
| Claude marketplace entry | `scs` | `.claude-plugin/marketplace.json`; `claude plugin marketplace add --help`; Anthropic plugin marketplace docs | Version and repo point at current rc.1 surface; GitHub, git URL, remote marketplace JSON, and local path marketplace sources are supported | Keep |
| Codex plugin slug | `scs` | `node -p "require('./.codex-plugin/plugin.json').name"` | `scs` | Keep |
| Codex plugin version | `2.0.0-rc.1` | `node tests/plugin-manifest.test.js`; `node tests/docs/scs2-release-surface.test.js` | Plugin manifest passed 54/54; release surface passed 21/21 on Codex CLI `0.131.0` | Ready for Codex marketplace/manual marketplace gate |
| Codex repo marketplace | `scs` | `.agents/plugins/marketplace.json`; `codex plugin marketplace add --help`; OpenAI Codex plugin docs | Repo marketplace add supports GitHub shorthand, Git URLs, SSH URLs, local roots, `--ref`, and `--sparse`; local and GitHub-ref temp-home add smokes passed | Use as rc.1 Codex distribution path |
| OpenCode package | `stijncodesetup` | `node -p "require('./.opencode/package.json').name"` | `stijncodesetup` | Keep |
| OpenCode build | Generated package output | `npm run build:opencode` | Passed | Ready for package dry-run gate |
| npm pack surface | Reduced runtime package | `NPM_CONFIG_USERCONFIG=/dev/null npm pack --dry-run --json` | Produced `stijncodesetup-2.0.0-rc.1.tgz`, 2228 entries, 4,348,504 bytes packed, 13,024,929 bytes unpacked | Needs final release-commit rerun |
...
| Codex plugin | `codex plugin marketplace` supports local and Git marketplace sources; `.codex-plugin/plugin.json` is present; `.agents/plugins/marketplace.json` exposes `scs` from the repo root; temp-home local and GitHub-ref marketplace adds passed | Publish rc.1 docs with the repo-marketplace command, then monitor OpenAI's official Plugin Directory path | Do not claim official Plugin Directory listing before OpenAI submission evidence |
...
| Codex official Plugin Directory | OpenAI docs describe the curated official directory; SCS has not submitted or received listing evidence | Directory submission link or OpenAI approval path once available | Plugin owner | Track as an ITO-56/ITO-46 follow-up; do not claim an official listing |
...
If the package layer moves from `stijncodesetup` toward a shorter npm surface
after rc.1, do it as a staged migration:

1. Keep `stijncodesetup` as the npm package until a replacement package has a
   verified owner, deprecation plan, and install migration.
2. Keep `StijnCodeSetup/StijnCodeSetup` as the canonical repo for public docs, release notes,
...
npm view scs name version description repository.url --json
scs@0.0.2 is occupied by an unrelated elliptic curve cryptography package.

npm view stijncodesetup name version dist-tags --json
registry latest is 1.10.0; next is 2.0.0-rc.1.

npm view stijncodesetup@2.0.0-rc.1 name version dist.tarball dist.integrity time --json
...
Would create and push tag scs--v2.0.0-rc.1.

node tests/docs/scs2-release-surface.test.js
...
Produced stijncodesetup-2.0.0-rc.1.tgz, 2228 entries, 4,348,504 bytes
packed, and 13,024,929 bytes unpacked.

npm publish --tag next --dry-run
Dry run would publish stijncodesetup@2.0.0-rc.1 to npm with tag next.
...
Added marketplace scs and recorded the installed marketplace root as
<local-checkout> without touching the real Codex config.

HOME="$(mktemp -d)" codex plugin marketplace add StijnCodeSetup/StijnCodeSetup --ref "$(git rev-parse HEAD)"
Added marketplace scs from the GitHub repo pinned to
