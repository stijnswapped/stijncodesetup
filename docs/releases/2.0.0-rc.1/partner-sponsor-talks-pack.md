# SCS v2.0.0-rc.1 Partner, Sponsor, and Talks Pack
...
| Core revenue lanes | Sponsors, SCS Tools Pro, consulting, talks | Repeatable growth loop | Approval-gated outbound |
...
```text
SCS gives teams one reusable layer for skills, hooks, rules, MCP conventions,
...
```text
SCS 2.0 sponsor slot for cross-harness agent workflows
```
...
I am getting SCS v2.0.0-rc.1 ready for release review.
...
The sponsor fit is pretty direct: SCS reaches the exact builders who are
...
```text
SCS 2.0 is getting close to rc.1.
...
```text
I am open to a small number of SCS 2.0 implementation sprints for teams that
are standardizing AI coding workflows.

The useful scope is usually:

1. audit the current harness setup;
2. turn repeated workflows into SCS skills, hooks, and rules;
...
```text
SCS started as an open-source workflow layer for Claude Code and is now moving
...
I can show how SCS uses reusable skills, hooks, MCP conventions, release gates,
...
```text
SCS v2.0.0-rc.1 preview pack is ready for final release review.

The main point: SCS 2.0 is a meta-harness for agentic work.
...
- "If your AI coding setup only works in one harness, it is not an operating
  system yet."
- "SCS 2.0 is the shared layer: skills, hooks, MCPs, release gates, and team
...
- The message claims native payments are ready before SCS Tools billing readback
  passes.
