# SCS v2.0.0-rc.1 Preview Pack Manifest
...
| `skills/hermes-imports/SKILL.md` | Sanitized Hermes-to-SCS import workflow | Includes import rules, sanitization checklist, conversion pattern, and output contract |
...
| `docs/releases/2.0.0-rc.1/naming-and-publication-matrix.md` | Naming, slug, and publication-path decision record | Keeps `SCS`, npm `stijncodesetup`, and plugin slug `scs` for rc.1 |
...
| `docs/releases/2.0.0-rc.1/ito-prediction-market-skill-pack.md` | Public Itô skill-pack distribution note | Keeps Itô API access gated, non-advisory, and separate from SCS Tools billing |
...
operator patterns feeding back into SCS without merging business ownership.
...
place trades, do not provide investment advice, and do not merge SCS Tools with
...
skills should enter SCS only after they pass the same rules:
...
| Reference pressure | SCS preview-pack interpretation |
| --- | --- |
...
- npm `stijncodesetup@2.0.0-rc.1` on the `next` dist-tag live readback;
- Claude plugin tag / marketplace propagation for `scs@scs`;
...
- SCS Tools billing/product readiness evidence remains fresh: the May 20
