# SCS v2.0.0-rc.1 Publication Evidence - 2026-05-13 Post-Hardening
...
| --- | --- | --- |
| GitHub PRs and issues | `gh pr list` / `gh issue list` across trunk, AgentShield, and JARVIS | 0 open PRs and 0 open issues on accessible `StijnCodeSetup` repos |
| Trunk discussions | GraphQL discussion count for `StijnCodeSetup/stijncodesetup` | 0 open discussions |
...
SCS-Tools organization repo counts were not rechecked through the current
GraphQL token in this pass because the token cannot resolve those org repos.
The prior post-#42 local checkout handoff recorded both SCS-Tools repos at
...
| Release surface | `node tests/docs/scs2-release-surface.test.js` | Passed 18/18 |
...
| Rust surface | `cd scs2 && cargo test` | Passed 462/462; warnings only for unused functions/fields |
...
| Rust advisory audit | `cd scs2 && cargo audit -q` | Passed silently |
...
The May 2026 TanStack incident maps to SCS release risk through three workflow
classes:
...
SCS's current guardrails cover those classes through:
