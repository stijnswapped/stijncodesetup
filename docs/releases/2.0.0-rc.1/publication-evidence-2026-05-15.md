# SCS v2.0.0-rc.1 Publication Evidence - 2026-05-15
...
| Evidence scope | Current `main` after PR #1932, #1933, #1934, #1935, and #1936; AgentShield #86; and SCS-Tools #75 |
...
| --- | --- | --- |
| Trunk PRs/issues | `gh pr list` and `gh issue list` for `StijnCodeSetup/stijncodesetup` | 0 open PRs, 0 open issues |
| AgentShield PRs/issues | `gh pr list` and `gh issue list` for `StijnCodeSetup/agentshield` | 0 open PRs, 0 open issues |
| JARVIS PRs/issues | `gh pr list` and `gh issue list` for `StijnCodeSetup/JARVIS` | 0 open PRs, 0 open issues |
| SCS Tools PRs/issues | `env -u GITHUB_TOKEN gh pr list` and `env -u GITHUB_TOKEN gh issue list` for `SCS-Tools/SCS-Tools` | 0 open PRs, 0 open issues |
| SCS website PRs/issues | `env -u GITHUB_TOKEN gh pr list` and `env -u GITHUB_TOKEN gh issue list` for `SCS-Tools/SCS-website` | 0 open PRs, 0 open issues |
...
| Other repo discussions | GraphQL discussion count for AgentShield, JARVIS, SCS Tools, and SCS website | Discussions disabled or 0 total |
...
The SCS Tools organization is reachable with the configured GitHub host
credential. In this shell, the exported `GITHUB_TOKEN` overrides that credential
and causes false 404/403 failures for `SCS-Tools/*`. Use `env -u GITHUB_TOKEN`
for SCS Tools verification commands until that environment override is cleaned
up.
...
| SCS 2.0 Preview and Publication | `ITO-45`, `ITO-46`, `ITO-47`, `ITO-56` |
| AgentShield Enterprise Iteration | `ITO-48`, `ITO-49` |
| SCS Tools Next-Level Platform | `ITO-50`, `ITO-51`, `ITO-52`, `ITO-53`, `ITO-54`, `ITO-59` |
...
- SCS-Tools Access Evidence 2026-05-15
...
| PR #1935 | Stabilized `scs2` current-directory-mutating tests with a test-only serialized current-dir guard, preserving the Rust release-surface gate under parallel test execution |
...
| SCS-Tools PR #75 | Tightened the native GitHub payments announcement gate so public billing claims remain blocked until live Marketplace-managed test-account readback is ready |
...
| SCS-Tools merge commits | `6d00d67043e92cadc80f160bfe947115bfef33b1` |
...
| IOC scan | `node scripts/ci/scan-supply-chain-iocs.js --root <SCS-workspace> --home` passed with 229 files inspected after the no-lifecycle install refresh |
...
| Rust release-surface gate | `cd scs2 && cargo test` passed 462/462 with the existing 14 dead-code/unused warnings |
...
- explicit blockers for GitHub release, npm `next` publish, Claude plugin,
  Codex plugin, SCS Tools billing/product-readiness, and announcements.
...
| Repo marketplace | `.agents/plugins/marketplace.json` exposes `scs@2.0.0-rc.1` with `source.path: "./"` from the marketplace root |
| Local add smoke | `HOME="$(mktemp -d)" codex plugin marketplace add <local-checkout>` added marketplace `scs` and recorded the installed marketplace root as `<local-checkout>` without touching the real Codex config |
...
- npm `stijncodesetup@2.0.0-rc.1` is still not published to the `next` dist-tag.
...
- SCS Tools PR #73 added a fail-closed `/api/billing/readiness`
  `announcementGate` for native GitHub payments claims, and SCS Tools PR #74
