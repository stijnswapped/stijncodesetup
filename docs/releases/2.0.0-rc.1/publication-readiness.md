# SCS v2.0.0-rc.1 Publication Readiness
...
AgentShield #86 evidence-pack provenance, and `scs2` current-dir guard evidence
...
export, AgentShield #92 checksum-verified policy promotion, SCS-Tools #76
fleet-summary consumption, SCS-Tools #77 hosted finding evidence paths,
SCS-Tools #78 harness policy-route linking, operator dashboard refresh, and
...
| Claude plugin slug | `scs` / `scs@scs` install path | `.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json` | `node tests/hooks/hooks.test.js` | `publication-evidence-2026-05-12.md` | Plugin owner | Evidence recorded |
...
| Codex plugin manifest | `2.0.0-rc.1` with shared skill source | `.codex-plugin/plugin.json` | `node tests/docs/scs2-release-surface.test.js` | `publication-evidence-2026-05-12.md` | Plugin owner | Evidence recorded |
| Codex repo marketplace | `scs@2.0.0-rc.1` exposed through `.agents/plugins/marketplace.json` | `.agents/plugins/marketplace.json`, `.codex-plugin/README.md` | `HOME="$(mktemp -d)" codex plugin marketplace add <local-checkout>` | `publication-evidence-2026-05-15.md` | Plugin owner | Repo-marketplace path verified; do not claim official Plugin Directory listing before OpenAI submission evidence |
| OpenCode package | `stijncodesetup` plugin module | `.opencode/package.json`, `.opencode/index.ts` | `npm run build:opencode` | `publication-evidence-2026-05-12.md` | Package owner | Evidence recorded |
...
| npm package | `npm pack --dry-run` has expected files, version matches, rc goes to `next` | `npm pack --dry-run`; `npm view stijncodesetup name version dist-tags --json`; `npm view stijncodesetup@2.0.0-rc.1 name version dist.tarball dist.integrity time --json` | `Ready: stijncodesetup@2.0.0-rc.1 is live on next; latest remains 1.10.0` | Package owner | Evidence recorded |
...
| Codex plugin | Manifest version matches package and docs, repo marketplace points at the plugin root, and OpenAI's current official Plugin Directory status is recorded | `node tests/docs/scs2-release-surface.test.js`; `node tests/plugin-manifest.test.js`; `codex plugin marketplace add --help`; temp-home `codex plugin marketplace add <local-checkout>` | `Blocker: official Plugin Directory listing requires OpenAI submission/listing evidence` | Plugin owner | Repo-marketplace distribution verified; official directory pending |
...
| SCS Tools billing reference | Any billing claim links to verified Marketplace/App state | `env -u GITHUB_TOKEN gh repo view SCS-Tools/SCS-Tools --json nameWithOwner,isPrivate,viewerPermission` plus internal `/api/billing/readiness?selectReadyTarget=1` readback using the operator bearer path | `Ready: SCS-Tools #92 main CI and SCS-Tools #93 main CI passed; live selected-target readback returned announcementGate.ready === true on 2026-05-20; repeat before payment announcement` | SCS Tools owner | Billing evidence ready; final copy still waits on release/plugin/live URL approvals |
...
| Markdown lint | `npx markdownlint-cli '**/*.md' --ignore node_modules` | 0 failures | Current release gate: focused lint passed for `publication-readiness.md`, `publication-evidence-2026-05-19.md`, and `docs/SCS-2.0-GA-ROADMAP.md` |
...
| Release surface | `node tests/docs/scs2-release-surface.test.js` | 0 failures | Current release gate: 27/27 passed after refreshing the discussion-count assertion to the post-PR #2005 baseline |
| Optional Rust surface | `cd scs2 && cargo test` | 0 failures or explicit deferral | `publication-evidence-2026-05-16.md`: 462/462 passed, existing warnings only |
| Queue baseline | `node scripts/platform-audit.js --json` across trunk, AgentShield, JARVIS, SCS Tools, and SCS website | Under 20 open PRs and under 20 open issues | Current May 20 baseline after PR #2020: platform audit ready true, 0 open PRs, 0 open issues, 0 discussion gaps, 0 conflicting PRs, and 0 blocking dirty files across tracked repos |
...
| Release name and plugin publication checklist | `docs/releases/2.0.0-rc.1/release-name-plugin-publication-checklist-2026-05-18.md` | Name/package/plugin values are frozen, final-release commands are listed, and Claude/Codex publication paths cite current official docs | Checklist keeps `SCS`, `stijncodesetup`, and plugin slug `scs` for rc.1; no npm rename, npm publish, plugin tag, official listing, billing claim, or announcement before final evidence |

## Do Not Publish If

- `main` has unreviewed release-surface changes after the evidence was recorded.
- `npm view stijncodesetup dist-tags --json` contradicts the intended rc/GA tag.
