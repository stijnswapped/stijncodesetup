...
```bash
npm install stijncodesetup@next
```

`next` currently resolves to `stijncodesetup@2.0.0-rc.1`; `latest` remains on
...
for loop status, session traces, harness audit, and SCS2 tool-risk logs.

## First Skill

Read `skills/hermes-imports/SKILL.md` first.

It shows the intended SCS 2.0 pattern:
...
- Claude Code consumes SCS through the Claude plugin and native hooks.
- Codex consumes SCS through `AGENTS.md`, `.codex-plugin/plugin.json`, and MCP reference config.
- OpenCode consumes SCS through the OpenCode package/plugin surface.
