# SCS v2.0.0-rc.1 Release Name And Plugin Publication Checklist

Snapshot date: 2026-05-18. Canonical repo decision refreshed 2026-05-19
after the public repo rename to `StijnCodeSetup/StijnCodeSetup`; release/package state refreshed
...
- Keep the GitHub repo at `StijnCodeSetup/StijnCodeSetup`.
- Keep the npm package as `stijncodesetup`.
- Keep Claude and Codex plugin slugs as `scs`.
- Publish the npm prerelease on the `next` dist-tag, not `latest`.
- Do not rename the npm package to `scs` or `@StijnCodeSetup/scs` before rc.1.
- Treat `StijnCodeSetup/StijnCodeSetup` as the canonical public repo for rc.1 and GA release
  copy.

Reasons:

- `stijncodesetup` is the current working install and package surface.
- `scs` on npm is occupied by an unrelated elliptic-curve package.
- `@StijnCodeSetup/scs` is unclaimed on npm, but would require a migration plan.
- `StijnCodeSetup/StijnCodeSetup` is now the live public GitHub repo.
- Claude and Codex already expose the desired short namespace as `scs`.
...
| npm package | `stijncodesetup@2.0.0-rc.1` local and registry next, `1.10.0` registry latest | `node -p "require('./package.json').name + '@' + require('./package.json').version"` and `npm view stijncodesetup name version dist-tags --json` | Local rc.1 ready; registry `next` points to `2.0.0-rc.1`; `latest` remains `1.10.0` | Keep rc.1 on `next`; do not move to `latest` before GA approval |
| Exact npm short name | `scs` | `npm view scs name version description repository.url --json` | Occupied by unrelated `scs@0.0.2` | Do not use |
| Scoped npm short name | `@StijnCodeSetup/scs` | `npm view @StijnCodeSetup/scs name version --json` | 404 | Candidate only after migration plan |
| Claude plugin | `scs@2.0.0-rc.1` | `claude plugin validate .claude-plugin/plugin.json`; `claude plugin validate .`; `claude plugin tag .claude-plugin --dry-run` | Validation passed on Claude Code `2.1.143`; full plugin validation has one expected root `CLAUDE.md` context warning; dry run would create `scs--v2.0.0-rc.1` | Run dry-run tag again from the final commit, then tag/push only after approval |
...
npm view @StijnCodeSetup/scs name version --json
...
| 3 | Verify package and plugin manifests | `node tests/plugin-manifest.test.js` and `node tests/docs/scs2-release-surface.test.js` pass | Manifest or release-surface failure |
...
| 10 | Verify npm rc | `npm view stijncodesetup version dist-tags --json` shows rc.1 on `next` and latest still on GA/stable | rc.1 lands on `latest` or registry output is unclear |
...
- Do not announce billing, Marketplace, or native payments until SCS Tools live
...
an official Plugin Directory, but SCS has not submitted or received an official
