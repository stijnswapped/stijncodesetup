# SCS v2.0.0-rc.1 Release Notes

## Positioning

SCS v2.0.0-rc.1 is the first release-candidate surface for SCS as a cross-harness operating system for agentic work.

Claude Code remains a core target. Codex, OpenCode, Cursor, Gemini, and other harnesses are treated as execution surfaces that can share the same skills, rules, MCP conventions, and operator workflows. SCS is the reusable substrate; Hermes is documented as the operator shell that can sit on top of that layer.
...
- Clarified the split between SCS as the reusable substrate and Hermes as the operator shell.
- Documented the cross-harness portability model for skills, hooks, MCPs, rules, and instructions.
- Added a Hermes import playbook for turning local operator patterns into publishable SCS skills.
- Added Zed as a project-local planning/install target while keeping BYOK and OpenRouter secrets outside SCS-managed project files.
...
- Added a local [observability readiness gate](../../architecture/observability-readiness.md) for loop status, session traces, harness audit, and SCS2 tool-risk logs.
- Added the public teaser [Itô prediction-market skill pack](ito-prediction-market-skill-pack.md)
  ...
  and risk review. Live Itô API access remains gated and separate from SCS
  Tools billing.
...
- Published `stijncodesetup@2.0.0-rc.1` to npm on the `next` dist-tag. The
...
- SCS Tools roadmap mirrors for hosted analysis, fleet-summary consumption,
...
- gated Itô skill distribution as a public workflow teaser, not a live trading
  claim or a merge of SCS Tools and Itô ownership.
- a release URL ledger that separates links which already resolve from links
  that must wait for the plugin tag/directory, video upload, and SCS Tools
  billing readback.

## Why This Matters

SCS is no longer only a Claude Code plugin or config bundle.
...
7. Import only sanitized operator patterns into SCS skills.
8. Treat `scs2/` as an alpha control plane until release packaging and installer
...
SCS Tools billing/native-payments readiness, and final outbound copy.
