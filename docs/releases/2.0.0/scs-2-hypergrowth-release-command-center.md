# SCS 2.0 Hypergrowth Release Command Center

Snapshot date: 2026-05-19.

This is the execution map for turning SCS 2.0 into a complete public release,
...
- `scs2/` as the alpha control-plane/TUI direction;
- Hermes as the optional operator shell for chat, cron, handoffs, and daily
  work routing;
- SCS Tools Pro, GitHub Sponsors, and consulting as the business surface that
  funds the OSS layer.
...
| --- | --- | --- |
| Product category | SCS as the harness-native operator system, not a Claude-only config pack | Converts confused OSS traffic into install, Pro, and sponsor intent |
...
| Control plane | `scs2/` alpha dashboard/status/session surface and Hermes operator shell clearly framed as directionally live | Consulting and team implementation sprints |
...
SCS should stay useful as free OSS while the managed value accrues around team
...
4. convert sanitized product usage back into SCS skills, docs, or evidence;
...
through SCS, while live Itô data and account-specific calls remain gated by
`ITO_API_KEY` and separate from SCS Tools billing.

## Release Gates

| Lane | Done when | Current action |
| --- | --- | --- |
| Repo identity | README, package metadata, plugin metadata, release docs, workflows, and launch copy all use `StijnCodeSetup/StijnCodeSetup` where public URLs are needed | Canonical URL sweep |
| Package and plugin publication | `stijncodesetup@2.0.0-rc.1` dry-runs clean, npm `next` is approved, Claude plugin tag dry-runs, Codex repo marketplace smoke passes, OpenCode build passes | Refresh publication evidence from final commit |
| Product proof | Quickstart, cross-harness architecture, demo prompts, `scs2/` alpha boundary, AgentShield safety proof, and hosted SCS Tools links are consistent | Keep proof surfaces concrete |
...
The video lane should use the existing SCS video-editing skill plus the
...
Primary source classes already exist in the local SCS media library. Keep raw
...
| --- | ---: | --- | --- |
| Primary launch video | 90-150s | `longform-full-wide.mp4`, `sf-longform-full.mp4`, `architecture-2-wide.mp4`, `terminal-scan-2-wide.mp4`, `new_site_raw.mp4` | SCS 2.0 as the operator system |
| Install proof | 30s | README install, terminal scan, quickstart, plugin install | Fewer-click adoption |
| What is SCS | 45-60s | `sf-thread-2-whatisscs.mp4`, `vertical-2-whatisscs.mp4`, `architecture-2-*` | Product category clarity |
...
- `Use SCS for skills, hooks, rules, MCP conventions, release gates, and operator workflows.`
...
5. Browser-capture the README, SCS Tools app, install flow, and relevant proof
   surfaces for b-roll.
