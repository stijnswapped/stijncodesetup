...
![StijnCodeSetup — система повышения эффективности сред агентного ИИ](../../assets/hero.png)
...
SCS v2.0.0-rc.1 добавляет публичную историю оператора Hermes поверх этого переиспользуемого слоя: начните с [руководства по настройке Hermes](../HERMES-SETUP.md), затем прочитайте [примечания к выпуску rc.1](../releases/2.0.0-rc.1/release-notes.md) и [архитектуру для разных сред](../architecture/cross-harness.md).
...
<img src="../../assets/images/guides/shorthand-guide.png" alt="Краткое руководство по StijnCodeSetup" />
</a>
</td>
<td width="33%">
<a href="https://x.com/StijnCodeSetup/status/2014040193557471352">
<img src="../../assets/images/guides/longform-guide.png" alt="Подробное руководство по StijnCodeSetup" />
</a>
</td>
<td width="33%">
...
### v2.0.0-rc.1 — Обновление публичного контура, операторские рабочие процессы и SCS 2.0 Alpha (апрель 2026)

- **Dashboard GUI** — новое настольное приложение на Tkinter (`scs_dashboard.py` или `npm run dashboard`) с переключателем тёмной/светлой темы, настройкой шрифта и логотипом проекта в заголовке и панели задач.
...
- **Расширение операторских и outbound-рабочих процессов** — `brand-voice`, `social-graph-ranker`, `connections-optimizer`, `customer-billing-ops`, `scs-tools-cost-audit`, `google-workspace-ops`, `project-flow-ops` и `workspace-surface-audit` закрывают операторское направление.
...
- **SCS 2.0 alpha находится в дереве репозитория** — прототип control plane на Rust в `scs2/` теперь собирается локально и предоставляет команды `dashboard`, `start`, `sessions`, `status`, `stop`, `resume` и `daemon`. Это пригодная к использованию alpha-версия, но ещё не общий релиз.
- **Укрепление экосистемы** — AgentShield, контроль затрат SCS Tools, работа над billing portal и обновления сайта продолжают поставляться вокруг основного плагина, а не расползаются по отдельным направлениям.
...
- **Релиз с фокусом на средах агентного ИИ** — SCS теперь явно позиционируется как система повышения эффективности таких сред, а не просто набор конфигов.
...
- **Управление хуками во время выполнения** — `SCS_HOOK_PROFILE=minimal|standard|strict` и `SCS_DISABLED_HOOKS=...` для runtime-ограничений без редактирования файлов хуков.
...
- **GitHub Marketplace** — GitHub App SCS Tools доступен на [github.com/marketplace/scs-tools](https://github.com/marketplace/scs-tools) с тарифами free/pro/enterprise
...
- **Исправлена потеря содержимого при импорте инстинктов** — `parse_instinct_file()` незаметно отбрасывал всё содержимое после frontmatter (разделы Action, Evidence, Examples) во время `/instinct-import`. ([#148](https://github.com/StijnCodeSetup/stijncodesetup/issues/148), [#161](https://github.com/StijnCodeSetup/stijncodesetup/pull/161))

### v1.4.0 — Многоязычные правила, мастер установки и PM2 (февраль 2026)

- **Интерактивный мастер установки** — новый навык `configure-scs` предоставляет пошаговую настройку с обнаружением merge/overwrite
...
Полный журнал изменений смотрите в [Releases](https://github.com/StijnCodeSetup/stijncodesetup/releases).
...
- **Не накладывайте методы установки друг на друга.** Самая частая сломанная конфигурация: сначала `/plugin install`, затем `install.sh --profile full` или `npx scs-install --profile full`.

Если вы уже наложили несколько установок и видите дублирование, сразу переходите к разделу [Сброс / удаление SCS](#сброс--удаление-scs).

### Путь с малым контекстом / без хуков

Если хуки кажутся слишком глобальными или вам нужны только правила, агенты, команды и основные навыки рабочих процессов SCS, пропустите плагин и используйте минимальный ручной профиль:
...
Если вы не уверены, какой профиль SCS или компонент установить, спросите упакованный advisor из любого проекта:
...
У SCS теперь три публичных идентификатора, и они не взаимозаменяемы:

- исходный репозиторий GitHub: `StijnCodeSetup/stijncodesetup`
- идентификатор Claude marketplace/plugin: `scs@scs`
- npm-пакет: `stijncodesetup`

Это сделано намеренно. Установки Anthropic marketplace/plugin ключуются каноническим идентификатором плагина, поэтому SCS использует `scs@scs`, чтобы имена инструментов и пространства имен slash-команд оставались достаточно короткими для строгих валидаторов Desktop/API. Старые публикации могут всё ещё показывать прежний длинный marketplace-идентификатор; считайте его только устаревшим alias. Отдельно npm-пакет остался `stijncodesetup`, поэтому npm-установки и marketplace-установки намеренно используют разные имена.

### Шаг 2: Установите правила (обязательно)

> ПРЕДУПРЕЖДЕНИЕ: **Важно:** плагины Claude Code не могут автоматически распространять `rules`.
>
> Если вы уже установили SCS через `/plugin install`, **не запускайте после этого `./install.sh --profile full`, `.\install.ps1 --profile full` или `npx scs-install --profile full`**. Плагин уже загружает навыки, команды и хуки SCS. Запуск полного установщика после установки плагина скопирует те же компоненты в пользовательские директории и может создать дублирующиеся навыки и дублирующееся runtime-поведение.
>
> Для установки через плагин вручную скопируйте только нужные директории `rules/` в `~/.claude/rules/scs/`. Начните с `rules/common` плюс один языковой или framework-пакет, который вы действительно используете. Не копируйте все директории правил, если явно не хотите весь этот контекст в Claude.
>
> Используйте полный установщик только если делаете полностью ручную установку SCS вместо пути через плагин.
>
> Если ваша локальная установка Claude была очищена или сброшена, это не значит, что нужно повторно покупать SCS. Начните с `node scripts/scs.js list-installed`, затем запустите `node scripts/scs.js doctor` и `node scripts/scs.js repair` перед любой переустановкой. Обычно это восстанавливает файлы, управляемые SCS, без пересборки всей настройки. Если проблема связана с аккаунтом или marketplace-доступом к SCS Tools, восстановление billing/account нужно делать отдельно.
...
# Путь установки через плагин: скопируйте только правила SCS в пространство имён SCS
mkdir -p ~/.claude/rules/scs
cp -R rules/common ~/.claude/rules/scs/
cp -R rules/typescript ~/.claude/rules/scs/

# Полностью ручной путь установки SCS (используйте вместо /plugin install)
...
Если SCS кажется дублированным, навязчивым или сломанным, не переустанавливайте его снова поверх самого себя.

- **Путь через плагин:** удалите плагин из Claude Code, затем удалите конкретные папки правил, которые вы вручную скопировали в `~/.claude/rules/scs/`.
...
Затем удалите файлы, управляемые SCS:
...
SCS удаляет только файлы, записанные в его install-state. Он не удалит посторонние файлы, которые сам не устанавливал.

Если вы смешали методы, очищайте в таком порядке:

1. Удалите установку плагина Claude Code.
2. Запустите команду удаления SCS из корня репозитория, чтобы удалить файлы, управляемые install-state.
...
# Существующие slash-style имена команд продолжают работать, пока SCS мигрирует с commands/.

# Установка через плагин использует каноническую форму с namespace
/scs:plan "Добавить аутентификацию пользователей"
...
Запустите настольную панель управления, чтобы визуально изучить компоненты SCS:
...
|   |-- configure-scs/              # Интерактивный мастер установки (НОВОЕ)
...
|
|-- rules/            # Always-follow guidelines (копируйте в ~/.claude/rules/scs/)
...
|
|-- scs_dashboard.py  # Настольная GUI-панель управления (Tkinter)
|
...
[Установить GitHub App](https://github.com/apps/scs-skill-creator) | [scs.tools](https://scs.tools)
...
Используйте `/security-scan` в Claude Code, чтобы запустить его, или добавьте в CI через [GitHub Action](https://github.com/StijnCodeSetup/scs-agentshield).
...
**История:** это уже приводило к повторяющимся циклам fix/revert в репозитории ([#29](https://github.com/StijnCodeSetup/stijncodesetup/issues/29), [#52](https://github.com/StijnCodeSetup/stijncodesetup/issues/52), [#103](https://github.com/StijnCodeSetup/stijncodesetup/issues/103)). Поведение менялось между версиями Claude Code, что вызывало путаницу. Теперь есть регрессионный тест, который не даёт вернуть эту ошибку.
...
> mkdir -p ~/.claude/rules/scs
> cp -r stijncodesetup/rules/common ~/.claude/rules/scs/
> cp -r stijncodesetup/rules/typescript ~/.claude/rules/scs/   # выберите свой стек
> cp -r stijncodesetup/rules/python ~/.claude/rules/scs/
> cp -r stijncodesetup/rules/golang ~/.claude/rules/scs/
> cp -r stijncodesetup/rules/php ~/.claude/rules/scs/
>
...
cp stijncodesetup/agents/*.md ~/.claude/agents/
...
Не копируйте сырой repo-файл `hooks/hooks.json` в `~/.claude/settings.json` или `~/.claude/hooks/hooks.json`. Этот файл ориентирован на плагин/репозиторий и должен устанавливаться через установщик SCS или загружаться как плагин, поэтому прямое копирование не является поддерживаемым ручным способом установки.
...
Если вы установили SCS через `/plugin install`, не копируйте эти хуки в `settings.json`. Claude Code v2.1+ уже автоматически загружает plugin `hooks/hooks.json`, а дублирование в `settings.json` вызывает двойное выполнение и кроссплатформенные конфликты хуков.

Примечание для Windows: директория конфигурации Claude — `%USERPROFILE%\\.claude`, а не `~/claude`.

#### Настроить MCP

Установки Claude plugin намеренно не включают автоматически bundled MCP server definitions SCS. Это предотвращает слишком длинные имена plugin MCP tools на строгих сторонних gateway, но оставляет доступной ручную настройку MCP.
...
Если у вас уже запущены собственные копии MCP, bundled в SCS, задайте:

```bash
export SCS_DISABLED_MCPS="github,context7,exa,playwright,sequential-thinking,memory"
```

SCS-managed install и Codex sync flows будут пропускать или удалять эти bundled servers вместо повторного добавления дубликатов. `SCS_DISABLED_MCPS` — это фильтр установки/синхронизации SCS, а не live-переключатель Claude Code.
...
|-----------|---------------|-------|
| Спланировать новую функцию | `/scs:plan "Добавить auth"` | planner |
| Спроектировать архитектуру системы | `/scs:plan` + агент architect | architect |
...
```
/scs:plan "Добавить OAuth-аутентификацию пользователей"
...
Это самая частая проблема. **НЕ добавляйте поле `"hooks"` в `.claude-plugin/plugin.json`.** Claude Code v2.1+ автоматически загружает `hooks/hooks.json` из установленных плагинов. Явное объявление вызывает ошибки обнаружения дубликатов. См. [#29](https://github.com/StijnCodeSetup/stijncodesetup/issues/29), [#52](https://github.com/StijnCodeSetup/stijncodesetup/issues/52), [#103](https://github.com/StijnCodeSetup/stijncodesetup/issues/103).
</details>

<details>
<summary><b>Можно ли использовать SCS с Claude Code на custom API endpoint или model gateway?</b></summary>

Да. SCS не хардкодит транспортные настройки Anthropic-hosted окружения. Он запускается локально через обычный CLI/plugin-интерфейс Claude Code, поэтому работает с:
...
Если ваш gateway переименовывает модели, настраивайте это в Claude Code, а не в SCS. Хуки, навыки, команды и правила SCS не зависят от model provider, если CLI `claude` уже работает.
...
Слишком много MCP-серверов съедают контекст. Каждое описание MCP tool потребляет токены из вашего окна 200k, потенциально сокращая его до ~70k. Контекст SessionStart по умолчанию ограничен 8000 символами; уменьшите его через `SCS_SESSION_START_MAX_CHARS=4000` или отключите через `SCS_SESSION_START_CONTEXT=off` для local-model или low-context setups.
...
- **Codex**: первоклассная поддержка macOS app и CLI, с guards против adapter drift и SessionStart fallback. См. PR [#257](https://github.com/StijnCodeSetup/stijncodesetup/pull/257).
...
Они не поставляются вместе с SCS и не аудируются этим репозиторием, но о них стоит знать, если вы изучаете более широкую экосистему Claude Code skills:
...
SCS предоставляет поддержку Cursor IDE с хуками, правилами, агентами, навыками, командами и MCP-конфигами, адаптированными под layout проекта Cursor.
...
| Agents | 50 | `.cursor/agents/scs-*.md` при установке; с префиксом, чтобы избежать конфликтов с user или marketplace agents |
...
SCS не устанавливает root `AGENTS.md` в `.cursor/`. Cursor воспринимает вложенные `AGENTS.md` как directory context, поэтому копирование identity SCS-репозитория в host project загрязняло бы этот проект.

Cursor-native loading behavior может различаться между сборками Cursor. SCS устанавливает агентов как `.cursor/agents/scs-*.md`; если ваша сборка Cursor не показывает project agents, эти файлы всё равно работают как явные reference definitions, а не скрытый global prompt context.
...
SCS предоставляет **первоклассную поддержку Codex** как для macOS app, так и для CLI: reference configuration, Codex-specific supplement `AGENTS.md` и общие skills.
...
# Автоматическая настройка: синхронизировать assets SCS (AGENTS.md, skills, MCP servers) в ~/.codex
npm install && bash scripts/sync-scs-to-codex.sh
# или: pnpm install && bash scripts/sync-scs-to-codex.sh
# или: yarn install && bash scripts/sync-scs-to-codex.sh
# или: bun install && bash scripts/sync-scs-to-codex.sh

# Или вручную: скопировать reference config в домашнюю директорию
cp .codex/config.toml ~/.codex/config.toml
```

Sync script безопасно сливает MCP-серверы SCS в существующий `~/.codex/config.toml` через стратегию **add-only**: он никогда не удаляет и не изменяет ваши существующие серверы. Запускайте с `--dry-run`, чтобы посмотреть изменения, или с `--update-mcp`, чтобы принудительно обновить SCS-серверы до последнего рекомендуемого конфига.

Для Context7 SCS использует каноническое имя секции Codex `[mcp_servers.context7]`, но всё ещё запускает пакет `@upstash/context7-mcp`. Если у вас уже есть legacy-запись `[mcp_servers.context7-mcp]`, `--update-mcp` мигрирует её на каноническое имя секции.
...
| stijncodesetup | Development conventions и patterns для проекта |
...
Codex **пока не предоставляет parity с Claude-style hook execution**. Принуждение SCS там instruction-based через `AGENTS.md`, опциональные overrides `model_instructions_file` и настройки sandbox/approval.
...
SCS поставляет три sample role configs:
...
SCS предоставляет **полную поддержку OpenCode**, включая плагины и хуки.
...
Эта npm plugin entry включает опубликованный OpenCode plugin module SCS (hooks/events и plugin tools).
Она **не** добавляет автоматически полный catalog команд/агентов/instructions SCS в конфиг вашего проекта.

Для полной настройки SCS OpenCode:
...
SCS — **первый плагин, который помогает максимально использовать каждый крупный инструмент AI-кодинга**. Вот как сравниваются среды:
...
- Используйте `SCS_DISABLED_MCPS` только для фильтрации MCP-конфигов, генерируемых SCS, во время install/sync flows
...
Проекты, построенные на StijnCodeSetup или вдохновлённые им:
...
Построили что-то с SCS? Откройте PR, чтобы добавить это сюда.
...
[**Стать спонсором**](https://github.com/sponsors/StijnCodeSetup) | [Уровни спонсорства](../../SPONSORS.md) | [Программа спонсорства](../../SPONSORING.md)
...
- **Краткое руководство (начните здесь):** [The Shorthand Guide to StijnCodeSetup](https://x.com/StijnCodeSetup/status/2012378465664745795)
- **Подробное руководство (продвинутый уровень):** [The Longform Guide to StijnCodeSetup](https://x.com/StijnCodeSetup/status/2014040193557471352)
- **Руководство по безопасности:** [Security Guide](../../the-security-guide.md) | [Тред](https://x.com/StijnCodeSetup/status/2033263813387223421)
- **Подписаться:** [@StijnCodeSetup](https://x.com/StijnCodeSetup)
