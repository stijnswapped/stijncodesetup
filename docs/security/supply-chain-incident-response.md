...
This playbook is the SCS operator runbook for npm, GitHub Actions, and
...
supply-chain compromise and broader Mini Shai-Hulud campaign. SCS keeps the
...
- Socket's 2026-05-14 `node-ipc` report describes a separate active npm
  ...
  with historical malicious `node-ipc` versions also blocked by SCS because
...
SCS also runs `.github/workflows/supply-chain-watch.yml` every six hours and on
...
If SCS or a maintainer machine installed a known-bad package version:
...
SCS enforces these rules through `scripts/ci/validate-workflow-security.js`:
...
Before tagging or publishing SCS:
