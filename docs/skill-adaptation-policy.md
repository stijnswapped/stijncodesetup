...
SCS accepts ideas from outside repos, but shipped skills need to become SCS-native surfaces.
...
- adapt it to SCS's current install surfaces, validation flow, and repo conventions
- remove unnecessary external branding, dependency assumptions, and upstream-specific framing

The goal is reuse without turning SCS into a thin wrapper around someone else's runtime.
...
- there is no better SCS-native name already in the repo
...
Rename the skill when SCS meaningfully expands, narrows, or repackages the original work.

Typical triggers:

- SCS adds substantial new behavior, structure, or guidance
- the original name is vendor-forward or community-brand-forward instead of workflow-forward
- the contribution overlaps an existing SCS surface and needs a clearer boundary
...
- prefer SCS-native names like `product-capability` over vague imported planning labels if the scope changed materially

## Dependency Policy

SCS prefers the narrowest native surface that gets the job done:
...
- vendor or recreate the relevant logic inside SCS when practical
...
1. Is this a real reusable surface in SCS, or just documentation for another tool?
2. Does the current name still match the SCS-shaped surface?
3. Is there already an SCS skill that owns most of this behavior?
4. Are we importing a concept, or importing someone else's product identity?
5. Would an SCS user understand the purpose of this skill without knowing the upstream repo?
