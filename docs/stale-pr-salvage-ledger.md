...
| #1478 | HarmonyOS/ArkTS rules, resolver agent, and CLAUDE example | Salvaged in #1769 with current install wiring; stale `scs2` session/TUI edits were not carried. |
...
| #1659 | Frontend design direction and interface-polish skills | Salvaged in the May 12 frontend-design maintainer pass with canonical `skills/` layout and current SCS frontend guidance, while preserving the repo guardrail that the official `frontend-design` skill should be installed from `anthropics/skills`. |
...
The SCS release repos were already clean, but an owner-wide `gh search` sweep
...
legacy/outreach/placeholder issues. The `StijnCodeSetup` owner namespace is now at 0
...
| Legacy issues in `payments0-api`, `payments0-sdk`, `agent-payments-gateway`, `0EM_Frontend`, `0em-payments-dashboard`, and `yield-optimizer` | Superseded by SCS Tools native-payments, hosted analysis, billing-readback, and Linear/project roadmap lanes. |
...
| Final PR/issue sweep | Closed the remaining generated SCS bundles, stale Cloudflare rename PRs, stale README-card PR, test/noise PR, public outreach issues, and empty placeholder issue. Preserved `dexploy#25` findings in Linear `ITO-62` before closure. |
