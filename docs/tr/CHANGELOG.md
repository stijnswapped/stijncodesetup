...
- Hermes operatör hikayesi için genel SCS 2.0 sürüm adayı yüzeyi eklendi.
- SCS, Claude Code, Codex, Cursor, OpenCode ve Gemini genelinde yeniden kullanılabilir cross-harness altyapı olarak belgelendi.
...
- SCS/Hermes sınırı için `docs/architecture/cross-harness.md` ve regresyon kapsamı eklendi.
- `scs2/` sürümlemesi bağımsız tutuldu; release engineering aksi karar vermedikçe alpha control-plane scaffold olarak kalır.

### Notlar

- Bu bir sürüm adayıdır; tam SCS 2.0 control-plane yol haritası için GA iddiası değildir.
...
- SCS 2.0 alpha control-plane binary artık `scs2/` üzerinden yerelde build ediliyor ve ilk kullanılabilir CLI/TUI yüzeyini sunuyor.
...
- Bu sürüm bir repo-yüzeyi düzeltmesi ve ekosistem senkronizasyonudur; tam SCS 2.0 yol haritasının tamamlandığı iddiası değildir.
...
- `stijncodesetup` — Öz-referansiyel SCS becerisi (#335)
...
- `SCS_HOOK_PROFILE` ve `SCS_DISABLED_HOOKS` kontrolleri eklendi.
