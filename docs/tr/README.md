...
[![npm stijncodesetup](https://img.shields.io/npm/dw/stijncodesetup?label=stijncodesetup%20haftalık%20indirme&logo=npm)](https://www.npmjs.com/package/stijncodesetup)
[![npm scs-agentshield](https://img.shields.io/npm/dw/scs-agentshield?label=scs-agentshield%20haftalık%20indirme&logo=npm)](https://www.npmjs.com/package/scs-agentshield)
[![GitHub App Install](https://img.shields.io/badge/GitHub%20App-150%20kurulum-2ea44f?logo=github)](https://github.com/marketplace/scs-tools)
...
<img src="../../assets/images/guides/shorthand-guide.png" alt="StijnCodeSetup Kısa Rehberi" />
</a>
</td>
<td width="33%">
<a href="https://x.com/StijnCodeSetup/status/2014040193557471352">
<img src="../../assets/images/guides/longform-guide.png" alt="StijnCodeSetup Uzun Rehberi" />
</a>
</td>
<td width="33%">
...
### v2.0.0-rc.1 — Surface Sync, Operatör İş Akışları ve SCS 2.0 Alpha (Nis 2026)
...
- **SCS 2.0 alpha repoda** — `scs2/` altındaki Rust kontrol katmanı artık yerelde derleniyor ve `dashboard`, `start`, `sessions`, `status`, `stop`, `resume` ve `daemon` komutlarını sunuyor.
- **Ekosistem sağlamlaştırma** — AgentShield, SCS Tools maliyet kontrolleri, billing portal işleri ve web yüzeyi çekirdek plugin etrafında birlikte gelişmeye devam ediyor.
...
- **Harness-first release** — SCS artık açıkça bir agent harness performans sistemi olarak çerçevelendi, sadece bir config paketi değil.
...
- **Hook runtime kontrolleri** — `SCS_HOOK_PROFILE=minimal|standard|strict` ve `SCS_DISABLED_HOOKS=...` hook dosyalarını düzenlemeden runtime gating için.
...
[Tam değişiklik günlüğü için Releases bölümüne bakın](https://github.com/StijnCodeSetup/stijncodesetup/releases).
...
/scs:plan "Kullanıcı kimlik doğrulaması ekle"
...
|---------------------|---------------------|------------------|
| Yeni bir feature planla | `/scs:plan "Auth ekle"` | planner |
| Sistem mimarisi tasarla | `/scs:plan` + architect agent | architect |
...
```
/scs:plan "OAuth ile kullanıcı kimlik doğrulaması ekle"
...
Bu en yaygın sorundur. `.claude-plugin/plugin.json`'a bir `"hooks"` alanı **EKLEMEYİN**. Claude Code v2.1+ kurulu plugin'lerden `hooks/hooks.json`'ı otomatik olarak yükler. Açıkça belirtmek duplicate algılama hatalarına neden olur. Bkz. [#29](https://github.com/StijnCodeSetup/stijncodesetup/issues/29), [#52](https://github.com/StijnCodeSetup/stijncodesetup/issues/52), [#103](https://github.com/StijnCodeSetup/stijncodesetup/issues/103).
</details>
...
- **Codex**: macOS app ve CLI için birinci sınıf destek. PR [#257](https://github.com/StijnCodeSetup/stijncodesetup/pull/257)'ye bakın.
