...
* 为 Hermes 操作员叙事新增公开的 SCS 2.0 release candidate 表面。
* 将 SCS 明确记录为跨 Claude Code、Codex、Cursor、OpenCode 和 Gemini 的可复用 cross-harness 基础层。
...
* 新增 `docs/architecture/cross-harness.md`，并补充 SCS/Hermes 边界的回归覆盖。
* `scs2/` 版本保持独立；除非 release engineering 另有决定，它仍是 alpha control-plane scaffold。

### 备注

* 这是 release candidate，不是完整 SCS 2.0 control-plane 路线图的 GA 声明。
...
* SCS 2.0 alpha control-plane binary 现在可从 `scs2/` 本地构建，并提供首个可用的 CLI/TUI 表面。
...
* 这是仓库表面校正和生态同步版本，不表示完整 SCS 2.0 路线图已经完成。
...
* 增加了 `SCS_HOOK_PROFILE` 和 `SCS_DISABLED_HOOKS` 控制。
