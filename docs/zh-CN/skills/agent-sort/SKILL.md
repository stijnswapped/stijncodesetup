---
name: agent-sort
description: 通过将技能、命令、规则、钩子和额外内容并行进行仓库感知审查，为特定仓库构建基于证据的 SCS 安装计划，将其分为 DAILY 和 LIBRARY 两类。当 SCS 应精简为项目实际所需而非加载完整包时使用。
origin: SCS
---

# 技能分类

当仓库需要项目特定的 SCS 表面而非默认完整安装时，使用此技能。

目标不是猜测"什么感觉有用"。目标是根据实际代码库中的证据对 SCS 组件进行分类。

## 何时使用

* 项目只需要 SCS 的子集，完整安装过于嘈杂
