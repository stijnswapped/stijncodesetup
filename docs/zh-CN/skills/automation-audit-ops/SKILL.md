...
description: 面向SCS的以证据为先的自动化清单与重叠审计工作流。当用户希望在修复任何内容之前了解哪些作业、钩子、连接器、MCP服务器或包装器是活跃的、损坏的、冗余的或缺失时使用。
origin: SCS
---
...
* 用户想知道从其他代理系统移植了什么，以及哪些还需要在 SCS 内部重建
...
其价值在于将杂乱的自动化整合到一条规范的 SCS 路径中，而不是保留每一条历史路径。
