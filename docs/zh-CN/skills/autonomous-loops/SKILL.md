...
**SCS 内置的持久循环。** 一个具有会话感知的 REPL，它使用完整的对话历史同步调用 `claude -p`。
...
3. **任何循环 + 验证** — 在提交前，使用 SCS 的 `/verify` 命令或 `verification-loop` 技能作为关卡。
...
| NanoClaw | SCS | 此仓库中的 `/claw` 命令 |
| Verification Loop | SCS | 此仓库中的 `skills/verification-loop/` |
