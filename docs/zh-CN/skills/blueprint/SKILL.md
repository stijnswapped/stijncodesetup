...
此技能随 StijnCodeSetup 附带。安装 SCS 时无需单独安装。

### 完整 SCS 安装

如果您从 SCS 仓库检出中工作，请验证技能是否存在：

```bash
test -f skills/blueprint/SKILL.md
```

后续更新时，请在更新前查看 SCS 的差异：
...
如果您在完整 SCS 安装之外仅内嵌此技能，请将 SCS 仓库中已审查的文件复制到 `~/.claude/skills/blueprint/SKILL.md`。内嵌副本没有 git 远程仓库，因此应通过从已审查的 SCS 提交中重新复制文件来更新，而不是运行 `git pull`。
