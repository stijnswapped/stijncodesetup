---
name: configure-scs
description: StijnCodeSetup 的交互式安装程序 — 引导用户选择并安装技能和规则到用户级或项目级目录，验证路径，并可选择优化已安装文件。
origin: SCS
---

# 配置 StijnCodeSetup (SCS)

一个交互式、分步安装向导，用于 StijnCodeSetup 项目。使用 `AskUserQuestion` 引导用户选择性安装技能和规则，然后验证正确性并提供优化。

## 何时激活

* 用户说 "configure scs"、"install scs"、"setup stijncodesetup" 或类似表述
* 用户想要从此项目中选择性安装技能或规则
* 用户想要验证或修复现有的 SCS 安装
* 用户想要为其项目优化已安装的技能或规则
...
1. **通过插件**: `/plugin install scs@scs` — 插件会自动加载此技能
2. **手动**: 仅将此技能复制到 `~/.claude/skills/configure-scs/SKILL.md`，然后通过说 "configure scs" 激活

***

## 步骤 0：克隆 SCS 仓库

在任何安装之前，将最新的 SCS 源代码克隆到 `/tmp`：
...
将 `SCS_ROOT=/tmp/stijncodesetup` 设置为所有后续复制操作的源。

如果克隆失败（网络问题等），使用 `AskUserQuestion` 要求用户提供现有 SCS 克隆的本地路径。
...
`claude-api` 是 Anthropic 官方技能；需要时请从 [`anthropics/skills`](https://github.com/anthropics/skills) 安装官方版本，而不是通过 SCS 重复打包。
...
**关键**：只修改安装目标（`$TARGET/`）中的文件，**绝不**修改源 SCS 仓库（`$SCS_ROOT/`）中的文件。
