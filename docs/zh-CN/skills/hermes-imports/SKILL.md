---
name: hermes-imports
description: 将本地 Hermes 操作员工作流转换为经过清理的 SCS 技能和发布包工件。在准备将 Hermes 工作流用于公共 SCS 重用而不泄露私有工作区状态、凭据或仅本地路径时使用。
origin: SCS
---

# Hermes 导入

当需要将重复的 Hermes 工作流转化为可在 SCS 中安全发布的内容时，使用此技能。

Hermes 是操作员外壳。SCS 是可复用工作流层。导入操作应将稳定模式从 Hermes 迁移至 SCS，同时避免移动私有状态。

## 使用时机

* Hermes 工作流重复次数足够多，已具备可复用性
* 本地操作员提示词需要升级为公共 SCS 技能
