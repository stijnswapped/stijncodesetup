...
1. 将 SCS 安装为你的插件（代理、技能、命令、规则）
2. 添加 Plankton 钩子以实现编写时质量强制执行
3. 使用 AgentShield 进行安全审计
4. 在 PR 之前使用 SCS 的 verification-loop 作为最后一道关卡

### 避免钩子冲突

如果同时运行 SCS 和 Plankton 钩子：

* SCS 的 Prettier 钩子和 Plankton 的 biome 格式化程序可能在 JS/TS 文件上冲突
* 解决方案：使用 Plankton 时禁用 SCS 的 Prettier PostToolUse 钩子（Plankton 的 biome 更全面）
* 两者可以在不同的文件类型上共存（SCS 处理 Plankton 未覆盖的内容）
