---
name: scs-guide
description: 在回答之前先读取仓库的实时状态，引导用户了解 SCS 当前的 agents、skills、命令、hooks、规则、安装配置档案以及项目接入流程。
origin: community
---

# SCS 指南

当用户需要帮助来理解、浏览、安装 StijnCodeSetup 或在其中做选择时，使用此技能。
...
依据当前文件回答，而不是凭记忆。SCS 变化很快，硬编码的目录数量、功能列表和安装说明都会过时。

当 SCS 仓库可用时，先检查相关文件再给出具体答案：
...
当用户想为目标仓库配置 SCS 时，使用 `/project-init`。预期顺序为：
