...
description: 审计活跃仓库、MCP服务器、插件、连接器、环境表面和工具设置，然后推荐最高价值的SCS原生技能、钩子、代理和操作员工作流。当用户希望帮助设置Claude Code或了解其环境中实际可用的功能时使用。
origin: SCS
---
...
* 当 SCS 能够合理拥有该表面时，优先选择 SCS 原生工作流，而非通用的"安装另一个插件"建议。
...
* "Stripe 可通过连接的应用使用，但 SCS 缺少计费操作技能"
* "Google Drive 已连接，但 SCS 没有原生的 Google Workspace 操作工作流"
...
2. **同等能力**
   * SCS 已匹配或超越基准的地方
3. **仅有原始形式的差距**
   * 工具存在，但 SCS 缺少简洁的操作技能
4. **缺失的集成**
   * 尚不可用的能力
5. **前 3-5 个下一步行动**
   * 具体的 SCS 原生新增内容，按影响排序
...
* 如果 SCS 已有强大的原始形式，建议封装技能而非发明全新的子系统。

## 良好结果

* 用户可以立即看到已连接的内容、缺失的内容以及 SCS 下一步应拥有的内容。
