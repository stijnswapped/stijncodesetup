...
2. Retrieve the Deep Analyzer Evidence contract from `../SCS-Tools/README.md`
   and the follow-up logic in `../SCS-Tools/src/lib/analyzer.ts`.
3. Match the change to maintained corpus or reference evidence:
   - `../SCS-Tools/src/analyzers/fixtures/deep-analyzer-corpus.ts`
   - `../SCS-Tools/src/analyzers/deep-analyzer-corpus.test.ts`
   - `../SCS-Tools/src/lib/analyzer.compare.test.ts`
...
5. Add or update analyzer corpus, expected-output snapshots, fixtures,
   ...
   surface.
6. Run the relevant validation gate from `../SCS-Tools/`:
