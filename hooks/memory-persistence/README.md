...
These lifecycle hook definitions document SCS's memory persistence contract for Claude Code plugin and manual installs.
...
- `session-activity-tracker.js` records tool usage and file activity for SCS2 status and observability.
...
| `PostToolUse` | `post:session-activity-tracker` | Record tool and file activity for SCS2 metrics | no |
...
- Bound context loaded at session start with `SCS_SESSION_START_MAX_CHARS`.
- Allow opt-out with `SCS_SESSION_START_CONTEXT=off`.
- Keep lifecycle hooks profile-gated through `SCS_HOOK_PROFILE` and `SCS_DISABLED_HOOKS`.
