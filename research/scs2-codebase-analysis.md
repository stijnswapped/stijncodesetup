# SCS2 Codebase Research Report

**Date:** 2026-03-26
**Subject:** `scs-tui` v0.1.0 — Agentic IDE Control Plane
**Total Lines:** 4,417 across 15 `.rs` files

## 1. Architecture Overview

SCS2 is a Rust TUI application that orchestrates AI coding agent sessions. It uses:
...
`dashboard.rs:495` — `new_session()` logs `"New session dialog requested"` but does nothing. Users must use the CLI (`scs start --task "..."`) to create sessions; the TUI dashboard cannot.
...
`Config::load()` reads `~/.claude/scs2.toml` only. The implementation lacks environment variable overrides (e.g., `SCS_DB_PATH`, `SCS_WORKTREE_ROOT`) and CLI flags for configuration.
...
1. **Add environment variable support to `Config::load()`** — `SCS_DB_PATH`, `SCS_WORKTREE_ROOT`, `SCS_DEFAULT_AGENT`. Standard practice for CLI tools.
...
**Bottom line:** SCS2 is a well-structured Rust project with clean error handling, good separation of concerns, and strong security features (risk scoring). The main gaps are incomplete features (comms, new-session dialog, single agent) rather than architectural problems. The codebase is ready for feature work on top of the solid foundation.
