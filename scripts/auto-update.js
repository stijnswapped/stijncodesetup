...
Pull the latest SCS repo changes and reinstall the current context's managed targets
...
function deriveRepoRootFromState(state) {
  ...
  throw new Error('Unable to infer SCS repo root from install-state operations');
}
...
// Recognized SCS package names. A repo root is only trusted to run its
// install-apply.js if its package.json identifies it as SCS — otherwise a
...
const SCS_PACKAGE_NAMES = new Set(['stijncodesetup', 'stijncodesetup']);
...
function validateRepoRoot(repoRoot) {
  ...
  if (!fs.existsSync(packageJsonPath)) {
    throw new Error(`Invalid SCS repo root: missing package.json at ${packageJsonPath}`);
  }

  if (!fs.existsSync(installApplyPath)) {
    throw new Error(`Invalid SCS repo root: missing install script at ${installApplyPath}`);
  }
  ...
  try {
    ...
    throw new Error(`Invalid SCS repo root: unreadable package.json at ${packageJsonPath}`);
  }
  if (!SCS_PACKAGE_NAMES.has(pkgName)) {
    throw new Error(`Refusing to run install from untrusted repo root ${normalized}: package.json name '${pkgName}' is not an official SCS package.`);
  }
...
function runAutoUpdate(options = {}, dependencies = {}) {
  ...
  if (!requestedRepoRoot) {
    ...
    if (uniqueRepoRoots.length > 1) {
      throw new Error(`Multiple SCS repo roots detected: ${uniqueRepoRoots.join(', ')}`);
    }
  }
...
function printHuman(result) {
  if (result.results.length === 0) {
    console.log('No SCS install-state files found for the current home/project context.');
    return;
  }
