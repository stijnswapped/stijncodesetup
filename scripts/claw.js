#!/usr/bin/env node
/**
 * NanoClaw v2 — Barebones Agent REPL for StijnCodeSetup
 *
...
function loadSCSContext(skillList) {
...
function main() {
  ...
  let scsContext = loadSCSContext(state.skills);
  ...
  const prompt = () => {
    rl.question('claw> ', input => {
      ...
      if (line.startsWith('/load ')) {
        ...
        scsContext = loadSCSContext(state.skills);
      ...
      const response = askClaude(scsContext, history, line, state.model);
      console.log(`\n${response}\n`);
