...
# SCS Codex global regression sanity check.
# Validates that global ~/.codex state matches expected SCS integration.
...
HOOKS_DIR_EXPECT="${SCS_GLOBAL_HOOKS_DIR:-$CODEX_HOME/git-hooks}"
...
printf 'SCS GLOBAL SANITY CHECK\n'
printf 'Repo: %s\n' "$REPO_ROOT"
printf 'Codex home: %s\n\n' "$CODEX_HOME"
...
if [[ -f "$AGENTS_FILE" ]]; then
  if search_file '^# StijnCodeSetup \(SCS\)' "$AGENTS_FILE"; then
    ok "AGENTS contains SCS root instructions"
  else
    fail "AGENTS missing SCS root instructions"
  fi

  if search_file '^# Codex Supplement \(From SCS \.codex/AGENTS\.md\)' "$AGENTS_FILE"; then
    ok "AGENTS contains SCS Codex supplement"
  else
    fail "AGENTS missing SCS Codex supplement"
  fi
fi
...
if [[ -f "$CONFIG_FILE" ]]; then
  ...
  # SCS <= 2.0.0 emitted a url-only exa entry that Codex's stdio-only
  ...
  if search_file '^\[mcp_servers\.exa\]' "$CONFIG_FILE"; then
    ...
    if printf '%s\n' "$exa_block" | grep -Eq '^[[:space:]]*url[[:space:]]*=' \
      ...
      fail "MCP section [mcp_servers.exa] uses a url key, which Codex rejects for stdio servers — re-run scs-sync-codex to repair (#2224)"
    else
...
if [[ -d "$SKILLS_DIR" ]]; then
  ...
  if [[ "$missing_skills" -eq 0 ]]; then
    ok "All 16 SCS skills are present in $SKILLS_DIR"
  else
    warn "$missing_skills SCS skills missing from $SKILLS_DIR (install via SCS installer or npx skills)"
  fi
else
  warn "Skills directory missing ($SKILLS_DIR) — install via SCS installer or npx skills"
fi

if [[ -f "$PROMPTS_DIR/scs-prompts-manifest.txt" ]]; then
  ok "Command prompts manifest exists"
else
  fail "Command prompts manifest missing"
fi

if [[ -f "$PROMPTS_DIR/scs-extension-prompts-manifest.txt" ]]; then
  ok "Extension prompts manifest exists"
else
  fail "Extension prompts manifest missing"
fi

command_prompts_count="$(find "$PROMPTS_DIR" -maxdepth 1 -type f -name 'scs-*.md' 2>/dev/null | wc -l | tr -d ' ')"
if [[ "$command_prompts_count" -ge 43 ]]; then
  ok "SCS prompts count is $command_prompts_count (expected >= 43)"
else
  fail "SCS prompts count is $command_prompts_count (expected >= 43)"
fi
...
if command -v scs-sync-codex >/dev/null 2>&1; then
  ok "scs-sync-codex command is in PATH"
else
  warn "scs-sync-codex is not in PATH"
fi

if command -v scs-install-git-hooks >/dev/null 2>&1; then
  ok "scs-install-git-hooks command is in PATH"
else
  warn "scs-install-git-hooks is not in PATH"
fi

if command -v scs-check-codex >/dev/null 2>&1; then
  ok "scs-check-codex command is in PATH"
else
  warn "scs-check-codex is not in PATH (this is expected before alias setup)"
fi

printf '\nSummary: checks=%d, warnings=%d, failures=%d\n' "$checks" "$warnings" "$failures"
if [[ "$failures" -eq 0 ]]; then
  printf 'SCS GLOBAL SANITY: PASS\n'
else
  printf 'SCS GLOBAL SANITY: FAIL\n'
  exit 1
fi
