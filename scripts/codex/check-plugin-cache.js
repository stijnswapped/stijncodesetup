...
function usage() {
  console.log([
    ...
    '  --marketplace <name> Marketplace cache name (default: scs)',
    '  --plugin <name>      Plugin cache name (default: scs)',
...
function checkCache(options) {
  ...
  if (!fs.existsSync(manifestPath)) {
    ...
    if (versions.length > 0) {
      ...
      if (options.marketplace === 'scs' && options.plugin === 'scs') {
        log('Run: codex plugin marketplace add StijnCodeSetup/StijnCodeSetup');
      } else {
  ...
  if (failures > 0) {
    ...
    const syncScript = path.join(REPO_ROOT, 'scripts', 'sync-scs-to-codex.sh');
    if (fs.existsSync(syncScript)) {
      log('Use the supported sync path until the cache contains the referenced files:');
      log('npm install && bash scripts/sync-scs-to-codex.sh');
    } else {
      log('Use the supported manual sync workflow from your SCS installation.');
    }
    return 1;
  }
...
function main() {
  ...
  try {
    ...
    console.error(`[scs-codex] ${error.message}`);
    usage();
    process.exit(1);
  }
