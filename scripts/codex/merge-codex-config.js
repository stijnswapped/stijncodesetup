...
try {
  ...
  console.error('[scs-codex] Missing dependency: @iarna/toml');
  console.error('[scs-codex] Run: npm install   (from the SCS repo root)');
  process.exit(1);
}
...
function warn(message) {
  console.warn(`[scs-codex] WARNING: ${message}`);
}
...
function main() {
  ...
  if (!fs.existsSync(referencePath)) {
    console.error(`[scs-codex] Reference config not found: ${referencePath}`);
    process.exit(1);
  }

  if (!fs.existsSync(configPath)) {
    console.error(`[scs-codex] Config file not found: ${configPath}`);
    process.exit(1);
  }
  ...
  try {
    ...
    console.error(`[scs-codex] Failed to parse TOML: ${error.message}`);
    process.exit(1);
  }
