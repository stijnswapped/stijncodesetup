...
/**
 * Merge SCS-recommended MCP servers into a Codex config.toml.
 *
 ...
 *   - Log warnings when an existing server's config differs from the SCS recommendation.
 *   - With --update-mcp, also replace existing SCS-managed servers.
 *
...
try {
  ...
  console.error('[scs-mcp] Missing dependency: @iarna/toml');
  console.error('[scs-mcp] Run: npm install   (from the SCS repo root)');
  process.exit(1);
}
...
// SCS <= 2.0.0 emitted [mcp_servers.exa] with a `url` key. Codex rejects
// `url` for stdio servers, which makes the *entire* config.toml fail to
// load (#2224). Repair exactly that SCS-emitted form on every merge so
...
// Legacy section names that should be treated as an existing SCS server.
...
function log(msg) {
  console.log(`[scs-mcp] ${msg}`);
}

function warn(msg) {
  console.warn(`[scs-mcp] WARNING: ${msg}`);
}
...
function main() {
  ...
  const disabledServers = new Set(parseDisabledMcpServers(process.env.SCS_DISABLED_MCPS));
  ...
  if (!fs.existsSync(configPath)) {
    console.error(`[scs-mcp] Config file not found: ${configPath}`);
    process.exit(1);
  }

  log(`Package manager: ${PM_NAME} (exec: ${PM_EXEC})`);
  if (disabledServers.size > 0) {
    log(`Disabled via SCS_DISABLED_MCPS: ${[...disabledServers].join(', ')}`);
  }
  ...
  try {
    ...
    console.error(`[scs-mcp] Failed to parse ${configPath}: ${err.message}`);
    process.exit(1);
  }
  ...
  for (const [name, spec] of Object.entries(SCS_SERVERS)) {
    ...
    // (Codex itself rejects url-form stdio servers; SCS only ever emits
    ...
    if (finalEntry) {
      if (updateMcp) {
        ...
        if (legacyName && !hasCanonical) {
          ...
          warn(`mcp_servers.${name} differs from SCS recommendation (run with --update-mcp to refresh)`);
        } else {
  ...
  if (toAppend.length === 0 && !hasRemovals) {
    log('All SCS MCP servers already present. Nothing to do.');
    return;
  }
