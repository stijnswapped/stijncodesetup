...
const MAX_LIMIT = 20;
const SCHEMA_VERSION = 'scs.consult.v1';
...
function showHelp(exitCode = 0) {
  console.log(`
Consult SCS install components and profiles from any project
...
function commandFor(kind, id, target) {
  if (kind === 'profile') {
    return `npx scs install --profile ${id} --target ${target}`;
  }

  return `npx scs install --profile minimal --target ${target} --with ${id}`;
}

function planCommandFor(componentId, target) {
  return `npx scs plan --profile minimal --target ${target} --with ${componentId}`;
}
...
function buildConsultation(options) {
  ...
  return {
    ...
    nextSteps: matches.length > 0
      ...
      : [
        'Run `npx scs catalog components` to browse all components.',
...
function formatText(payload) {
  const lines = [
    `SCS consult (${payload.generatedAt})`,
    `Query: ${payload.query}`,
  ...
  if (payload.matches.length === 0) {
    ...
    lines.push('Try: npx scs catalog components');
  } else {
