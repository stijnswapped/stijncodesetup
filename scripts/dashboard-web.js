#!/usr/bin/env node
/**
 * SCS Capabilities Dashboard — agents, skills, commands, MCPs, rules & hooks
 * With multi-language, routing, search suggestions, recently viewed, fine UI
 *
 * Usage: node scripts/dashboard-web.js [port]
 * Open http://localhost:3456
 *
 * Contribution: https://github.com/StijnCodeSetup/StijnCodeSetup
 */
...
function parsePort(v) {
  ...
  if (isNaN(n) || n < 1 || n > 65535) { console.error('[SCS] Invalid port: ' + v + ' — using 3456'); return 3456; }
  return n;
}
const PORT = parsePort(process.argv[2] || process.env.SCS_DASHBOARD_PORT || '3456');
...
<div class="header">
  <div class="brand" id="brand-link">
    ...
    <h1><span id="t-title">SCS Capabilities</span> <span class="ver">v2.0.0-rc.1</span></h1>
  </div>
...
<div class="footer">
  <a href="https://github.com/StijnCodeSetup/StijnCodeSetup" target="_blank">github.com/StijnCodeSetup/StijnCodeSetup</a>
  <span class="dt"></span>
  <span>SCS v2.0.0-rc.1</span>
  <span class="dt"></span>
  <span id="t-contribution">Contribution to SCS</span>
  <span class="dt"></span>
...
let lang = localStorage.getItem('scs-lang') || 'en';
let suggIdx = -1;
...
// Recently viewed
function recents() { try { return JSON.parse(localStorage.getItem('scs-recent') || '[]'); } catch { return []; } }
function addRecent(type, name) {
  ...
  localStorage.setItem('scs-recent', JSON.stringify(r));
}
function clearRecents() { localStorage.removeItem('scs-recent'); location.hash=''; location.reload(); }
...
// Language
function setLang(l) {
  lang = l; localStorage.setItem('scs-lang', l);
...
function toggleTheme() {
  ...
  localStorage.setItem('scs-theme', h.dataset.theme);
}
if (localStorage.getItem('scs-theme')) document.documentElement.dataset.theme = localStorage.getItem('scs-theme');
...
if (require.main === module) {
  server.listen(PORT, () => {
    console.log(`\n    SCS Capabilities  →  http://localhost:${PORT}\n`);
