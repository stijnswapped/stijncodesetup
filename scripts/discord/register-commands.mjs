#!/usr/bin/env node
// Registers the SCS bot's guild slash commands (bulk overwrite, instant).
...
const COMMANDS = [
  { name: 'scs', description: 'What SCS is + all the links' },
  { name: 'help', description: 'List SCS bot commands' },
  {
    name: 'skill',
    description: 'Look up an SCS skill by name',
    options: [{ type: 3, name: 'name', description: 'skill name or keyword', required: true }],
  },
  {
    name: 'docs',
    description: 'Search the SCS docs',
    options: [{ type: 3, name: 'query', description: 'search terms', required: true }],
  },
  { name: 'release', description: 'Latest SCS release' },
];
