#!/usr/bin/env node
// SCS community Discord bot — dependency-free (Node 22+ native WebSocket).
// Slash commands: /scs /help /skill /docs /release
//
// Env: DISCORD_BOT_TOKEN (required), DISCORD_APP_ID (required),
//      SCS_REPO (path to local clone, default ~/GitHub/SCS/stijncodesetup),
//      DISCORD_INVITE (optional, shown in /scs)
//
...
const REPO = process.env.SCS_REPO || join(homedir(), 'GitHub/SCS/stijncodesetup');
const REPO_URL = 'https://github.com/StijnCodeSetup/StijnCodeSetup';
...
const HELP = [
  '**SCS bot commands**',
  '- `/scs` — what SCS is + all the links',
  '- `/skill name:<query>` — look up an SCS skill',
  '- `/docs query:<terms>` — search the SCS docs',
  '- `/release` — latest SCS release',
  '- `/help` — this message',
].join('\n');

const handlers = {
  scs: () => [
    '**StijnCodeSetup (SCS)** — the agent harness performance system.',
    ...
    '',
    `- repo: ${REPO_URL}`,
    '- site: https://scs.tools',
    `- install: \`/plugin marketplace add StijnCodeSetup/stijncodesetup\` then \`/plugin install scs\``,
  ...
  release: async () => {
    const res = await fetch('https://api.github.com/repos/StijnCodeSetup/StijnCodeSetup/releases/latest', {
      headers: { 'User-Agent': 'scs-discord-bot' },
    });
  ...
  ws.onmessage = ev => {
    ...
    switch (msg.op) {
      case 10: { // HELLO
        ...
        send({ op: 2, d: { token: TOKEN, intents: 1, properties: { os: 'darwin', browser: 'scs-bot', device: 'scs-bot' } } });
        break;
      }
