...
function detectTargetMode(rootDir) {
  ...
  if (packageJson?.name === 'stijncodesetup') {
    return 'repo';
  }
...
const SCS_PLUGIN_KEY_PATTERNS = [
  /^scs@/i,
  /^stijncodesetup@/i,
];

const SCS_LEGACY_PLUGIN_DIRS = [
  'scs',
  'scs@scs',
  'stijncodesetup',
...
const SCS_CACHE_MARKETPLACES = ['stijncodesetup', 'scs'];
const SCS_CACHE_PLUGIN_NAMES = ['scs', 'stijncodesetup'];
...
function findPluginInstallFromManifest(installedPluginsPaths) {
  for (const installedPath of installedPluginsPaths) {
    ...
    for (const [key, value] of Object.entries(manifest.plugins)) {
      if (!SCS_PLUGIN_KEY_PATTERNS.some(pattern => pattern.test(key))) {
        continue;
      }
...
function findPluginInstallFlatLayout(candidateRoots) {
  for (const pluginsDir of candidateRoots) {
    for (const pluginDir of SCS_LEGACY_PLUGIN_DIRS) {
...
function findPluginInstallMarketplaceCache(candidateRoots) {
  for (const pluginsDir of candidateRoots) {
    for (const marketplace of SCS_CACHE_MARKETPLACES) {
      for (const pluginName of SCS_CACHE_PLUGIN_NAMES) {
  ...
  return [
    {
      ...
      points: 4,
      scopes: ['repo'],
      path: '~/.claude/plugins/scs/ (legacy stijncodesetup paths also supported)',
      description: 'StijnCodeSetup is installed for the active user or project',
      pass: Boolean(pluginInstall),
      fix: 'Install the SCS plugin for this user or project before auditing project-specific harness quality.',
    },
    {
      ...
      fix: 'Add project-local .claude hooks, commands, skills, or settings that tailor SCS to this repo.',
    },
    {
...
Audits the current working directory by default and auto-detects SCS repo mode vs consumer-project mode.
`);
