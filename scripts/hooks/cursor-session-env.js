#!/usr/bin/env node
/**
 * Cursor sessionStart hook — inject SCS_AGENT_DATA_HOME for the composer session.
 *
  ...
  readStdinJson()
    .then(() => {
      ...
      const payload = {
        env: envPayload,
        additional_context: [
          'SCS memory persistence uses a dedicated agent data root for this Cursor session.',
          `${AGENT_DATA_HOME_ENV}=${agentDataHome}`,
          'Session summaries, learned skills, aliases, and metrics live under that directory.',
          'Override via shell env, project .cursor/scs-agent-data.json, or SCS docs (issue #2065).',
        ].join('\n'),
      };
