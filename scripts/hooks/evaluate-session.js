...
async function main() {
  ...
  try {
    ...
    const fallbackPath = path.join(os.tmpdir(), 'scs-learned-skills');
    log(`[ContinuousLearning] Failed to create learned skills directory: ${error.message}`);
    log(`[ContinuousLearning] Falling back to: ${fallbackPath}`);
    ensureDir(fallbackPath);
    learnedSkillsPath = fallbackPath;
  }
