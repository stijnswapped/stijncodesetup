...
const SCS_DISABLE_VALUES = new Set(['0', 'false', 'off', 'disabled', 'disable']);
const SCS_ENABLE_VALUES = new Set(['1', 'true', 'on', 'enabled', 'enable', 'yes']);
...
function isRoutineBashGateDisabled() {
  return SCS_ENABLE_VALUES.has(normalizeEnvValue(process.env.GATEGUARD_BASH_ROUTINE_DISABLED));
}
...
function isGateGuardDisabled() {
  ...
  return SCS_DISABLE_VALUES.has(normalizeEnvValue(process.env.SCS_GATEGUARD));
}
...
function resolveSessionKey(data) {
  const directCandidates = [data && data.session_id, data && data.sessionId, data && data.session && data.session.id, process.env.CLAUDE_SESSION_ID, process.env.SCS_SESSION_ID];
...
function condensedGateMsg(action, filePath, ordinal) {
  ...
  return (
    ...
    '(SCS_GATEGUARD=off disables this gate.)'
  );
}
...
function withRecoveryHint(message, hookIds = [EDIT_WRITE_HOOK_ID]) {
  ...
  return [message, '', `Recovery: if GateGuard is blocking setup or repair work, run this session with \`SCS_GATEGUARD=off\` or add ${disableTargets} to \`SCS_DISABLED_HOOKS\`.`].join('\n');
}
