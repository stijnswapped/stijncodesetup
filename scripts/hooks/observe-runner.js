...
function getPluginRoot(options = {}) {
  ...
  if (process.env.SCS_PLUGIN_ROOT && process.env.SCS_PLUGIN_ROOT.trim()) {
    return process.env.SCS_PLUGIN_ROOT.trim();
  }
...
function getPhaseFromHookId(hookId) {
  const prefix = String(hookId || process.env.SCS_HOOK_ID || '').split(':')[0];
  return prefix === 'pre' || prefix === 'post' ? prefix : null;
}

function getTimeoutMs() {
  const parsed = Number.parseInt(process.env.SCS_OBSERVE_RUNNER_TIMEOUT_MS || '', 10);
...
if (require.main === module) {
  ...
  const output = run(raw, { hookId: process.argv[2] || process.env.SCS_HOOK_ID });
