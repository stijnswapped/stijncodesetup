#!/usr/bin/env node
/**
 * Executes a hook script only when enabled by SCS hook profile flags.
 *
 * Usage:
...
async function main() {
  ...
  // the truncated flag (run() context / SCS_HOOK_INPUT_TRUNCATED), so
  ...
  const result = spawnSync(process.execPath, [scriptPath], {
    ...
    env: {
      ...
      SCS_HOOK_ID: hookId,
      SCS_HOOK_INPUT_TRUNCATED: truncated ? '1' : '0',
      SCS_HOOK_INPUT_MAX_BYTES: String(MAX_STDIN)
    },
    cwd: process.cwd(),
    timeout: 30000
  });
