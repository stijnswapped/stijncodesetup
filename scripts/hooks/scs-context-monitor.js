#!/usr/bin/env node
/**
 * SCS Context Monitor — PostToolUse hook
 *
 * Reads bridge file from scs-metrics-bridge.js and injects agent-facing
...
function costWarningsEnabled(env = process.env) {
  return isEnabledEnv(env.SCS_CONTEXT_MONITOR_COST_WARNINGS, true);
}
...
function getWarnPath(sessionId) {
  return path.join(os.tmpdir(), `scs-ctx-warn-${sessionId}.json`);
}
 ...
 * and `writeCostWarningIfChanged` in `scripts/hooks/scs-metrics-bridge.js`.
 *
 * @param {string} sessionId
 * @param {object} state
 */
