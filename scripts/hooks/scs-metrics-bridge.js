#!/usr/bin/env node
/**
 * SCS Metrics Bridge — PostToolUse hook
 *
 * Maintains a running session aggregate in /tmp/scs-metrics-{session}.json.
 * This bridge file is read by scs-statusline.js and scs-context-monitor.js,
...
const WARNING_CACHE_PREFIX = 'scs-metrics-cost-warnings-';
  ...
  try {
    ...
    if (malformed > 0) {
      writeCostWarningIfChanged(
        ...
        `[scs-metrics-bridge] skipped ${malformed} malformed line(s) in ${costsPath}\n`
      );
    }
    ...
    if (err && err.code !== 'ENOENT') {
      writeCostWarningIfChanged(
        ...
        `[scs-metrics-bridge] failing open after ${err.name || 'error'} reading ${costsPath}: ${err.message || String(err)}\n`
      );
    }
...
function run(rawInput) {
  try {
    ...
    const sessionId = sanitizeSessionId(input.session_id) || sanitizeSessionId(process.env.SCS_SESSION_ID) || sanitizeSessionId(process.env.CLAUDE_SESSION_ID);
