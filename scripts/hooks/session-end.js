...
const SUMMARY_START_MARKER = '<!-- SCS:SUMMARY:START -->';
const SUMMARY_END_MARKER = '<!-- SCS:SUMMARY:END -->';
...
function getSessionMetadata() {
  const branchResult = runCommand('git symbolic-ref --short HEAD');
  const fallbackBranchResult = branchResult.success
    ? branchResult
    : runCommand('git rev-parse --abbrev-ref HEAD');

  return {
    project: getProjectName() || 'unknown',
    branch: fallbackBranchResult.success ? fallbackBranchResult.output : 'unknown',
...
async function main() {
  ...
  if (transcriptPath && summary && fs.existsSync(transcriptPath)) {
    ...
    const interval = parseInt(process.env.SCS_LLM_SUMMARY_INTERVAL || '50', 10);
