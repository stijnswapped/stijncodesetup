 ...
 *
 * Bootstrap loader for the SCS SessionStart hook.
 *
 ...
 *   2. Resolves the SCS plugin root directory (via CLAUDE_PLUGIN_ROOT env var
...
const { resolveScsRoot } = require('../lib/resolve-scs-root');
...
// Resolve the SCS plugin root via the shared resolver, probing for the runner
// so a valid root is one that actually contains run-with-flags.js.
const root = resolveScsRoot({ probe: rel });
...
process.stderr.write(
  '[SessionStart] WARNING: could not resolve SCS plugin root; skipping session-start hook\n'
);
process.stdout.write(raw);
