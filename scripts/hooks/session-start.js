...
/**
 * Resolve session retention days from the SCS_SESSION_RETENTION_DAYS env var.
 *
...
function getSessionRetentionDays() {
  const raw = process.env.SCS_SESSION_RETENTION_DAYS;
...
function isSessionStartContextDisabled() {
  const raw = String(process.env.SCS_SESSION_START_CONTEXT || '').trim().toLowerCase();
  return ['0', 'false', 'off', 'none', 'disabled'].includes(raw);
}

function getSessionStartMaxContextChars() {
  const raw = process.env.SCS_SESSION_START_MAX_CHARS;
 ...
 * SessionStart. Overridable via `SCS_INSTINCT_CONFIDENCE_THRESHOLD`
...
function getInstinctConfidenceThreshold() {
  const raw = process.env.SCS_INSTINCT_CONFIDENCE_THRESHOLD;
 ...
 * Overridable via `SCS_MAX_INJECTED_INSTINCTS` (a positive integer);
...
function getMaxInjectedInstincts() {
  const raw = process.env.SCS_MAX_INJECTED_INSTINCTS;
...
function limitSessionStartContext(additionalContext, maxChars = getSessionStartMaxContextChars()) {
  ...
  const marker = '\n\n[SessionStart truncated context. Set SCS_SESSION_START_MAX_CHARS to raise the cap or SCS_SESSION_START_CONTEXT=off to disable injected context.]';
...
async function main() {
  ...
  if (retentionDays === null) {
    log('[SessionStart] Pruning disabled via SCS_SESSION_RETENTION_DAYS');
  } else {
  ...
  if (explicitContextDisabled) {
    log('[SessionStart] Additional context injection disabled by SCS_SESSION_START_CONTEXT');
  } else if (maxContextChars === 0) {
    log('[SessionStart] Additional context injection disabled by SCS_SESSION_START_MAX_CHARS=0');
  }
