/**
 * Resolve SCS agent data home (memory persistence root) across harnesses.
 *
 ...
 * purpose, consistent with SCS script modules elsewhere. Automated PR reviewers
...
const AGENT_DATA_HOME_ENV = 'SCS_AGENT_DATA_HOME';
const DEFAULT_CLAUDE_DIR_NAME = '.claude';
const DEFAULT_CURSOR_SCS_DIR_SEGMENTS = ['.cursor', 'scs'];
const PROJECT_CONFIG_RELATIVE = path.join('.cursor', 'scs-agent-data.json');
 ...
 * dependency and risk divergent defaults for `~/.cursor/scs` vs `~/.claude`.
 *
...
/**
 * Project root for a config file under .cursor/scs-agent-data.json.
 */
...
function getDefaultCursorAgentDataHome() {
  return path.join(getHomeDirFromEnv(), ...DEFAULT_CURSOR_SCS_DIR_SEGMENTS);
}
...
function readProjectConfigAt(configPath) {
  ...
  try {
    ...
    const candidate = parsed.agentDataHome || parsed.SCS_AGENT_DATA_HOME;
    if (typeof candidate !== 'string' || !candidate.trim()) return null;
    const projectRoot = resolveProjectRootFromConfigPath(configPath);
    return expandHomePath(candidate, projectRoot);
  } catch (error) {
    console.error(
      `[SCS] Failed to read or parse agent data config at ${configPath}: ${error.message}`
    );
    return null;
  }
}
...
/**
 * Set SCS_AGENT_DATA_HOME on the current process when unset (hook subprocess safety net).
...
function getCursorSessionEnvPayload(options = {}) {
  ...
  return {
    SCS_AGENT_DATA_HOME: agentDataHome,
  };
}
...
module.exports = {
  ...
  DEFAULT_CURSOR_SCS_DIR_SEGMENTS,
