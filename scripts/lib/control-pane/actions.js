...
const ACTION_DEFINITIONS = new Map([
  [
    'sync-knowledge',
    {
      ...
      description: 'Import all configured SCS2 memory connectors into the context graph.',
      args: ({ limit }) => [
        'run',
        '--quiet',
        '--',
  ...
  [
    'recall-knowledge',
    {
      ...
      description: 'Run SCS2 context recall for the current operator query.',
      args: ({ query, limit }) => [
        'run',
        '--quiet',
        '--',
  ...
  [
    'graph-sync',
    {
      ...
      description: 'Backfill the SCS2 graph from sessions, decisions, file activity, and messages.',
      args: ({ limit }) => [
        'run',
        '--quiet',
        '--',
  ...
  [
    ...
    {
      label: 'Open TUI',
      description: 'Launch the SCS2 terminal dashboard.',
...
function buildControlPaneAction(actionId, options = {}) {
  ...
  const cwd = path.join(repoRoot, 'scs2');
