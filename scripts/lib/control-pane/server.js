...
function usage() {
  return [
    'Usage:',
    '  node scripts/control-pane.js [--host 127.0.0.1] [--port 8765] [--db <scs2.db>] [--state-db <state.db>] [--config <scs2.toml>] [--query <text>]',
    '',
    'Options:',
    '  --state-db <path>  Read agent work items from an SCS state-store database',
...
function createControlPaneServer(options = {}) {
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (req.method === 'GET' && requestUrl.pathname === '/assets/scs-icon.svg') {
        const iconPath = path.join(repoRoot, 'assets', 'scs-icon.svg');
