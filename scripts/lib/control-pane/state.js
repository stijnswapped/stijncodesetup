...
const SNAPSHOT_SCHEMA_VERSION = 'scs.control-pane.snapshot.v1';
const DEFAULT_STATE_STORE_RELATIVE_PATH = path.join('.claude', 'scs', 'state.db');
...
function defaultDbPath(env = process.env) {
  return path.join(homeDir(env), '.claude', 'scs2.db');
}
...
function defaultConfigPaths(cwd = process.cwd(), env = process.env) {
  ...
  const paths = [path.join(home, 'Library', 'Application Support', 'scs2', 'config.toml'), path.join(home, '.config', 'scs2', 'config.toml'), path.join(home, '.claude', 'scs2.toml')];

  let current = path.resolve(cwd);
  while (current && current !== path.dirname(current)) {
    paths.push(path.join(current, '.claude', 'scs2.toml'));
    paths.push(path.join(current, 'scs2.toml'));
...
function resolveControlPaneConfig(options = {}) {
  ...
  return {
    ...normalizeConfig(merged, {
      env,
      dbPath: options.dbPath || env.SCS2_DB_PATH || null,
      stateDbPath: options.stateDbPath || env.SCS_STATE_DB_PATH || null
    }),
