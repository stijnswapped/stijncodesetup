...
<body>
  <div class="shell">
    <header>
      <div class="brand">
        <img src="/assets/scs-icon.svg" alt="">
        <h1>SCS Control Pane</h1>
      </div>
  ...
  <script>
    ...
    function renderSessions(sessions) {
      if (!sessions.length) {
        $('#sessions').innerHTML = '<div class="empty">No SCS2 sessions found.</div>';
        return;
      }
      ...
      document.querySelectorAll('#work-items [data-wi-action]').forEach(button => {
        button.addEventListener('click', () => {
          ...
          if (button.getAttribute('data-wi-action') === 'claim') {
            window.scsClaimItem(id);
          } else {
            window.scsMoveItem(id, button.getAttribute('data-wi-lane'));
          }
        });
      });
    }
    ...
    window.scsClaimItem = function (id) {
    ...
    window.scsMoveItem = function (id, lane) {
      if (!state.allowActions) return;
