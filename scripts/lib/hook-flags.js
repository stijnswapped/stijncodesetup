 ...
 *
 * Controls:
 * - SCS_HOOK_PROFILE=minimal|standard|strict (default: standard)
 * - SCS_DISABLED_HOOKS=comma,separated,hook,ids
 */
...
function getHookProfile() {
  const raw = String(process.env.SCS_HOOK_PROFILE || 'standard').trim().toLowerCase();
  return VALID_PROFILES.has(raw) ? raw : 'standard';
}

function getDisabledHookIds() {
  const raw = String(process.env.SCS_DISABLED_HOOKS || '');
...
function isDryRun() {
  return process.env.SCS_DRY_RUN === '1';
}
