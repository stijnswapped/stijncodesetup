...
const CLAUDE_SCS_NAMESPACE = 'scs';
const EXCLUDED_GENERATED_SOURCE_SUFFIXES = ['/scs-install-state.json', '/scs/install-state.json'];
...
function addCursorAgentDataScaffoldOperations(operations, options) {
  ...
  addFileCopyOperation(operations, {
    ...
    sourceRelativePath: path.join('scaffolds', 'cursor', 'scs-agent-data.json'),
    destinationPath: path.join(options.targetRoot, 'scs-agent-data.json'),
  ...
  addFileCopyOperation(operations, {
    moduleId: options.moduleId,
    sourceRoot: options.sourceRoot,
    sourceRelativePath: path.join('scaffolds', 'cursor', 'rules', 'scs-agent-data-home.mdc'),
    destinationPath: path.join(options.targetRoot, 'rules', 'scs-agent-data-home.mdc'),
    strategy: 'preserve-relative-path'
  });
...
function planClaudeStyleLegacyInstall(context, { adapterId, adapterRootInput, rulesDir: rulesDirOverride }) {
  ...
  const rulesDir = rulesDirOverride || path.join(targetRoot, 'rules', CLAUDE_SCS_NAMESPACE);
