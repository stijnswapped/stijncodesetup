 ...
 * own authentication.
 *
 * Recursion guard: sets SCS_SKIP_LLM_SUMMARY=1 in subprocess env so any Stop
...
function getLLMModel() {
  return process.env.SCS_LLM_SUMMARY_MODEL || 'haiku';
}

function getContextThreshold() {
  const raw = parseInt(process.env.SCS_LLM_SUMMARY_CONTEXT_THRESHOLD || '20', 10);
...
function generateSessionSummary(transcriptPath) {
  if (process.env.SCS_SKIP_LLM_SUMMARY) return null;
