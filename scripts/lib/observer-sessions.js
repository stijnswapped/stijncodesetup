...
function getHomunculusDir() {
  ...
  if (override) {
    ...
    process.stderr.write(`[scs] CLV2_HOMUNCULUS_DIR=${override} is not absolute; ignoring\n`);
  }

  const xdgDataHome = process.env.XDG_DATA_HOME;
  if (xdgDataHome) {
    if (path.isAbsolute(xdgDataHome)) {
      return path.join(xdgDataHome, 'scs-homunculus');
    }
    process.stderr.write(`[scs] XDG_DATA_HOME=${xdgDataHome} is not absolute; ignoring\n`);
  }

  return path.join(os.homedir(), '.local', 'share', 'scs-homunculus');
}
