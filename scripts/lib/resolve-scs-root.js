...
const CURRENT_PLUGIN_SLUG = 'scs';
const LEGACY_PLUGIN_SLUG = 'stijncodesetup';
...
/**
 * Resolve the SCS source root directory.
 *
 * Tries, in order:
 ...
 *   4. Plugin cache auto-detection — scans ~/.claude/plugins/cache/{scs,stijncodesetup}/
 ...
 *                                    contains SCS scripts. Default: 'scripts/lib/utils.js'
 * @returns {string} Resolved SCS root path
 */
function resolveScsRoot(options = {}) {
 ...
 *
 * Earlier revisions inlined the *entire* resolveScsRoot() search (~700 chars,
 ...
 * locations resolveScsRoot() knows about — ~/.claude, the exact plugin roots
 * under ~/.claude/plugins/, and the versioned plugin cache — only far enough to
 * load the committed resolve-scs-root module, then delegates the authoritative
 * decision to resolveScsRoot(). This keeps discovery behaviour identical to the
