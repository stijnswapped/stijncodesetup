...
const SESSION_SCHEMA_VERSION = 'scs.session.v1';
const SESSION_RECORDING_SCHEMA_VERSION = 'scs.session.recording.v1';
const DEFAULT_RECORDING_DIR = path.join(os.tmpdir(), 'scs-session-recordings');
...
function resolveRecordingDir(options = {}) {
  ...
  if (typeof process.env.SCS_SESSION_RECORDING_DIR === 'string' && process.env.SCS_SESSION_RECORDING_DIR.length > 0) {
    return path.resolve(process.env.SCS_SESSION_RECORDING_DIR);
  }
