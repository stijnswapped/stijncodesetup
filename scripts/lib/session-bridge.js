...
/**
 * Shared session bridge utilities for SCS hooks.
 *
...
function getBridgePath(sessionId) {
  return path.join(os.tmpdir(), `scs-metrics-${sessionId}.json`);
}
 ...
 * writers (e.g. PostToolUse `scs-metrics-bridge` and the background
 * `scs-statusline`, both writing to the same session bridge) do not
 ...
 * `scripts/hooks/scs-metrics-bridge.js` (commit 9b1d8918) for the
...
function resolveSessionId() {
  const raw = process.env.SCS_SESSION_ID || process.env.CLAUDE_SESSION_ID || '';
