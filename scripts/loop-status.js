...
function buildStatus(options = {}) {
  ...
  return {
    ...
    errors,
    schemaVersion: 'scs.loop-status.v1',
    sessions,
    source: {
...
function formatText(payload) {
  ...
  if (payload.sessions.length === 0) {
    const lines = [
      `SCS loop status (${payload.generatedAt})`,
  ...
  const lines = [`SCS loop status (${payload.generatedAt})`];
...
function writeStatusSnapshots(payload, writeDir) {
  ...
  const sessions = payload.sessions.map(session => {
    ...
    atomicWriteJson(snapshotPath, {
      ...
      schemaVersion: 'scs.loop-status.session.v1',
      session,
    });
  ...
  atomicWriteJson(indexPath, {
    ...
    generatedAt: payload.generatedAt,
    schemaVersion: 'scs.loop-status.index.v1',
