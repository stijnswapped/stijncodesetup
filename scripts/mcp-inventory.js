...
function usage() {
  return [
    ...
    'Codex, OpenCode), normalize them to scs.mcp.v1, and report which servers',
    'are configured in more than one harness. Secrets are never printed; only',
    'env key names are shown.',
    '',
    'Options:',
    '  --json              Print the full scs.mcp.v1 inventory as JSON',
...
function formatHumanReport(inventory, options = {}) {
  ...
  lines.push('MCP Inventory (scs.mcp.v1)');
  lines.push(
