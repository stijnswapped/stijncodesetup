...
function usage() {
  console.log([
    ...
    '',
    'Deterministic SCS 2.0 observability readiness gate.',
    '',
    'Options:',
...
function buildChecks(rootDir) {
  ...
  const observabilityRust = readText(rootDir, 'scs2/src/observability/mod.rs');
  const sessionStoreRust = readText(rootDir, 'scs2/src/session/store.rs');
  const sessionManagerRust = readText(rootDir, 'scs2/src/session/manager.rs');
  ...
  const gaRoadmap = readText(rootDir, 'docs/SCS-2.0-GA-ROADMAP.md');
  ...
  const releaseSurfaceTest = readText(rootDir, 'tests/docs/scs2-release-surface.test.js');
  ...
  return [
    ...
    },
    {
      ...
      pass: fileExists(rootDir, 'docs/architecture/hud-status-session-control.md')
        ...
        && hudStatusFixture.schema_version === 'scs.hud-status.v1'
    ...
    {
      id: 'scs2-tool-risk-ledger',
      category: 'Tool Activity',
      points: 3,
      path: 'scs2/src/observability/mod.rs',
      description: 'SCS2 records tool calls with risk scoring and paginated queries',
      pass: fileExists(rootDir, 'scs2/src/observability/mod.rs')
      ...
      fix: 'Restore SCS2 tool logging, risk scoring, store queries, and metrics sync.'
    },
    {
    ...
    },
    {
      ...
      pass: fileExists(rootDir, 'docs/releases/2.0.0-rc.1/publication-readiness.md')
        ...
        && fileExists(rootDir, 'tests/docs/scs2-release-surface.test.js')
...
function buildReport(rootDir) {
  ...
  return {
    schema_version: 'scs.observability-readiness.v1',
