...
const SCHEMA_VERSION = 'scs.operator-readiness-dashboard.v1';
...
function usage() {
  console.log([
    ...
    '',
    'Generate the SCS operator readiness dashboard and prompt-to-artifact audit.',
    '',
    'Options:',
...
function scsToolsNextLevelEvidence(roadmap) {
  if (roadmap.includes('announcementGateReady` is `true')
    ...
    return 'billing announcement gate, selected-target announcement gate, billing gate env-file operator path, non-breaking operator bearer path, hosted analysis lanes, AgentShield fleet-summary consumption, hosted finding evidence paths, harness-route policy linking, policy-promotion Action-output telemetry, operator-visible promotion output details, hosted promotion judge audit traces, billing announcement preflight, aggregate production billing KV readback, Wrangler selected-target readback, target-account billing readback, provenance-aware Marketplace billing-state gates, sanitized Marketplace plan/action provenance counts, ready Marketplace Pro target selection, hosted team-learning feedback controls, and SCS-Tools Dependabot alert remediation are mirrored in the GA roadmap';
  }
  ...
  if (roadmap.includes('selected-target official announcement gate')
    ...
    return 'billing announcement gate, selected-target announcement gate, billing gate env-file operator path, hosted analysis lanes, AgentShield fleet-summary consumption, hosted finding evidence paths, harness-route policy linking, policy-promotion Action-output telemetry, operator-visible promotion output details, hosted promotion judge audit traces, billing announcement preflight, aggregate production billing KV readback, Wrangler OAuth readback, target-account billing readback, provenance-aware Marketplace billing-state gates, sanitized Marketplace plan/action provenance counts, ready Marketplace Pro target selection, hosted team-learning feedback controls, and SCS-Tools Dependabot alert remediation are mirrored in the GA roadmap';
  }

  if (roadmap.includes('69ca535')
    || roadmap.includes('team feedback controls')
    || roadmap.includes('e56fc1a')) {
    return 'billing announcement gate, hosted analysis lanes, AgentShield fleet-summary consumption, hosted finding evidence paths, harness-route policy linking, policy-promotion Action-output telemetry, operator-visible promotion output details, hosted promotion judge audit traces, billing announcement preflight, aggregate production billing KV readback, Wrangler OAuth readback, target-account billing readback, provenance-aware Marketplace billing-state gates, sanitized Marketplace plan/action provenance counts, hosted team-learning feedback controls, and SCS-Tools Dependabot alert remediation are mirrored in the GA roadmap';
  }
...
function scsToolsNextLevelGap(roadmap) {
  if (roadmap.includes('announcementGateReady` is `true')
    || roadmap.includes('Native GitHub payments announcement gate is ready')
    || roadmap.includes('d3d62df83fa075660fa4530c3e0edc311a4355fe')) {
...
function hasCurrentLinearProgressSync({ roadmap, progressSync }) {
  ...
  const hasMay19ProgressSurface = roadmap.includes('scs-may-19-post-pr-2002-sync-64cef8f668e0')
...
function linearProgressEvidence(context) {
  if (hasCurrentLinearProgressSync(context)) {
    if (context.roadmap.includes('467d148a-712a-4777-aad9-95593e9f1739')
      ...
      return 'Linear live sync is current with the May 20 Marketplace Pro release-gate comments on ITO-61 and the SCS platform roadmap; progress-sync contract defines the file-backed work-items/status path';
    }

    if (context.roadmap.includes('scs-may-19-post-pr-2002-sync-64cef8f668e0')) {
...
function buildGrowthSummary(rootDir) {
  const hypergrowth = readText(rootDir, 'docs/releases/2.0.0/scs-2-hypergrowth-release-command-center.md');
...
function buildRequirements(rootDir, platformReport) {
  const roadmap = readText(rootDir, 'docs/SCS-2.0-GA-ROADMAP.md');
  ...
  const hypergrowthCommandCenter = readText(rootDir, 'docs/releases/2.0.0/scs-2-hypergrowth-release-command-center.md');
  ...
  const releaseVideoSuiteReady = scripts['release:video-suite'] === 'node scripts/release-video-suite.js'
    ...
    && includesAll(releaseVideoProduction, [
      'SCS 2.0 Video Suite Production Manifest',
      'Primary launch video',
      'Self-Eval Gate',
      'timeline',
    ])
    && includesAll(releaseVideoSuite, [
      'scs.release-video-suite.v1',
  ...
  return [
    ...
    buildRequirement(
      ...
      'docs/releases/2.0.0/scs-2-hypergrowth-release-command-center.md plus May 19 evidence',
    ...
    buildRequirement(
      'release-video-suite',
      'Produce the SCS 2.0 release video suite',
    ...
    buildRequirement(
      'scs-tools-next-level',
      'Advance SCS Tools native payments and AI-native harness-agnostic app',
      'SCS Tools PR evidence, billing gate, hosted analysis lanes',
      includesAll(roadmap, ['SCS-Tools PR #78', 'hosted promotion', 'announcementGate'])
        ? 'in_progress'
        : 'not_complete',
      scsToolsNextLevelEvidence(roadmap),
      scsToolsNextLevelGap(roadmap)
    ),
    buildRequirement(
      'legacy-salvage',
    ...
    buildRequirement(
      ...
      'Provide SCS 2.0 observability for self-use',
...
function renderText(report) {
  const lines = [
    `SCS Operator Readiness Dashboard: ${report.ready ? 'objective ready' : 'work remaining'}`,
...
function renderMarkdown(report) {
  const lines = [
    '# SCS Operator Readiness Dashboard',
    '',
    ...
    '',
    'Growth lanes: GitHub Sponsors and OSS partner sponsors; SCS Tools Pro subscriptions; consulting and implementation contracts; talks, podcasts, conference demos, and partner webinars.',
    '',
