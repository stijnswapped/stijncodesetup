...
const SCHEMA_VERSION = 'scs.platform-audit.v1';
...
function usage() {
  console.log([
    ...
    '',
    'Operator readiness audit for SCS queue, discussion, roadmap, release, and security evidence.',
    '',
    'Options:',
...
function runGhJson(args, options = {}) {
  const shimPath = process.env.SCS_GH_SHIM;
...
function buildLocalEvidenceChecks(rootDir) {
  ...
  return [
    ...
    buildCheck(
      ...
      includesAll(roadmap, ['linear.app/itomarkets/project/scs-platform-roadmap', 'ITO-44', 'ITO-59']) ? 'pass' : 'fail',
      'repo roadmap mirrors the Linear roadmap and security/operator lanes',
      { path: 'docs/SCS-2.0-GA-ROADMAP.md' }
    ),
    buildCheck(
...
function buildReport(options) {
  ...
  checks.push(buildCheck(
    ...
    { fix: 'Re-run with working gh authentication or SCS_GH_SHIM for deterministic tests.' }
  ));
...
function renderText(report) {
  const lines = [
    `SCS Platform Audit: ${report.ready ? 'ready' : 'attention required'}`,
