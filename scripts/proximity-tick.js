 ...
 * sending session-to-session messages via `scs-tui messages send`.
 *
 ...
 *
 * Messages are internal SCS agent-to-agent coordination (the scs2 `messages`
...
const { createScsMessageSink } = require('./lib/control-pane/message-sink');
...
async function main() {
  ...
  const sink = opts.dryRun ? null : createScsMessageSink({});
