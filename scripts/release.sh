...
CODEX_MARKETPLACE_PLUGIN_JSON="plugins/scs/.codex-plugin/plugin.json"
OPENCODE_PACKAGE_JSON=".opencode/package.json"
OPENCODE_PACKAGE_LOCK_JSON=".opencode/package-lock.json"
OPENCODE_SCS_HOOKS_PLUGIN=".opencode/plugins/scs-hooks.ts"
...
for FILE in "$ROOT_PACKAGE_JSON" "$PACKAGE_LOCK_JSON" "$ROOT_AGENTS_MD" "$TR_AGENTS_MD" "$ZH_CN_AGENTS_MD" "$AGENT_YAML" "$VERSION_FILE" "$PLUGIN_JSON" "$MARKETPLACE_JSON" "$CODEX_MARKETPLACE_JSON" "$CODEX_PLUGIN_JSON" "$OPENCODE_PACKAGE_JSON" "$OPENCODE_PACKAGE_LOCK_JSON" "$OPENCODE_SCS_HOOKS_PLUGIN" "$README_FILE" "$ROOT_ZH_CN_README_FILE" "$TR_README_FILE" "$PT_BR_README_FILE" "$ZH_CN_README_FILE" "$SELECTIVE_INSTALL_ARCHITECTURE_DOC"; do
  if [[ ! -f "$FILE" ]]; then
...
update_codex_marketplace_version() {
  node -e '
    ...
    const plugin = marketplace.plugins.find(entry => entry && entry.name === "scs");
    if (!plugin || typeof plugin !== "object") {
      console.error(`Error: could not find scs plugin entry in ${file}`);
      process.exit(1);
    }
...
update_opencode_hook_banner_version() {
  node -e '
    ...
    const current = fs.readFileSync(file, "utf8");
    const updated = current.replace(
      /(## Active Plugin: StijnCodeSetup v)[0-9]+\.[0-9]+\.[0-9]+(?:-[0-9A-Za-z.-]+)?/,
      `$1${version}`
    );
    if (updated === current) {
    ...
    fs.writeFileSync(file, updated);
  ' "$OPENCODE_SCS_HOOKS_PLUGIN" "$VERSION"
}
...
git add "$ROOT_PACKAGE_JSON" "$PACKAGE_LOCK_JSON" "$ROOT_AGENTS_MD" "$TR_AGENTS_MD" "$ZH_CN_AGENTS_MD" "$AGENT_YAML" "$VERSION_FILE" "$PLUGIN_JSON" "$MARKETPLACE_JSON" "$CODEX_MARKETPLACE_JSON" "$CODEX_PLUGIN_JSON" "$OPENCODE_PACKAGE_JSON" "$OPENCODE_PACKAGE_LOCK_JSON" "$OPENCODE_SCS_HOOKS_PLUGIN" "$README_FILE" "$ROOT_ZH_CN_README_FILE" "$TR_README_FILE" "$PT_BR_README_FILE" "$ZH_CN_README_FILE" "$SELECTIVE_INSTALL_ARCHITECTURE_DOC"
