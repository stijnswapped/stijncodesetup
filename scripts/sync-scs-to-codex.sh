...
# Sync StijnCodeSetup (SCS) assets into a local Codex CLI setup.
# - Backs up ~/.codex config and AGENTS.md
# - Merges SCS AGENTS.md into existing AGENTS.md (marker-based, preserves user content)
...
# - Merges SCS MCP servers into config.toml (add-only via Node TOML parser)
...
BACKUP_DIR="$CODEX_HOME/backups/scs-$STAMP"

log() { printf '[scs-sync] %s\n' "$*"; }
...
generate_prompt_file() {
  ...
  local cmd_name="$3"
  {
    printf '# SCS Command Prompt: /%s\n\n' "$cmd_name"
    printf 'Source: %s\n\n' "$src"
    printf 'Use this prompt to run the SCS `%s` workflow.\n\n' "$cmd_name"
    awk '
      NR == 1 && $0 == "---" { fm = 1; next }
      fm == 1 && $0 == "---" { fm = 0; next }
      fm == 1 { next }
...
require_path "$REPO_ROOT/AGENTS.md" "SCS AGENTS.md"
require_path "$AGENTS_CODEX_SUPP_SRC" "SCS Codex AGENTS supplement"
require_path "$CODEX_AGENTS_SRC" "SCS Codex agent roles"
require_path "$PROMPTS_SRC" "SCS commands directory"
require_path "$BASELINE_MERGE_SCRIPT" "SCS Codex baseline merge script"
require_path "$HOOKS_INSTALLER" "SCS global git hooks installer"
require_path "$SANITY_CHECKER" "SCS global sanity checker"
require_path "$CURSOR_RULES_DIR" "SCS Cursor rules directory"
require_path "$CONFIG_FILE" "Codex config.toml"
require_path "$MCP_MERGE_SCRIPT" "SCS MCP merge script"
...
SCS_BEGIN_MARKER="<!-- BEGIN SCS -->"
SCS_END_MARKER="<!-- END SCS -->"

compose_scs_block() {
  printf '%s\n' "$SCS_BEGIN_MARKER"
  cat "$AGENTS_ROOT_SRC"
  printf '\n\n---\n\n'
  printf '# Codex Supplement (From SCS .codex/AGENTS.md)\n\n'
  cat "$AGENTS_CODEX_SUPP_SRC"
  printf '\n%s\n' "$SCS_END_MARKER"
}

log "Merging SCS AGENTS into $AGENTS_FILE (preserving user content)"
if [[ "$MODE" == "dry-run" ]]; then
  printf '[dry-run] merge SCS block into %s from %s + %s\n' "$AGENTS_FILE" "$AGENTS_ROOT_SRC" "$AGENTS_CODEX_SUPP_SRC"
else
  replace_scs_section() {
    # Replace the SCS block between markers in $AGENTS_FILE with fresh content.
    # Uses awk to correctly handle all positions including line 1.
    local tmp
    tmp="$(mktemp)"
    local scs_tmp
    scs_tmp="$(mktemp)"
    compose_scs_block > "$scs_tmp"
    awk -v begin="$SCS_BEGIN_MARKER" -v end="$SCS_END_MARKER" -v scs="$scs_tmp" '
      { gsub(/\r$/, "") }
      $0 == begin { skip = 1; while ((getline line < scs) > 0) print line; close(scs); next }
      $0 == end   { skip = 0; next }
      !skip        { print }
    ' "$AGENTS_FILE" > "$tmp"
  ...
  if [[ ! -f "$AGENTS_FILE" ]]; then
    ...
    compose_scs_block > "$AGENTS_FILE"
  elif awk -v b="$SCS_BEGIN_MARKER" -v e="$SCS_END_MARKER" '
        { gsub(/\r$/, "") }
        $0 == b { bc++; if (!fb) fb = NR }
        $0 == e { ec++; if (!fe) fe = NR }
        END { exit !(bc == 1 && ec == 1 && fb < fe) }
      ' "$AGENTS_FILE"; then
    # Exactly one BEGIN/END pair in correct order — replace only the SCS section
    replace_scs_section
  elif awk -v b="$SCS_BEGIN_MARKER" -v e="$SCS_END_MARKER" '
        { gsub(/\r$/, "") }
        $0 == b { bc++ } $0 == e { ec++ }
    ...
    log "WARNING: SCS markers found but not a clean pair — stripping markers and re-appending"
    _fix_tmp="$(mktemp)"
    awk -v b="$SCS_BEGIN_MARKER" -v e="$SCS_END_MARKER" '
      { gsub(/\r$/, "") }
      $0 == b { skip = 1; next }
      $0 == e { skip = 0; next }
      !skip   { print }
    ...
    # Existing file without markers — append SCS block, preserving existing content.
    # Legacy SCS-only files will have duplicate content after this first run, but
    ...
    log "No SCS markers found — appending managed block (backup saved)"
    {
      printf '\n\n'
      compose_scs_block
    } >> "$AGENTS_FILE"
...
log "Merging SCS Codex baseline into $CONFIG_FILE (add-only, preserving user config)"
if [[ "$MODE" == "dry-run" ]]; then
...
# ~/.agents/skills/ (installed by SCS installer / npx skills).
# Copying into ~/.codex/skills/ was unnecessary.

log "Generating prompt files from SCS commands"
run_or_echo mkdir -p "$PROMPTS_DEST"
manifest="$PROMPTS_DEST/scs-prompts-manifest.txt"
if [[ "$MODE" == "dry-run" ]]; then
  printf '[dry-run] > %s\n' "$manifest"
else
  : > "$manifest"
...
extension_manifest="$PROMPTS_DEST/scs-extension-prompts-manifest.txt"
if [[ "$MODE" == "dry-run" ]]; then
  printf '[dry-run] > %s\n' "$extension_manifest"
else
  : > "$extension_manifest"
...
write_extension_prompt "scs-tool-run-tests.md" <<EOF
# SCS Tool Prompt: run-tests
...
write_extension_prompt "scs-tool-check-coverage.md" <<EOF
# SCS Tool Prompt: check-coverage
...
write_extension_prompt "scs-tool-security-audit.md" <<EOF
# SCS Tool Prompt: security-audit
...
write_extension_prompt "scs-rules-pack-common.md" <<EOF
# SCS Rule Pack: common (optional)

Apply SCS common engineering rules for this session. Use these files as the source of truth:
...
write_extension_prompt "scs-rules-pack-typescript.md" <<EOF
# SCS Rule Pack: typescript (optional)

Apply SCS common rules plus TypeScript-specific rules for this session.

## Common
Use \`$PROMPTS_DEST/scs-rules-pack-common.md\`.
...
write_extension_prompt "scs-rules-pack-python.md" <<EOF
# SCS Rule Pack: python (optional)

Apply SCS common rules plus Python-specific rules for this session.
...
write_extension_prompt "scs-rules-pack-golang.md" <<EOF
# SCS Rule Pack: golang (optional)

Apply SCS common rules plus Go-specific rules for this session.
...
write_extension_prompt "scs-rules-pack-swift.md" <<EOF
# SCS Rule Pack: swift (optional)

Apply SCS common rules plus Swift-specific rules for this session.
...
log "Merging SCS MCP servers into $CONFIG_FILE (add-only, preserving user config)"
if [[ "$MODE" == "dry-run" ]]; then
...
if [[ "$MODE" == "dry-run" ]]; then
  ...
  SCS_GLOBAL_HOOKS_DIR="${SCS_GLOBAL_HOOKS_DIR:-$CODEX_HOME/git-hooks}" \
