...
Track Linear, GitHub, handoff, and manual roadmap items in the SCS SQLite state
store so "scs status" can include linked work and blocked operator follow-up.

Options:
  --id <id>                 Stable local work-item id for upsert
  --source <source>         Source system, e.g. linear, github, handoff, manual
  --source-id <id>          Source-local identifier, e.g. SCS-20 or PR number
  ...
  --session-id <id>         Optional SCS session id
...
function buildGithubPrWorkItem(repo, pr, options = {}) {
  return {
    ...
    metadata: {
      ...
      syncedBy: 'scs-work-items-sync-github'
    }
  };
}
...
function closeStaleGithubItems(store, repo, activeIds, options = {}) {
  ...
  for (const item of payload.items) {
    if (!item.metadata || item.metadata.syncedBy !== 'scs-work-items-sync-github') {
      continue;
    }
