...
`scs2/` is the current Rust-based SCS 2.0 control-plane scaffold.

It is usable as an alpha for local experimentation, but it is **not** the finished SCS 2.0 product yet.
...
SCS 2.0 is the layer above individual harness installs.
...
Open issue clusters for the broader roadmap live in the main repo issue tracker under the `scs-2.0` label.
...
The alpha is missing the higher-level operator surface that defines SCS 2.0:
...
Do not market `scs2/` as done just because the scaffold builds.

The right framing is:

- SCS 2.0 alpha exists
