...
impl Default for Config {
    fn default() -> Self {
        ...
        Self {
            db_path: home.join(".claude").join("scs2.db"),
            worktree_root: PathBuf::from("/tmp/scs-worktrees"),
...
impl Config {
    ...
    pub fn config_path() -> PathBuf {
        Self::config_root().join("scs2").join("config.toml")
    }
    ...
    fn load_from_paths(
        ...
        let mut merged = toml::Value::try_from(Self::default())
            .context("serialize default SCS 2.0 config for layered merge")?;
        ...
        merged
            .try_into()
            .context("deserialize merged SCS 2.0 config")
    }
    ...
    fn project_config_paths_from(start: &std::path::Path) -> Vec<PathBuf> {
        ...
        while let Some(path) = current {
            let legacy = path.join(".claude").join("scs2.toml");
            let primary = path.join("scs2.toml");
    ...
    fn merge_config_file(base: &mut toml::Value, path: &std::path::Path) -> Result<()> {
        let content = std::fs::read_to_string(path)
            .with_context(|| format!("read SCS 2.0 config from {}", path.display()))?;
        let overlay: toml::Value = toml::from_str(&content)
            .with_context(|| format!("parse SCS 2.0 config from {}", path.display()))?;
...
mod tests {
    ...
    fn missing_budget_fields_fall_back_to_defaults() {
        let legacy_config = r#"
db_path = "/tmp/scs2.db"
worktree_root = "/tmp/scs-worktrees"
max_parallel_sessions = 8
max_parallel_worktrees = 6
worktree_retention_secs = 0
session_timeout_secs = 3600
...
"#;
    ...
    fn worktree_branch_prefix_deserializes_from_toml() {
        let config: Config = toml::from_str(r#"worktree_branch_prefix = "bots/scs""#).unwrap();

        assert_eq!(config.worktree_branch_prefix, "bots/scs");
    }

    #[test]
    fn layered_config_merges_global_and_project_overrides() {
        let tempdir = std::env::temp_dir().join(format!("scs2-config-{}", Uuid::new_v4()));
        let legacy_global_path = tempdir.join("legacy-global.toml");
        let global_path = tempdir.join("config.toml");
        let project_path = tempdir.join("scs2.toml");
...
"#,
    ...
    fn project_config_discovery_prefers_nearest_directory_and_new_path() {
        ...
        std::fs::write(project_root.join(".claude").join("scs2.toml"), "").unwrap();
        std::fs::write(project_root.join("scs2.toml"), "").unwrap();

        let paths = Config::project_config_paths_from(&nested_dir);
        assert_eq!(
            paths,
            vec![
                project_root.join(".claude").join("scs2.toml"),
                project_root.join("scs2.toml")
            ]
        );
    ...
    fn primary_config_path_uses_xdg_style_location() {
        ...
        assert!(path.ends_with("scs2/config.toml"));
    }
...
"#,
        ...
        assert_eq!(
            runner.env.get("SCS_HARNESS").map(String::as_str),
            Some("cursor")
        );
    }
...
"#,
        ...
        let vars = BTreeMap::from([
            ...
            ("project".to_string(), "scs-core".to_string()),
        ...
        assert_eq!(template.project.as_deref(), Some("scs-core"));
...
"#,
    ...
    fn save_round_trips_automation_settings() {
        let path = std::env::temp_dir().join(format!("scs2-config-{}.toml", Uuid::new_v4()));
        ...
        config.worktree_branch_prefix = "bots/scs".to_string();
        ...
        assert_eq!(loaded.worktree_branch_prefix, "bots/scs");
        assert_eq!(
