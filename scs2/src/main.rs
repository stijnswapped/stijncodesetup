...
#[derive(Parser, Debug)]
#[command(name = "scs", version, about = "SCS 2.0 — Agentic IDE control plane")]
struct Cli {
    #[command(subcommand)]
...
enum Commands {
    ...
    Start {
        ...
        #[arg(short, long)]
        task: String,
        /// Agent type (defaults to `default_agent` from scs2.toml)
        #[arg(short, long)]
        agent: Option<String>,
        /// Agent profile defined in scs2.toml
        #[arg(long)]
    ...
    Template {
        /// Template name defined in scs2.toml
        name: String,
    ...
    DrainInbox {
        /// Lead session ID or alias
        session_id: String,
        /// Agent type for routed delegates (defaults to `default_agent` from scs2.toml)
        #[arg(short, long)]
        agent: Option<String>,
        #[command(flatten)]
        worktree: WorktreePolicyArgs,
    ...
    /// Audit Hermes/OpenClaw-style workspaces and map them onto SCS2
    Migrate {
        #[command(subcommand)]
...
enum RemoteCommands {
    ...
    Add {
        ...
        #[arg(long, value_enum, default_value_t = TaskPriorityArg::Normal)]
        priority: TaskPriorityArg,
        /// Agent type (defaults to SCS default agent)
        #[arg(short, long)]
        agent: Option<String>,
        /// Agent profile defined in scs2.toml
        #[arg(long)]
    ...
    ComputerUse {
        ...
        #[arg(long, value_enum, default_value_t = TaskPriorityArg::Normal)]
        priority: TaskPriorityArg,
        /// Agent type override (defaults to [computer_use_dispatch] or SCS default agent)
        #[arg(short, long)]
        agent: Option<String>,
        /// Agent profile override (defaults to [computer_use_dispatch] or SCS default profile)
        #[arg(long)]
        profile: Option<String>,
        #[command(flatten)]
...
enum MigrationCommands {
    /// Audit a Hermes/OpenClaw-style workspace and map it onto SCS2 features
    Audit {
    ...
    /// Generate an actionable SCS2 migration plan from a legacy workspace audit
    Plan {
        /// Path to the legacy Hermes/OpenClaw workspace root
        #[arg(long)]
        source: PathBuf,
    ...
    /// Import recurring jobs from a legacy cron/jobs.json into SCS2 schedules
    ImportSchedules {
        /// Path to the legacy Hermes/OpenClaw workspace root
        #[arg(long)]
        source: PathBuf,
        /// Preview detected jobs without creating SCS2 schedules
        #[arg(long)]
        dry_run: bool,
        /// Emit machine-readable JSON instead of the human summary
        #[arg(long)]
    ...
    /// Import legacy workspace memory into the SCS2 context graph
    ImportMemory {
        /// Path to the legacy Hermes/OpenClaw workspace root
        #[arg(long)]
        source: PathBuf,
    ...
    /// Import safe legacy env/service config context into the SCS2 context graph
    ImportEnv {
        /// Path to the legacy Hermes/OpenClaw workspace root
        #[arg(long)]
        source: PathBuf,
        /// Preview detected importable sources without writing to the SCS2 graph
        #[arg(long)]
        dry_run: bool,
        /// Maximum imported records across all synthesized connectors
        #[arg(long, default_value_t = 100)]
    ...
    /// Scaffold SCS-native orchestration templates from legacy skill markdown
    ImportSkills {
        /// Path to the legacy Hermes/OpenClaw workspace root
        #[arg(long)]
        source: PathBuf,
        /// Directory where imported SCS2 skill artifacts should be written
        #[arg(long)]
        output_dir: PathBuf,
        /// Emit machine-readable JSON instead of the human summary
        #[arg(long)]
    ...
    /// Scaffold SCS-native templates from legacy tool scripts
    ImportTools {
        /// Path to the legacy Hermes/OpenClaw workspace root
        #[arg(long)]
        source: PathBuf,
        /// Directory where imported SCS2 tool artifacts should be written
        #[arg(long)]
        output_dir: PathBuf,
        /// Emit machine-readable JSON instead of the human summary
        #[arg(long)]
    ...
    /// Scaffold SCS-native templates from legacy bridge plugins
    ImportPlugins {
        /// Path to the legacy Hermes/OpenClaw workspace root
        #[arg(long)]
        source: PathBuf,
        /// Directory where imported SCS2 plugin artifacts should be written
        #[arg(long)]
        output_dir: PathBuf,
        /// Emit machine-readable JSON instead of the human summary
        #[arg(long)]
    ...
    /// Import legacy gateway/dispatch tasks into the SCS2 remote queue
    ImportRemote {
        /// Path to the legacy Hermes/OpenClaw workspace root
        #[arg(long)]
        source: PathBuf,
        /// Preview detected requests without creating SCS2 remote queue entries
        #[arg(long)]
        dry_run: bool,
        /// Emit machine-readable JSON instead of the human summary
        #[arg(long)]
...
fn build_worktree_resolution_report(
    ...
    let resolution_steps = if conflicted {
        vec![
            format!(
                "Inspect current patch: scs worktree-status {} --patch",
                session.id
            ),
            ...
            format!(
                "Re-check readiness: scs worktree-status {} --check",
                session.id
            ),
            format!("Merge when clear: scs merge-worktree {}", session.id),
        ]
    } else {
        Vec::new()
    };
...
fn build_legacy_migration_audit_report(source: &Path) -> Result<LegacyMigrationAuditReport> {
    ...
    if !scheduler_paths.is_empty() {
        artifacts.push(LegacyMigrationArtifact {
            ...
            mapping: vec![
                "scs schedule add".to_string(),
                "scs schedule list".to_string(),
                "scs schedule run-due".to_string(),
                "scs daemon".to_string(),
            ],
            notes: vec![
                "Recurring jobs can be recreated directly in SCS2's persistent scheduler."
                    .to_string(),
                "Translate each legacy cron prompt into an explicit SCS task body before enabling it."
                    .to_string(),
            ],
        });
    }
    ...
    if gateway_dir.is_dir() {
        artifacts.push(LegacyMigrationArtifact {
            ...
            mapping: vec![
                "scs remote serve".to_string(),
                "scs remote add".to_string(),
                "scs remote computer-use".to_string(),
                "scs remote run".to_string(),
            ],
            notes: vec![
                "SCS2 already ships a token-authenticated remote dispatch queue and HTTP intake."
                    .to_string(),
                "Remote handlers should be translated to SCS task bodies instead of copied verbatim."
                    .to_string(),
            ],
        });
    }
    ...
    if !memory_paths.is_empty() {
        artifacts.push(LegacyMigrationArtifact {
            ...
            mapping: vec![
                "scs graph add-observation".to_string(),
                "scs graph connector-sync".to_string(),
                "scs graph recall".to_string(),
                "scs graph connectors".to_string(),
            ],
            notes: vec![
                "SCS2 deep memory now supports persistent observations, recall, compaction, and external connectors."
                    .to_string(),
            ],
        });
    }
    ...
    let skills_paths = collect_existing_relative_paths(&source, &["skills", "skills/scs-imports"]);
    if !skills_paths.is_empty() {
        artifacts.push(LegacyMigrationArtifact {
            ...
            mapping: vec![
                "skills/".to_string(),
                "scs template".to_string(),
                "configure-scs".to_string(),
            ],
            notes: vec![
                "Reusable skills should be ported one by one into SCS-native skills or orchestration templates."
                    .to_string(),
    ...
    if tools_dir.is_dir() {
        artifacts.push(LegacyMigrationArtifact {
            ...
            notes: vec![
                "Legacy tool wrappers should be rebuilt as SCS agents, commands, hooks, or configured harness runners."
                    .to_string(),
    ...
    if plugins_dir.is_dir() {
        artifacts.push(LegacyMigrationArtifact {
            ...
            notes: vec![
                "Bridge plugins normally translate into SCS hooks, commands, or skills instead of one-for-one plugin copies."
                    .to_string(),
            ],
        });
    }
    ...
    if !env_service_paths.is_empty() {
        artifacts.push(LegacyMigrationArtifact {
            ...
            notes: vec![
                "Secret material should not be imported into SCS2."
                    .to_string(),
...
fn build_legacy_migration_next_steps(artifacts: &[LegacyMigrationArtifact]) -> Vec<String> {
    ...
    if categories.contains("scheduler") {
        steps.push(
            "Recreate recurring jobs with `scs schedule add`, verify them with `scs schedule list`, then enable processing through `scs daemon`."
                .to_string(),
        );
    }
    if categories.contains("gateway_dispatch") {
        steps.push(
            "Replace gateway/dispatch entrypoints with `scs remote serve`, preview/import legacy requests with `scs migrate import-remote`, then verify them with `scs remote list` / `scs remote run`."
                .to_string(),
        );
    }
    if categories.contains("memory_tool") || categories.contains("workspace_memory") {
        steps.push(
            "Import sanitized operator memory through `scs graph connector-sync`, then use `scs graph recall` and pinned observations for durable context."
                .to_string(),
        );
    }
    if categories.contains("skills") {
        steps.push(
            "Scaffold translated legacy skills with `scs migrate import-skills --source <legacy-workspace> --output-dir <dir>`, then promote the reusable ones into SCS skills or orchestration templates one lane at a time instead of bulk-copying them."
                .to_string(),
        );
    }
    if categories.contains("tools") {
        steps.push(
            "Scaffold translated legacy tools with `scs migrate import-tools --source <legacy-workspace> --output-dir <dir>`, then rebuild the valuable ones as SCS-native commands, hooks, or harness runners instead of shelling back out to the old stack."
                .to_string(),
        );
    }
    if categories.contains("plugins") {
        steps.push(
            "Scaffold translated bridge plugins with `scs migrate import-plugins --source <legacy-workspace> --output-dir <dir>`, then port the valuable ones into SCS-native hooks, commands, or skills."
                .to_string(),
        );
    }
    if categories.contains("env_services") {
        steps.push(
            "Preview safe env/service context with `scs migrate import-env --source <legacy-workspace> --dry-run`, then reconfigure credentials locally through Claude connectors, MCP config, OAuth, or local API key setup without importing raw secret material."
                .to_string(),
        );
    }
...
fn import_legacy_skills(source: &Path, output_dir: &Path) -> Result<LegacySkillImportReport> {
    ...
    let templates_path = output_dir.join("scs2.imported-skills.toml");
    fs::write(
        &templates_path,
...
fn import_legacy_tools(source: &Path, output_dir: &Path) -> Result<LegacyToolImportReport> {
        ...
        templates.insert(
            ...
            config::OrchestrationTemplateConfig {
                ...
                steps: vec![config::OrchestrationTemplateStepConfig {
                    name: Some("operator".to_string()),
                    task: format!(
                        "Use the migrated legacy tool context from {}.\nSuggested SCS target surface: {}\nLegacy tool title: {}\nLegacy summary: {}\nLegacy excerpt:\n{}\nRebuild or wrap that behavior as an SCS-native {} for {{{{task}}}}.",
                        draft.source_path,
    ...
    let templates_path = output_dir.join("scs2.imported-tools.toml");
    fs::write(
        &templates_path,
...
fn import_legacy_plugins(source: &Path, output_dir: &Path) -> Result<LegacyPluginImportReport> {
        ...
        templates.insert(
            ...
            config::OrchestrationTemplateConfig {
                ...
                steps: vec![config::OrchestrationTemplateStepConfig {
                    name: Some("operator".to_string()),
                    task: format!(
                        "Use the migrated legacy plugin context from {}.\nSuggested SCS target surface: {}\nLegacy plugin title: {}\nLegacy summary: {}\nLegacy excerpt:\n{}\nPort that behavior into an SCS-native {} for {{{{task}}}}.",
                        draft.source_path,
                        draft.suggested_surface,
                        draft.title,
                        draft.summary,
    ...
    let templates_path = output_dir.join("scs2.imported-plugins.toml");
    fs::write(
        &templates_path,
    ...
    for artifact in &audit.artifacts {
        let step = match artifact.category.as_str() {
            "scheduler" => LegacyMigrationPlanStep {
                ...
                title: "Recreate Hermes/OpenClaw recurring jobs in SCS2 scheduler".to_string(),
                target_surface: "SCS2 scheduler".to_string(),
                source_paths: artifact.source_paths.clone(),
                command_snippets: if schedule_commands.is_empty() {
                    vec![
                        "scs schedule add --cron \"<legacy-cron>\" --task \"Translate legacy recurring job from cron/scheduler.py\"".to_string(),
                        "scs schedule list".to_string(),
                        "scs daemon".to_string(),
                    ]
                } else {
                    let mut commands = schedule_commands.clone();
                    commands.push("scs schedule list".to_string());
                    commands.push("scs daemon".to_string());
                    commands
                },
                ...
                notes: {
                    ...
                    if disabled_schedule_jobs > 0 {
                        notes.push(format!(
                            "{disabled_schedule_jobs} legacy recurring job(s) are disabled and were left out of generated SCS2 commands."
                        ));
                    }
            ...
            "gateway_dispatch" => LegacyMigrationPlanStep {
                category: artifact.category.clone(),
                readiness: artifact.readiness,
                title: "Replace legacy gateway intake with SCS2 remote dispatch".to_string(),
                target_surface: "SCS2 remote dispatch".to_string(),
                source_paths: artifact.source_paths.clone(),
                command_snippets: if remote_commands.is_empty() {
                    vec![
                        "scs remote serve --bind 127.0.0.1:8787 --token <token>".to_string(),
                        "scs remote add --task \"Translate legacy dispatch workflow\"".to_string(),
                        "scs remote computer-use --goal \"Translate legacy browser/operator flow\"".to_string(),
                    ]
                } else {
                    ...
                    commands.push("scs remote list".to_string());
                    commands.push("scs remote run".to_string());
                    commands
                },
                config_snippets: Vec::new(),
                notes: {
                    ...
                    if disabled_remote_requests > 0 {
                        notes.push(format!(
                            "{disabled_remote_requests} legacy remote dispatch request(s) are disabled and were left out of generated SCS2 commands."
                        ));
                    }
            ...
            "memory_tool" => LegacyMigrationPlanStep {
                category: artifact.category.clone(),
                readiness: artifact.readiness,
                title: "Port legacy memory tool usage to SCS2 deep memory".to_string(),
                target_surface: "SCS2 context graph".to_string(),
                source_paths: artifact.source_paths.clone(),
                command_snippets: vec![
                    "scs graph add-observation --entity-id <id> --type migration_note --summary \"Imported legacy memory pattern\"".to_string(),
                    "scs graph recall \"<query>\"".to_string(),
                    "scs graph connectors".to_string(),
                ],
                config_snippets: Vec::new(),
            ...
            "workspace_memory" => LegacyMigrationPlanStep {
                category: artifact.category.clone(),
                readiness: artifact.readiness,
                title: "Import sanitized workspace memory through SCS2 connectors".to_string(),
                target_surface: "SCS2 memory connectors".to_string(),
                source_paths: artifact.source_paths.clone(),
                command_snippets: vec![
                    "scs graph connector-sync hermes_workspace".to_string(),
                    "scs graph recall \"<query>\"".to_string(),
                ],
            ...
            "skills" => LegacyMigrationPlanStep {
                category: artifact.category.clone(),
                readiness: artifact.readiness,
                title: "Translate reusable legacy skills into SCS-native surfaces".to_string(),
                target_surface: "SCS skills / orchestration templates".to_string(),
                source_paths: artifact.source_paths.clone(),
                command_snippets: vec![
                    format!(
                        "scs migrate import-skills --source {} --output-dir migration-artifacts/skills",
                        shell_quote_double(&audit.source)
                    ),
                    "scs template <template-name> --task \"<translated workflow goal>\"".to_string(),
                ],
                config_snippets: vec![
            ...
            "tools" => LegacyMigrationPlanStep {
                category: artifact.category.clone(),
                readiness: artifact.readiness,
                title: "Rebuild valuable legacy tools as SCS agents, hooks, commands, or harness runners".to_string(),
                target_surface: "SCS agents / hooks / commands / harness runners".to_string(),
                source_paths: artifact.source_paths.clone(),
                command_snippets: vec![
                    format!(
                        "scs migrate import-tools --source {} --output-dir migration-artifacts/tools",
                        shell_quote_double(&audit.source)
                    ),
                    "scs template <template-name> --task \"Rebuild one legacy tool as an SCS-native command, hook, or harness runner\"".to_string(),
                ],
                config_snippets: vec![
            ...
            "plugins" => LegacyMigrationPlanStep {
                category: artifact.category.clone(),
                readiness: artifact.readiness,
                title: "Translate legacy bridge plugins into SCS-native automation".to_string(),
                target_surface: "SCS hooks / commands / skills".to_string(),
                source_paths: artifact.source_paths.clone(),
                command_snippets: vec![
                    format!(
                        "scs migrate import-plugins --source {} --output-dir migration-artifacts/plugins",
                        shell_quote_double(&audit.source)
                    ),
                    "scs template <template-name> --task \"Port one bridge plugin behavior into an SCS hook, command, or skill\"".to_string(),
                ],
                config_snippets: Vec::new(),
                notes: artifact.notes.clone(),
            },
            "env_services" => LegacyMigrationPlanStep {
                ...
                command_snippets: vec![
                    format!(
                        "scs migrate import-env --source {} --dry-run",
                        shell_quote_double(&audit.source)
                    ),
                    format!(
                        "scs migrate import-env --source {}",
                        shell_quote_double(&audit.source)
                    ),
                    "scs graph recall \"<service or env key>\"".to_string(),
                ],
                config_snippets: vec![
                    "# Re-enter connector auth locally; do not copy legacy secrets into SCS2.\n# Typical targets: Google Drive OAuth, GitHub, Stripe, Linear, browser creds.".to_string(),
                ],
                notes: artifact.notes.clone(),
            },
            _ => LegacyMigrationPlanStep {
                ...
                target_surface: "Manual SCS2 translation".to_string(),
                source_paths: artifact.source_paths.clone(),
...
fn write_legacy_migration_scaffold(
    ...
    let config_path = output_dir.join("scs2.migration.toml");
...
fn render_legacy_migration_config_scaffold(plan: &LegacyMigrationPlanReport) -> String {
    let mut sections = vec![
        format!(
            "# SCS2 migration scaffold generated from {}\n# Review every section before merging it into a real scs2.toml.",
            plan.source
        ),
    ];
...
fn build_otel_export(
    ...
    Ok(OtlpExport {
        resource_spans: vec![OtlpResourceSpans {
            resource: OtlpResource {
                attributes: vec![
                    otlp_string_attr("service.name", "scs2"),
...
fn build_session_otel_spans(
    ...
    let mut spans = vec![OtlpSpan {
        ...
        attributes: vec![
            otlp_string_attr("scs.session.id", &session.id),
            otlp_string_attr("scs.session.state", &session.state.to_string()),
            otlp_string_attr("scs.agent.type", &session.agent_type),
            otlp_string_attr("scs.session.task", &session.task),
            otlp_string_attr(
                "scs.working_dir",
                session.working_dir.to_string_lossy().as_ref(),
            ),
            otlp_int_attr("scs.metrics.input_tokens", session.metrics.input_tokens),
            otlp_int_attr("scs.metrics.output_tokens", session.metrics.output_tokens),
            otlp_int_attr("scs.metrics.tokens_used", session.metrics.tokens_used),
            otlp_int_attr("scs.metrics.tool_calls", session.metrics.tool_calls),
            otlp_int_attr(
                "scs.metrics.files_changed",
                u64::from(session.metrics.files_changed),
            ),
            otlp_int_attr("scs.metrics.duration_secs", session.metrics.duration_secs),
            otlp_double_attr("scs.metrics.cost_usd", session.metrics.cost_usd),
        ],
        links: parent_link
            .into_iter()
    ...
    for entry in db.list_tool_logs_for_session(&session.id)? {
        ...
        spans.push(OtlpSpan {
            ...
            attributes: vec![
                otlp_string_attr("scs.session.id", &entry.session_id),
...
mod tests {
    ...
    impl TestDir {
        fn new(label: &str) -> Result<Self> {
            let path =
                std::env::temp_dir().join(format!("scs2-main-{label}-{}", uuid::Uuid::new_v4()));
    ...
    fn build_session(id: &str, task: &str, state: SessionState) -> Session {
        ...
        Session {
            ...
            working_dir: PathBuf::from("/tmp/scs"),
            state,
            pid: None,
            worktree: None,
    ...
    fn cli_parses_resume_command() {
        let cli = Cli::try_parse_from(["scs", "resume", "deadbeef"])
    ...
    fn cli_parses_export_otel_command() {
        ...
        match cli.command {
            Some(Commands::ExportOtel { session_id, output }) => {
                ...
                assert_eq!(output.as_deref(), Some(Path::new("/tmp/scs-otel.json")));
            }
    ...
    fn cli_parses_schedule_add_command() {
        ...
        match cli.command {
            Some(Commands::Schedule {
                ...
                assert_eq!(project.as_deref(), Some("scs-core"));
    ...
    fn cli_parses_remote_computer_use_command() {
        ...
        match cli.command {
            Some(Commands::Remote {
                ...
                assert_eq!(target_url.as_deref(), Some("https://scs.tools/account"));
    ...
    fn cli_parses_start_without_agent_override() {
        let cli = Cli::try_parse_from(["scs", "start", "--task", "Follow up"])
    ...
    fn cli_parses_start_no_worktree_override() {
        let cli = Cli::try_parse_from(["scs", "start", "--task", "Follow up", "--no-worktree"])
    ...
    fn cli_parses_delegate_worktree_override() {
        let cli = Cli::try_parse_from(["scs", "delegate", "planner", "--worktree"])
    ...
    fn cli_parses_team_command() {
        let cli = Cli::try_parse_from(["scs", "team", "planner", "--depth", "3"])
    ...
    fn cli_parses_worktree_status_command() {
        let cli = Cli::try_parse_from(["scs", "worktree-status", "planner"])
    ...
    fn cli_parses_worktree_status_json_flag() {
        let cli = Cli::try_parse_from(["scs", "worktree-status", "--json"])
    ...
    fn cli_parses_worktree_status_all_flag() {
        let cli = Cli::try_parse_from(["scs", "worktree-status", "--all"])
    ...
    fn cli_parses_worktree_status_session_id_with_all_flag() {
        let err = Cli::try_parse_from(["scs", "worktree-status", "planner", "--all"])
    ...
    fn cli_parses_worktree_status_patch_flag() {
        let cli = Cli::try_parse_from(["scs", "worktree-status", "--patch"])
    ...
    fn build_otel_export_includes_session_and_tool_spans() -> Result<()> {
        ...
        assert_eq!(
            attr_value(&session_span.attributes, "scs.session.id")
    ...
    fn build_otel_export_links_delegated_session_to_parent_trace() -> Result<()> {
        ...
        assert_eq!(
            attr_value(&session_span.links[0].attributes, "scs.parent_session.id")
                .and_then(|value| value.string_value.as_deref()),
            Some("lead-1")
        );
    ...
    fn cli_parses_worktree_status_check_flag() {
        let cli = Cli::try_parse_from(["scs", "worktree-status", "--check"])
    ...
    fn cli_parses_worktree_resolution_flags() {
        let cli =
            Cli::try_parse_from(["scs", "worktree-resolution", "planner", "--json", "--check"])
    ...
    fn cli_parses_worktree_resolution_all_flag() {
        let cli = Cli::try_parse_from(["scs", "worktree-resolution", "--all"])
    ...
    fn cli_parses_prune_worktrees_json_flag() {
        let cli = Cli::try_parse_from(["scs", "prune-worktrees", "--json"])
    ...
    fn cli_parses_merge_worktree_all_flags() {
        let cli = Cli::try_parse_from(["scs", "merge-worktree", "--all", "--json"])
    ...
    fn cli_parses_merge_queue_json_flag() {
        let cli = Cli::try_parse_from(["scs", "merge-queue", "--json"])
    ...
    fn cli_parses_merge_queue_apply_flag() {
        let cli = Cli::try_parse_from(["scs", "merge-queue", "--apply", "--json"])
    ...
    fn format_worktree_status_human_includes_readiness_and_conflicts() {
        let report = WorktreeStatusReport {
            ...
            patch_included: true,
            attached: true,
            path: Some("/tmp/scs/wt-1".to_string()),
            branch: Some("scs/deadbeefcafefeed".to_string()),
        ...
        assert!(text.contains("Branch scs/deadbeefcafefeed (base main)"));
    ...
    fn format_worktree_resolution_human_includes_protocol_steps() {
        let report = WorktreeResolutionReport {
            ...
            resolution_steps: vec![
                "Inspect current patch: scs worktree-status deadbeefcafefeed --patch".to_string(),
                "Open worktree: cd /tmp/scs/wt-1".to_string(),
                "Resolve conflicts and stage files: git add <paths>".to_string(),
            ],
        };
    ...
    fn worktree_resolution_reports_exit_code_tracks_conflicts() {
        ...
        let conflicted = WorktreeResolutionReport {
            ...
            conflicted: true,
            check_exit_code: 2,
            path: Some("/tmp/scs/wt-2".to_string()),
            branch: Some("scs/conflicted".to_string()),
            base_branch: Some("main".to_string()),
    ...
    fn format_worktree_merge_human_reports_merge_and_cleanup() {
        ...
        assert!(text.contains("Branch scs/deadbeef -> main"));
    ...
    fn format_merge_queue_human_reports_ready_and_blocked_entries() {
        let text = format_merge_queue_human(&session::manager::MergeQueueReport {
            ready_entries: vec![session::manager::MergeQueueEntry {
                ...
                project: "scs".to_string(),
                task_group: "checkout".to_string(),
                branch: "scs/alpha1234".to_string(),
                base_branch: "main".to_string(),
    ...
    fn format_bulk_worktree_merge_human_reports_summary_and_skips() {
        let text = format_bulk_worktree_merge_human(&session::manager::WorktreeBulkMergeOutcome {
            merged: vec![session::manager::WorktreeMergeOutcome {
                session_id: "deadbeefcafefeed".to_string(),
                branch: "scs/deadbeefcafefeed".to_string(),
                base_branch: "main".to_string(),
                already_up_to_date: false,
                cleaned_worktree: true,
            }],
            rebased: vec![session::manager::WorktreeRebaseOutcome {
                ...
                branch: "scs/rebased12345678".to_string(),
                base_branch: "main".to_string(),
                already_up_to_date: false,
            }],
        ...
        assert!(text.contains("- merged scs/deadbeefcafefeed -> main for deadbeef"));
        assert!(text.contains("Rebased 1 blocked worktree(s) onto their base branch"));
        assert!(text.contains("- rebased scs/rebased12345678 onto main for rebased1"));
    ...
    fn worktree_status_exit_code_tracks_health() {
        ...
        let conflicted = WorktreeStatusReport {
            ...
            patch_included: false,
            attached: true,
            path: Some("/tmp/scs/wt-3".to_string()),
            branch: Some("scs/c".to_string()),
            base_branch: Some("main".to_string()),
            diff_summary: Some("Branch 1 file changed".to_string()),
            file_preview: vec!["Branch M README.md".to_string()],
            patch_preview: None,
    ...
    fn cli_parses_coordinate_backlog_json_flag() {
        let cli = Cli::try_parse_from(["scs", "coordinate-backlog", "--json"])
    ...
    fn cli_parses_coordinate_backlog_check_flag() {
        let cli = Cli::try_parse_from(["scs", "coordinate-backlog", "--check"])
    ...
    fn cli_parses_coordination_status_command() {
        let cli = Cli::try_parse_from(["scs", "coordination-status"])
    ...
    fn cli_parses_decisions_command() {
        let cli = Cli::try_parse_from(["scs", "decisions", "--all", "--limit", "5", "--json"])
    ...
    fn cli_parses_graph_add_entity_command() {
        ...
        match cli.command {
            Some(Commands::Graph {
                ...
                assert_eq!(path.as_deref(), Some("scs2/src/tui/dashboard.rs"));
    ...
    fn cli_parses_graph_sync_command() {
        let cli = Cli::try_parse_from(["scs", "graph", "sync", "--all", "--limit", "12", "--json"])
    ...
    fn cli_parses_graph_connectors_command() {
        let cli = Cli::try_parse_from(["scs", "graph", "connectors", "--json"])
    ...
    fn legacy_migration_audit_report_maps_detected_artifacts() -> Result<()> {
        ...
        fs::create_dir_all(root.join("skills/scs-imports"))?;
        ...
        fs::write(root.join("skills/scs-imports/research.md"), "# skill\n")?;
        ...
        assert!(report
            ...
            .iter()
            .any(|step| step.contains("scs schedule add")));
        assert!(report
            .recommended_next_steps
            .iter()
            .any(|step| step.contains("scs remote serve")));
    ...
    fn legacy_migration_plan_report_generates_workspace_connector_step() -> Result<()> {
        ...
        fs::write(
            ...
            [
                ...
                serde_json::json!({
                    ...
                    "target_url": "https://scs.tools/account",
        ...
        fs::write(root.join("workspace/notes/recovery.md"), "# recovery\n")?;
        fs::write(root.join("skills/scs-imports/research.md"), "# research\n")?;
        fs::create_dir_all(root.join("tools"))?;
        fs::write(
        ...
        assert!(workspace_step
            .command_snippets
            .contains(&"scs graph connector-sync hermes_workspace".to_string()));
        ...
        assert!(scheduler_step
            .command_snippets
            .iter()
            .any(|command| command.contains("scs schedule add --cron \"*/15 * * * *\"")));
        assert!(!scheduler_step
            .command_snippets
            .iter()
        ...
        assert!(gateway_step
            ...
            .iter()
            .any(|command| command
                .contains("scs remote add --task \"Handle account recovery triage\"")));
        assert!(gateway_step
            .command_snippets
            .iter()
            .any(|command| command.contains(
                "scs remote computer-use --goal \"Verify the billing portal warning banner\""
            )));
        assert!(!gateway_step
            .command_snippets
            .iter()
        ...
        assert!(rendered.contains("Import sanitized workspace memory through SCS2 connectors"));
        let env_step = plan
            .steps
            .iter()
        ...
        assert!(env_step
            .command_snippets
            .iter()
            .any(|command| command.contains("scs migrate import-env --source")));
        ...
        assert!(skills_step
            .command_snippets
            .iter()
            .any(|command| command.contains("scs migrate import-skills --source")));
        let tools_step = plan
            .steps
            .iter()
        ...
        assert!(tools_step
            .command_snippets
            .iter()
            .any(|command| command.contains("scs migrate import-tools --source")));
        ...
        assert!(plugins_step
            .command_snippets
            .iter()
            .any(|command| command.contains("scs migrate import-plugins --source")));
    ...
    fn import_legacy_schedules_dry_run_reports_ready_disabled_and_invalid_jobs() -> Result<()> {
        ...
        assert!(report
            .jobs
            .iter()
            .any(|job| job.command_snippet.as_deref() == Some("scs schedule add --cron \"*/15 * * * *\" --task \"Check portal-first recovery flow\" --agent \"codex\" --no-worktree --project \"billing-web\" --task-group \"recovery\"")));

        Ok(())
    }

    #[test]
    fn import_legacy_schedules_creates_real_scs2_schedules() -> Result<()> {
...
"#,
        ...
        fs::write(
            ...
            [
                ...
                serde_json::json!({
                    ...
                    "summary": "Recommend scs repair before purchase flows"
                })
                .to_string(),
            ]
            .join("\n"),
    ...
    fn import_legacy_remote_dispatch_dry_run_reports_ready_disabled_and_invalid_requests(
        ...
        assert!(report.requests.iter().any(|request| request.command_snippet.as_deref()
            == Some("scs remote add --task \"Handle account recovery triage\" --priority high --agent \"codex\" --no-worktree --project \"scs-tools\" --task-group \"recovery\"")));
        assert!(report.requests.iter().any(|request| request.command_snippet.as_deref()
            == Some("scs remote computer-use --goal \"Verify the billing portal warning banner\" --target-url \"https://scs.tools/account\" --context \"Use the production account flow\" --priority critical")));
    ...
    fn import_legacy_remote_dispatch_creates_real_pending_requests() -> Result<()> {
        ...
        assert_eq!(requests[1].project, "scs-tools");
    ...
    fn import_legacy_env_dry_run_reports_importable_and_manual_sources() -> Result<()> {
        ...
        fs::write(
            root.join(".env.local"),
            "STRIPE_SECRET_KEY=sk_test_secret\nPUBLIC_BASE_URL=https://scs.tools\n",
        )?;
        fs::write(
            root.join(".envrc"),
            "export OPENAI_API_KEY=sk-openai-secret\nexport PUBLIC_DOCS_URL=https://docs.scs.tools\n",
        )?;
        fs::write(root.join("config.yaml"), "model: claude\n")?;
        fs::write(
    ...
    fn import_legacy_env_imports_safe_context_into_graph() -> Result<()> {
        ...
        let recalled = db.recall_context_entities(None, "stripe docs scs.tools", 10)?;
        assert!(recalled
            .iter()
    ...
    fn import_legacy_skills_writes_template_artifacts() -> Result<()> {
        ...
        fs::write(
            root.join("skills/scs-imports/research.md"),
        ...
        assert!(report
            .skills
            .iter()
            .any(|skill| skill.template_name == "scs_imports_research_md"));
        assert!(report
            .skills
            .iter()
            .any(|skill| skill.template_name == "ops_recovery_markdown"));

        let config_text = fs::read_to_string(output_dir.join("scs2.imported-skills.toml"))?;
        assert!(config_text.contains("[orchestration_templates.scs_imports_research_md]"));
        ...
        assert!(summary_text.contains("skills/scs-imports/research.md"));
    ...
    fn import_legacy_tools_writes_template_artifacts() -> Result<()> {
        ...
        let config_text = fs::read_to_string(output_dir.join("scs2.imported-tools.toml"))?;
        ...
        assert!(config_text.contains("Rebuild or wrap that behavior as an SCS-native"));
    ...
    fn import_legacy_plugins_writes_template_artifacts() -> Result<()> {
        ...
        let config_text = fs::read_to_string(output_dir.join("scs2.imported-plugins.toml"))?;
        ...
        assert!(config_text.contains("Port that behavior into an SCS-native"));
    ...
    fn legacy_migration_scaffold_writes_plan_and_config_files() -> Result<()> {
        ...
        fs::write(root.join("config.yaml"), "model: claude\n")?;
        fs::write(root.join("workspace/notes/recovery.md"), "# recovery\n")?;
        fs::write(root.join("skills/scs-imports/triage.md"), "# triage\n")?;
        ...
        let config_text = fs::read_to_string(output_dir.join("scs2.migration.toml"))?;
    ...
    fn format_graph_entity_detail_human_renders_relations() {
        let detail = session::ContextGraphEntityDetail {
            entity: session::ContextGraphEntity {
                ...
                path: Some("scs2/src/tui/dashboard.rs".to_string()),
...
"#,
    ...
    fn sync_memory_connector_imports_markdown_file_sections() -> Result<()> {
        ...
        db.insert_session(&session::Session {
            ...
            project: "stijncodesetup".to_string(),
...
"#,
        ...
        let recalled = db.recall_context_entities(None, "stripe scs.tools", 10)?;
        assert!(recalled
            .iter()
            .any(|entry| entry.entity.name == "STRIPE_SECRET_KEY"));
        assert!(recalled
...
"#,
    ...
    fn cli_parses_coordination_status_json_flag() {
        let cli = Cli::try_parse_from(["scs", "coordination-status", "--json"])
    ...
    fn cli_parses_coordination_status_check_flag() {
        let cli = Cli::try_parse_from(["scs", "coordination-status", "--check"])
    ...
    fn cli_parses_maintain_coordination_command() {
        let cli = Cli::try_parse_from(["scs", "maintain-coordination"])
    ...
    fn cli_parses_maintain_coordination_json_flag() {
        let cli = Cli::try_parse_from(["scs", "maintain-coordination", "--json"])
    ...
    fn cli_parses_maintain_coordination_check_flag() {
        let cli = Cli::try_parse_from(["scs", "maintain-coordination", "--check"])
