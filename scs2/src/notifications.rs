...
mod tests {
    ...
    fn macos_notifications_use_osascript() {
        let (program, args) =
            notification_command("macos", "SCS 2.0: Completed", "Task finished").unwrap();

        assert_eq!(program, "osascript");
        assert_eq!(args[0], "-e");
        assert!(args[1].contains("display notification"));
        assert!(args[1].contains("SCS 2.0: Completed"));
    }

    #[test]
    fn linux_notifications_use_notify_send() {
        let (program, args) =
            notification_command("linux", "SCS 2.0: Approval needed", "worker-123").unwrap();

        assert_eq!(program, "notify-send");
        assert_eq!(args[0], "--app-name");
        assert_eq!(args[1], "SCS 2.0");
        assert_eq!(args[2], "SCS 2.0: Approval needed");
    ...
    fn slack_webhook_payload_uses_text() {
        ...
        assert_eq!(payload, json!({ "text": "*SCS 2.0* hello" }));
    }
