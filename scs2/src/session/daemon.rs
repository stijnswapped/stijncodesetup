...
pub async fn run(db: StateStore, cfg: Config) -> Result<()> {
    tracing::info!("SCS daemon started");
...
mod tests {
    ...
    fn temp_db_path() -> PathBuf {
        std::env::temp_dir().join(format!("scs2-daemon-test-{}.db", uuid::Uuid::new_v4()))
    }
