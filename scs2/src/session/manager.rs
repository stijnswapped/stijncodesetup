...
pub async fn run_due_schedules(
    ...
    let runner_program =
        std::env::current_exe().context("Failed to resolve SCS executable path")?;
...
async fn resume_session_with_program(
    ...
    let runner_executable = match runner_executable_override {
        ...
        None => std::env::current_exe().context("Failed to resolve SCS executable path")?,
    };
...
async fn spawn_session_runner_for_program(
    ...
    if let Some(parent) = stderr_log_path.parent() {
        std::fs::create_dir_all(parent).with_context(|| {
            format!(
                "Failed to create SCS runner log directory {}",
                parent.display()
            )
        })?;
    }
    let stderr_log = OpenOptions::new()
        ...
        .with_context(|| {
            format!(
                "Failed to open SCS runner stderr log {}",
    ...
    let child = command
        .spawn()
        .with_context(|| format!("Failed to spawn SCS runner from {}", current_exe.display()))?;

    child
        .id()
        .ok_or_else(|| anyhow::anyhow!("SCS runner did not expose a process id"))?;
    Ok(())
}
...
fn configure_background_runner_command(command: &mut Command) {
    #[cfg(unix)]
    {
        ...
        // processing a live harness session after `scs-tui start` returns.
        unsafe {
...
fn build_configured_harness_command(
    ...
    if let Some(flag) = runner.session_name_flag.as_deref() {
        command.arg(flag).arg(format!("scs-{session_id}"));
    }
    if let Some(profile) = profile {
...
fn apply_shared_harness_runtime_env(
    ...
    command.env("SCS_SESSION_ID", session_id);
    command.env("SCS_HARNESS", &harness_label);
    command.env("SCS_WORKING_DIR", working_dir);
    command.env("SCS_PROJECT_DIR", working_dir);
    ...
    if let Some(plugin_root) = resolve_scs_plugin_root() {
        command.env("SCS_PLUGIN_ROOT", &plugin_root);
        command.env("CLAUDE_PLUGIN_ROOT", &plugin_root);
    }
}

fn resolve_scs_plugin_root() -> Option<PathBuf> {
    ...
    for seed in seeds {
        for candidate in seed.ancestors() {
            if is_scs_plugin_root(candidate) {
...
fn is_scs_plugin_root(candidate: &Path) -> bool {
...
mod tests {
    ...
    impl TestDir {
        fn new(label: &str) -> Result<Self> {
            let path =
                std::env::temp_dir().join(format!("scs2-{}-{}", label, uuid::Uuid::new_v4()));
    ...
    fn build_agent_command_normalizes_runner_flags_for_codex() {
        ...
        assert_eq!(envs.get("SCS_SESSION_ID"), Some(&"sess-1234".to_string()));
        assert_eq!(
        ...
        assert_eq!(envs.get("SCS_HARNESS"), Some(&"codex".to_string()));
        assert_eq!(envs.get("CLAUDE_MODEL"), Some(&"gpt-5.4".to_string()));
        assert!(
            envs.contains_key("CLAUDE_PLUGIN_ROOT"),
            "shared compatibility env should expose the SCS plugin root"
        );
    }
    ...
    fn build_agent_command_uses_configured_runner_for_cursor() {
        ...
        cfg.harness_runners.insert(
            ...
            crate::config::HarnessRunnerConfig {
                ...
                env: BTreeMap::from([("SCS_HARNESS".to_string(), "cursor".to_string())]),
                ..Default::default()
            },
        );
        let profile = SessionAgentProfile {
        ...
        let envs = command_env_map(&command);
        assert_eq!(envs.get("SCS_SESSION_ID"), Some(&"sess-cur1".to_string()));
        assert_eq!(
            envs.get("CLAUDE_SESSION_ID"),
        ...
        assert_eq!(envs.get("CLAUDE_CODE_ENTRYPOINT"), Some(&"cli".to_string()));
        assert_eq!(envs.get("SCS_HARNESS"), Some(&"cursor".to_string()));
        assert_eq!(envs.get("CLAUDE_MODEL"), Some(&"gpt-5.4".to_string()));
        assert_eq!(envs.get("SCS_PLUGIN_ROOT"), envs.get("CLAUDE_PLUGIN_ROOT"));
    }
    ...
    fn background_runner_stderr_log_path_is_session_scoped() {
        let path =
            background_runner_stderr_log_path(Path::new("/tmp/scs-repo"), "session-123");
        assert_eq!(
            path,
            PathBuf::from("/tmp/scs-repo/.claude/scs2/logs/session-123.runner-stderr.log")
        );
    }
    ...
    fn write_package_manager_project_files(
        ...
        let package_json = match package_manager_field {
            Some(package_manager_field) => format!(
                "{{\"name\":\"scs-smoke\",\"packageManager\":\"{package_manager_field}\"}}\n"
            ),
            None => "{\"name\":\"scs-smoke\"}\n".to_string(),
        };
    ...
    async fn create_session_spawns_process_and_marks_session_running() -> Result<()> {
        ...
        assert!(log.contains(&format!("SCS_SESSION_ID={session_id}")));
        ...
        assert!(log.contains("SCS_HARNESS=claude"));
    ...
    async fn run_due_schedules_dispatches_due_tasks_and_advances_next_run() -> Result<()> {
        ...
        assert_eq!(session.project, "scs-core");
    ...
    fn create_computer_use_remote_dispatch_request_uses_config_defaults() -> Result<()> {
        ...
        assert!(request
            .task
            .contains("Target URL: https://scs.tools/account"));
        assert!(request
            .task
    ...
    fn enforce_budget_hard_limits_stops_active_sessions_without_cleaning_worktrees() -> Result<()> {
        ...
        db.insert_session(&Session {
            ...
            worktree: Some(crate::session::WorktreeInfo {
                ...
                branch: "scs/active-over-budget".to_string(),
    ...
    async fn assign_session_inherits_lead_grouping_for_spawned_delegate() -> Result<()> {
        ...
        db.insert_session(&Session {
            id: "lead".to_string(),
            task: "lead task".to_string(),
            project: "scs-platform".to_string(),
        ...
        let spawned = db
            .get_session(&outcome.session_id)?
            .context("spawned delegated session missing")?;
        assert_eq!(spawned.project, "scs-platform");
