...
impl SessionCompletionSummary {
    fn title(&self) -> String {
        match self.state {
            SessionState::Completed => "SCS 2.0: Session completed".to_string(),
            SessionState::Failed => "SCS 2.0: Session failed".to_string(),
            _ => "SCS 2.0: Session summary".to_string(),
        }
    }
...
impl Dashboard {
    ...
    fn render_header(&self, frame: &mut Frame, area: Rect) {
        ...
        let title = format!(
            " SCS 2.0 | {running} running / {total} total | {} {}% | {} ",
            self.layout_label(),
    ...
    fn build_pull_request_body(&self, session: &Session) -> String {
        ...
        lines.push("- Verified in SCS 2.0 dashboard workflow".to_string());
        lines.join("\n")
    }
                    ...
                    match session.state {
                        ...
                        SessionState::Failed => {
                            ...
                            failed_notifications.push((
                                "SCS 2.0: Session failed".to_string(),
                                format!(
                                    "{} | {}",
                                    format_session_id(&session.id),
                                    truncate_for_dashboard(&session.task, 96)
    ...
    fn new_session_task(&self) -> String {
        self.sessions
            ...
            .unwrap_or_else(|| "New SCS 2.0 session".to_string())
    }
...
fn session_started_webhook_body(session: &Session, compare_url: Option<&str>) -> String {
    let mut lines = vec![
        "*SCS 2.0: Session started*".to_string(),
        format!(
            "`{}` {}",
            format_session_id(&session.id),
            truncate_for_dashboard(&session.task, 96)
...
fn budget_alert_webhook_body(
    ...
    [
        "*SCS 2.0: Budget alert*".to_string(),
...
fn approval_request_webhook_body(message: &SessionMessage, preview: &str) -> String {
    [
        "*SCS 2.0: Approval needed*".to_string(),
        format!(
            "To `{}` from `{}`",
            format_session_id(&message.to_session),
            format_session_id(&message.from_session)
...
fn build_conflict_protocol(
    ...
    lines.push(format!(
        "1. Inspect current patch: scs worktree-status {session_id} --patch"
    ));
    ...
    lines.push(format!(
        "5. Re-check readiness: scs worktree-status {session_id} --check"
    ));
    lines.push(format!(
        "6. Merge when clear: scs merge-worktree {session_id}"
    ));
...
fn build_session_conflict_protocol(
    ...
    lines.push(format!(
        "3. Resume the paused session only after reviewing the overlap: scs resume {}",
        session_id
    ));
...
mod tests {
    ...
    fn selected_session_metrics_text_includes_worktree_output_and_attention_queue() {
        ...
        assert!(text.contains("Branch scs/focus | Base main"));
        assert!(text.contains("Worktree /tmp/scs/focus"));
    ...
    fn toggle_git_status_mode_renders_selected_worktree_status() -> Result<()> {
        let root = std::env::temp_dir().join(format!("scs2-git-status-{}", Uuid::new_v4()));
        init_git_repo(&root)?;
    ...
    fn toggle_output_mode_from_git_status_opens_selected_file_patch() -> Result<()> {
        let root = std::env::temp_dir().join(format!("scs2-git-patch-view-{}", Uuid::new_v4()));
        init_git_repo(&root)?;
        fs::write(
            root.join("README.md"),
            "line 1\nline 2\nline 3\nline 4\nline 5\nline 6 updated\n",
    ...
    fn git_patch_mode_stages_only_selected_hunk() -> Result<()> {
        let root = std::env::temp_dir().join(format!("scs2-git-patch-stage-{}", Uuid::new_v4()));
        init_git_repo(&root)?;
        let original = (1..=12)
    ...
    fn begin_pr_prompt_seeds_latest_commit_subject() -> Result<()> {
        let root = std::env::temp_dir().join(format!("scs2-pr-prompt-{}", Uuid::new_v4()));
        init_git_repo(&root)?;
    ...
    fn submit_pr_prompt_passes_custom_metadata_to_gh() -> Result<()> {
        let temp_root =
            std::env::temp_dir().join(format!("scs2-dashboard-pr-submit-{}", Uuid::new_v4()));
...
+second new"
    ...
    fn timeline_and_metrics_render_recent_file_activity_details() -> Result<()> {
        let root = std::env::temp_dir().join(format!("scs2-file-activity-{}", Uuid::new_v4()));
        ...
        fs::write(
            ...
            concat!(
                ...
                "{\"id\":\"evt-2\",\"session_id\":\"focus-12345678\",\"tool_name\":\"Write\",\"input_summary\":\"Write README.md\",\"output_summary\":\"updated readme\",\"file_paths\":[\"README.md\"],\"file_events\":[{\"path\":\"README.md\",\"action\":\"create\",\"diff_preview\":\"+ # SCS 2.0\",\"patch_preview\":\"+ # SCS 2.0\\n+ \\n+ A richer dashboard\"}],\"timestamp\":\"2026-04-09T00:01:00Z\"}\n"
            ),
        )?;
        ...
        assert!(rendered.contains("+ # SCS 2.0"));
        ...
        assert!(metrics_text.contains("+ # SCS 2.0"));
    ...
    fn metrics_text_surfaces_file_activity_conflicts() -> Result<()> {
        let root = std::env::temp_dir().join(format!("scs2-file-overlaps-{}", Uuid::new_v4()));
        fs::create_dir_all(&root)?;
        let now = Utc::now();
    ...
    fn toggle_context_graph_mode_renders_selected_session_entities_and_relations() -> Result<()> {
        ...
        let file = dashboard.db.upsert_context_entity(
            ...
            "file",
            "dashboard.rs",
            Some("scs2/src/tui/dashboard.rs"),
            "dashboard renderer",
...
+new line";
    ...
    fn toggle_conflict_protocol_mode_switches_to_protocol_view() {
        ...
        dashboard.selected_conflict_protocol = Some(
            "Conflict protocol for focus-12\nResolution steps\n1. Inspect current patch: scs worktree-status focus-12345678 --patch"
                .to_string(),
        );
    ...
    fn selected_session_metrics_text_includes_delegate_task_board() {
        ...
        dashboard.selected_child_sessions = vec![DelegatedChildSummary {
            ...
            branch: Some("scs/delegate-12345678".to_string()),
        ...
        assert!(
            text.contains(
                "- delegate [Running] | next resolve conflict | worktree conflicted | approvals 1 | backlog 2 | progress 1,280 tok / 3 files / 00:00:12 | task Implement rust tui delegate board | branch scs/delegate-12345678"
            )
        );
    ...
    fn selected_session_metrics_text_marks_focused_delegate_row() {
        ...
        dashboard.selected_child_sessions = vec![
            ...
            DelegatedChildSummary {
                ...
                branch: Some("scs/delegate-22345678".to_string()),
        ...
        assert!(text.contains(
            ">> delegate [Idle] | next review approvals | worktree in progress | approvals 1 | backlog 2 | progress 64 tok / 2 files / 00:00:10 | task Second delegate | branch scs/delegate-22345678"
        ));
    ...
    fn open_focused_delegate_switches_selected_session() {
        ...
        dashboard.selected_child_sessions = vec![DelegatedChildSummary {
            ...
            branch: Some("scs/delegate".to_string()),
    ...
    fn toggle_auto_worktree_policy_persists_config() {
        let tempdir = std::env::temp_dir().join(format!("scs2-worktree-policy-{}", Uuid::new_v4()));
    ...
    fn sync_selected_lineage_populates_delegate_task_and_output_previews() {
        ...
        child.task = "Implement delegate metrics board for SCS 2.0".to_string();
    ...
    fn refresh_builds_completion_summary_popup_from_metrics_activity_and_logs() -> Result<()> {
        let root = std::env::temp_dir().join(format!("scs2-completion-popup-{}", Uuid::new_v4()));
    ...
    fn refresh_persists_completion_summary_observation() -> Result<()> {
        let root =
            std::env::temp_dir().join(format!("scs2-completion-observation-{}", Uuid::new_v4()));
        fs::create_dir_all(root.join(".claude").join("metrics"))?;
    ...
    fn refresh_syncs_tool_activity_metrics_from_hook_file() {
        let tempdir = std::env::temp_dir().join(format!("scs2-activity-sync-{}", Uuid::new_v4()));
    ...
    fn refresh_enforces_conflicts_and_surfaces_active_incidents() -> Result<()> {
        ...
        assert!(conflict_protocol.contains("scs resume session-b"));
    ...
    fn selected_session_metrics_text_includes_harness_summary() -> Result<()> {
        let tempdir = std::env::temp_dir().join(format!(
            "scs2-dashboard-harness-metrics-{}",
            uuid::Uuid::new_v4()
        ));
    ...
    async fn submit_spawn_prompt_launches_orchestration_template() -> Result<()> {
        ...
        cfg.orchestration_templates = BTreeMap::from([(
            ...
            crate::config::OrchestrationTemplateConfig {
                description: None,
                project: Some("scs2-smoke".to_string()),
        ...
        assert!(dashboard
            .sessions
            .iter()
            .all(|session| session.project == "scs2-smoke"));
        assert!(dashboard
            .sessions
            .iter()
    ...
    fn refresh_preserves_selected_session_by_id() -> Result<()> {
        let db_path = std::env::temp_dir().join(format!("scs2-dashboard-{}.db", Uuid::new_v4()));
    ...
    fn toggle_output_filter_cycles_tool_calls_and_file_changes() {
        ...
        dashboard.session_output_cache.insert(
            ...
            vec![
                ...
                test_output_line(OutputStream::Stdout, "Updated scs2/src/tui/dashboard.rs"),
                test_output_line(OutputStream::Stderr, "stderr line"),
            ],
        );
        ...
        assert_eq!(
            ...
            "Updated scs2/src/tui/dashboard.rs"
        );
    ...
    async fn resume_selected_requeues_failed_session() -> Result<()> {
        ...
        db.insert_session(&Session {
            ...
            working_dir: PathBuf::from("/tmp/scs2-resume"),
            pid: None,
            worktree: Some(WorktreeInfo {
                path: PathBuf::from("/tmp/scs2-resume"),
                branch: "scs/failed-1".to_string(),
                base_branch: "main".to_string(),
            }),
            created_at: now,
            updated_at: now,
    ...
    async fn cleanup_selected_worktree_clears_session_metadata() -> Result<()> {
        ...
        let db = StateStore::open(&db_path)?;
        let now = Utc::now();
        let worktree_path = std::env::temp_dir().join(format!("scs2-cleanup-{}", Uuid::new_v4()));
        ...
        db.insert_session(&Session {
            ...
            worktree: Some(WorktreeInfo {
                ...
                branch: "scs/stopped-1".to_string(),
                base_branch: "main".to_string(),
            }),
            created_at: now,
            updated_at: now,
    ...
    async fn prune_inactive_worktrees_reports_pruned_and_skipped_counts() -> Result<()> {
        ...
        let db = StateStore::open(&db_path)?;
        let now = Utc::now();
        let active_path = std::env::temp_dir().join(format!("scs2-active-{}", Uuid::new_v4()));
        let stopped_path = std::env::temp_dir().join(format!("scs2-stopped-{}", Uuid::new_v4()));
        ...
        db.insert_session(&Session {
            ...
            worktree: Some(WorktreeInfo {
                ...
                branch: "scs/running-1".to_string(),
                base_branch: "main".to_string(),
            }),
            created_at: now,
            updated_at: now,
    ...
    async fn prune_inactive_worktrees_reports_retained_sessions_within_retention() -> Result<()> {
        ...
        let db = StateStore::open(&db_path)?;
        let now = Utc::now();
        let retained_path = std::env::temp_dir().join(format!("scs2-retained-{}", Uuid::new_v4()));
    ...
    async fn merge_selected_worktree_sets_operator_note_when_ready() -> Result<()> {
        ...
        assert!(note.contains("merged scs/merge1234 into"));
    ...
    fn pane_resize_clamps_to_bounds() {
        ...
        for _ in 0..20 {
            dashboard.adjust_pane_size_with_save(5, Path::new("/tmp/scs2-noop.toml"), |_| Ok(()));
        }
    ...
    fn pane_command_mode_sets_layout() {
        let tempdir = std::env::temp_dir().join(format!("scs2-pane-command-{}", Uuid::new_v4()));
        std::fs::create_dir_all(&tempdir).unwrap();
        let previous_home = std::env::var_os("HOME");
        std::env::set_var("HOME", &tempdir);
    ...
    fn cycle_pane_layout_rotates_and_hides_log_when_leaving_grid() {
        let tempdir = std::env::temp_dir().join(format!("scs2-cycle-pane-{}", Uuid::new_v4()));
        std::fs::create_dir_all(&tempdir).unwrap();
        let previous_home = std::env::var_os("HOME");
        std::env::set_var("HOME", &tempdir);
    ...
    fn cycle_pane_layout_persists_config() {
        let mut dashboard = test_dashboard(Vec::new(), 0);
        let tempdir = std::env::temp_dir().join(format!("scs2-layout-policy-{}", Uuid::new_v4()));
        std::fs::create_dir_all(&tempdir).unwrap();
        let config_path = tempdir.join("scs2.toml");
    ...
    fn pane_resize_persists_linear_setting() {
        let mut dashboard = test_dashboard(Vec::new(), 0);
        let tempdir = std::env::temp_dir().join(format!("scs2-pane-size-{}", Uuid::new_v4()));
        std::fs::create_dir_all(&tempdir).unwrap();
        let config_path = tempdir.join("scs2.toml");
    ...
    fn cycle_pane_layout_uses_persisted_grid_size() {
        ...
        dashboard.cycle_pane_layout_with_save(Path::new("/tmp/scs2-noop.toml"), |_| Ok(()));
    ...
    fn toggle_theme_persists_config() {
        let mut dashboard = test_dashboard(Vec::new(), 0);
        let tempdir = std::env::temp_dir().join(format!("scs2-theme-policy-{}", Uuid::new_v4()));
        std::fs::create_dir_all(&tempdir).unwrap();
        let config_path = tempdir.join("scs2.toml");
    ...
    fn build_config(root: &Path) -> Config {
        Config {
            ...
            worktree_branch_prefix: "scs".to_string(),
    ...
    fn init_git_repo(path: &Path) -> Result<()> {
        ...
        run_git(path, &["init", "-q"])?;
        run_git(path, &["config", "user.name", "SCS Tests"])?;
        run_git(path, &["config", "user.email", "scs-tests@example.com"])?;
