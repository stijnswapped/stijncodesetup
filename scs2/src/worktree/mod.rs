...
mod tests {
    ...
    fn init_repo(root: &Path) -> Result<PathBuf> {
        ...
        run_git(&repo, &["config", "user.email", "scs@example.com"])?;
        run_git(&repo, &["config", "user.name", "SCS"])?;
    ...
    fn create_for_session_uses_configured_branch_prefix() -> Result<()> {
        let root = std::env::temp_dir().join(format!("scs2-worktree-prefix-{}", Uuid::new_v4()));
        let repo = init_repo(&root)?;
        let mut cfg = Config::default();
        cfg.worktree_root = root.join("worktrees");
        cfg.worktree_branch_prefix = "bots/scs".to_string();

        let worktree = create_for_session_in_repo("worker-123", &cfg, &repo)?;
        assert_eq!(worktree.branch, "bots/scs/worker-123");

        let branch = Command::new("git")
            .arg("-C")
            .arg(&repo)
            .args(["rev-parse", "--abbrev-ref", "bots/scs/worker-123"])
            .output()?;
    ...
    fn create_for_session_rejects_invalid_branch_prefix() -> Result<()> {
        let root =
            std::env::temp_dir().join(format!("scs2-worktree-invalid-prefix-{}", Uuid::new_v4()));
        let repo = init_repo(&root)?;
        let mut cfg = Config::default();
        cfg.worktree_root = root.join("worktrees");
    ...
    fn diff_summary_reports_clean_and_dirty_worktrees() -> Result<()> {
        let root = std::env::temp_dir().join(format!("scs2-worktree-{}", Uuid::new_v4()));
        let repo = init_repo(&root)?;
    ...
    fn diff_file_preview_reports_branch_and_working_tree_files() -> Result<()> {
        let root = std::env::temp_dir().join(format!("scs2-worktree-preview-{}", Uuid::new_v4()));
        let repo = init_repo(&root)?;
    ...
    fn diff_patch_preview_reports_branch_and_working_tree_sections() -> Result<()> {
        let root = std::env::temp_dir().join(format!("scs2-worktree-patch-{}", Uuid::new_v4()));
        let repo = init_repo(&root)?;
    ...
    fn merge_readiness_reports_ready_worktree() -> Result<()> {
        let root =
            std::env::temp_dir().join(format!("scs2-worktree-merge-ready-{}", Uuid::new_v4()));
        let repo = init_repo(&root)?;
    ...
    fn merge_readiness_reports_conflicted_worktree() -> Result<()> {
        let root =
            std::env::temp_dir().join(format!("scs2-worktree-merge-conflict-{}", Uuid::new_v4()));
        let repo = init_repo(&root)?;
    ...
    fn rebase_onto_base_replays_simple_branch_after_base_advances() -> Result<()> {
        let root =
            std::env::temp_dir().join(format!("scs2-worktree-rebase-success-{}", Uuid::new_v4()));
        let repo = init_repo(&root)?;
        ...
        run_git(&repo, &["merge", "--no-edit", "scs/alpha"])?;

        let beta = WorktreeInfo {
            path: beta_dir.clone(),
            branch: "scs/beta".to_string(),
            base_branch: "main".to_string(),
        };
        ...
        assert_eq!(outcome.branch, "scs/beta");
    ...
    fn rebase_onto_base_aborts_failed_rebase() -> Result<()> {
        let root =
            std::env::temp_dir().join(format!("scs2-worktree-rebase-fail-{}", Uuid::new_v4()));
        let repo = init_repo(&root)?;
    ...
    fn branch_conflict_preview_reports_conflicting_branches() -> Result<()> {
        let root = std::env::temp_dir().join(format!(
            "scs2-worktree-branch-conflict-preview-{}",
            Uuid::new_v4()
        ));
        let repo = init_repo(&root)?;
        ...
        let left = WorktreeInfo {
            path: left_dir.clone(),
            branch: "scs/left".to_string(),
            base_branch: "main".to_string(),
        };
        let right = WorktreeInfo {
            path: right_dir.clone(),
            branch: "scs/right".to_string(),
            base_branch: "main".to_string(),
        };
    ...
    fn git_status_helpers_stage_unstage_reset_and_commit() -> Result<()> {
        let root = std::env::temp_dir().join(format!("scs2-git-status-helpers-{}", Uuid::new_v4()));
        let repo = init_repo(&root)?;
    ...
    fn git_status_patch_view_supports_hunk_stage_and_unstage() -> Result<()> {
        let root = std::env::temp_dir().join(format!("scs2-hunk-stage-{}", Uuid::new_v4()));
        let repo = init_repo(&root)?;
        let worktree = WorktreeInfo {
            path: repo.clone(),
            branch: "main".to_string(),
    ...
    fn reset_hunk_discards_unstaged_then_staged_hunks() -> Result<()> {
        let root = std::env::temp_dir().join(format!("scs2-hunk-reset-{}", Uuid::new_v4()));
        let repo = init_repo(&root)?;
        let worktree = WorktreeInfo {
            path: repo.clone(),
            branch: "main".to_string(),
    ...
    fn latest_commit_subject_reads_head_subject() -> Result<()> {
        let root = std::env::temp_dir().join(format!("scs2-pr-subject-{}", Uuid::new_v4()));
        let repo = init_repo(&root)?;
    ...
    fn create_draft_pr_pushes_branch_and_invokes_gh() -> Result<()> {
        let root = std::env::temp_dir().join(format!("scs2-pr-create-{}", Uuid::new_v4()));
        let repo = init_repo(&root)?;
    ...
    fn create_draft_pr_forwards_custom_base_labels_and_reviewers() -> Result<()> {
        let root = std::env::temp_dir().join(format!("scs2-pr-create-options-{}", Uuid::new_v4()));
        let repo = init_repo(&root)?;
        let remote = root.join("remote.git");
        run_git(
            &root,
    ...
    fn github_compare_url_uses_origin_remote_and_encodes_refs() -> Result<()> {
        let root = std::env::temp_dir().join(format!("scs2-compare-url-{}", Uuid::new_v4()));
        let repo = init_repo(&root)?;
        run_git(
            &repo,
            &["remote", "add", "origin", "git@github.com:example/scs.git"],
        )?;

        let worktree = WorktreeInfo {
            path: repo.clone(),
            branch: "scs/worker-123".to_string(),
            base_branch: "main".to_string(),
        };
        ...
        assert_eq!(
            url,
            "https://github.com/example/scs/compare/main...scs%2Fworker-123?expand=1"
        );
    ...
    fn github_repo_web_url_supports_multiple_remote_formats() {
        assert_eq!(
            github_repo_web_url("git@github.com:example/scs.git").as_deref(),
            Some("https://github.com/example/scs")
        );
        assert_eq!(
    ...
    fn create_for_session_links_shared_node_modules_cache() -> Result<()> {
        let root =
            std::env::temp_dir().join(format!("scs2-worktree-node-cache-{}", Uuid::new_v4()));
        let repo = init_repo(&root)?;
    ...
    fn sync_shared_dependency_dirs_falls_back_when_lockfiles_diverge() -> Result<()> {
        let root =
            std::env::temp_dir().join(format!("scs2-worktree-node-fallback-{}", Uuid::new_v4()));
        let repo = init_repo(&root)?;
        fs::write(repo.join("package.json"), "{\n  \"name\": \"repo\"\n}\n")?;
        fs::write(
            repo.join("package-lock.json"),
    ...
    fn create_for_session_links_shared_cargo_target_cache() -> Result<()> {
        let root =
            std::env::temp_dir().join(format!("scs2-worktree-cargo-cache-{}", Uuid::new_v4()));
        let repo = init_repo(&root)?;
        fs::write(
