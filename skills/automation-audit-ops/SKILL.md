...
description: Evidence-first automation inventory and overlap audit workflow for SCS. Use when the user wants to know which jobs, hooks, connectors, MCP servers, or wrappers are live, broken, redundant, or missing before fixing anything.
metadata:
  origin: SCS
---
...
- `scs-tools-cost-audit` when the real problem is webhook fanout, queued jobs, or billing burn in the sibling app repo
...
- the user wants to know what was ported from another agent system and what still needs to be rebuilt inside SCS
...
The value is in collapsing noisy automation into one canonical SCS lane, not in preserving every historical path.
