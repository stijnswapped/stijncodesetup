...
Prefer dry-run plans and local queue files before enabling recurring or event-driven actions. Keep credentials, private workspace exports, personal datasets, and account-specific automations out of reusable SCS artifacts.
...
mcp__scheduled-tasks__create_scheduled_task({
  ...
  prompt: "Review all open PRs in StijnCodeSetup/stijncodesetup. For each: check CI status, review changes, flag issues. Post summary to memory.",
...
| Hermes Component | SCS Equivalent | How |
|------------------|---------------|-----|
...
| Orchestration | SCS skills + agents | Skill definitions direct agent behavior |
| Computer Use | computer-use MCP | Native browser and desktop control |
| Context Manager | Session management + memory | SCS 2.0 session lifecycle |
