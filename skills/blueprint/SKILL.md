...
This skill ships with StijnCodeSetup. No separate installation is needed when SCS is installed.

### Full SCS install

If you are working from the SCS repository checkout, verify the skill is present with:

```bash
test -f skills/blueprint/SKILL.md
```

To update later, review the SCS diff before updating:
...
If you are vendoring only this skill outside the full SCS install, copy the reviewed file from the SCS repository into `~/.claude/skills/blueprint/SKILL.md`. Vendored copies do not have a git remote, so update them by re-copying the file from a reviewed SCS commit rather than running `git pull`.
