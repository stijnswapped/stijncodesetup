...
**Opt-in (SCS):** The `codescene` block in `mcp-configs/mcp-servers.json` is a template only. SCS plugin installs do not auto-enable bundled MCP servers. Copy the entry into your config only if you want it. You can exclude it during SCS install/sync with `SCS_DISABLED_MCPS=codescene,...`.
...
Same triggers as **When to Use** above — this heading is what SCS uses for skill auto-activation.
...
| SCS skill / flow | Code Health MCP role |
|------------------|----------------------|
...
**Context tip:** SCS recommends keeping MCP count low. Enable `codescene` when doing substantive edits; disable when not needed.
