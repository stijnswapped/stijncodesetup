---
name: configure-scs
description: Interactive installer for StijnCodeSetup — guides users through selecting and installing skills and rules to user-level or project-level directories, verifies paths, and optionally optimizes installed files.
metadata:
  origin: SCS
---

# Configure StijnCodeSetup (SCS)

An interactive, step-by-step installation wizard for the StijnCodeSetup project. Uses `AskUserQuestion` to guide users through selective installation of skills and rules, then verifies correctness and offers optimization.

## When to Activate

- User says "configure scs", "install scs", "setup stijncodesetup", or similar
- User wants to selectively install skills or rules from this project
- User wants to verify or fix an existing SCS installation
- User wants to optimize installed skills or rules for their project

## Prerequisites

This skill must be accessible to Claude Code before activation. Two ways to bootstrap:
1. **Via Plugin**: `/plugin install scs@scs` — the plugin loads this skill automatically
2. **Manual**: Copy only this skill to `~/.claude/skills/configure-scs/SKILL.md`, then activate by saying "configure scs"

---

## Step 0: Clone SCS Repository

Before any installation, clone the latest SCS source to `/tmp`:

```bash
rm -rf /tmp/stijncodesetup
git clone https://github.com/StijnCodeSetup/stijncodesetup.git /tmp/stijncodesetup
```

Set `SCS_ROOT=/tmp/stijncodesetup` as the source for all subsequent copy operations.

If the clone fails (network issues, etc.), use `AskUserQuestion` to ask the user to provide a local path to an existing SCS clone.
...
```
Question: "Where should SCS components be installed?"
Options:
...
`claude-api` is an Anthropic canonical skill. Install it from [`anthropics/skills`](https://github.com/anthropics/skills) when you want the official Claude API workflow instead of an SCS-bundled copy.
...
cp -R "$SCS_ROOT/.agents/skills/<skill-name>" "$TARGET/skills/"

# Niche skills live under skills/
cp -R "$SCS_ROOT/skills/<skill-name>" "$TARGET/skills/"
```
...
```bash
# Common rules
cp -r $SCS_ROOT/rules/common $TARGET/rules/common

# Language-specific rules (preserve per-language directories)
cp -r $SCS_ROOT/rules/typescript $TARGET/rules/typescript   # if selected
cp -r $SCS_ROOT/rules/python $TARGET/rules/python            # if selected
cp -r $SCS_ROOT/rules/golang $TARGET/rules/golang            # if selected
```
...
**Critical**: Only modify files in the installation target (`$TARGET/`), NEVER modify files in the source SCS repository (`$SCS_ROOT/`).
