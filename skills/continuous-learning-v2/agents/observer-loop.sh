...
ANALYSIS_COOLDOWN="${SCS_OBSERVER_ANALYSIS_COOLDOWN:-60}"
IDLE_TIMEOUT_SECONDS="${SCS_OBSERVER_IDLE_TIMEOUT_SECONDS:-1800}"
...
analyze_observations() {
  ...
  if [ "${CLV2_IS_WINDOWS:-false}" = "true" ] && [ "${SCS_OBSERVER_ALLOW_WINDOWS:-false}" != "true" ]; then
    echo "[$(date)] Skipping claude analysis on Windows due to known non-interactive hang issue (#295). Set SCS_OBSERVER_ALLOW_WINDOWS=true to override." >> "$LOG_FILE"
    return
  fi
  ...
  MAX_ANALYSIS_LINES="${SCS_OBSERVER_MAX_ANALYSIS_LINES:-500}"
  ...
  analysis_file="$(mktemp "${observer_tmp_dir}/scs-observer-analysis.jsonl.XXXXXX")"
  ...
  prompt_file="$(mktemp "${observer_tmp_dir}/scs-observer-prompt.XXXXXX")"
...
PROMPT
  ...
  timeout_seconds="${SCS_OBSERVER_TIMEOUT_SECONDS:-120}"
  ...
  if [ -n "${SCS_OBSERVER_MAX_TURNS:-}" ]; then
    max_turns="${SCS_OBSERVER_MAX_TURNS}"
  else
  ...
  # but an explicit SCS_OBSERVER_MAX_TURNS override may be non-numeric, empty, or too
  ...
  # Model is configurable via SCS_OBSERVER_MODEL (defaults to haiku for cost efficiency);
  # e.g. SCS_OBSERVER_MODEL=opus for higher-quality instinct extraction. Heavier models are
  # slower — consider raising SCS_OBSERVER_TIMEOUT_SECONDS (default 120s) so the watchdog
  # doesn't kill the analysis mid-run.
  SCS_SKIP_OBSERVE=1 SCS_HOOK_PROFILE=minimal claude --model "${SCS_OBSERVER_MODEL:-haiku}" --max-turns "$max_turns" --print \
