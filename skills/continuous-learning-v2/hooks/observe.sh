...
#   - SCS observing its own Haiku observer sessions (self-loop)
#   - SCS observing other tools' automated sessions
...
# (SCS_HOOK_PROFILE=minimal, SCS_SKIP_OBSERVE=1, agent_id, path exclusions).
...
[ "${SCS_HOOK_PROFILE:-standard}" = "minimal" ] && exit 0

# Layer 3: cooperative skip env var for automated sessions.
[ "${SCS_SKIP_OBSERVE:-0}" = "1" ] && exit 0

# Layer 4: subagent sessions are automated by definition.
_SCS_AGENT_ID=$(echo "$INPUT_JSON" | "$PYTHON_CMD" -c "import json,sys; print(json.load(sys.stdin).get('agent_id',''))" 2>/dev/null || true)
[ -n "$_SCS_AGENT_ID" ] && exit 0

# Layer 5: known observer-session path exclusions.
_SCS_SKIP_PATHS="${SCS_OBSERVE_SKIP_PATHS:-observer-sessions,.claude-mem}"
if [ -n "$STDIN_CWD" ]; then
  IFS=',' read -ra _SCS_SKIP_ARRAY <<< "$_SCS_SKIP_PATHS"
  for _pattern in "${_SCS_SKIP_ARRAY[@]}"; do
...
SIGNAL_EVERY_N="${SCS_OBSERVER_SIGNAL_EVERY_N:-20}"
