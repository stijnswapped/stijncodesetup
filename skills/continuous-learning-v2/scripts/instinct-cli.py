...
def _resolve_homunculus_dir() -> Path:
    ...
    if override:
        ...
        print(f"[scs] CLV2_HOMUNCULUS_DIR={override!r} is not absolute; ignoring", file=sys.stderr)

    xdg = os.environ.get("XDG_DATA_HOME")
    if xdg:
        if Path(xdg).is_absolute():
            return Path(xdg) / "scs-homunculus"
        print(f"[scs] XDG_DATA_HOME={xdg!r} is not absolute; ignoring", file=sys.stderr)

    return Path.home() / ".local" / "share" / "scs-homunculus"
...
def _fetch_import_url(source: str, *, max_bytes: int = 2 * 1024 * 1024) -> str:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "SCS-instinct-import/2"})
