---
name: hermes-imports
description: Convert local Hermes operator workflows into sanitized SCS skills and release-pack artifacts. Use when preparing a Hermes workflow for public SCS reuse without leaking private workspace state, credentials, or local-only paths.
metadata:
  origin: SCS
---
...
Use this skill when turning a repeated Hermes workflow into something safe to ship in SCS.

Hermes is the operator shell. SCS is the reusable workflow layer. Imports should move stable patterns from Hermes into SCS without moving private state.

## When To Use

- A Hermes workflow has repeated enough times to become reusable.
- A local operator prompt should become a public SCS skill.
