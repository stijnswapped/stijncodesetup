...
## SCS install form and namespacing
...
|---|---|---|---|
| Plugin install (2.0.0+) | `<claude-home>/plugins/marketplaces/scs/` exists | `/scs:orchestrate` | `scs:<name>` |
| Legacy bare install | Above absent; agent files under `<claude-home>/agents/` | `/orchestrate` | `<name>` |

Why this matters: under the plugin install, agents register as `scs:tdd-guide`. Bare names force fuzzy matching, which fails intermittently under parallel calls. Under legacy, the prefixed forms are not registered and fail outright.
...
### Phase 0 — Detect SCS mode + language

1. Read `<plan-doc-path>`. If missing or empty, report and stop.
2. Detect SCS install form once and freeze it into `SCS_MODE`. Algorithm (run in order, stop at the first match):
   1. If `<claude-home>/plugins/marketplaces/scs/` exists → `SCS_MODE=plugin`.
   2. Else if `<claude-home>/agents/` exists and contains at least one SCS agent file (e.g. `tdd-guide.md`, `code-reviewer.md`) → `SCS_MODE=legacy`.
   3. Else → default to `SCS_MODE=legacy` and emit a one-line warning at the top of the output: `> Warning: could not detect SCS install; defaulting to legacy form. If you use the plugin install, edit the prefixes manually.`
...
5. **Normalize any agent names declared in the plan**: if the plan text references agents by their plugin-prefixed form (e.g. `scs:tdd-guide`), strip the prefix to get the bare catalogue name before validating or composing chains. Re-prefixing happens only at output time per `SCS_MODE` (Phase 4). Never let a pre-prefixed name flow into chain composition — it would double-prefix in plugin mode.
...
Emit Markdown using **the form determined by `SCS_MODE`**. The output uses one form throughout — every `{ORCH_CMD}` and every agent name is rendered with the matching prefix from Phase 0. **Do not emit both forms; do not include "this is plugin form" / "strip the prefix" instructions in the rendered output.**

Concrete rendering rules:

- `{ORCH_CMD}` = `/scs:orchestrate` under `plugin`, `/orchestrate` under `legacy`.
- `{AGENT(name)}` = `scs:<name>` under `plugin`, `<name>` under `legacy`.
...
- [ ] Every agent in every chain comes from the catalogue (after stripping any `scs:` prefix that appeared in the plan; see Phase 0 step 5).
...
- **Plan declares agents** (rare): first **strip any `scs:` prefix** to get the bare catalogue name (Phase 0 step 5), then validate against the catalogue. Replace invalid agents and explain under "Chain rationale". The bare name is re-prefixed at output time per `SCS_MODE`.
...
```bash
/scs:orchestrate custom "scs:tdd-guide,scs:database-reviewer,scs:python-reviewer,scs:security-reviewer" "[Plan: docs/plan/example-feature.md#step-2] Implement EncryptedString SQLAlchemy type and migrate UserProfile.birth_datetime/location columns; key from ENV APP_DB_KEY; Acceptance: encrypt/decrypt roundtrip tests pass; alembic upgrade/downgrade clean on empty DB; no plaintext in DB after migrate; Out of scope: cross-tenant profile sharing logic"
```
````

### Example 2 — Legacy mode, same step

If `SCS_MODE=legacy` were detected, the same step would be emitted as a single uniform command (no plugin-prefixed forms anywhere in the output):
