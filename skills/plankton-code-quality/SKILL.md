...
1. Install SCS as your plugin (agents, skills, commands, rules)
2. Add Plankton hooks for write-time quality enforcement
3. Use AgentShield for security audits
4. Use SCS's verification-loop as a final gate before PRs

### Avoiding Hook Conflicts

If running both SCS and Plankton hooks:
- SCS's Prettier hook and Plankton's biome formatter may conflict on JS/TS files
- Resolution: disable SCS's Prettier PostToolUse hook when using Plankton (Plankton's biome is more comprehensive)
- Both can coexist on different file types (SCS handles what Plankton doesn't cover)
...
```bash
export SCS_HOOK_PROFILE=strict
export SCS_QUALITY_GATE_FIX=true
export SCS_QUALITY_GATE_STRICT=true
```
