...
One entry point for "which group of SCS slash-commands runs my workflow, in what
order, and when do I stop." Also browses every command-group recipe family.

Fills the gap between two existing skills:

- `scs-guide` — lists commands and where to read docs, but as a flat catalog.
...
- "Show me all SCS command-group recipes" (catalog mode)
- "How many workflow pipelines does SCS have?"
- User invokes `/scs-recipes` with or without a description.

### Do Not Use When

- User wants the task done now — route to the actual command, don't describe it.
- User wants deep docs for ONE command — use `scs-guide`.
...
```bash
for D in \
  "$HOME"/.claude/plugins/marketplaces/scs/commands \
  "$HOME"/.claude/plugins/cache/scs/scs/*/commands \
  ./commands \
  ./.claude/commands \
  "$HOME"/.claude/commands; do
  [ -d "$D" ] && CMD_DIR="$D" && break
done
[ -z "${CMD_DIR:-}" ] && { echo "No SCS commands directory found."; return 1; }
...
5. **Where to read** — the `commands/<name>.md` path plus `/scs-guide <name>`.
...
Read full docs:
  commands/<cmd1>.md   (or: /scs-guide <cmd1>)
```

## Examples

**Catalog:** `/scs-recipes` → prints the family table and total count.

**Match:** `/scs-recipes plan a whole app upfront then auto-build with adversarial
...
**Match:** `/scs-recipes fix a bug in my Go service` → Best fit: `orch-fix-defect`
...
- Not per-command deep docs — that's `scs-guide`.
