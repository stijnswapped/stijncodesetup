---
name: scs-tools-cost-audit
description: Evidence-first SCS Tools burn and billing audit workflow. Use when investigating runaway PR creation, quota bypass, premium-model leakage, duplicate jobs, or GitHub App cost spikes in the SCS Tools repo.
metadata:
  origin: SCS
---
...
Use this skill when the user suspects the SCS Tools GitHub App is burning cost, over-creating PRs, bypassing usage limits, or routing free users into premium analysis paths.

This is a focused operator workflow for the sibling [SCS-Tools](../../SCS-Tools) repo. It is not a generic billing skill and it is not a repo-wide code review pass.

## Skill Stack

Pull these SCS-native skills into the workflow when relevant:
...
- user says SCS Tools burn rate, PR recursion, over-created PRs, usage-limit bypass, or premium-model leakage
- the task is in the sibling `SCS-Tools` repo and depends on webhook handlers, queue workers, usage reservation, PR creation logic, or paid-gate enforcement
...
- work in the sibling `SCS-Tools` repo, not in `stijncodesetup`
...
- switch into the sibling `SCS-Tools` repo
