...
1. **`claude agents` command** (primary) — run `claude agents` to get all agents known to the CLI, including user agents, plugin agents (e.g. `scs:architect`), and built-in agents. This automatically covers SCS marketplace installs without any path configuration.
...
- **Plugin agents** are prefixed with `plugin-name:` (e.g., `scs:security-reviewer`). Use the part after `:` as the agent name and the plugin name as the domain.
