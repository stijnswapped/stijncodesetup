...
description: Audit the active repo, MCP servers, plugins, connectors, env surfaces, and harness setup, then recommend the highest-value SCS-native skills, hooks, agents, and operator workflows. Use when the user wants help setting up Claude Code or understanding what capabilities are actually available in their environment.
metadata:
  origin: SCS
---
...
This is the SCS-native answer to setup-audit plugins. It does not modify files unless the user explicitly asks for follow-up implementation.
...
- Comparing official marketplace plugins against SCS-native coverage
...
- Prefer SCS-native workflows over generic "install another plugin" advice when SCS can reasonably own the surface.
...
- Separate three things clearly:
  ...
  - available but not wrapped well in SCS
...
2. Environment surface
   - `.env*` files in the active repo and obvious adjacent SCS workspaces
...
- existing SCS skills already relevant to the workspace

If a surface exists only as a primitive, call that out. Example:

- "Stripe is available via connected app, but SCS lacks a billing-operator skill"
- "Google Drive is connected, but there is no SCS-native Google Workspace operator workflow"
...
2. whether SCS already has parity
3. whether SCS only has primitives
4. whether SCS is missing the workflow entirely

### Phase 3: Turn Gaps Into SCS Decisions

For every real gap, recommend the correct SCS-native shape:

| Gap Type | Preferred SCS Shape |
|----------|---------------------|
...
2. **Parity**
   - where SCS already matches or exceeds the benchmark
3. **Primitive-only gaps**
   - tools exist, but SCS lacks a clean operator skill
4. **Missing integrations**
   - capability not available yet
5. **Top 3-5 next moves**
   - concrete SCS-native additions, ordered by impact
...
- If SCS already has a strong primitive, propose a wrapper skill instead of inventing a brand-new subsystem.

## Good Outcomes

- The user can immediately see what is connected, what is missing, and what SCS should own next.
