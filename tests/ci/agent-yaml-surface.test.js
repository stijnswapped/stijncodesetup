...
function run() {
  ...
  if (test('canonical Anthropic skills are not re-bundled in active SCS skill surfaces', () => {
    for (const skillName of CANONICAL_ANTHROPIC_SKILLS) {
      assert.ok(
        !fs.existsSync(path.join(SKILLS_DIR, skillName, 'SKILL.md')),
        `${skillName} should be installed from anthropics/skills, not SCS skills/`
      );
      assert.ok(
        !fs.existsSync(path.join(CODEX_SKILLS_DIR, skillName, 'SKILL.md')),
        `${skillName} should be installed from anthropics/skills, not SCS .agents/skills/`
      );
    }
  })) passed++; else failed++;
