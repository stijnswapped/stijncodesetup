...
async function run() {
  ...
  if (await test('offline report emits passing coverage checks and Linear-ready ITO-57 payload', async () => {
    ...
    assert.strictEqual(report.schema_version, 'scs.supply-chain-advisory-sources.v1');
  ...
  if (await test('CLI JSON can be written as a scheduled workflow artifact', async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-advisory-sources-'));
    ...
    try {
      ...
      assert.strictEqual(parsed.schema_version, 'scs.supply-chain-advisory-sources.v1');
