...
test('fixtures use one scenario id and declare read-only behavior', () => {
  ...
  assert.strictEqual(scenario.schema_version, 'scs.evaluator-rag.scenario.v1');
  assert.strictEqual(trace.schema_version, 'scs.evaluator-rag.trace.v1');
  assert.strictEqual(report.schema_version, 'scs.evaluator-rag.report.v1');
  assert.strictEqual(verifier.schema_version, 'scs.evaluator-rag.verifier.v1');
...
test('billing readiness scenario rejects launch copy overclaims', () => {
  ...
  assert.ok(playbook.includes('https://github.com/marketplace/scs-tools'));
});
...
test('AgentShield policy exception scenario rejects blanket suppression', () => {
  ...
  for (const blocked of [
    ...
    'editing AgentShield code or policy files from this SCS evaluator run'
  ]) {
  ...
  assert.ok(playbook.includes('npx scs-agentshield scan --format json'));
});
...
test('deep analyzer evidence scenario rejects no-corpus analyzer changes', () => {
  ...
  assert.ok(playbook.includes('../SCS-Tools/src/analyzers/fixtures/deep-analyzer-corpus.ts'));
