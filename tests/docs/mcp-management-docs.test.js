...
test('token optimization guide separates Claude MCP disables from SCS config filters', () => {
  ...
  assert.ok(
    source.includes('`SCS_DISABLED_MCPS` only affects SCS-generated MCP config output'),
    'Token guide should scope SCS_DISABLED_MCPS to config generation'
  );
  assert.ok(
...
test('README MCP guidance avoids settings.json disable instructions', () => {
  ...
  assert.ok(
    source.includes('`SCS_DISABLED_MCPS` is an SCS install/sync filter, not a live Claude Code toggle.'),
    'README should explain SCS_DISABLED_MCPS scope'
  );
  assert.ok(
