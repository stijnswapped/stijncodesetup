...
console.log('\n=== Testing SCS platform value loop docs ===\n');

test('platform value loop doc defines the three-layer SCS 2.0 direction', () => {
  ...
  for (const marker of [
    ...
    'reproducible demo',
    'SCS can be used full-stack as a meta-harness + agent + control pane',
  ]) {
...
test('Ito example preserves non-advisory and gated-access boundaries', () => {
  ...
  for (const marker of [
    ...
    'do not merge SCS Tools billing with Ito billing',
  ]) {
...
test('platform value loop does not overclaim release status or trading ability', () => {
  ...
  const forbidden = [
    ...
    'public SCS skills place trades',
  ];
