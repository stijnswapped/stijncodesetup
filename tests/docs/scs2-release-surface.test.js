...
console.log('\n=== Testing SCS 2.0 release surface ===\n');
...
test('announcement drafts avoid live-release claims before publication', () => {
  ...
  for (const relativePath of announcementFiles) {
    ...
    assert.ok(!/SCS v2\.0\.0-rc\.1 is live\./.test(source), `${relativePath} must not claim rc.1 is live before the release gate completes`);
  }
});
...
test('Hermes setup uses release-candidate wording for the rc.1 surface', () => {
  ...
  assert.ok(source.includes('SCS v2.0.0-rc.1 documents the Hermes surface'));
...
test('Hermes setup cross-links adjacent migration and architecture docs', () => {
  ...
  assert.ok(!source.includes('0.5. Generate and review artifacts with `scs migrate plan` /'));
});

test('release docs preserve the SCS/Hermes boundary', () => {
  const releaseNotes = read('docs/releases/2.0.0-rc.1/release-notes.md');
  assert.ok(releaseNotes.includes('SCS is the reusable substrate'));
...
test('preview pack manifest assembles release, Hermes, and publication gates', () => {
  ...
  for (const blocker of [
    ...
    'npm `stijncodesetup@2.0.0-rc.1`',
    'Claude plugin tag',
    'Codex repo-marketplace distribution evidence',
    'SCS Tools billing/product readiness'
  ]) {
...
test('owner approval packet consolidates the final gated decisions', () => {
  ...
  const hypergrowth = read('docs/releases/2.0.0/scs-2-hypergrowth-release-command-center.md');
  ...
  for (const urlSurface of ['GitHub prerelease URL', 'npm rc package URL', 'Claude plugin tag URL', 'Primary launch video URL', 'SCS Tools billing/readiness URL']) {
...
test('GA roadmap mirrors the current May 19 release evidence', () => {
  const roadmap = read('docs/SCS-2.0-GA-ROADMAP.md');
...
test('rc.1 quickstart gives a clone-to-cross-harness path', () => {
  ...
  assert.ok(quickstart.includes('git clone https://github.com/StijnCodeSetup/StijnCodeSetup.git'));
  assert.ok(quickstart.includes('cd SCS'));
...
test('launch checklist records the scs2 alpha version policy', () => {
  const cargoToml = read('scs2/Cargo.toml');
  const launchChecklist = read('docs/releases/2.0.0-rc.1/launch-checklist.md');
  assert.ok(cargoToml.includes('version = "0.1.0"'));
  assert.ok(launchChecklist.includes('`scs2/Cargo.toml` stays at `0.1.0`'));
  assert.ok(!launchChecklist.includes('confirm whether `scs2/Cargo.toml` moves'));
});
...
test('release video suite manifest gates the content launch lane', () => {
  ...
  for (const marker of [
    ...
    'SCS_VIDEO_SOURCE_ROOT',
    'SCS_VIDEO_RELEASE_SUITE_ROOT',
  ...
  for (const asset of ['longform-full-wide.mp4', 'sf-thread-2-whatisscs.mp4', 'thread-2-ghapp-money.mp4', 'coverage-montage-wide.mp4', 'star_history.png', 'x_analytics.png']) {
...
test('release video suite public docs do not expose private media paths', () => {
  const releaseVideoDocs = ['docs/releases/2.0.0-rc.1/video-suite-production.md', 'docs/releases/2.0.0/scs-2-hypergrowth-release-command-center.md'];
...
test('publication readiness checklist gates public release actions on evidence', () => {
  ...
  for (const surface of ['GitHub release', 'npm package', 'Claude plugin', 'Codex plugin', 'Codex repo marketplace', 'OpenCode package', 'SCS Tools billing reference', 'Announcement copy']) {
  ...
  assert.ok(may15Evidence.includes('SCS Tools PR #73'));
  assert.ok(may15Evidence.includes('SCS-Tools PR #75'));
  ...
  assert.ok(source.includes('SCS-Tools #92 main CI'));
  assert.ok(source.includes('SCS-Tools #93 main CI'));
...
test('release name and plugin publication checklist freezes rc.1 surfaces', () => {
  ...
  const launchChecklist = read('docs/releases/2.0.0-rc.1/launch-checklist.md');
  const referenceArchitecture = read('docs/SCS-2.0-REFERENCE-ARCHITECTURE.md');

  for (const value of [
    'Ship `v2.0.0-rc.1` as **SCS**',
    '`StijnCodeSetup/StijnCodeSetup`',
    '`stijncodesetup`',
    '`scs` on npm is occupied',
    '`@StijnCodeSetup/scs` is unclaimed on npm',
    'Claude plugin',
    'Codex plugin',
...
test('active release identity surfaces use canonical SCS repo URLs', () => {
  const activeFiles = [
    'README.md',
  ...
  for (const relativePath of activeFiles) {
    const source = read(relativePath);
    if (source.includes('StijnCodeSetup/stijncodesetup')) {
      offenders.push(relativePath);
    }
  }
