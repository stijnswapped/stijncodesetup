...
function runScript(args = [], envOverrides = {}) {
  ...
  if (!envOverrides.SCS_HOOK_PROFILE) delete env.SCS_HOOK_PROFILE;
  if (!envOverrides.SCS_DISABLED_HOOKS) delete env.SCS_DISABLED_HOOKS;
...
function runTests() {
  ...
  if (test('returns no when hook is disabled via env', () => {
    const result = runScript(['my-hook'], { SCS_DISABLED_HOOKS: 'my-hook' });
    assert.strictEqual(result.stdout, 'no');
  })) passed++; else failed++;

  if (test('returns yes when different hook is disabled', () => {
    const result = runScript(['my-hook'], { SCS_DISABLED_HOOKS: 'other-hook' });
    assert.strictEqual(result.stdout, 'yes');
  })) passed++; else failed++;
  ...
  if (test('returns yes for strict profile with strict-only hook', () => {
    const result = runScript(['my-hook', 'strict'], { SCS_HOOK_PROFILE: 'strict' });
    assert.strictEqual(result.stdout, 'yes');
  })) passed++; else failed++;

  if (test('returns yes for minimal profile with minimal-only hook', () => {
    const result = runScript(['my-hook', 'minimal'], { SCS_HOOK_PROFILE: 'minimal' });
    assert.strictEqual(result.stdout, 'yes');
  })) passed++; else failed++;
