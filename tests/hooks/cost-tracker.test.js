...
function runTests() {
  ...
  // 6. Prefers SCS_SESSION_ID for SCS2 session correlation
  (test('prefers SCS_SESSION_ID over CLAUDE_SESSION_ID when both are present', () => {
    const tmpHome = makeTempDir();
    const input = {
      model: 'claude-sonnet-4-20250514',
    ...
    const metricsFile = path.join(tmpHome, '.claude', 'metrics', 'costs.jsonl');
    const row = JSON.parse(fs.readFileSync(metricsFile, 'utf8').trim());
    assert.strictEqual(row.session_id, 'scs-session-1234', 'Expected SCS_SESSION_ID to win');
