...
function runWrapper(input, env = {}) {
  ...
  const result = spawnSync('node', [wrapper], {
    input: rawInput,
    encoding: 'utf8',
    env: { ...process.env, SCS_HOOK_PROFILE: 'standard', ...env },
    timeout: 15000,
...
// --- Disable via SCS_DISABLED_HOOKS ---

if (test('respects SCS_DISABLED_HOOKS=pre:bash:block-no-verify', () => {
  const r = runWrapper(
    { command: 'git commit --no-verify -m "msg"' },
    { SCS_DISABLED_HOOKS: 'pre:bash:block-no-verify' }
  );
