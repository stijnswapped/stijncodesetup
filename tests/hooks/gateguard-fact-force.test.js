...
function runTests() {
  ...
  // --- Test 9: respects SCS_DISABLED_HOOKS ---
  clearState();
  if (
    test('respects SCS_DISABLED_HOOKS (skips when disabled)', () => {
      const input = {
        tool_name: 'Edit',
        tool_input: { file_path: '/src/disabled.js', old_string: 'a', new_string: 'b' }
      };
      const result = runHook(input, {
        SCS_DISABLED_HOOKS: 'pre:edit-write:gateguard-fact-force'
      });
  ...
  clearState();
  if (
    test('respects SCS_GATEGUARD=off without writing gate state', () => {
      const input = {
        tool_name: 'Write',
        tool_input: { file_path: '/src/env-disabled.js', content: 'export const ok = true;' }
      };
      const result = runHook(input, { SCS_GATEGUARD: 'off' });
      const output = parseOutput(result.stdout);
  ...
  if (
    test('denial messages include direct recovery escape hatch', () => {
      ...
      assert.strictEqual(output.hookSpecificOutput.permissionDecision, 'deny');
      assert.ok(output.hookSpecificOutput.permissionDecisionReason.includes('SCS_GATEGUARD=off'), 'denial reason should show the direct recovery env toggle');
      assert.ok(output.hookSpecificOutput.permissionDecisionReason.includes('SCS_DISABLED_HOOKS'), 'denial reason should mention the existing hook-id disable control');
    })
  )
    passed++;
  else failed++;
  ...
  if (
    test('destructive Bash denials omit recovery escape hatch', () => {
      ...
      assert.ok(!output.hookSpecificOutput.permissionDecisionReason.includes('SCS_GATEGUARD=off'), 'destructive gate should not advertise disabling GateGuard');
    })
  )
    passed++;
  else failed++;
  ...
  function runFreshSessionEdit(filePath, extra = {}) {
    return runHook(
      ...
      { CLAUDE_SESSION_ID: '', SCS_SESSION_ID: '' }
    );
  }
  ...
  if (
    test('emits a condensed single-line denial once the full-block budget is spent', () => {
      ...
      assert.ok(reason.includes('SCS_GATEGUARD=off'), 'condensed message keeps a recovery hint');
    })
  )
    passed++;
  else failed++;
