...
async function runTests() {
  ...
  if (await test('run() passes through input when feature flag is off', async () => {
    const original = process.env.SCS_GOVERNANCE_CAPTURE;
    delete process.env.SCS_GOVERNANCE_CAPTURE;
    ...
    try {
      ...
      if (original !== undefined) {
        process.env.SCS_GOVERNANCE_CAPTURE = original;
      }
    }
  })) passed += 1; else failed += 1;
  ...
  if (await test('run() passes through input when feature flag is on', async () => {
    const original = process.env.SCS_GOVERNANCE_CAPTURE;
    process.env.SCS_GOVERNANCE_CAPTURE = '1';
