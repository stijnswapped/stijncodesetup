...
function runScript(scriptPath, input = '', env = {}) {
  return new Promise((resolve, reject) => {
    ...
    delete childEnv.SCS_AGENT_DATA_HOME;
...
function assertNoProjectDetectionSideEffects(homeDir, testName) {
  const homunculusDir = path.join(homeDir, '.local', 'share', 'scs-homunculus');
...
async function runTests() {
  ...
  if (
    await asyncTest('exits 0 even with isolated empty HOME', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-iso-start-'));
  ...
  if (
    await asyncTest('skips template session content', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-tpl-start-'));
  ...
  if (
    await asyncTest('injects real session content', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-real-start-'));
      const sessionsDir = getLegacySessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('caps very large session-start context by default', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-large-start-'));
      const sessionsDir = getLegacySessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('honors SCS_SESSION_START_MAX_CHARS for injected context', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-max-start-'));
      const sessionsDir = getLegacySessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
      ...
      try {
        const result = await runScript(path.join(scriptsDir, 'session-start.js'), '', {
          HOME: isoHome,
          USERPROFILE: isoHome,
          SCS_SESSION_START_MAX_CHARS: '700'
        });
        assert.strictEqual(result.code, 0);
        const additionalContext = getSessionStartAdditionalContext(result.stdout);
  ...
  if (
    await asyncTest('honors SCS_MAX_INJECTED_INSTINCTS for injected instincts', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-max-instincts-'));
      ...
      try {
        ...
        const capped = await runScript(path.join(scriptsDir, 'session-start.js'), '', { ...baseEnv, SCS_MAX_INJECTED_INSTINCTS: '3' });
        ...
        const garbage = await runScript(path.join(scriptsDir, 'session-start.js'), '', { ...baseEnv, SCS_MAX_INJECTED_INSTINCTS: 'not-a-number' });
        ...
        const partial = await runScript(path.join(scriptsDir, 'session-start.js'), '', { ...baseEnv, SCS_MAX_INJECTED_INSTINCTS: '3.9' });
        ...
        const exponent = await runScript(path.join(scriptsDir, 'session-start.js'), '', { ...baseEnv, SCS_MAX_INJECTED_INSTINCTS: '1e2' });
  ...
  if (
    await asyncTest('honors SCS_INSTINCT_CONFIDENCE_THRESHOLD for injected instincts', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-instinct-threshold-'));
      const homunculusDir = path.join(isoHome, 'homunculus');
      const instinctsDir = path.join(homunculusDir, 'instincts', 'personal');
      fs.mkdirSync(instinctsDir, { recursive: true });
      ...
      try {
        ...
        const raised = await runScript(path.join(scriptsDir, 'session-start.js'), '', { ...baseEnv, SCS_INSTINCT_CONFIDENCE_THRESHOLD: '0.95' });
        ...
        const lowered = await runScript(path.join(scriptsDir, 'session-start.js'), '', { ...baseEnv, SCS_INSTINCT_CONFIDENCE_THRESHOLD: '0.5' });
        ...
        const hex = await runScript(path.join(scriptsDir, 'session-start.js'), '', { ...baseEnv, SCS_INSTINCT_CONFIDENCE_THRESHOLD: '0x1' });
  ...
  if (
    await asyncTest('disables session-start additional context when requested', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-disabled-start-'));
      const sessionsDir = getLegacySessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('prefers canonical session-data content over legacy duplicates', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-canonical-start-'));
  ...
  if (
    await asyncTest('strips ANSI escape codes from injected session content', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-ansi-start-'));
      const sessionsDir = getLegacySessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('skips prior session summary on Desktop SessionStart resume', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-resume-start-'));
  ...
  if (
    await asyncTest('skips prior session summary on CLI SessionStart resume', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-cli-resume-start-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('skips prior session summary on clear SessionStart payloads', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-clear-start-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('does not log malformed SessionStart stdin content while skipping prior summary', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-invalid-start-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('does not fall back to unrelated recent session content', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-cross-project-start-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('does not inject same-project sessions from a different explicit worktree', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-cross-worktree-start-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('allows project fallback only for legacy sessions without worktree metadata', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-project-only-start-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('reports learned skills count', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-skills-start-'));
  ...
  if (
    await asyncTest('injects learned skills into session-start additional context', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-skills-context-'));
      const learnedDir = path.join(isoHome, '.claude', 'skills', 'learned');
      fs.mkdirSync(learnedDir, { recursive: true });
      fs.mkdirSync(getCanonicalSessionsDir(isoHome), { recursive: true });
  ...
  if (
    await asyncTest('creates or updates session file', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-session-create-'));
  ...
  if (
    await asyncTest('includes session ID in filename', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-session-id-'));
  ...
  if (
    await asyncTest('derives shortId from transcript_path UUID when available', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-session-transcript-'));
  ...
  if (
    await asyncTest('normalizes transcript UUID shortId to lowercase', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-session-transcript-upper-'));
      const transcriptUuid = 'ABCDEF12-3456-4789-A012-BCDEF3456789';
  ...
  if (
    await asyncTest('matches getSessionIdShort when transcript UUID equals CLAUDE_SESSION_ID', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-session-transcript-match-'));
  ...
  if (
    await asyncTest('writes project, branch, and worktree metadata into new session files', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-session-metadata-'));
  ...
  if (
    await asyncTest('creates compaction log', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-compact-log-'));
      try {
        const result = await runScript(path.join(scriptsDir, 'pre-compact.js'), '', {
          HOME: isoHome,
          USERPROFILE: isoHome
  ...
  if (
    await asyncTest('annotates active session file with compaction marker', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-compact-annotate-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('compaction log contains timestamp', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-compact-ts-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    test('SessionStart hook uses safe inline resolver without plugin-tree scanning', () => {
      ...
      assert.ok(bootstrapSrc.includes('resolve-scs-root'), 'Bootstrap should delegate to the committed resolver module');
      assert.ok(bootstrapSrc.includes('resolveScsRoot({ probe: rel })'), 'Bootstrap should call resolveScsRoot with the hook probe');
    })
  )
    passed++;
  else failed++;
  if (
    test('Stop and SessionEnd hooks use the safe inline resolver when plugin root may be unset', () => {
      ...
      for (const hook of [...stopHooks, ...sessionEndHooks]) {
        ...
        assert.ok(commandText.includes('resolve-scs-root'), 'Lifecycle hook should delegate to the committed resolver module');
  ...
  if (
    test('plugin.json does NOT have explicit hooks declaration', () => {
      ...
      // See: https://github.com/StijnCodeSetup/stijncodesetup/issues/103
  ...
  if (
    test('observer-loop uses a configurable max-turn budget with safe default', () => {
      ...
      assert.ok(observerLoopSource.includes('SCS_OBSERVER_MAX_TURNS'), 'observer-loop should allow max-turn overrides');
      ...
      assert.ok(observerLoopSource.includes('SCS_SKIP_OBSERVE=1'), 'observer-loop should suppress observe.sh for automated sessions');
      assert.ok(observerLoopSource.includes('SCS_HOOK_PROFILE=minimal'), 'observer-loop should run automated analysis with the minimal hook profile');
  ...
  if (SKIP_BASH) {
    ...
    await asyncTest('detect-project writes project metadata to the registry and project directory', async () => {
      ...
      try {
        ...
        spawnSync('git', ['remote', 'add', 'origin', 'https://github.com/example/scs-test.git'], { cwd: repoDir, stdio: 'ignore' });
        ...
        const registryPath = path.join(homeDir, '.local', 'share', 'scs-homunculus', 'projects.json');
        const expectedProjectDir = path.join(homeDir, '.local', 'share', 'scs-homunculus', 'projects', projectId);
        ...
        assert.strictEqual(metadata.remote, 'https://github.com/example/scs-test.git', 'project.json should include the sanitized remote');
  ...
  if (SKIP_BASH) {
    ...
    await asyncTest('observe.sh skips minimal hook profile before project detection side effects', async () => {
      await assertObserveSkipBeforeProjectDetection({
        ...
        env: { CLAUDE_CODE_ENTRYPOINT: 'cli', SCS_HOOK_PROFILE: 'minimal' }
      });
    })
  )
    passed++;
  ...
  if (SKIP_BASH) {
    ...
    await asyncTest('observe.sh skips cooperative skip env before project detection side effects', async () => {
      await assertObserveSkipBeforeProjectDetection({
        ...
        env: { CLAUDE_CODE_ENTRYPOINT: 'cli', SCS_SKIP_OBSERVE: '1' }
      });
    })
  )
    passed++;
    ...
    await asyncTest('observe.sh skips configured observer-session paths before project detection side effects', async () => {
      await assertObserveSkipBeforeProjectDetection({
        ...
        env: {
          ...
          SCS_OBSERVE_SKIP_PATHS: ' observer-sessions , .claude-mem '
        },
  ...
  if (
    await asyncTest('config with custom learned_skills_path creates directory', async () => {
      ...
      const transcriptPath = path.join(testDir, 'transcript.jsonl');
      const lines = [];
      for (let i = 0; i < 10; i++) lines.push(`{"type":"user","content":"msg${i}"}`);
      fs.writeFileSync(transcriptPath, lines.join('\n'));
  ...
  if (
    await asyncTest('only annotates *-session.tmp files, not other .tmp files', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-compact-glob-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('handles no active session files gracefully', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-compact-nosession-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('exits 0 with empty sessions directory (no recent sessions)', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-start-empty-'));
      fs.mkdirSync(getCanonicalSessionsDir(isoHome), { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
      try {
        const result = await runScript(path.join(scriptsDir, 'session-start.js'), '', {
  ...
  if (
    await asyncTest('does not inject blank template session into context', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-start-blank-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('annotates only the newest session file when multiple exist', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-compact-multi-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('does not inject empty session file content into context', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-start-empty-file-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('summary omits Files Modified and Tools Used when none found', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-notools-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('reports available session aliases on startup', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-start-alias-'));
      fs.mkdirSync(getCanonicalSessionsDir(isoHome), { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('parallel compaction runs all append to log without loss', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-compact-par-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('exits 0 when sessions path is a file (not a directory)', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-start-blocked-'));
  ...
  if (
    await asyncTest('excludes session files older than 7 days', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-start-7day-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('prunes session files older than the retention window', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-start-prune-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('disables pruning when SCS_SESSION_RETENTION_DAYS=0', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-start-prune-off-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
      ...
      try {
        const result = await runScript(path.join(scriptsDir, 'session-start.js'), '', {
          HOME: isoHome,
          USERPROFILE: isoHome,
          SCS_SESSION_RETENTION_DAYS: '0'
        });

        assert.strictEqual(result.code, 0);
        assert.ok(fs.existsSync(expiredFile), 'Should keep all sessions when retention is opt-out=0');
        assert.ok(result.stderr.includes('Pruning disabled via SCS_SESSION_RETENTION_DAYS'), `Should log pruning disabled, stderr: ${result.stderr}`);
  ...
  if (
    await asyncTest('disables pruning when SCS_SESSION_RETENTION_DAYS=off', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-start-prune-offstr-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
      ...
      try {
        const result = await runScript(path.join(scriptsDir, 'session-start.js'), '', {
          HOME: isoHome,
          USERPROFILE: isoHome,
          SCS_SESSION_RETENTION_DAYS: 'off'
        });
  ...
  if (
    await asyncTest('falls back to default retention when SCS_SESSION_RETENTION_DAYS is garbage', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-start-prune-garbage-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
      ...
      try {
        const result = await runScript(path.join(scriptsDir, 'session-start.js'), '', {
          HOME: isoHome,
          USERPROFILE: isoHome,
          SCS_SESSION_RETENTION_DAYS: 'bogus-value'
        });
  ...
  if (
    await asyncTest('injects newest session when multiple recent sessions exist', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-start-multi-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      fs.mkdirSync(path.join(isoHome, '.claude', 'skills', 'learned'), { recursive: true });
  ...
  if (
    await asyncTest('does not inject content when session file is unreadable', async () => {
      ...
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-start-unreadable-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('exits 0 when session file is read-only (appendFile fails)', async () => {
      ...
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-compact-ro-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('logs warning when existing session file lacks Last Updated field', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-end-nots-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('extracts user messages from role-only format (no type field)', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-role-only-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('logs "Transcript not found" for nonexistent transcript_path', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-notfound-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('extracts tool name and file path from entry.name/entry.input (not tool_name/tool_input)', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-r70-entryname-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('shows selection prompt when no package manager preference found (default source)', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-r71-ss-default-'));
  ...
  // ── Round 76: evaluate-session.js no-transcript guard ──
  console.log('\nRound 76: evaluate-session.js (no-transcript guard):');

  if (
    await asyncTest('evaluate-session exits 0 before touching HOME when no transcript is provided', async () => {
      if (process.platform === 'win32') {
        console.log('    (skipped — /dev/null not available on Windows)');
        return;
      }
      ...
      assert.strictEqual(result.code, 0, `Should exit 0 (don't block on errors), got ${result.code}`);
      assert.strictEqual(result.stderr, '', `Should skip before directory writes when no transcript is provided, got: ${result.stderr}`);
    })
  )
    passed++;
  else failed++;
  ...
  if (
    await asyncTest('extracts user messages from entries where only message.role is user (not type or role)', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-msgrole-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('skips user messages with numeric content (non-string non-array branch)', async () => {
      ...
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-r81-numcontent-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
      const transcriptPath = path.join(isoHome, 'transcript.jsonl');
  ...
  if (
    await asyncTest('collects tool name from entry with tool_name but non-tool_use type', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-r82-toolname-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (
    await asyncTest('preserves file when marker present but regex does not match corrupted template', async () => {
      const isoHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-r82-tmpl-'));
      const sessionsDir = getCanonicalSessionsDir(isoHome);
      fs.mkdirSync(sessionsDir, { recursive: true });
...
`;
  ...
  // ── Round 95: pre-compact.js — SCS_SKIP_LLM_SUMMARY guard ──
  console.log('\nRound 95: pre-compact.js (transcript_path provided + SCS_SKIP_LLM_SUMMARY=1 — LLM skipped):');

  if (
    await asyncTest('pre-compact falls back to compaction log entry when SCS_SKIP_LLM_SUMMARY=1', async () => {
      const testDir = createTestDir();
  ...
  // ── Round 95: session-end.js — SCS_LLM_SUMMARY_INTERVAL controls trigger ──
  console.log('\nRound 95: session-end.js (SCS_LLM_SUMMARY_INTERVAL — controls LLM trigger cadence):');
  ...
  if (
    await asyncTest('session-end triggers LLM when totalMessages % interval === 0', async () => {
      ...
      const result = await runScript(path.join(scriptsDir, 'session-end.js'), stdinJson, {
        HOME: testDir,
        SCS_LLM_SUMMARY_INTERVAL: '3',
        SCS_SKIP_LLM_SUMMARY: '1' // prevent actual claude -p invocation in tests
      });
