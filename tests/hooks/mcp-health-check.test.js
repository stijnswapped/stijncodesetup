...
function createTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'scs-mcp-health-'));
}
...
async function runTests() {
  ...
  if (test('blocks truncated MCP hook input by default', () => {
    ...
    const result = runRawHook(rawInput, {
      CLAUDE_HOOK_EVENT_NAME: 'PreToolUse',
      SCS_HOOK_INPUT_TRUNCATED: '1',
      SCS_HOOK_INPUT_MAX_BYTES: '512'
    });
  ...
  if (test('allows truncated MCP hook input when fail-open mode is enabled', () => {
    ...
    const result = runRawHook(rawInput, {
      CLAUDE_HOOK_EVENT_NAME: 'PreToolUse',
      SCS_HOOK_INPUT_TRUNCATED: 'true',
      SCS_HOOK_INPUT_MAX_BYTES: '256',
      SCS_MCP_HEALTH_FAIL_OPEN: 'yes'
    });
  ...
  if (await asyncTest('uses default cwd config path and default home state path', async () => {
    ...
    try {
      ...
      const result = runHook(
        ...
        {
          CLAUDE_HOOK_EVENT_NAME: 'PreToolUse',
          SCS_MCP_CONFIG_PATH: null,
          SCS_MCP_HEALTH_STATE_PATH: null,
          SCS_MCP_HEALTH_TIMEOUT_MS: '100',
          HOME: homeDir,
          USERPROFILE: homeDir
        },
        { cwd: tempDir }
  ...
  if (test('uses cached healthy and unhealthy states without probing configs', () => {
    ...
    try {
      ...
      const healthy = runHook(
        ...
        {
          CLAUDE_HOOK_EVENT_NAME: 'PreToolUse',
          SCS_MCP_CONFIG_PATH: path.join(tempDir, 'missing.json'),
          SCS_MCP_HEALTH_STATE_PATH: healthyStatePath
        }
      );
      const unhealthy = runHook(
        { tool_name: 'mcp__blocked__query', tool_input: {} },
        {
          CLAUDE_HOOK_EVENT_NAME: 'PreToolUse',
          SCS_MCP_CONFIG_PATH: path.join(tempDir, 'missing.json'),
          SCS_MCP_HEALTH_STATE_PATH: unhealthyStatePath
        }
      );
    ...
    try {
      ...
      const result = runHook(
        ...
        },
        {
          CLAUDE_HOOK_EVENT_NAME: 'PreToolUse',
          SCS_MCP_CONFIG_PATH: path.join(tempDir, 'missing.json'),
          SCS_MCP_HEALTH_STATE_PATH: statePath
        }
      );
  ...
  if (await asyncTest('supports explicit tool_input server targets and mcp_servers config aliases', async () => {
    ...
    try {
      ...
      const result = runHook(input, {
        CLAUDE_HOOK_EVENT_NAME: 'PreToolUse',
        SCS_MCP_CONFIG_PATH: configPath,
        SCS_MCP_HEALTH_STATE_PATH: statePath,
        SCS_MCP_HEALTH_TIMEOUT_MS: '100'
      });
    ...
    try {
      ...
      const result = runHook(
        { tool_name: 'mcp__flaky__search', tool_input: {} },
        {
          ...
          SCS_MCP_CONFIG_PATH: configPath,
          SCS_MCP_HEALTH_STATE_PATH: statePath,
          SCS_MCP_HEALTH_TIMEOUT_MS: '1000'
        }
      );
  ...
  if (await asyncTest('includes command stderr and config env in unhealthy probe reasons', async () => {
    ...
    try {
      fs.writeFileSync(
        serverScript,
        "console.error(`probe failed with ${process.env.SCS_MCP_TEST_MARKER}`); process.exit(1);\n"
      );
      writeConfig(configPath, {
        mcpServers: {
          stderrprobe: {
            ...
            args: [serverScript],
            env: {
              SCS_MCP_TEST_MARKER: 'marker-from-config'
            }
          }
        }
      });
    ...
    try {
      ...
      const result = runHook(
        { tool_name: 'mcp__sticky__search', tool_input: {} },
        {
          ...
          SCS_MCP_CONFIG_PATH: configPath,
          SCS_MCP_HEALTH_STATE_PATH: statePath,
          SCS_MCP_RECONNECT_COMMAND: `${JSON.stringify(process.execPath)} ${JSON.stringify(reconnectScript)}`,
          SCS_MCP_HEALTH_TIMEOUT_MS: '1000',
          SCS_MCP_HEALTH_BACKOFF_MS: '10'
        }
      );
    ...
    try {
      ...
      const result = runHook(
        ...
        },
        {
          ...
          SCS_MCP_CONFIG_PATH: configPath,
          SCS_MCP_HEALTH_STATE_PATH: statePath,
          SCS_MCP_RECONNECT_COMMAND: `node ${JSON.stringify(reconnectScript)}`,
          SCS_MCP_HEALTH_TIMEOUT_MS: '1000'
        }
      );
    ...
    try {
      const result = runHook(
        ...
        },
        {
          CLAUDE_HOOK_EVENT_NAME: 'PostToolUseFailure',
          SCS_MCP_HEALTH_STATE_PATH: statePath,
          SCS_MCP_RECONNECT_COMMAND: null
        }
      );
    ...
    try {
      ...
      const result = runHook(
        ...
        },
        {
          ...
          SCS_MCP_CONFIG_PATH: path.join(tempDir, 'missing.json'),
          SCS_MCP_RECONNECT_COMMAND: null,
          SCS_MCP_RECONNECT_FOO_BAR: `${JSON.stringify(process.execPath)} ${JSON.stringify(reconnectScript)} ${JSON.stringify(markerFile)} {server}`
        }
      );
  ...
  if (await asyncTest('treats HTTP 400 probe responses as healthy reachable servers', async () => {
    if (!(await canBindLoopback())) {
      console.log('    (skipped — loopback bind denied by sandbox)');
      return;
    }
    ...
    try {
      ...
      const result = runHook(input, {
        ...
        SCS_MCP_CONFIG_PATH: configPath,
        SCS_MCP_HEALTH_STATE_PATH: statePath,
        SCS_MCP_HEALTH_TIMEOUT_MS: '2000'
      });
