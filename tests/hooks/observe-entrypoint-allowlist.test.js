 ...
 * Layer 2 (the SCS_HOOK_PROFILE check) while denied entrypoints stop at
 * Layer 1's `exit 0`. We force Layer 2 to short-circuit via
 * SCS_HOOK_PROFILE=minimal so the test is fast and side-effect-free
...
function makeTempHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'scs-observe-allowlist-'));
}
 ...
 * Layer 2 is forced to short-circuit (SCS_HOOK_PROFILE=minimal) so the only
...
// `[ "${SCS_HOOK_PROFILE:-standard}" = "minimal" ] && exit 0` therefore
