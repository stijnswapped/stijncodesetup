...
function createTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'scs-observe-subdir-test-'));
}
...
function gitInit(dir) {
  ...
  const remoteResult = spawnSync(
    'git',
    ['remote', 'add', 'origin', 'https://github.com/example/scs-test.git'],
...
function readSingleProjectMetadata(homeDir) {
  const projectsDir = path.join(homeDir, '.local', 'share', 'scs-homunculus', 'projects');
