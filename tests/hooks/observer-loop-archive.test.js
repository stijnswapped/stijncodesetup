...
function createTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'scs-observer-archive-'));
}
...
function runAnalyzeOnce(claudeExitCode) {
  ...
  try {
    ...
    // Defensive: never leak CLAUDE_PLUGIN_ROOT into the SCS test shell (it
    ...
    childEnv.SCS_OBSERVER_TIMEOUT_SECONDS = '2';
