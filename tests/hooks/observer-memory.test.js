...
function createTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'scs-observer-test-'));
}
...
test('observe.sh default throttle is 20 observations per signal', () => {
  const content = fs.readFileSync(observeShPath, 'utf8');
  assert.ok(content.includes('SCS_OBSERVER_SIGNAL_EVERY_N:-20'), 'Default signal frequency should be every 20 observations');
});
...
test('default cooldown is 60 seconds', () => {
  const content = fs.readFileSync(observerLoopPath, 'utf8');
  assert.ok(content.includes('SCS_OBSERVER_ANALYSIS_COOLDOWN:-60'), 'Default cooldown should be 60 seconds');
});
...
test('observer-loop.sh defines idle timeout fallback', () => {
  ...
  assert.ok(content.includes('SCS_OBSERVER_IDLE_TIMEOUT_SECONDS:-1800'), 'Default idle timeout should be 30 minutes');
});
...
test('default max analysis lines is 500', () => {
  const content = fs.readFileSync(observerLoopPath, 'utf8');
  assert.ok(content.includes('SCS_OBSERVER_MAX_ANALYSIS_LINES:-500'), 'Default should sample last 500 lines');
});

test('analysis temp file is created and cleaned up', () => {
  const content = fs.readFileSync(observerLoopPath, 'utf8');
  assert.ok(content.includes('scs-observer-analysis'), 'Should create a temp analysis file');
...
test('observer-loop uses project-local temp directory for analysis artifacts', () => {
  ...
  assert.ok(content.includes('mktemp "${observer_tmp_dir}/scs-observer-analysis.'), 'Analysis temp file should use the project temp dir');
  assert.ok(content.includes('mktemp "${observer_tmp_dir}/scs-observer-prompt.'), 'Prompt temp file should use the project temp dir');
});
...
test('observe.sh creates counter file and increments on each call', () => {
  ...
  // We use SCS_SKIP_OBSERVE=0 and minimal JSON so observe.sh processes but
...
test('claude invocation still includes SCS_SKIP_OBSERVE and SCS_HOOK_PROFILE guards', () => {
  const content = fs.readFileSync(observerLoopPath, 'utf8');
  ...
  // Model is configurable via SCS_OBSERVER_MODEL but must still default to haiku.
  assert.ok(
    claudeLine.includes('${SCS_OBSERVER_MODEL:-haiku}'),
    `claude --model should default to haiku and honor SCS_OBSERVER_MODEL, got: ${claudeLine}`
  );
  ...
  assert.ok(fullCommand.includes('SCS_SKIP_OBSERVE=1'), 'claude invocation should include SCS_SKIP_OBSERVE=1 guard');
  assert.ok(fullCommand.includes('SCS_HOOK_PROFILE=minimal'), 'claude invocation should include SCS_HOOK_PROFILE=minimal guard');
});
