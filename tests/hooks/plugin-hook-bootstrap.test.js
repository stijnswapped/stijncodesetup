...
function run(args = [], options = {}) {
  return spawnSync(process.execPath, [SCRIPT, ...args], {
    ...
    env: {
      ...
      SCS_PLUGIN_ROOT: options.scsRoot || '',
      ...(options.env || {}),
    },
...
function runTests() {
  ...
  if (test('normalizes Windows Git Bash POSIX drive roots', () => {
    assert.strictEqual(
      normalizePluginRootForPlatform('/c/Users/x/.claude/plugins/scs', 'win32'),
      'C:/Users/x/.claude/plugins/scs'
    );
    assert.strictEqual(
      normalizePluginRootForPlatform('/z/Work/SCS/scripts/hooks/check-console-log.js', 'win32'),
      'Z:/Work/SCS/scripts/hooks/check-console-log.js'
    );
  })) passed++; else failed++;
  ...
  if (test('leaves already-Windows roots unchanged', () => {
    ...
    assert.strictEqual(
      normalizePluginRootForPlatform('D:\\Users\\x\\.claude\\plugins\\scs', 'win32'),
      'D:\\Users\\x\\.claude\\plugins\\scs'
    );
  })) passed++; else failed++;

  if (test('leaves POSIX-looking roots unchanged off Windows', () => {
    assert.strictEqual(
      normalizePluginRootForPlatform('/c/Users/x/.claude/plugins/scs', 'darwin'),
      '/c/Users/x/.claude/plugins/scs'
    );
    assert.strictEqual(
      normalizePluginRootForPlatform('/c/Users/x/.claude/plugins/scs', 'linux'),
      '/c/Users/x/.claude/plugins/scs'
    );
  })) passed++; else failed++;
  ...
  if (test('does not mangle UNC or non-drive absolute paths on Windows', () => {
    assert.strictEqual(
      normalizePluginRootForPlatform('\\\\server\\share\\scs', 'win32'),
      '\\\\server\\share\\scs'
    );
    assert.strictEqual(
      normalizePluginRootForPlatform('/workspace/scs', 'win32'),
      '/workspace/scs'
    );
  })) passed++; else failed++;
...
process.stdout.write(JSON.stringify({
  ...
  scsRoot: process.env.SCS_PLUGIN_ROOT,
}));
`);
      ...
      assert.strictEqual(parsed.scsRoot, root);
    } finally {
      cleanup(root);
    }
  })) passed++; else failed++;
