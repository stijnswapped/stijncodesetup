...
if (
  test('oversized payload never echoes truncated stdin for a disabled hook', () => {
    const result = runRunner(['pre:write:doc-file-warning', 'scripts/hooks/doc-file-warning.js', 'standard,strict'], oversizedPayload(), { SCS_DISABLED_HOOKS: 'pre:write:doc-file-warning' });
    assert.strictEqual(result.status, 0);
...
if (
  test('disabled-hook passthrough of a >64KB payload stays valid JSON', () => {
    ...
    const result = runRunner(['pre:write:doc-file-warning', 'scripts/hooks/doc-file-warning.js', 'standard,strict'], payload, { SCS_DISABLED_HOOKS: 'pre:write:doc-file-warning' });
    assert.strictEqual(result.status, 0);
