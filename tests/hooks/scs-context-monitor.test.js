/**
 * Tests for scripts/hooks/scs-context-monitor.js
 *
 * Run with: node tests/hooks/scs-context-monitor.test.js
 */
...
const { run, evaluateConditions, detectLoop, severityLabel, costWarningsEnabled } = require('../../scripts/hooks/scs-context-monitor');
...
function runTests() {
  console.log('\n=== Testing scs-context-monitor.js ===\n');
  ...
  if (
    test('SCS_CONTEXT_MONITOR_COST_WARNINGS=off disables only run-time cost warnings', () => {
      const sessionId = `ctx-monitor-cost-off-${process.pid}-${Date.now()}`;
      const input = JSON.stringify({ session_id: sessionId, tool_name: 'Bash' });
      const warnPath = path.join(os.tmpdir(), `scs-ctx-warn-${sessionId}.json`);
      try {
        ...
        const result = withEnv('SCS_CONTEXT_MONITOR_COST_WARNINGS', 'off', () => JSON.parse(run(input)));
  ...
  if (
    test('cost warning env defaults on and accepts false-like values', () => {
      assert.strictEqual(withEnv('SCS_CONTEXT_MONITOR_COST_WARNINGS', undefined, () => costWarningsEnabled()), true);
      assert.strictEqual(withEnv('SCS_CONTEXT_MONITOR_COST_WARNINGS', 'false', () => costWarningsEnabled()), false);
      assert.strictEqual(withEnv('SCS_CONTEXT_MONITOR_COST_WARNINGS', '0', () => costWarningsEnabled()), false);
      assert.strictEqual(withEnv('SCS_CONTEXT_MONITOR_COST_WARNINGS', 'yes', () => costWarningsEnabled()), true);
    })
  )
    passed++;
  else failed++;
