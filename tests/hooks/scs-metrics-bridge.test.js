/**
 * Tests for scripts/hooks/scs-metrics-bridge.js
 *
 * Run with: node tests/hooks/scs-metrics-bridge.test.js
 */
...
const { run, hashToolCall, extractFilePaths, readSessionCost } = require('../../scripts/hooks/scs-metrics-bridge');
...
function makeTempHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'scs-metrics-bridge-test-'));
}

function runTests() {
  console.log('\n=== Testing scs-metrics-bridge.js ===\n');
  ...
  if (
    test('readSessionCost suppresses repeated malformed breadcrumbs across hook subprocesses', () => {
      ...
      try {
        ...
        const bridgePath = path.resolve(__dirname, '../../scripts/hooks/scs-metrics-bridge');
