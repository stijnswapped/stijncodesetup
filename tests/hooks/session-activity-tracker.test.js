...
function runTests() {
  ...
  (test('creates tool activity metrics rows with file paths', () => {
    ...
    const result = runScript(input, {
      ...
      CLAUDE_HOOK_EVENT_NAME: 'PostToolUse',
      SCS_SESSION_ID: 'scs-session-1234',
    });
    assert.strictEqual(result.code, 0);
    ...
    assert.strictEqual(row.session_id, 'scs-session-1234');
  ...
  (test('captures typed move file events from source/destination inputs', () => {
    ...
    const result = runScript(input, {
      ...withTempHome(tmpHome),
      CLAUDE_HOOK_EVENT_NAME: 'PostToolUse',
      SCS_SESSION_ID: 'scs-session-5678',
    });
    assert.strictEqual(result.code, 0);
  ...
  (test('captures replacement diff previews for edit tool input', () => {
    ...
    const result = runScript(input, {
      ...withTempHome(tmpHome),
      CLAUDE_HOOK_EVENT_NAME: 'PostToolUse',
      SCS_SESSION_ID: 'scs-session-edit',
    });
    assert.strictEqual(result.code, 0);
  ...
  (test('captures MultiEdit nested edits with typed diff previews', () => {
    ...
    const result = runScript(input, {
      ...withTempHome(tmpHome),
      CLAUDE_HOOK_EVENT_NAME: 'PostToolUse',
      SCS_SESSION_ID: 'scs-session-multiedit',
    });
    assert.strictEqual(result.code, 0);
  ...
  (test('reclassifies tracked Write activity as modify using git diff context', () => {
    ...
    spawnSync('git', ['config', 'user.email', 'scs@example.com'], { cwd: repoDir, encoding: 'utf8' });
    spawnSync('git', ['config', 'user.name', 'SCS Tests'], { cwd: repoDir, encoding: 'utf8' });
    ...
    const result = runScript(input, {
      ...withTempHome(tmpHome),
      CLAUDE_HOOK_EVENT_NAME: 'PostToolUse',
      SCS_SESSION_ID: 'scs-session-write-modify',
    }, {
      cwd: repoDir,
    });
    assert.strictEqual(result.code, 0);
  ...
  (test('captures tracked Delete activity using git diff context', () => {
    ...
    const result = runScript(input, {
      ...withTempHome(tmpHome),
      CLAUDE_HOOK_EVENT_NAME: 'PostToolUse',
      SCS_SESSION_ID: 'scs-session-delete',
    }, {
      cwd: repoDir,
    });
    assert.strictEqual(result.code, 0);
  ...
  (test('resolves repo-relative paths even when the hook runs from a nested cwd', () => {
    ...
    const result = runScript(input, {
      ...withTempHome(tmpHome),
      CLAUDE_HOOK_EVENT_NAME: 'PostToolUse',
      SCS_SESSION_ID: 'scs-session-nested-cwd',
    }, {
      cwd: nestedCwd,
    });
    assert.strictEqual(result.code, 0);
  ...
  (test('prefers SCS_SESSION_ID over CLAUDE_SESSION_ID and redacts bash summaries', () => {
    const tmpHome = makeTempDir();
    const input = {
      tool_name: 'Bash',
      tool_input: {
    ...
    const metricsFile = path.join(tmpHome, '.claude', 'metrics', 'tool-usage.jsonl');
    const row = JSON.parse(fs.readFileSync(metricsFile, 'utf8').trim());
    assert.strictEqual(row.session_id, 'scs-session-1');
  ...
  (test('skips non-PostToolUse events and rows without required identifiers', () => {
    assert.strictEqual(buildActivityRow(
      ...
      { CLAUDE_HOOK_EVENT_NAME: 'PreToolUse', SCS_SESSION_ID: 'sess' }
    ), null);
    assert.strictEqual(buildActivityRow(
      { tool_name: 'Read', tool_input: { file_path: 'README.md' } },
      { CLAUDE_HOOK_EVENT_NAME: 'PostToolUse' }
    ...
    assert.strictEqual(buildActivityRow(
      { tool_input: { file_path: 'README.md' } },
      { CLAUDE_HOOK_EVENT_NAME: 'PostToolUse', SCS_SESSION_ID: 'sess' }
    ), null);
  }) ? passed++ : failed++);
  ...
  (test('records creation previews unchanged when running outside a git repository', () => {
    ...
    const result = runScript(input, {
      ...withTempHome(tmpHome),
      CLAUDE_HOOK_EVENT_NAME: 'PostToolUse',
      SCS_SESSION_ID: 'scs-session-non-git-create',
    }, {
      cwd: tmpCwd,
    });
  ...
  (test('preserves absolute paths outside the repo without git enrichment', () => {
    ...
    const result = runScript(input, {
      ...withTempHome(tmpHome),
      CLAUDE_HOOK_EVENT_NAME: 'PostToolUse',
      SCS_SESSION_ID: 'scs-session-absolute-outside',
    });
