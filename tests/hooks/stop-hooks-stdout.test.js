 ...
 *
 * Stop hooks follow the SCS pass-through convention (echo stdin on stdout).
...
const workDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-stop-stdout-')); // non-git cwd
const dataHome = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-stop-data-'));
...
function hookEnv() {
  const env = {
    ...
    SCS_AGENT_DATA_HOME: dataHome,
    CLAUDE_SESSION_ID: `stop-stdout-test-${process.pid}`
  };
  delete env.SCS_GATEGUARD;
  delete env.SCS_DISABLED_HOOKS;
  return env;
}
