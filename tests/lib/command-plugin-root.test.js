...
const { INLINE_RESOLVE } = require('../../scripts/lib/resolve-scs-root');
...
test('sessions command uses shared inline resolver in all node scripts', () => {
  ...
  assert.strictEqual((sessionsDoc.match(/scripts','lib','resolve-scs-root/g) || []).length, 6);
});
...
test('skill-health command uses shared inline resolver in all shell snippets', () => {
  ...
  assert.strictEqual((skillHealthDoc.match(/scripts','lib','resolve-scs-root/g) || []).length, 3);
});
...
test('instinct-status command uses shared inline resolver (no stale legacy fallback) (#2037)', () => {
  ...
  assert.strictEqual((instinctStatusDoc.match(/scripts','lib','resolve-scs-root/g) || []).length, 1);
...
test('resolveScsRoot module covers current and legacy marketplace plugin roots', () => {
  const { resolveScsRoot } = require('../../scripts/lib/resolve-scs-root');
  assert.ok(typeof resolveScsRoot === 'function');

  const legacyHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-marketplace-legacy-'));
  try {
    const legacyRoot = path.join(legacyHomeDir, '.claude', 'plugins', 'marketplaces', 'scs');
    ...
    assert.strictEqual(resolveScsRoot({ envRoot: '', homeDir: legacyHomeDir }), legacyRoot);
  } finally {
    fs.rmSync(legacyHomeDir, { recursive: true, force: true });
  }

  const cacheHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-marketplace-cache-'));
  try {
    const cacheRoot = path.join(cacheHomeDir, '.claude', 'plugins', 'cache', 'scs', 'StijnCodeSetup', '1.0.0');
    ...
    assert.strictEqual(resolveScsRoot({ envRoot: '', homeDir: cacheHomeDir }), cacheRoot);
  } finally {
  ...
  assert.ok(INLINE_RESOLVE.includes("scripts','lib','resolve-scs-root"));
});
