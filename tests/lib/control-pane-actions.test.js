/**
 * Tests for allowlisted SCS2 control-pane actions.
 */
...
function runTests() {
  ...
  if (test('builds copyable and executable allowlisted SCS2 actions', () => {
    ...
    assert.strictEqual(sync.cwd, path.join(repoRoot, 'scs2'));
  ...
  if (test('rejects unknown action identifiers', () => {
    assert.throws(
      () => buildControlPaneAction('rm -rf', { repoRoot: '/repo/scs' }),
