'use strict';
/**
 * Tests for the proximity message sink (scs-tui messages send) and the
 * deduping dispatcher.
 */

const assert = require('assert');

const { KIND_BY_TYPE, buildSendArgs, createScsMessageSink, resolveScsBin } = require('../../scripts/lib/control-pane/message-sink');
...
test('createScsMessageSink: delivers via the injected runner with the resolved binary', () => {
  const calls = [];
  const send = createScsMessageSink({
    binPath: '/fake/scs-tui',
    runCommand: (bin, args) => calls.push({ bin, args })
  });
  send({ fromSession: 'a', toSession: 'b', content: 'hello', msgType: 'proximity_transmit' });
  assert.strictEqual(calls.length, 1);
  assert.strictEqual(calls[0].bin, '/fake/scs-tui');
...
test('createScsMessageSink: a failing command propagates (dispatcher will count it skipped)', () => {
  const send = createScsMessageSink({
    binPath: 'scs-tui',
    runCommand: () => {
...
test('resolveScsBin: honors explicit override', () => {
  assert.strictEqual(resolveScsBin({ binPath: '/x/scs-tui' }), '/x/scs-tui');
});
