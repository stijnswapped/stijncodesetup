/**
 * Tests for the local SCS2 control-pane state projection.
 */
...
async function writeSampleScs2Database(dbPath) {
  ...
  insertSession.run([
    'lead-hermes',
    'Coordinate Hermes desktop and SCS release work',
    'SCS',
    '2.0-control-pane',
    'claude',
    'claude',
    ...
    'running',
    4242,
    '/tmp/scs-worktrees/hermes',
    'scs/hermes-control-pane',
    'main',
    1200,
    800,
    2000,
  ...
  insertEntity.run([
    ...
    '/notes/hermes.md',
    'How Affaan routes Hermes Desktop, Zellij panes, Devin-style delegation, and SCS release control work.',
  ...
  db.run('INSERT INTO context_graph_observations (session_id, entity_id, observation_type, priority, pinned, summary, details_json, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [
    ...
    3,
    1,
    'Hermes Desktop and SCS should share recall before dispatching work.',
...
async function writeSampleWorkItemsDatabase(dbPath) {
  ...
  insertWorkItem.run([
    ...
    'github-pr',
    '2131',
    'Merge SCS control pane',
    'done',
    'normal',
    'https://github.com/StijnCodeSetup/StijnCodeSetup/pull/2131',
    'stijn',
    '/repo/scs',
    null,
    JSON.stringify({
      branch: 'product/scs2-knowledge-control-pane',
  ...
  insertWorkItem.run([
    ...
    'open',
    'high',
    'https://github.com/StijnCodeSetup/StijnCodeSetup/issues/2290',
    null,
    '/repo/scs',
    null,
    JSON.stringify({}),
...
async function runTests() {
  ...
  if (
    await test('builds an operator snapshot from SCS2 SQLite and configured connectors', async () => {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-control-pane-state-'));
      const dbPath = path.join(tempDir, 'scs2.db');

      try {
        await writeSampleScs2Database(dbPath);
        ...
        assert.strictEqual(snapshot.schemaVersion, 'scs.control-pane.snapshot.v1');
  ...
  if (
    await test('projects state-store work items into agent Kanban summary', async () => {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-control-pane-work-items-'));
      const dbPath = path.join(tempDir, 'scs2.db');
  ...
  if (
    await test('treats an unreadable optional state-store database as empty work items', async () => {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-control-pane-corrupt-work-items-'));
      const dbPath = path.join(tempDir, 'scs2.db');
  ...
  if (
    await test('resolves config from explicit db path and TOML connector file', async () => {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-control-pane-config-'));
      const dbPath = path.join(tempDir, 'state.db');
      const configPath = path.join(tempDir, 'scs2.toml');
  ...
  if (
    await test('prefers the operator home config over stale app-support config', async () => {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-control-pane-precedence-'));
      const homeDir = path.join(tempDir, 'home');
      const homeConfigDir = path.join(homeDir, '.claude');
      const appConfigDir = path.join(homeDir, 'Library', 'Application Support', 'scs2');
      ...
      try {
        ...
        fs.writeFileSync(path.join(homeConfigDir, 'scs2.toml'), `db_path = "${homeDbPath.replace(/\\/g, '\\\\')}"\n`, 'utf8');
  ...
  if (
    await test('shows configured connectors even when the SQLite database is missing', async () => {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-control-pane-missing-db-'));
  ...
  if (
    await test('handles an existing SQLite database before SCS2 tables are created', async () => {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-control-pane-empty-db-'));
  ...
  if (
    await test('handles malformed JSON rows and all session state counters', async () => {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-control-pane-edge-db-'));
      const dbPath = path.join(tempDir, 'scs2.db');
      ...
      const observations = [
        {
          ...
          priority: 1,
          pinned: false,
          summary: 'Ito should expose market backtesting through SCS tools.'
        }
      ];
