...
function runTests() {
  ...
  if (test('returns false when SCS_DRY_RUN is unset', () => {
    const env = { ...process.env };
    delete env.SCS_DRY_RUN;
  ...
  if (test('returns true when SCS_DRY_RUN=1', () => {
    const env = { ...process.env, SCS_DRY_RUN: '1' };
    const result = spawnSync(process.execPath, [
      '-e',
      'const { isDryRun } = require("./scripts/lib/hook-flags"); process.exit(isDryRun() ? 1 : 0)',
    ], { cwd: path.resolve(__dirname, '..', '..'), env });
  ...
  if (test('returns false when SCS_DRY_RUN=0', () => {
    const env = { ...process.env, SCS_DRY_RUN: '0' };
    const result = spawnSync(process.execPath, [
      '-e',
      'const { isDryRun } = require("./scripts/lib/hook-flags"); process.exit(isDryRun() ? 1 : 0)',
    ], { cwd: path.resolve(__dirname, '..', '..'), env });
  ...
  if (test('skips hook execution and logs preview when SCS_DRY_RUN=1', () => {
  ...
  if (test('executes hook normally when SCS_DRY_RUN is not set', () => {
    const runWithFlags = path.resolve(__dirname, '..', '..', 'scripts', 'hooks', 'run-with-flags.js');
    const hookScript = 'scripts/hooks/doc-file-warning.js';
  ...
  console.log('\nscs.js --dry-run flag parsing:');

  if (test('--dry-run sets SCS_DRY_RUN env var for child commands', () => {
    const scsJs = path.resolve(__dirname, '..', '..', 'scripts', 'scs.js');
    const result = spawnSync(process.execPath, [scsJs, '--dry-run', '--help'], {
      encoding: 'utf8',
      env: { ...process.env },
    });
    assert.strictEqual(result.status, 0);
  ...
  if (test('--dry-run is stripped from args so command routing works', () => {
    const scsJs = path.resolve(__dirname, '..', '..', 'scripts', 'scs.js');
    const result = spawnSync(process.execPath, [scsJs, '--dry-run', 'doctor'], {
      encoding: 'utf8',
      env: { ...process.env },
    });
    assert.ok(
  ...
  if (test('--dry-run works with implicit install routing', () => {
    const scsJs = path.resolve(__dirname, '..', '..', 'scripts', 'scs.js');
    const result = spawnSync(process.execPath, [scsJs, '--dry-run', '--json', 'typescript'], {
      encoding: 'utf8',
      env: { ...process.env },
    });
