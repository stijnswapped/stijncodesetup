...
function runTests() {
  ...
  if (test('resolves relative config paths from the provided cwd', () => {
    ...
    const resolved = resolveInstallConfigPath('configs/scs-install.json', { cwd });
    assert.strictEqual(resolved, path.join(cwd, 'configs', 'scs-install.json'));
  })) passed++; else failed++;
  ...
  if (test('finds the default project install config in the provided cwd', () => {
    ...
    try {
      const configPath = path.join(cwd, 'scs-install.json');
  ...
  if (test('loads and normalizes a valid install config', () => {
    ...
    try {
      ...
      const config = loadInstallConfig('scs-install.json', { cwd });
  ...
  if (test('rejects invalid config schema values', () => {
    ...
    try {
      writeJson(path.join(cwd, 'scs-install.json'), {
        version: 2,
        target: 'ghost-target',
      });

      assert.throws(
        () => loadInstallConfig('scs-install.json', { cwd }),
