...
function writeManifestSourceFixture(root) {
    ...
    modules: [
      {
        ...
        paths: [
          ...
          'missing.txt',
          'skills/demo',
          path.join('runtime', 'scs', 'install-state.json'),
          '.claude-plugin',
        ],
        targets: ['claude'],
        dependencies: [],
  ...
  writeFile(root, path.join('src', 'nested', 'scs-install-state.json'), '{}\n');
  writeFile(root, path.join('rules', 'common', 'coding-style.md'), '# Common\n');
  writeFile(root, path.join('skills', 'demo', 'SKILL.md'), '# Demo\n');
  writeFile(root, 'standalone.txt', 'standalone\n');
  writeFile(root, path.join('runtime', 'scs', 'install-state.json'), '{}\n');
...
function runTests() {
  ...
  if (test('plans Claude legacy rules under the default SCS-managed rules directory', () => {
    const sourceRoot = createTempDir('install-executor-source-');
    const homeDir = createTempDir('install-executor-home-');
    const projectRoot = createTempDir('install-executor-project-');
    try {
      ...
      const managedRulesDir = path.join(homeDir, '.claude', 'rules', 'scs');
      assert.strictEqual(plan.installRoot, managedRulesDir);
      assert.ok(operationFor(plan, path.join('.claude', 'rules', 'scs', 'common', 'coding-style.md')));
      assert.ok(operationFor(plan, path.join('.claude', 'rules', 'scs', 'typescript', 'testing.md')));
  ...
  if (test('plans Cursor legacy assets and JSON merge payloads', () => {
    ...
    try {
      ...
      assert.ok(operationFor(plan, path.join('.cursor', 'agents', 'scs-planner.md')));
  ...
  if (test('materializes manifest scaffold operations and filters generated runtime state', () => {
    ...
    try {
      ...
      assert.ok(!normalizedSources.includes('runtime/scs/install-state.json'));
      assert.ok(!normalizedSources.includes('src/nested/scs-install-state.json'));
      ...
      assert.ok(plan.operations.some(operation => (
        ...
        && operation.destinationPath === path.join(homeDir, '.claude', 'rules', 'scs', 'common', 'coding-style.md')
      )));
      assert.ok(plan.operations.some(operation => (
        operation.sourceRelativePath === path.join('skills', 'demo', 'SKILL.md')
        && operation.destinationPath === path.join(homeDir, '.claude', 'skills', 'scs', 'demo', 'SKILL.md')
      )));
  ...
  if (test('applyInstallPlan re-export applies a manifest plan and writes install state', () => {
    ...
    try {
      ...
      assert.ok(fs.existsSync(path.join(homeDir, '.claude', 'rules', 'scs', 'common', 'coding-style.md')));
      assert.ok(fs.existsSync(path.join(homeDir, '.claude', 'skills', 'scs', 'demo', 'SKILL.md')));
      ...
      const state = JSON.parse(fs.readFileSync(path.join(homeDir, '.claude', 'scs', 'install-state.json'), 'utf8'));
