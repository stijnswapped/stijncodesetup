...
function createCursorStateOptions(projectRoot, overrides = {}) {
  ...
  const installStatePath = overrides.installStatePath || path.join(targetRoot, 'scs-install-state.json');
...
function runTests() {
  ...
  if (test('discovers installed states for multiple targets in the current context', () => {
    ...
    try {
      const claudeStatePath = path.join(homeDir, '.claude', 'scs', 'install-state.json');
      const cursorStatePath = path.join(projectRoot, '.cursor', 'scs-install-state.json');
  ...
  if (test('discovers missing and invalid install-state records', () => {
    ...
    try {
      ...
      const statePath = path.join(targetRoot, 'scs-install-state.json');
  ...
  if (test('doctor reports target mismatches, missing sources, unverified operations, and version drift', () => {
    ...
    try {
      ...
      const actualStatePath = path.join(actualTargetRoot, 'scs-install-state.json');
  ...
  if (test('doctor reports invalid install-state files as errors', () => {
    ...
    try {
      const statePath = path.join(projectRoot, '.cursor', 'scs-install-state.json');
  ...
  if (test('repair reports invalid states, missing sources, unsupported operations, and no-op refreshes', () => {
    ...
    try {
      const invalidStatePath = path.join(invalidProjectRoot, '.cursor', 'scs-install-state.json');
