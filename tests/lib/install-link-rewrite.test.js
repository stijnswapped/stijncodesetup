...
// A claude-style namespace placement: skills/<id> -> skills/scs/<id> and
// rules/<x> -> rules/scs/<x>. Mirrors what the real adapter emits.
function claudeNamespaceMappings() {
  return [
    { sourceRel: 'skills/react-patterns/SKILL.md', destRel: 'skills/scs/react-patterns/SKILL.md' },
    { sourceRel: 'skills/react-patterns/other.md', destRel: 'skills/scs/react-patterns/other.md' },
    { sourceRel: 'skills/react-patterns/sub/NOTE.md', destRel: 'skills/scs/react-patterns/sub/NOTE.md' },
    { sourceRel: 'rules/react/hooks.md', destRel: 'rules/scs/react/hooks.md' },
    { sourceRel: 'rules/react/testing.md', destRel: 'rules/scs/react/testing.md' },
    { sourceRel: 'rules/react/coding-style.md', destRel: 'rules/scs/react/coding-style.md' },
  ];
}
...
function runTests() {
  ...
  for (const skill of ['react-patterns', 'react-performance', 'react-testing']) {
    if (test(`rewrites ../../rules file link for ${skill}`, () => {
      const idx = buildInstallIndex([
        { sourceRel: `skills/${skill}/SKILL.md`, destRel: `skills/scs/${skill}/SKILL.md` },
        { sourceRel: 'rules/react/hooks.md', destRel: 'rules/scs/react/hooks.md' },
      ]);
      ...
      assert.ok(
        after.includes('](../../../rules/scs/react/hooks.md)'),
  ...
  if (test('rewrites a directory link and preserves the trailing slash', () => {
    ...
    assert.ok(after.includes('](../../../rules/scs/react/)'), `got: ${after}`);
  })) passed++; else failed++;
  ...
  if (test('preserves a #fragment on a rewritten link', () => {
    ...
    const after = rewriteRelativeLinks(before, { sourceRel: 'skills/react-patterns/SKILL.md', index });
    assert.ok(after.includes('](../../../rules/scs/react/hooks.md#use-effect)'), `got: ${after}`);
  })) passed++; else failed++;
  ...
  if (test('does not rewrite links inside fenced code blocks', () => {
    ...
    assert.ok(after.includes('[prose](../../../rules/scs/react/hooks.md)'), 'prose link must be rewritten');
  })) passed++; else failed++;

  if (test('computes depth from path math for a nested skill file', () => {
    // skills/react-patterns/sub/NOTE.md -> skills/scs/react-patterns/sub/NOTE.md
    ...
    assert.ok(after.includes('](../../../../rules/scs/react/hooks.md)'), `got: ${after}`);
  })) passed++; else failed++;
  ...
  if (test('real React skills: rewritten rules links resolve to installed targets', () => {
    ...
    const plan = createManifestInstallPlan({
      ...
      homeDir: '/tmp/scs-link-rewrite-it',
      target: 'claude',
    ...
    for (const skill of ['react-patterns', 'react-performance', 'react-testing']) {
      ...
      const installedSkillDir = path.posix.dirname(`skills/scs/${skill}/SKILL.md`);
