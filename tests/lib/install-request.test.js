...
function runTests() {
  ...
  if (test('parses manifest-mode CLI arguments', () => {
    ...
    assert.strictEqual(parsed.configPath, 'scs-install.json');
  ...
  if (test('merges config-backed component selections with CLI overrides', () => {
    const request = normalizeInstallRequest({
      ...
      excludeComponentIds: ['capability:media'],
      languages: [],
      configPath: '/workspace/app/scs-install.json',
      config: {
        path: '/workspace/app/scs-install.json',
        target: 'claude',
        profileId: 'developer',
    ...
    assert.strictEqual(request.configPath, '/workspace/app/scs-install.json');
  })) passed++; else failed++;
