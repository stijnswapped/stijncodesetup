...
function runTests() {
  ...
  if (test('creates a valid install-state payload', () => {
    const state = createInstallState({
      ...
      installStatePath: '/repo/.cursor/scs-install-state.json',
      request: {
        profile: 'developer',
    ...
    assert.strictEqual(state.schemaVersion, 'scs.install.v1');
  ...
  if (test('writes and reads install-state from disk', () => {
    ...
    const statePath = path.join(testDir, 'scs-install-state.json');
  ...
  if (test('rejects invalid install-state payloads on read', () => {
    ...
    try {
      fs.writeFileSync(statePath, JSON.stringify({ schemaVersion: 'scs.install.v1' }, null, 2));
      assert.throws(
