...
function runTests() {
  ...
  if (test('resolves cursor adapter root and install-state path from project root', () => {
    ...
    assert.strictEqual(statePath, path.join(projectRoot, '.cursor', 'scs-install-state.json'));
  })) passed++; else failed++;
  ...
  if (test('resolves claude adapter root and install-state path from home dir', () => {
    ...
    const root = adapter.resolveRoot({ homeDir, repoRoot: '/repo/scs' });
    const statePath = adapter.getInstallStatePath({ homeDir, repoRoot: '/repo/scs' });

    assert.strictEqual(root, path.join(homeDir, '.claude'));
    assert.strictEqual(statePath, path.join(homeDir, '.claude', 'scs', 'install-state.json'));
  })) passed++; else failed++;

  if (test('plans claude rules and skills under SCS-managed subdirectories', () => {
    ...
    assert.ok(
      plan.operations.some(operation => (
        ...
        && operation.destinationPath === path.join(homeDir, '.claude', 'rules', 'scs')
      )),
      'Should install bundled Claude rules under rules/scs'
    );
    assert.ok(
      plan.operations.some(operation => (
        normalizedRelativePath(operation.sourceRelativePath) === 'skills/tdd-workflow'
        && operation.destinationPath === path.join(homeDir, '.claude', 'skills', 'scs', 'tdd-workflow')
      )),
      'Should install bundled Claude skills under skills/scs'
    );
  })) passed++; else failed++;
  ...
  if (test('plans scaffold operations and flattens native target roots', () => {
    ...
    assert.strictEqual(plan.installStatePath, path.join(projectRoot, '.cursor', 'scs-install-state.json'));
  ...
  if (test('does not install root AGENTS.md into Cursor nested context', () => {
    ...
    assert.ok(
      ...
      'Cursor installs should not copy SCS root AGENTS.md into host project context'
    );
    assert.ok(
      !plan.operations.some(operation => (
  ...
  if (test('plans cursor agents with scs-prefixed filenames to avoid agent collisions', () => {
    const repoRoot = path.join(__dirname, '..', '..');
    const projectRoot = '/workspace/app';
    ...
    assert.ok(
      plan.operations.some(operation => (
        ...
        && operation.destinationPath === path.join(projectRoot, '.cursor', 'agents', 'scs-architect.md')
      )),
      'Should prefix Cursor agent files with scs-'
    );
    assert.ok(
      !plan.operations.some(operation => (
  ...
  if (test('exposes validate and planOperations on adapters', () => {
    ...
    assert.deepStrictEqual(
      claudeAdapter.validate({ homeDir: '/Users/example', repoRoot: '/repo/scs' }),
      []
    );
    ...
    assert.deepStrictEqual(
      cursorAdapter.validate({ projectRoot: '/workspace/app', repoRoot: '/repo/scs' }),
      []
    );
  })) passed++; else failed++;
  ...
  if (test('resolves codebuddy adapter root and install-state path from project root', () => {
    ...
    assert.strictEqual(statePath, path.join(projectRoot, '.codebuddy', 'scs-install-state.json'));
  })) passed++; else failed++;
  ...
  if (test('resolves gemini adapter root and install-state path from project root', () => {
    ...
    assert.strictEqual(statePath, path.join(projectRoot, '.gemini', 'scs-install-state.json'));
  })) passed++; else failed++;
  ...
  if (test('resolves joycode adapter root and install-state path from project root', () => {
    ...
    assert.strictEqual(statePath, path.join(projectRoot, '.joycode', 'scs-install-state.json'));
  })) passed++; else failed++;
  ...
  if (test('resolves qwen adapter root and install-state path from home dir', () => {
    ...
    assert.strictEqual(statePath, path.join(homeDir, '.qwen', 'scs-install-state.json'));
  })) passed++; else failed++;
  ...
  if (test('resolves zed adapter root and install-state path from project root', () => {
    ...
    assert.strictEqual(statePath, path.join(projectRoot, '.zed', 'scs-install-state.json'));
  })) passed++; else failed++;
  ...
  if (test('plans codebuddy rules with flat namespaced filenames', () => {
    ...
    assert.strictEqual(plan.installStatePath, path.join(projectRoot, '.codebuddy', 'scs-install-state.json'));
  ...
  if (test('plans joycode commands, agents, skills, and flattened rules', () => {
    ...
    assert.strictEqual(plan.installStatePath, path.join(projectRoot, '.joycode', 'scs-install-state.json'));
  ...
  if (test('plans qwen commands, agents, skills, and native config under home root', () => {
    ...
    assert.strictEqual(plan.installStatePath, path.join(homeDir, '.qwen', 'scs-install-state.json'));
    assert.ok(
      plan.operations.some(operation => (
        normalizedRelativePath(operation.sourceRelativePath) === 'rules'
  ...
  if (test('plans zed project settings, commands, agents, skills, and flattened rules', () => {
    ...
    assert.strictEqual(plan.installStatePath, path.join(projectRoot, '.zed', 'scs-install-state.json'));
    assert.ok(
      plan.operations.some(operation => (
  ...
  if (test('exposes validate and planOperations on codebuddy adapter', () => {
    ...
    assert.deepStrictEqual(
      codebuddyAdapter.validate({ projectRoot: '/workspace/app', repoRoot: '/repo/scs' }),
      []
    );
  })) passed++; else failed++;

  if (test('every schema target enum value has a matching adapter (regression guard)', () => {
    const schemaPath = path.join(__dirname, '..', '..', 'schemas', 'scs-install-config.schema.json');
  ...
  if (test('resolves claude-project adapter root and install-state path from project root', () => {
    ...
    assert.strictEqual(statePath, path.join(projectRoot, '.claude', 'scs', 'install-state.json'));
  })) passed++; else failed++;
  ...
  if (test('plans claude-project rules and skills under project-scope SCS-managed subdirectories', () => {
    const repoRoot = path.join(__dirname, '..', '..');
    const projectRoot = '/workspace/app';
    ...
    assert.strictEqual(plan.installStatePath, path.join(projectRoot, '.claude', 'scs', 'install-state.json'));
    assert.ok(
      plan.operations.some(operation => (
        normalizedRelativePath(operation.sourceRelativePath) === 'rules'
        && operation.destinationPath === path.join(projectRoot, '.claude', 'rules', 'scs')
      )),
      'Should install bundled rules under project-scope rules/scs'
    );
    assert.ok(
      plan.operations.some(operation => (
        normalizedRelativePath(operation.sourceRelativePath) === 'skills/tdd-workflow'
        && operation.destinationPath === path.join(projectRoot, '.claude', 'skills', 'scs', 'tdd-workflow')
      )),
      'Should install bundled skills under project-scope skills/scs'
    );
  })) passed++; else failed++;
  ...
  if (test('resolves opencode adapter root and install-state path from home dir', () => {
    ...
    assert.strictEqual(statePath, path.join(homeDir, '.opencode', 'scs-install-state.json'));
  })) passed++; else failed++;
