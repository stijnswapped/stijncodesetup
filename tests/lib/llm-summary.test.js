...
test('returns haiku by default', () => {
  const orig = process.env.SCS_LLM_SUMMARY_MODEL;
  delete process.env.SCS_LLM_SUMMARY_MODEL;
  assert.strictEqual(getLLMModel(), 'haiku');
  if (orig !== undefined) process.env.SCS_LLM_SUMMARY_MODEL = orig;
});

test('reads SCS_LLM_SUMMARY_MODEL env var', () => {
  const orig = process.env.SCS_LLM_SUMMARY_MODEL;
  process.env.SCS_LLM_SUMMARY_MODEL = 'sonnet';
  assert.strictEqual(getLLMModel(), 'sonnet');
  if (orig !== undefined) process.env.SCS_LLM_SUMMARY_MODEL = orig;
  else delete process.env.SCS_LLM_SUMMARY_MODEL;
});
...
test('returns 20 by default', () => {
  const orig = process.env.SCS_LLM_SUMMARY_CONTEXT_THRESHOLD;
  delete process.env.SCS_LLM_SUMMARY_CONTEXT_THRESHOLD;
  assert.strictEqual(getContextThreshold(), 20);
  if (orig !== undefined) process.env.SCS_LLM_SUMMARY_CONTEXT_THRESHOLD = orig;
});

test('reads SCS_LLM_SUMMARY_CONTEXT_THRESHOLD env var', () => {
  const orig = process.env.SCS_LLM_SUMMARY_CONTEXT_THRESHOLD;
  process.env.SCS_LLM_SUMMARY_CONTEXT_THRESHOLD = '70';
  assert.strictEqual(getContextThreshold(), 70);
  if (orig !== undefined) process.env.SCS_LLM_SUMMARY_CONTEXT_THRESHOLD = orig;
  else delete process.env.SCS_LLM_SUMMARY_CONTEXT_THRESHOLD;
});

test('falls back to 20 on invalid value', () => {
  const orig = process.env.SCS_LLM_SUMMARY_CONTEXT_THRESHOLD;
  process.env.SCS_LLM_SUMMARY_CONTEXT_THRESHOLD = 'notanumber';
  assert.strictEqual(getContextThreshold(), 20);
  if (orig !== undefined) process.env.SCS_LLM_SUMMARY_CONTEXT_THRESHOLD = orig;
  else delete process.env.SCS_LLM_SUMMARY_CONTEXT_THRESHOLD;
});
...
test('falls back to 20 when value exceeds 100', () => {
  const orig = process.env.SCS_LLM_SUMMARY_CONTEXT_THRESHOLD;
  process.env.SCS_LLM_SUMMARY_CONTEXT_THRESHOLD = '150';
  assert.strictEqual(getContextThreshold(), 20);
  if (orig !== undefined) process.env.SCS_LLM_SUMMARY_CONTEXT_THRESHOLD = orig;
  else delete process.env.SCS_LLM_SUMMARY_CONTEXT_THRESHOLD;
});
...
test('returns null when SCS_SKIP_LLM_SUMMARY is set', () => {
  const orig = process.env.SCS_SKIP_LLM_SUMMARY;
  process.env.SCS_SKIP_LLM_SUMMARY = '1';
  const p = writeTranscript([userEntry('test')]);
  assert.strictEqual(generateSessionSummary(p), null);
  if (orig !== undefined) process.env.SCS_SKIP_LLM_SUMMARY = orig;
  else delete process.env.SCS_SKIP_LLM_SUMMARY;
});

test('returns null for missing transcript (no conversation to summarize)', () => {
  const orig = process.env.SCS_SKIP_LLM_SUMMARY;
  delete process.env.SCS_SKIP_LLM_SUMMARY;
