...
function withIsolatedHome(fn) {
  ...
  const originalAgentDataHome = process.env.SCS_AGENT_DATA_HOME;

  process.env.HOME = isolatedHome;
  process.env.USERPROFILE = isolatedHome;
  process.env.SCS_AGENT_DATA_HOME = path.join(isolatedHome, '.claude');
  ...
  try {
    ...
    if (originalAgentDataHome !== undefined) {
      process.env.SCS_AGENT_DATA_HOME = originalAgentDataHome;
    } else {
...
function runTests() {
  ...
  if (test('setPreferredPackageManager throws wrapped error when save fails', () => {
    ...
    const isoHome = path.join(os.tmpdir(), `scs-pm-r71-${Date.now()}`);
  ...
  if (test('setProjectPackageManager throws wrapped error when write fails', () => {
    ...
    const isoProject = path.join(os.tmpdir(), `scs-pm-proj-r72-${Date.now()}`);
