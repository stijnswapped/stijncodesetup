...
function runTests() {
  ...
  if (test('end-to-end: installs --profile core --with capability:security and writes state', () => {
    ...
    try {
      ...
      assert.ok(fs.existsSync(path.join(claudeRoot, 'skills', 'scs', 'security-review', 'SKILL.md')),
      ...
      const statePath = path.join(claudeRoot, 'scs', 'install-state.json');
  ...
  if (test('end-to-end: installs --profile developer --without capability:orchestration and state reflects exclusion', () => {
    ...
    try {
      ...
      assert.ok(!fs.existsSync(path.join(claudeRoot, 'skills', 'scs', 'dmux-workflows', 'SKILL.md')),
  ...
  if (test('end-to-end: --skills continuous-learning-v2 installs only that skill', () => {
    ...
    try {
      ...
      const claudeRoot = path.join(homeDir, '.claude');
      assert.ok(
        fs.existsSync(path.join(claudeRoot, 'skills', 'scs', 'continuous-learning-v2', 'SKILL.md')),
        'Should install continuous-learning-v2'
      );
      assert.ok(
        !fs.existsSync(path.join(claudeRoot, 'skills', 'scs', 'tdd-workflow', 'SKILL.md')),
