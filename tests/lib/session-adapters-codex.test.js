...
function writeRolloutFixture() {
  const sessionsDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-codex-sessions-'));
  const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-codex-worktree-'));
  ...
  const lines = [
    ...
    { type: 'response_item', timestamp: now, payload: {
      type: 'message', role: 'user',
      content: [{ type: 'text', text: 'continue our scs 2.0 session and build the codex-worktree adapter' }]
    } }
  ];
...
test('normalizeCodexWorktreeSession produces a valid scs.session.v1 snapshot', () => {
...
test('adapter reads latest rollout, skips preamble, derives objective + model', () => {
  ...
  const recordingDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-codex-rec-'));
  ...
  assert.strictEqual(
    ...
    'continue our scs 2.0 session and build the codex-worktree adapter'
  );
...
test('registry routes structured codex-worktree target and direct rollout path', () => {
  const { sessionsDir, repoRoot, rolloutPath } = writeRolloutFixture();
  const recordingDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-codex-reg-'));
...
test('adapter throws clear errors for missing sessions and unknown ids', () => {
  const sessionsDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-codex-empty-'));
...
test('findRolloutById + direct file target + isCodexRolloutFileTarget', () => {
  const sessionsDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-codex-byid-'));
...
test('parseCodexRollout: model fallbacks, objective truncation, corrupt-line skip, mtime fallback', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-codex-parse-'));
...
test('resolveGitBranch returns null when cwd is not a git repo (real path)', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-codex-nogit-'));
