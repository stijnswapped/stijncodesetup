...
function writeOpencodeFixture({ title = 'rebuild the basket trader rebalancer', updatedAgoMs = 0 } = {}) {
  const storageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-opencode-store-'));
  const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-opencode-repo-'));
...
test('normalizeOpencodeSession produces a valid scs.session.v1 snapshot', () => {
...
test('adapter reads latest session, extracts model from messages, derives objective from title', () => {
  ...
  const recordingDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-opencode-rec-'));
...
test('auto-title "New session - ..." yields empty objective; stale session is recorded', () => {
  ...
  const recordingDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-opencode-rec2-'));
...
test('registry routes structured opencode target and lists the adapter', () => {
  const { storageDir, repoRoot } = writeOpencodeFixture();
  const recordingDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-opencode-reg-'));
...
test('adapter throws for empty store and unknown id; findLatest on empty => null', () => {
  const storageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-oc-empty-'));
...
test('findSessionInfoById + direct file target + isOpencodeSessionFileTarget', () => {
  const storageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-oc-byid-'));
  const now = Date.now();
...
test('parseOpencodeSession: model from later assistant message, missing-time => recorded', () => {
  const storageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-oc-parse-'));
...
test('resolveGitBranch real path returns null outside a repo', () => {
  const storageDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-oc-nogit-'));
