...
test('dmux adapter normalizes orchestration snapshots into canonical form', () => {
  const recordingDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-session-recordings-'));
  ...
  try {
    ...
    assert.strictEqual(snapshot.schemaVersion, 'scs.session.v1');
...
test('claude-history adapter loads the latest recorded session', () => {
  const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-session-adapter-home-'));
  const recordingDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-session-recordings-'));
  ...
  fs.writeFileSync(sessionPath, [
    ...
    '**Started:** 09:00',
    '**Last Updated:** 11:30',
    '**Project:** stijncodesetup',
    '**Branch:** feat/session-adapter',
    '**Worktree:** /tmp/scs-worktree',
    '',
    '### Completed',
  ...
  try {
    withHome(homeDir, () => {
      ...
      assert.strictEqual(snapshot.workers[0].worktree, '/tmp/scs-worktree');
...
test('adapter registry routes plan files to dmux and explicit claude targets to history', () => {
  const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-session-registry-repo-'));
  ...
  const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-session-registry-home-'));
  const sessionsDir = path.join(homeDir, '.claude', 'sessions');
  fs.mkdirSync(sessionsDir, { recursive: true });
  fs.writeFileSync(
...
test('adapter registry resolves structured target types into the correct adapter', () => {
  const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-session-typed-repo-'));
  const planPath = path.join(repoRoot, 'workflow.json');
  fs.writeFileSync(planPath, JSON.stringify({
  ...
  const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-session-typed-home-'));
  const sessionsDir = path.join(homeDir, '.claude', 'sessions');
  fs.mkdirSync(sessionsDir, { recursive: true });
  fs.writeFileSync(
    path.join(sessionsDir, '2026-03-13-z9y8x7w6-session.tmp'),
...
test('canonical snapshot validation rejects malformed required fields', () => {
  const invalidCases = [
    ...
    [canonicalSnapshot({ schemaVersion: 'scs.session.v0' }), /Unsupported canonical session schema version/],
...
test('claude history normalization falls back to filename ids and empty metadata defaults', () => {
  ...
  assert.deepStrictEqual(pathOnly.workers[0].intent.seedPaths, ['README.md', 'scripts/scs.js']);
...
test('fallback recordings sanitize paths, use env dirs, and preserve changed history', () => {
  const recordingDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-session-recordings-env-'));
  const previousRecordingDir = process.env.SCS_SESSION_RECORDING_DIR;

  try {
    process.env.SCS_SESSION_RECORDING_DIR = recordingDir;
    ...
    assert.strictEqual(persisted.schemaVersion, 'scs.session.recording.v1');
    ...
    if (typeof previousRecordingDir === 'string') {
      process.env.SCS_SESSION_RECORDING_DIR = previousRecordingDir;
    } else {
      delete process.env.SCS_SESSION_RECORDING_DIR;
    }
    fs.rmSync(recordingDir, { recursive: true, force: true });
  }
});
...
test('persistence supports skip mode, writer variants, and missing state-store fallback', () => {
  ...
  const noWriterDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-session-no-writer-'));
  const missingModuleDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-session-missing-module-'));
  try {
