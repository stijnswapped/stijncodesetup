...
const tmpHome = path.join(os.tmpdir(), `scs-alias-test-${Date.now()}`);
fs.mkdirSync(path.join(tmpHome, '.claude'), { recursive: true });
const origHome = process.env.HOME;
const origUserProfile = process.env.USERPROFILE;
const origAgentDataHome = process.env.SCS_AGENT_DATA_HOME;
process.env.HOME = tmpHome;
process.env.USERPROFILE = tmpHome; // Windows: os.homedir() uses USERPROFILE
process.env.SCS_AGENT_DATA_HOME = path.join(tmpHome, '.claude');
...
process.once('exit', () => {
  ...
  if (origAgentDataHome !== undefined) {
    process.env.SCS_AGENT_DATA_HOME = origAgentDataHome;
  } else {
...
function withTempWriteFailure(fn) {
  const originalWriteFileSync = fs.writeFileSync;
  fs.writeFileSync = function mockedWriteFileSync(filePath, ...args) {
    if (String(filePath).endsWith('session-aliases.json.tmp')) {
      throw new Error('mock temp write failure');
    }
...
function runTests() {
  ...
  if (test('updateAliasTitle returns failure when saveAliases fails (read-only dir)', () => {
    resetAliases();
    aliases.setAlias('title-save-fail', '/path/session', 'Original Title');
    withTempWriteFailure(() => {
      const result = aliases.updateAliasTitle('title-save-fail', 'New Title');
  ...
  if (test('deleteAlias returns failure when saveAliases fails (read-only dir)', () => {
    resetAliases();
    aliases.setAlias('to-delete', '/path/session', 'Test');
    withTempWriteFailure(() => {
      const result = aliases.deleteAlias('to-delete');
      assert.strictEqual(result.success, false, 'Should fail when save is blocked');
  ...
  if (test('cleanupAliases returns failure when saveAliases fails after removing aliases', () => {
    resetAliases();
    aliases.setAlias('keep-me', '/sessions/real', 'Kept');
    aliases.setAlias('remove-me', '/sessions/gone', 'Gone');
    withTempWriteFailure(() => {
      const result = aliases.cleanupAliases((p) => p === '/sessions/real');
      assert.strictEqual(result.success, false, 'Should fail when save is blocked');
  ...
  if (test('setAlias returns failure when saveAliases fails', () => {
    resetAliases();
    withTempWriteFailure(() => {
      const result = aliases.setAlias('my-alias', '/sessions/test', 'Test');
      assert.strictEqual(result.success, false, 'Should fail when save is blocked');
  ...
  if (test('saveAliases triggers inner restoreErr catch when both save and restore fail', () => {
    resetAliases();
    aliases.setAlias('before-double-fail', '/path/session');
    const backupPath = aliases.getAliasesPath() + '.bak';
    const originalWriteFileSync = fs.writeFileSync;
    ...
    fs.copyFileSync = function mockedCopyFileSync(...args) {
      copyCalls += 1;
      if (copyCalls > 1) {
    ...
    try {
      const result = aliases.saveAliases({ aliases: { x: 1 }, version: '1.0' });
