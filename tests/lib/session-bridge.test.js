...
function runTests() {
  ...
  if (
    test('returns path containing scs-metrics-', () => {
      const p = getBridgePath('test-session');
      assert.ok(p.includes('scs-metrics-'), `Expected scs-metrics- in path, got: ${p}`);
    })
  )
    passed++;
  else failed++;
  ...
  if (
    test('resolveSessionId uses SCS_SESSION_ID env var', () => {
      const original = process.env.SCS_SESSION_ID;
      try {
        process.env.SCS_SESSION_ID = 'env-session-42';
        const result = resolveSessionId();
        assert.strictEqual(result, 'env-session-42');
      } finally {
        if (original === undefined) {
          delete process.env.SCS_SESSION_ID;
        } else {
          process.env.SCS_SESSION_ID = original;
        }
      }
    })
  )
