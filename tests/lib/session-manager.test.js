...
function createTempSessionDir() {
  const dir = path.join(os.tmpdir(), `scs-test-sessions-${Date.now()}`);
...
\`\`\``;
    ...
    assert.strictEqual(meta.project, 'stijncodesetup');
    assert.strictEqual(meta.branch, 'feature/session-metadata');
    assert.strictEqual(meta.worktree, '/tmp/scs-worktree');
...
`;
  ...
  const tmpHome = path.join(os.tmpdir(), `scs-session-mgr-test-${Date.now()}`);
  ...
  const r33Home = path.join(os.tmpdir(), `scs-r33-birthtime-${Date.now()}`);
  ...
  if (test('getAllSessions hasContent is true for non-empty and false for empty files', () => {
    const isoHome = path.join(os.tmpdir(), `scs-hascontent-${Date.now()}`);
  ...
  if (test('getAllSessions skips broken symlink .tmp files gracefully', () => {
    ...
    const isoHome = path.join(os.tmpdir(), `scs-r83-toctou-${Date.now()}`);
  ...
  if (test('getSessionById returns null when matching session is a broken symlink', () => {
    ...
    const isoHome = path.join(os.tmpdir(), `scs-r84-getbyid-toctou-${Date.now()}`);
    const sessionsDir = path.join(isoHome, '.claude', 'sessions');
    fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  if (test('getAllSessions skips subdirectories inside sessions dir', () => {
    ...
    const isoHome = path.join(os.tmpdir(), `scs-r89-subdir-skip-${Date.now()}`);
    const sessionsDir = path.join(isoHome, '.claude', 'sessions');
    fs.mkdirSync(sessionsDir, { recursive: true });
  ...
  const tmpHome2 = path.join(os.tmpdir(), `scs-session-mgr-test-2-${Date.now()}`);
...
`;
  ...
  if (test('getAllSessions coerces array/object limit via Number() with NaN fallback to 50', () => {
    const isoHome = path.join(os.tmpdir(), `scs-r106-limit-coerce-${Date.now()}`);
  ...
  if (test('getAllSessions ignores .tmp files with non-matching filenames', () => {
    const isoHome = path.join(os.tmpdir(), `scs-r109-nonsession-${Date.now()}`);
    const isoSessionsDir = path.join(isoHome, '.claude', 'sessions');
    fs.mkdirSync(isoSessionsDir, { recursive: true });
