...
test('observation layer writes and reads structured skill outcomes', () => {
  const projectRoot = makeProjectRoot('scs-skill-observe-');
  ...
  try {
    ...
    assert.strictEqual(records[0].schemaVersion, 'scs.skill-observation.v1');
    ...
    assert.strictEqual(getSkillObservationsPath({ projectRoot }), path.join(projectRoot, '.claude', 'scs', 'skills', 'observations.jsonl'));
  } finally {
    cleanup(projectRoot);
  }
});
...
test('health inspector traces recurring failures for a skill across runs', () => {
  const projectRoot = makeProjectRoot('scs-skill-health-');
...
test('amendify proposes SKILL.md patch content from failure evidence', () => {
  ...
  assert.strictEqual(proposal.schemaVersion, 'scs.skill-amendment-proposal.v1');
...
test('evaluation scaffold compares amended and baseline performance', () => {
  ...
  assert.strictEqual(evaluation.schemaVersion, 'scs.skill-evaluation.v1');
