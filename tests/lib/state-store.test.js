/**
 * Tests for the SQLite-backed SCS state store and CLI commands.
 */
...
const SCS_SCRIPT = path.join(__dirname, '..', '..', 'scripts', 'scs.js');
...
const mode = process.env.SCS_FAKE_GH_MODE || 'open';
...
if (args[0] === 'pr' && args[1] === 'list') {
  if (mode === 'empty') write([]);
  else write([
    {
      ...
      url: 'https://github.com/StijnCodeSetup/stijncodesetup/pull/3',
    ...
    },
    {
      ...
      url: 'https://github.com/StijnCodeSetup/stijncodesetup/pull/4',
  ...
  if (mode === 'empty') write([]);
  else write([
    {
      ...
      url: 'https://github.com/StijnCodeSetup/stijncodesetup/issues/9',
...
async function seedStore(dbPath) {
  ...
  store.upsertSession({
    ...
    harness: 'claude',
    state: 'active',
    repoRoot: '/tmp/scs-repo',
    startedAt: '2026-03-15T08:00:00.000Z',
    endedAt: null,
    snapshot: {
      schemaVersion: 'scs.session.v1',
      adapterId: 'dmux-tmux',
      session: {
        id: 'session-active',
        kind: 'orchestrated',
      ...
      workers: [
        {
          ...
          worktree: '/tmp/scs-repo/.worktrees/worker-1',
        },
        {
          id: 'worker-2',
          label: 'Worker 2',
          state: 'idle',
          branch: 'feat/state-store',
          worktree: '/tmp/scs-repo/.worktrees/worker-2',
        },
      ],
      aggregates: {
        workerCount: 2,
  ...
  store.insertSkillRun({
    ...
    skillVersion: '1.0.0',
    sessionId: 'session-recorded',
    taskDescription: 'Outline SCS 2.0 work',
    outcome: 'unknown',
    failureReason: null,
    tokensUsed: 300,
    durationMs: 500,
  ...
  store.insertDecision({
    ...
    rationale: 'Need queryable local state for SCS control plane',
...
async function runTests() {
  ...
  if (await test('creates the default state.db path and applies migrations idempotently', async () => {
    const homeDir = createTempDir('scs-state-home-');

    try {
      const expectedPath = path.join(homeDir, '.claude', 'scs', 'state.db');
  ...
  if (await test('preserves SQLite special database names like :memory:', async () => {
    const tempDir = createTempDir('scs-state-memory-');
  ...
  if (await test('stores sessions and returns detailed session views with workers, skill runs, and decisions', async () => {
    const testDir = createTempDir('scs-state-db-');
  ...
  if (await test('builds an empty status snapshot with null rates and missing install health', async () => {
    const testDir = createTempDir('scs-state-empty-');
    const dbPath = path.join(testDir, 'state.db');
  ...
  if (await test('tracks linked work items for Linear, GitHub, and handoff progress', async () => {
    const testDir = createTempDir('scs-state-work-items-');
    const dbPath = path.join(testDir, 'state.db');
    ...
    try {
      ...
      const linearItem = store.upsertWorkItem({
        id: 'linear-scs-20-control-plane',
        source: 'linear',
        sourceId: 'SCS-20',
        title: 'Define harness-neutral session/worktree contract',
        status: 'in-progress',
        priority: 'high',
        url: 'https://linear.app/scstools/issue/SCS-20',
        owner: 'control-plane',
        repoRoot: '/tmp/scs-repo',
        sessionId: 'session-active',
        metadata: {
          project: 'SCS 2.0: Control Plane',
        },
      ...
      store.upsertWorkItem({
        ...
        source: 'handoff',
        sourceId: 'scs-rc1-release-decision-20260511.md',
      ...
      store.upsertWorkItem({
        ...
        status: 'merged',
        priority: 'normal',
        url: 'https://github.com/StijnCodeSetup/stijncodesetup/pull/1738',
        owner: 'maintainer',
      ...
      assert.strictEqual(linearItem.id, 'linear-scs-20-control-plane');
      assert.strictEqual(linearItem.metadata.project, 'SCS 2.0: Control Plane');
  ...
  if (await test('normalizes default optional fields and reports warning install health', async () => {
    const testDir = createTempDir('scs-state-defaults-');
    const dbPath = path.join(testDir, 'state.db');
  ...
  if (await test('rejects invalid limits and unserializable JSON payloads', async () => {
    const testDir = createTempDir('scs-state-errors-');
    const dbPath = path.join(testDir, 'state.db');
  ...
  if (await test('status CLI supports human-readable and --json output', async () => {
    const testDir = createTempDir('scs-state-cli-');
    const dbPath = path.join(testDir, 'state.db');
  ...
  if (await test('status CLI --exit-code reports attention without suppressing output', async () => {
    const attentionDir = createTempDir('scs-state-attention-');
    const okDir = createTempDir('scs-state-ok-');
  ...
  if (await test('status CLI can emit and write markdown operator snapshots', async () => {
    ...
    try {
      ...
      assert.match(written, /^# SCS Status/m);
  ...
  if (await test('work-items CLI supports upsert, list, show, and close', async () => {
    const testDir = createTempDir('scs-work-items-cli-');
    const dbPath = path.join(testDir, 'state.db');
    ...
    try {
      const upsertResult = runNode(WORK_ITEMS_SCRIPT, [
        ...
        'high',
        '--url',
        'https://linear.app/example/issue/SCS-99',
        '--owner',
        'control-plane',
        '--metadata-json',
        '{"project":"SCS 2.0"}',
      ...
      assert.strictEqual(listPayload.items[0].id, 'linear-scs-99');

      const showResult = runNode(WORK_ITEMS_SCRIPT, ['show', 'linear-scs-99', '--db', dbPath]);
      assert.strictEqual(showResult.status, 0, showResult.stderr);
      assert.match(showResult.stdout, /linear\/#SCS-99 in-progress: Ship work item CLI/);

      const closeResult = runNode(WORK_ITEMS_SCRIPT, ['close', 'linear-scs-99', '--db', dbPath, '--json']);
  ...
  if (await test('work-items CLI syncs GitHub PRs and issues into readiness', async () => {
    const testDir = createTempDir('scs-work-items-github-');
    const dbPath = path.join(testDir, 'state.db');
    const binDir = path.join(testDir, 'bin');
    const repo = 'StijnCodeSetup/stijncodesetup';

    try {
      const env = {
        SCS_GH_SHIM: createGhShim(binDir),
      };
      ...
      assert.strictEqual(syncPayload.items[0].id, 'github-stijncodesetup-stijncodesetup-pr-3');
  ...
  if (await test('scs CLI delegates the new status, sessions, and work-items subcommands', async () => {
    const testDir = createTempDir('scs-state-cli-');
    const dbPath = path.join(testDir, 'state.db');
    ...
    try {
      ...
      const statusResult = runNode(SCS_SCRIPT, ['status', '--db', dbPath, '--json']);
      ...
      const sessionsResult = runNode(SCS_SCRIPT, ['sessions', 'session-active', '--db', dbPath, '--json']);
      ...
      const workItemResult = runNode(SCS_SCRIPT, [
        'work-items',
        'upsert',
        'handoff-roadmap',
        '--db',
      ...
      const delegatedStatusResult = runNode(SCS_SCRIPT, ['status', '--db', dbPath, '--json']);
