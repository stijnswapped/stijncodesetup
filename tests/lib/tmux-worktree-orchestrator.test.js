...
test('buildOrchestrationPlan creates worktrees, branches, and tmux commands', () => {
  const repoRoot = path.join('/tmp', 'scs');
  ...
  assert.ok(
    plan.workerPlans[0].worktreePath.endsWith(path.join('scs-skill-audit-docs-a')),
...
test('normalizeSeedPaths rejects paths outside the repo root', () => {
  assert.throws(
    () => normalizeSeedPaths(['../outside.txt'], '/tmp/scs'),
    /inside repoRoot/
  );
});

test('materializePlan keeps worker instructions inside the worktree boundary', () => {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-orchestrator-test-'));
...
test('overlaySeedPaths copies local overlays into the worker worktree', () => {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-orchestrator-overlay-'));
...
test('executePlan rolls back partial setup when orchestration fails mid-run', () => {
  const plan = {
    ...
    coordinationDir: '/tmp/scs/.orchestration/rollback-test',
    replaceExisting: false,
    workerPlans: [
      {
        workerName: 'Docs',
        workerSlug: 'docs',
        worktreePath: '/tmp/scs-rollback-docs',
        seedPaths: ['commands/multi-workflow.md'],
        gitArgs: ['worktree', 'add', '-b', 'orchestrator-rollback-test-docs', '/tmp/scs-rollback-docs', 'HEAD'],
...
test('executePlan does not mark pre-existing resources for rollback when worktree creation fails', () => {
  const plan = {
    ...
    coordinationDir: '/tmp/scs/.orchestration/rollback-existing',
    replaceExisting: false,
    workerPlans: [
      {
        workerName: 'Docs',
        workerSlug: 'docs',
        worktreePath: '/tmp/scs-existing-docs',
        seedPaths: [],
        gitArgs: ['worktree', 'add', '-b', 'orchestrator-rollback-existing-docs', '/tmp/scs-existing-docs', 'HEAD'],
        launchCommand: 'echo run',
