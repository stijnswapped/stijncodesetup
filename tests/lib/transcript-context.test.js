...
delete process.env.SCS_CONTEXT_WINDOW_TOKENS;
...
test('honors an explicit SCS_CONTEXT_WINDOW_TOKENS override (e.g. 400k models, #2290)', () => {
  process.env.SCS_CONTEXT_WINDOW_TOKENS = '400000';
  try {
...
test('ignores a non-positive / invalid window override', () => {
  process.env.SCS_CONTEXT_WINDOW_TOKENS = 'not-a-number';
  try {
    assert.strictEqual(resolveContextWindowTokens(50000, 'claude-sonnet-4-6'), STANDARD_CONTEXT_WINDOW_TOKENS);
  } finally {
    delete process.env.SCS_CONTEXT_WINDOW_TOKENS;
