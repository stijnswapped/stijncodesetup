...
function runTests() {
  ...
  if (test('getAgentDataHome honors SCS_AGENT_DATA_HOME', () => {
    const original = process.env.SCS_AGENT_DATA_HOME;
    const overrideRoot = path.join(utils.getTempDir(), `scs-agent-data-${Date.now()}`);
    try {
      process.env.SCS_AGENT_DATA_HOME = overrideRoot;
      ...
      if (original === undefined) {
        delete process.env.SCS_AGENT_DATA_HOME;
      } else {
        process.env.SCS_AGENT_DATA_HOME = original;
      }
    }
  })) passed++; else failed++;

  if (test('getAgentDataHome defaults to ~/.cursor/scs when CURSOR_VERSION is set', () => {
    const originalVersion = process.env.CURSOR_VERSION;
    const originalHome = process.env.SCS_AGENT_DATA_HOME;
    try {
      delete process.env.SCS_AGENT_DATA_HOME;
      ...
      const reloaded = require('../../scripts/lib/utils');
      const expected = path.join(reloaded.getHomeDir(), '.cursor', 'scs');
      ...
      if (originalHome === undefined) {
        ...
        process.env.SCS_AGENT_DATA_HOME = originalHome;
      }
    }
  })) passed++; else failed++;

  if (test('getAgentDataHome expands tilde in SCS_AGENT_DATA_HOME', () => {
    const original = process.env.SCS_AGENT_DATA_HOME;
    try {
      process.env.SCS_AGENT_DATA_HOME = path.join('~', '.cursor', 'scs-test');
      delete require.cache[require.resolve('../../scripts/lib/utils')];
      const reloaded = require('../../scripts/lib/utils');
      const expected = path.join(reloaded.getHomeDir(), '.cursor', 'scs-test');
      assert.strictEqual(reloaded.getAgentDataHome(), expected);
    } finally {
      delete require.cache[require.resolve('../../scripts/lib/utils')];
      if (original === undefined) {
  ...
  if (test('isGitRepo returns true in a git repo', () => {
    // We're running from within the SCS repo, so this should be true
  ...
  if (test('findFiles recursive scan skips unreadable subdirectory silently', () => {
    ...
    const tmpDir = path.join(utils.getTempDir(), `scs-findfiles-r71-${Date.now()}`);
  ...
  if (test('findFiles skips broken symlinks that match the pattern', () => {
    ...
    const tmpDir = path.join(utils.getTempDir(), `scs-r84-findfiles-toctou-${Date.now()}`);
  ...
  if (test('replaceInFile with empty string replacement deletes matched text', () => {
    const tmpDir = path.join(utils.getTempDir(), `scs-r88-replace-empty-${Date.now()}`);
    fs.mkdirSync(tmpDir, { recursive: true });
  ...
  if (test('countInFile returns 0 for valid file with no pattern matches', () => {
    const tmpDir = path.join(utils.getTempDir(), `scs-r88-count-zero-${Date.now()}`);
    fs.mkdirSync(tmpDir, { recursive: true });
  ...
  if (test('countInFile with /apple|banana/ (alternation, no g) counts all matches', () => {
    const tmpDir = path.join(utils.getTempDir(), `scs-r95-alternation-${Date.now()}`);
    fs.mkdirSync(tmpDir, { recursive: true });
