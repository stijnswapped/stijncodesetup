...
async function loadPlugin() {
  ...
  const pluginUrl = pathToFileURL(
    path.join(repoRoot, ".opencode", "dist", "plugins", "scs-hooks.js")
  ).href
...
async function withTempProject(files, fn) {
  const projectDir = fs.mkdtempSync(path.join(os.tmpdir(), "scs-opencode-plugin-"))
  try {
...
async function main() {
  ...
  const { SCSHooksPlugin } = await loadPlugin()
  const tests = [
    [
      ...
      async () => withTempProject(
        ...
        async (projectDir) => {
          ...
          const hooks = await SCSHooksPlugin({ client, $, directory: projectDir })
          ...
          // Verify SCS_VERSION is not hardcoded
          assert.ok(env.SCS_VERSION !== "1.8.0", "SCS_VERSION should not be hardcoded to 1.8.0")
          assert.ok(env.SCS_VERSION.match(/^\d+\.\d+\.\d+$/), "SCS_VERSION should be a valid semver version")
        }
      ),
    ],
    [
      ...
      async () => withTempProject(["CLAUDE.md"], async (projectDir) => {
        ...
        assert.deepStrictEqual($.calls, [], `Unexpected shell probes: ${$.calls.join(", ")}`)
        assert.ok(
          client.logs.some((entry) => entry.message === "[SCS] Found CLAUDE.md - loading project context"),
