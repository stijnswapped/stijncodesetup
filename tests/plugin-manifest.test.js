...
const opencodeHooksPluginPath = path.join(repoRoot, '.opencode', 'plugins', 'scs-hooks.ts');
...
test('agent.yaml uses canonical SCS identity', () => {
  const agentYamlSource = fs.readFileSync(agentYamlPath, 'utf8');
  assert.ok(/^name:\s*scs$/m.test(agentYamlSource), 'Expected agent.yaml to use the scs name');
});
...
test('.opencode/plugins/scs-hooks.ts active plugin banner matches package.json', () => {
  const source = fs.readFileSync(opencodeHooksPluginPath, 'utf8');
  const match = source.match(new RegExp(`## Active Plugin: SCS v(${semverPattern})`));
  assert.ok(match, 'Expected .opencode/plugins/scs-hooks.ts to declare an active plugin banner');
  assert.strictEqual(match[1], expectedVersion);
});
...
test('claude plugin.json uses short plugin slug', () => {
  assert.strictEqual(claudePlugin.name, 'scs');
});
...
test('claude marketplace.json has plugins array with the published plugin entry', () => {
  ...
  assert.strictEqual(claudeMarketplace.name, 'scs');
  assert.strictEqual(claudeMarketplace.plugins[0].name, 'scs');
});
...
test('codex plugin.json uses short plugin slug', () => {
  assert.strictEqual(codexPlugin.name, 'scs');
});
...
test('marketplace.json uses short marketplace slug', () => {
  assert.strictEqual(marketplace.name, 'scs');
});
...
test('marketplace.json plugin entry uses short plugin slug', () => {
  assert.strictEqual(marketplace.plugins[0].name, 'scs');
});
...
// ── plugins/scs marketplace plugin folder ─────────────────────────────────────
...
console.log('\n=== plugins/scs Codex marketplace plugin folder ===\n');

const marketplacePluginManifestPath = path.join(repoRoot, 'plugins', 'scs', '.codex-plugin', 'plugin.json');
const marketplacePluginManifest = loadJsonObject(marketplacePluginManifestPath, 'plugins/scs/.codex-plugin/plugin.json');
const rootCodexManifest = loadJsonObject(path.join(repoRoot, '.codex-plugin', 'plugin.json'), '.codex-plugin/plugin.json');

test('plugins/scs manifest name matches the root Codex manifest', () => {
  assert.strictEqual(marketplacePluginManifest.name, rootCodexManifest.name);
});

test('plugins/scs manifest version matches package.json', () => {
  assert.strictEqual(marketplacePluginManifest.version, expectedVersion);
});

test('plugins/scs manifest version matches the root Codex manifest', () => {
  assert.strictEqual(marketplacePluginManifest.version, rootCodexManifest.version);
});

test('plugins/scs manifest reuses root skills and MCP config without vendoring', () => {
  ...
  assert.ok(!fs.existsSync(path.join(pluginDir, 'skills')), 'plugins/scs must not vendor a second skills/ copy (see #2097 review)');
  assert.ok(!fs.existsSync(path.join(pluginDir, '.mcp.json')), 'plugins/scs must not vendor a second .mcp.json (see #2097 review)');
});

test('plugins/scs manifest interface assets resolve to root assets', () => {
  const pluginDir = path.dirname(path.dirname(marketplacePluginManifestPath));
...
test('plugins/scs README documents the upstream Codex fragility', () => {
  const readmePath = path.join(repoRoot, 'plugins', 'scs', 'README.md');
  assert.ok(fs.existsSync(readmePath), 'Expected plugins/scs/README.md');
  const source = fs.readFileSync(readmePath, 'utf8');
  assert.ok(source.includes('openai/codex'), 'plugins/scs README must link the upstream Codex discovery issue');
  assert.ok(source.includes('check-plugin-cache.js'), 'plugins/scs README must point at the cache health check');
  assert.ok(source.includes('sync-scs-to-codex.sh'), 'plugins/scs README must point at the supported manual sync flow');
});
...
test('README version row matches package.json', () => {
  ...
  const match = readme.match(new RegExp(`^\\*\\*Version:\\*\\* (${semverPattern})$`, 'm'));
  assert.ok(match, 'Expected README version line');
  assert.strictEqual(match[1], expectedVersion);
});
...
test('user-facing docs do not use overlong legacy marketplace install commands', () => {
  const markdownFiles = [
    ...
    path.join(repoRoot, 'skills', 'configure-scs', 'SKILL.md'),
  ...
  for (const filePath of markdownFiles) {
    ...
    if (/\/plugin\s+(install|list)\s+stijncodesetup(?:@stijncodesetup)?\b/.test(source)) {
...
test('user-facing docs do not use the legacy non-URL marketplace add form', () => {
  ...
  for (const filePath of markdownFiles) {
    const source = fs.readFileSync(filePath, 'utf8');
    if (source.includes('/plugin marketplace add StijnCodeSetup/stijncodesetup')) {
      offenders.push(path.relative(repoRoot, filePath));
    }
  }
