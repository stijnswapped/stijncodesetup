...
function makeRecord({ repoRoot, homeDir, projectRoot, adapter, request, resolution, operations }) {
  ...
  const installStatePath = adapter.kind === 'project'
    ? path.join(targetRoot, 'scs-install-state.json')
    : path.join(targetRoot, 'scs', 'install-state.json');
...
function ensureFakeRepo(repoRoot) {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ name: 'stijncodesetup', version: '1.10.0' }, null, 2)
  );
...
function runTests() {
  ...
  if (test('parseArgs reads repo-root, target, dry-run, and json flags', () => {
    ...
    assert.strictEqual(parsed.repoRoot, '/tmp/scs');
  ...
  if (test('deriveRepoRootFromState uses sourcePath and sourceRelativePath', () => {
    const state = {
      operations: [
        {
          sourcePath: path.join('/tmp', 'scs', 'scripts', 'setup-package-manager.js'),
    ...
    assert.strictEqual(
      ...
      path.resolve(path.join('/tmp', 'scs'))
    );
  })) passed += 1; else failed += 1;
  ...
  if (test('determineInstallCwd uses the project root for project installs', () => {
    ...
    assert.strictEqual(determineInstallCwd(record, '/tmp/scs'), path.join('/tmp', 'project'));
  })) passed += 1; else failed += 1;
  ...
  if (test('runAutoUpdate rejects mixed inferred repo roots', () => {
    ...
    try {
      ...
      assert.throws(
        ...
        /Multiple SCS repo roots detected/
      );
    } finally {
      cleanup(homeDir);
      cleanup(projectRoot);
            ...
            if (command === process.execPath) {
              return {
                stdout: JSON.stringify({
                  dryRun: false,
                  result: {
                    installStatePath: path.join(projectRoot, '.cursor', 'scs-install-state.json'),
                  },
                }),
                stderr: '',
              };
