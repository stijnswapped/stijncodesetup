...
// Test helper — matches SCS's custom test pattern
...
function runTests() {
  ...
  if (test('loadSCSContext() returns "" when no skills specified', () => {
    const result = loadSCSContext('');
    assert.strictEqual(result, '');
  })) passed++; else failed++;

  if (test('loadSCSContext() skips missing skill directories gracefully', () => {
    const result = loadSCSContext('nonexistent-skill-xyz');
    assert.strictEqual(result, '');
  })) passed++; else failed++;

  if (test('loadSCSContext() concatenates multiple skill files', () => {
    // Use real skills from the SCS repo if they exist
    ...
    const result = loadSCSContext(twoSkills);
  ...
  if (test('module exports all required functions', () => {
    ...
    const required = [
      ...
      'appendTurn', 'loadSCSContext', 'askClaude', 'main'
    ];
