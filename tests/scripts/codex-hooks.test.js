...
const syncScript = path.join(repoRoot, 'scripts', 'sync-scs-to-codex.sh');
...
function seedPluginCache(codexDir, manifest, files = []) {
  const cacheDir = path.join(codexDir, 'plugins', 'cache', 'scs', 'scs', packageVersion);
...
const cacheManifestWithLocalRefs = {
  ...
  mcpServers: './.mcp.json',
  interface: {
    composerIcon: './assets/scs-icon.svg',
...
if (
  test('check-plugin-cache fails when the installed cache is missing manifest-referenced files', () => {
    ...
    try {
      ...
      assert.match(result.stdout, /sync-scs-to-codex\.sh/);
    } finally {
      cleanup(homeDir);
    }
  })
  ...
  test('install-global-git-hooks.sh handles quoted hook paths without shell injection', () => {
    ...
    try {
      const result = runBash(installScript, [], {
        HOME: homeDir,
        SCS_GLOBAL_HOOKS_DIR: weirdHooksDir,
      });
...
if (
  test('merge-mcp-config no-ops after all recommended servers are present', () => {
    ...
    try {
      ...
      assert.match(second.stdout, /All SCS MCP servers already present/);
...
if (
  test('merge-mcp-config repairs the invalid exa url entry from earlier SCS versions (#2224)', () => {
...
if (
  test('merge-mcp-config update dry-run refreshes managed sections and leaves user servers alone', () => {
    ...
    try {
      ...
      assert.match(result.stdout, /\[mcp_servers\.chrome-devtools\]/);
      // Retired servers are no longer SCS-managed: never removed or re-added.
  ...
  test('merge-mcp-config removes disabled servers without appending replacements', () => {
    ...
    try {
      ...
      const result = runNode(mergeMcpConfigScript, [configPath], {
        ...
        SCS_DISABLED_MCPS: 'chrome-devtools',
      });

      assert.strictEqual(result.status, 0, `${result.stdout}\n${result.stderr}`);
      assert.match(result.stdout, /Disabled via SCS_DISABLED_MCPS/);
...
if (
  test('sync installs the missing Codex baseline and accepts the legacy context7 MCP section', () => {
    ...
    try {
      ...
      assert.match(syncedAgents, /^# StijnCodeSetup \(SCS\) — Agent Instructions/m);
      assert.match(syncedAgents, /^# Codex Supplement \(From SCS \.codex\/AGENTS\.md\)/m);
