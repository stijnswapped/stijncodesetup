...
function runTests() {
  ...
  if (test('shows help with an explicit help flag', () => {
    ...
    assert.match(result.stdout, /Consult SCS install components/);
  ...
  if (test('recommends security components and profile for a natural language query', () => {
    ...
    assert.strictEqual(payload.schemaVersion, 'scs.consult.v1');
    ...
    assert.strictEqual(
      ...
      'npx scs install --profile minimal --target claude --with capability:security'
    );
  ...
  if (test('prints text recommendations with install and plan commands', () => {
    ...
    assert.strictEqual(result.status, 0, result.stderr);
    assert.match(result.stdout, /SCS consult/);
    assert.match(result.stdout, /capability:security/);
    assert.match(result.stdout, /npx scs install --profile minimal --target claude --with capability:security/);
    assert.match(result.stdout, /npx scs plan --profile minimal --target claude --with capability:security/);
  })) passed++; else failed++;
  ...
  if (test('works from outside the SCS repository', () => {
    const projectDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-consult-project-'));
    try {
