...
async function test(name, fn) {
  try {
    ...
    if (String(error && error.message || error).includes('listen EPERM: operation not permitted 127.0.0.1')) {
      console.log(`  SKIP ${name}`);
      console.log('    Reason: loopback binding is not permitted in this sandbox');
      return true;
    }
...
async function writeMinimalDatabase(dbPath) {
  ...
  db.run(`
    ...
    INSERT INTO sessions (
      ...
      'session-a', 'Build the control pane', 'codex', 'codex', '["codex"]', '/repo/scs',
...
function waitForCliReady(child) {
  return new Promise((resolve, reject) => {
    ...
    child.stdout.on('data', chunk => {
      ...
      if (!settled && stdout.includes('SCS Control Pane:') && stdout.includes('Actions:')) {
        settled = true;
        clearTimeout(timer);
...
async function runTests() {
  ...
  if (
    await test('parses CLI arguments for local-only serving', async () => {
      ...
      assert.strictEqual(parsed.dbPath, '/tmp/scs2.db');
      assert.strictEqual(parsed.stateDbPath, '/tmp/scs-state.db');
  ...
  if (
    await test('serves HTML and snapshot JSON from a temp SCS2 database', async () => {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-control-pane-server-'));
      ...
      try {
        ...
        try {
          ...
          assert.ok(html.includes('SCS Control Pane'));
          ...
          assert.ok(!/onclick="scs(Claim|Move)Item\(/.test(html), 'no inline onclick handlers with interpolated ids');
  ...
  if (
    await test('serves the 3D agent-airspace page and the proximity JSON feed', async () => {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-control-pane-proximity-'));
      const dbPath = path.join(tempDir, 'scs2.db');
  ...
  if (
    await test('serves health, asset, not-found, invalid body, and read-only action responses', async () => {
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-control-pane-routes-'));
      ...
      try {
        ...
        try {
          ...
          try {
            const realAsset = await fetchLocal(`${realAssetApp.url}/assets/scs-icon.svg`);
          ...
          const missingAsset = await fetchLocal(`${app.url}/assets/scs-icon.svg`);
  ...
  if (
    await test('runAction captures success, failure, and bounded output', async () => {
      ...
      const spawnError = await runAction({
        id: 'spawn-error',
        command: 'definitely-not-scs-control-pane-command',
        args: [],
        cwd: repoRoot
      });
  ...
  if (
    await test('CLI starts a read-only local server and shuts down on SIGTERM', async () => {
      const child = spawn(process.execPath, [SCRIPT, '--host', '127.0.0.1', '--port', '0', '--read-only', '--no-open'], {
        ...
        env: {
          ...process.env,
          SCS2_DB_PATH: path.join(os.tmpdir(), 'missing-scs2-cli.db')
        }
      });
      ...
      try {
        let ready;
        try {
          ready = await waitForCliReady(child);
        } catch (error) {
          if (String(error && error.message || error).includes('listen EPERM: operation not permitted 127.0.0.1')) {
            loopbackDenied = true;
            return;
        ...
        assert.match(ready.stdout, /SCS Control Pane: http:\/\/127\.0\.0\.1:\d+/);
  ...
  if (
    await test('interactive board: claim and move work items via POST endpoints', async () => {
      ...
      const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-control-pane-board-'));
