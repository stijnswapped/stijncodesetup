...
test('readFrontmatter parses simple frontmatter', () => {
  ...
  testRoot = createTempDir('scs-test-');
...
test('renderHTML includes LANG and LANG_KEYS in the output', () => {
  ...
  const html = renderHTML(data);
  assert.ok(html.includes('SCS Capabilities'));
...
test('renderHTML includes the dashboard title and footer', () => {
  ...
  const html = renderHTML(data);
  assert.ok(html.includes('SCS Capabilities'));
  assert.ok(html.includes('github.com/StijnCodeSetup/StijnCodeSetup'));
});
  ...
  testServer.listen(0, () => {
    ...
    http.get(`http://localhost:${port}/`, (res) => {
      ...
      res.on('end', () => {
        ...
        assert.ok(body.includes('SCS Capabilities'));
        testServer.close();
        done();
      });
    });
