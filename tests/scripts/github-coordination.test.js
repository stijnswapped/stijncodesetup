...
const key = args.join(' ');
const logPath = process.env.SCS_GH_SHIM_LOG;
if (logPath) {
    ...
    await test('claims an epic issue, updates GitHub state, and caches a work item', async () => {
      ...
      try {
        ...
        const issueView = {
          ...
          body: epicBody,
          url: 'https://github.com/StijnCodeSetup/StijnCodeSetup/issues/12',
          state: 'OPEN',
          labels: [{ name: 'epic' }],
        ...
        const shim = writeGhShim(rootDir, {
          'issue view 12 --repo StijnCodeSetup/StijnCodeSetup --json number,title,body,url,state,labels,author,updatedAt,assignees': issueView
        });

        const result = run(['claim', '12', '--repo', 'StijnCodeSetup/StijnCodeSetup', '--actor', 'codex', '--db', dbPath, '--json'], {
          cwd: rootDir,
          env: {
            SCS_GH_SHIM: shim.shimPath,
            SCS_GH_SHIM_LOG: shim.logPath
          }
        });
    ...
    await test('unblocks an epic when dependencies are closed', async () => {
      ...
      try {
        const blockedBody = [
          ...
          'Dependencies: #2',
          '',
          '<!-- scs-coordination:start -->',
          '```json',
          JSON.stringify(
            {
              schemaVersion: 'scs.github.coordination.v1',
              kind: 'epic',
              status: 'blocked',
              owner: 'codex',
          ...
          '```',
          '<!-- scs-coordination:end -->'
        ].join('\n');
        const openIssue = {
          number: 1,
          title: 'Release readiness',
          body: blockedBody,
          url: 'https://github.com/StijnCodeSetup/StijnCodeSetup/issues/1',
          state: 'OPEN',
        ...
        const closedDependency = {
          ...
          url: 'https://github.com/StijnCodeSetup/StijnCodeSetup/issues/2',
          state: 'CLOSED',
        ...
        const shim = writeGhShim(rootDir, {
          'issue list --repo StijnCodeSetup/StijnCodeSetup --state all --limit 100 --json number,title,body,url,state,labels,author,updatedAt,assignees': [openIssue, closedDependency],
          'issue view 1 --repo StijnCodeSetup/StijnCodeSetup --json number,title,body,url,state,labels,author,updatedAt,assignees': openIssue
        });

        const result = run(['unblock', '--repo', 'StijnCodeSetup/StijnCodeSetup', '--db', dbPath, '--json'], {
          cwd: rootDir,
          env: {
            SCS_GH_SHIM: shim.shimPath,
            SCS_GH_SHIM_LOG: shim.logPath
