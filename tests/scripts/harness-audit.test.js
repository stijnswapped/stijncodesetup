...
function runTests() {
  ...
  if (test('GitHub Integration category scores against a fully-wired consumer fixture', () => {
    ...
    try {
      fs.mkdirSync(path.join(homeDir, '.claude', 'plugins', 'scs', '.claude-plugin'), { recursive: true });
      fs.writeFileSync(
        path.join(homeDir, '.claude', 'plugins', 'scs', '.claude-plugin', 'plugin.json'),
        JSON.stringify({ name: 'scs' }, null, 2)
      );
  ...
  if (test('detects repo mode from structural markers when package name differs', () => {
    ...
    try {
      ...
      fs.writeFileSync(path.join(projectRoot, '.claude-plugin', 'plugin.json'), JSON.stringify({ name: 'scs' }, null, 2));
      fs.writeFileSync(
        path.join(projectRoot, 'package.json'),
  ...
  if (test('audits consumer projects from cwd instead of the SCS repo root', () => {
  ...
  if (test('prints no top actions when consumer checks all pass', () => {
    ...
    try {
      fs.mkdirSync(path.join(projectRoot, '.claude', 'plugins', 'scs@scs'), { recursive: true });
      fs.writeFileSync(
        path.join(projectRoot, '.claude', 'plugins', 'scs@scs', 'plugin.json'),
        JSON.stringify({ name: 'scs' }, null, 2)
      );
      fs.mkdirSync(path.join(projectRoot, '.claude'), { recursive: true });
  ...
  if (test('detects marketplace-installed Claude plugins under home marketplaces/', () => {
    ...
    try {
      fs.mkdirSync(path.join(homeDir, '.claude', 'plugins', 'marketplaces', 'stijncodesetup', '.claude-plugin'), { recursive: true });
      fs.writeFileSync(
        path.join(homeDir, '.claude', 'plugins', 'marketplaces', 'stijncodesetup', '.claude-plugin', 'plugin.json'),
        JSON.stringify({ name: 'stijncodesetup' }, null, 2)
      );
  ...
  if (test('detects marketplace-installed Claude plugins under project marketplaces/', () => {
    ...
    try {
      fs.mkdirSync(path.join(projectRoot, '.claude', 'plugins', 'marketplaces', 'stijncodesetup', '.claude-plugin'), { recursive: true });
      fs.writeFileSync(
        path.join(projectRoot, '.claude', 'plugins', 'marketplaces', 'stijncodesetup', '.claude-plugin', 'plugin.json'),
        JSON.stringify({ name: 'stijncodesetup' }, null, 2)
      );
  ...
  if (test('detects Claude plugin installs from installed_plugins.json', () => {
    ...
    const installRoot = path.join(pluginsDir, 'cache', 'stijncodesetup', 'scs', '2.0.0');

    try {
      fs.mkdirSync(path.join(installRoot, '.claude-plugin'), { recursive: true });
      fs.writeFileSync(
        path.join(installRoot, '.claude-plugin', 'plugin.json'),
        JSON.stringify({ name: 'scs', version: '2.0.0' }, null, 2)
      );
      fs.writeFileSync(
        ...
        JSON.stringify({
          plugins: {
            'scs@stijncodesetup': [
              { installPath: path.join('cache', 'stijncodesetup', 'scs', '2.0.0') },
            ],
          },
        }, null, 2)
      );
      ...
      try {
        ...
        assert.ok(found);
        assert.ok(found.includes(`${path.sep}cache${path.sep}stijncodesetup${path.sep}scs${path.sep}2.0.0${path.sep}`));
      } finally {
  ...
  if (test('detects newest Claude plugin install from cache marketplace layout', () => {
    ...
    const pluginRoot = path.join(homeDir, '.claude', 'plugins', 'cache', 'stijncodesetup', 'scs');
    ...
    try {
      for (const version of ['1.8.0', '1.10.0']) {
        ...
        fs.writeFileSync(
          ...
          JSON.stringify({ name: 'scs', version }, null, 2)
        );
      }
