...
function runTests() {
  ...
  if (test('installs Claude rules and writes install-state', () => {
    ...
    try {
      ...
      assert.ok(fs.existsSync(path.join(claudeRoot, 'rules', 'scs', 'common', 'coding-style.md')));
      assert.ok(fs.existsSync(path.join(claudeRoot, 'rules', 'scs', 'typescript', 'testing.md')));
      ...
      assert.ok(fs.existsSync(path.join(claudeRoot, 'skills', 'scs', 'tdd-workflow', 'SKILL.md')));
      assert.ok(fs.existsSync(path.join(claudeRoot, 'skills', 'scs', 'coding-standards', 'SKILL.md')));
      assert.ok(fs.existsSync(path.join(claudeRoot, 'plugin.json')));

      const statePath = path.join(homeDir, '.claude', 'scs', 'install-state.json');
      ...
      assert.ok(
        state.operations.some(operation => (
          operation.destinationPath === path.join(claudeRoot, 'rules', 'scs', 'common', 'coding-style.md')
        )),
  ...
  if (test('rewrites namespaced skill links to the scs/ rules path (#2340)', () => {
    const homeDir = createTempDir('install-apply-home-');
    const projectDir = createTempDir('install-apply-project-');
    ...
    try {
      ...
      const claudeRoot = path.join(homeDir, '.claude');
      const skillPath = path.join(claudeRoot, 'skills', 'scs', 'react-patterns', 'SKILL.md');
      assert.ok(fs.existsSync(skillPath), 'react-patterns SKILL.md should be installed');

      const content = fs.readFileSync(skillPath, 'utf8');
      assert.ok(
        content.includes('../../../rules/scs/react/'),
        'source-relative rules link should be rewritten for the scs/ namespace'
      );
      assert.ok(
  ...
  if (test('installs Cursor configs and writes install-state', () => {
    ...
    try {
      ...
      assert.ok(fs.existsSync(path.join(projectDir, '.cursor', 'agents', 'scs-architect.md')));
      ...
      const statePath = path.join(projectDir, '.cursor', 'scs-install-state.json');
      const state = readJson(statePath);
  ...
  if (test('installs Antigravity configs and writes install-state', () => {
    ...
    try {
      ...
      const statePath = path.join(projectDir, '.agent', 'scs-install-state.json');
      const state = readJson(statePath);
  ...
  if (test('installs JoyCode profile through managed install-state', () => {
    ...
    try {
      ...
      const statePath = path.join(projectDir, '.joycode', 'scs-install-state.json');
      const state = readJson(statePath);
  ...
  if (test('installs Qwen profile through managed home install-state', () => {
    ...
    try {
      ...
      const statePath = path.join(homeDir, '.qwen', 'scs-install-state.json');
      const state = readJson(statePath);
  ...
  if (test('supports dry-run without mutating the target project', () => {
    ...
    try {
      ...
      assert.ok(!fs.existsSync(path.join(projectDir, '.cursor', 'hooks.json')));
      assert.ok(!fs.existsSync(path.join(projectDir, '.cursor', 'scs-install-state.json')));
    } finally {
      cleanup(homeDir);
      cleanup(projectDir);
    }
  ...
  if (test('supports manifest profile dry-runs through the installer', () => {
    ...
    try {
      ...
      assert.ok(!fs.existsSync(path.join(homeDir, '.claude', 'scs', 'install-state.json')));
    } finally {
      cleanup(homeDir);
      cleanup(projectDir);
    }
  ...
  if (test('installs manifest profiles and writes non-legacy install-state', () => {
    ...
    try {
      ...
      const state = readJson(path.join(claudeRoot, 'scs', 'install-state.json'));
  ...
  if (test('installs antigravity manifest profiles while skipping only unsupported modules', () => {
    ...
    try {
      ...
      const state = readJson(path.join(projectDir, '.agent', 'scs-install-state.json'));
      assert.strictEqual(state.request.profile, 'core');
      assert.strictEqual(state.request.legacyMode, false);
  ...
  if (test('installs explicit modules for cursor using manifest operations', () => {
    ...
    try {
      ...
      const state = readJson(path.join(projectDir, '.cursor', 'scs-install-state.json'));
      ...
      assert.ok(state.resolution.selectedModules.includes('platform-configs'));
      assert.ok(
        !state.operations.some(operation => operation.destinationPath.endsWith('scs-install-state.json')),
  ...
  if (test('filters copied mcp config files when SCS_DISABLED_MCPS is set', () => {
    ...
    const installStatePath = path.join(tempDir, 'installed', 'scs-install-state.json');
    const previousValue = process.env.SCS_DISABLED_MCPS;
    ...
    try {
      ...
      process.env.SCS_DISABLED_MCPS = 'github,memory';

      applyInstallPlan({
        targetRoot: path.join(tempDir, 'installed'),
        installStatePath,
        statePreview: {
          schemaVersion: 'scs.install.v1',
      ...
      if (previousValue === undefined) {
        delete process.env.SCS_DISABLED_MCPS;
      } else {
        process.env.SCS_DISABLED_MCPS = previousValue;
      }
      cleanup(tempDir);
    }
  })) passed++; else failed++;
  ...
  if (test('ignores malformed existing settings.json during claude install', () => {
    ...
    try {
      ...
      assert.ok(fs.existsSync(path.join(claudeRoot, 'scs', 'install-state.json')), 'install state should still be written');
    } finally {
      cleanup(homeDir);
      cleanup(projectDir);
    }
  ...
  if (test('fails when source hooks.json root is not an object before copying files', () => {
    ...
    const installStatePath = path.join(targetRoot, 'scs', 'install-state.json');
  ...
  if (test('installs from scs-install.json and persists component selections', () => {
    const homeDir = createTempDir('install-apply-home-');
    const projectDir = createTempDir('install-apply-project-');
    const configPath = path.join(projectDir, 'scs-install.json');
    ...
    try {
      ...
      assert.ok(fs.existsSync(path.join(homeDir, '.claude', 'skills', 'scs', 'security-review', 'SKILL.md')));
      assert.ok(!fs.existsSync(path.join(homeDir, '.claude', 'skills', 'scs', 'dmux-workflows', 'SKILL.md')));

      const state = readJson(path.join(homeDir, '.claude', 'scs', 'install-state.json'));
  ...
  if (test('auto-detects scs-install.json from the project root', () => {
    const homeDir = createTempDir('install-apply-home-');
    const projectDir = createTempDir('install-apply-project-');
    const configPath = path.join(projectDir, 'scs-install.json');
