...
function runTests() {
  ...
  if (test('shows help with no arguments', () => {
    ...
    assert.ok(result.stdout.includes('Inspect SCS selective-install manifests'));
  })) passed++; else failed++;
  ...
  if (test('loads planning intent from scs-install.json', () => {
    const configDir = path.join(__dirname, '..', 'fixtures', 'tmp-install-plan-config');
    const configPath = path.join(configDir, 'scs-install.json');
  ...
  if (test('auto-detects planning intent from project scs-install.json', () => {
