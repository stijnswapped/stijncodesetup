...
function runTests() {
  ...
  if (test('README documents reset and uninstall flow', () => {
    assert.ok(
      readme.includes('### Reset / Uninstall SCS'),
    ...
    assert.ok(
      readme.includes('node scripts/scs.js list-installed'),
      'README should document install-state inspection before reinstalling'
    );
    assert.ok(
      readme.includes('node scripts/scs.js doctor'),
      'README should document doctor before reinstalling'
    );
    assert.ok(
      readme.includes('SCS only removes files recorded in its install-state.'),
  ...
  if (test('README documents low-context no-hooks install path', () => {
    ...
    assert.ok(
      readme.includes('npx scs-install --profile minimal --target claude'),
  ...
  if (test('README documents consult-based component discovery', () => {
    ...
    assert.ok(
      readme.includes('npx scs consult "security reviews" --target claude'),
  ...
  if (test('README documents Cursor agent namespace and loading caveat', () => {
    assert.ok(
      readme.includes('`.cursor/agents/scs-*.md`'),
    ...
    assert.ok(
      readme.includes('SCS does not install root `AGENTS.md` into `.cursor/`.'),
  ...
  if (test('README explains plugin-path cleanup and rules scoping', () => {
    ...
    assert.ok(
      readme.includes('~/.claude/rules/scs/'),
      'README should steer plugin-path rules into an SCS-owned namespace'
    );
  })) passed++; else failed++;

  if (test('rules README mirrors SCS namespaced install path', () => {
    assert.ok(
      rulesReadme.includes('mkdir -p ~/.claude/rules/scs'),
      'rules README should create the SCS-owned user-level rules namespace'
    );
    assert.ok(
      rulesReadme.includes('cp -r rules/common ~/.claude/rules/scs/'),
      'rules README should copy common rules under ~/.claude/rules/scs/'
    );
    assert.ok(
      rulesReadme.includes('cp -r rules/typescript ~/.claude/rules/scs/'),
      'rules README should copy language rules under ~/.claude/rules/scs/'
    );
    assert.ok(
      rulesReadme.includes('mkdir -p .claude/rules/scs'),
      'rules README should document the project-local SCS namespace'
    );
    assert.ok(
