...
function createTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'scs-instinct-cli-projects-'));
}
...
test('status migrates legacy no-remote linked worktree project dirs to main worktree id', () => {
  ...
  try {
    ...
    runGit(mainWorktree, ['config', 'user.email', 'scs@example.test']);
    runGit(mainWorktree, ['config', 'user.name', 'SCS Test']);
