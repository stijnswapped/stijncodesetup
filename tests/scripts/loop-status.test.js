...
function createTempHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'scs-loop-status-home-'));
}
...
function runTests() {
  ...
  if (test('reports overdue ScheduleWakeup calls from Claude transcripts', () => {
    ...
    try {
      ...
      assert.strictEqual(payload.schemaVersion, 'scs.loop-status.v1');
  ...
  if (test('parses write-dir snapshot option', () => {
    const options = parseArgs([
      ...
      'scripts/loop-status.js',
      '--write-dir',
      '/tmp/scs-loop-snapshots',
    ]);

    assert.strictEqual(options.writeDir, '/tmp/scs-loop-snapshots');
  })) passed++; else failed++;
  ...
  if (test('exit-code mode returns 1 for scan errors without attention signals', () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-loop-status-missing-'));
  ...
  if (test('watch mode emits repeated JSON status frames', () => {
    ...
    try {
      ...
      assert.strictEqual(frames[0].schemaVersion, 'scs.loop-status.v1');
      assert.strictEqual(frames[1].schemaVersion, 'scs.loop-status.v1');
  ...
  if (test('writes per-session status snapshots and index when write-dir is set', () => {
    const homeDir = createTempHome();
    const snapshotDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-loop-status-snapshots-'));
    ...
    try {
      ...
      assert.strictEqual(stdoutPayload.schemaVersion, 'scs.loop-status.v1');
      ...
      assert.strictEqual(indexPayload.schemaVersion, 'scs.loop-status.index.v1');
      ...
      assert.strictEqual(snapshotPayload.schemaVersion, 'scs.loop-status.session.v1');
  ...
  if (test('keeps index.json reserved when session id sanitizes to index', () => {
    const homeDir = createTempHome();
    const snapshotDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-loop-status-index-collision-'));
  ...
  if (test('avoids Windows reserved basenames for session snapshots', () => {
    const homeDir = createTempHome();
    const snapshotDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-loop-status-windows-name-'));
  ...
  if (test('cleans temporary snapshot files when atomic rename fails', () => {
    const snapshotDir = fs.mkdtempSync(path.join(os.tmpdir(), 'scs-loop-status-rename-failure-'));
