...
function seedMinimalRepo(rootDir, overrides = {}) {
  const files = {
    ...
    'scripts/work-items.js': 'sync-github github-pr github-issue sourceClosedAt scs-work-items-sync-github',
    'scripts/hooks/session-activity-tracker.js': 'tool-usage.jsonl session_id tool_name',
    'scs2/src/observability/mod.rs': 'ToolCallEvent RiskAssessment ToolLogger',
    'scs2/src/session/store.rs': 'insert_tool_log query_tool_logs',
    'scs2/src/session/manager.rs': 'sync_tool_activity_metrics tool-usage.jsonl',
    ...
    'examples/hud-status-contract.json': JSON.stringify({
      schema_version: 'scs.hud-status.v1',
      context: {},
      toolCalls: {},
      activeAgents: [],
      todos: {},
    ...
    'tests/docs/scs2-release-surface.test.js': 'publication-readiness.md',
  };
...
function runTests() {
  ...
  if (test('current repo reports a complete readiness score', () => {
    ...
    assert.strictEqual(parsed.schema_version, 'scs.observability-readiness.v1');
  ...
  if (test('missing tool logger surfaces become prioritized top actions', () => {
    ...
    try {
      seedMinimalRepo(projectRoot, {
        'scs2/src/observability/mod.rs': 'ToolCallEvent only'
      });
      const report = buildReport(projectRoot);

      assert.strictEqual(report.ready, false);
      assert.ok(report.top_actions.some(action => action.id === 'scs2-tool-risk-ledger'));
      assert.ok(report.checks.some(check => check.id === 'scs2-tool-risk-ledger' && !check.pass));
    } finally {
      cleanup(projectRoot);
    }
  })) passed++; else failed++;
