...
function seedRepo(rootDir, overrides = {}) {
  const files = {
    ...
    'docs/releases/2.0.0/scs-2-hypergrowth-release-command-center.md': [
...
function runTests() {
  ...
  if (test('parseArgs accepts dashboard flags and rejects invalid values', () => {
    ...
    try {
      ...
      assert.deepStrictEqual(parsed.repos, ['StijnCodeSetup/StijnCodeSetup']);
  ...
  if (test('seeded repo emits an objective audit with remaining work', () => {
    ...
    try {
      ...
      assert.strictEqual(report.schema_version, 'scs.operator-readiness-dashboard.v1');
      ...
      assert.ok(report.requirements.some(item => item.id === 'scs-tools-next-level' && item.status === 'in_progress'));
      assert.ok(report.requirements.some(item => (
      ...
      assert.ok(report.requirements.some(item => (
        item.id === 'scs-tools-next-level'
          ...
          && item.evidence.includes('SCS-Tools Dependabot alert remediation')
      )));
      assert.ok(report.requirements.some(item => (
      ...
      assert.ok(!report.top_actions.some(item => item.id === 'scs-preview-pack'));
  ...
  if (test('preview pack and Hermes gates stay in progress until smoke gate is wired', () => {
    ...
    try {
      ...
      const report = buildSeededReport(rootDir);
      const previewPack = report.requirements.find(item => item.id === 'scs-preview-pack');
  ...
  if (test('markdown output can be written as the dashboard artifact', () => {
    ...
    try {
      ...
      assert.ok(written.includes('# SCS Operator Readiness Dashboard'));
  ...
  if (test('text output renders compact status and top actions', () => {
    ...
    try {
      ...
      assert.ok(stdout.includes('SCS Operator Readiness Dashboard'));
