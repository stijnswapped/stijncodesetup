...
function runTests() {
  ...
  if (test('skip-github report checks local release and security evidence', () => {
    ...
    try {
      ...
      assert.strictEqual(parsed.schema_version, 'scs.platform-audit.v1');
  ...
  if (test('markdown output can be written as an operator artifact', () => {
    ...
    try {
      ...
      assert.ok(written.includes('# SCS Platform Audit'));
  ...
  if (test('github queue and discussion budgets pass with maintainer touch', () => {
    ...
    try {
      ...
      const shimPath = writeGhShim(projectRoot, {
        'pr list --repo StijnCodeSetup/StijnCodeSetup --state open --json number,title,isDraft,mergeStateStatus,updatedAt,url,author': [],
        'issue list --repo StijnCodeSetup/StijnCodeSetup --state open --json number,title,updatedAt,url,author,labels': [],
  ...
  if (test('threshold failures and untouched discussions become top actions', () => {
    ...
    try {
      ...
      const shimPath = writeGhShim(projectRoot, {
        'pr list --repo StijnCodeSetup/StijnCodeSetup --state open --json number,title,isDraft,mergeStateStatus,updatedAt,url,author': prs,
        'issue list --repo StijnCodeSetup/StijnCodeSetup --state open --json number,title,updatedAt,url,author,labels': [],
        [discussionEnabledGhKey('StijnCodeSetup', 'StijnCodeSetup')]: {
          data: { repository: { hasDiscussionsEnabled: true } }
        },
  ...
  if (test('discussion-disabled repos skip the heavy discussion query', () => {
    ...
    try {
      seedRepo(projectRoot);
      const shimPath = writeGhShim(projectRoot, {
        'pr list --repo SCS-Tools/SCS-website --state open --json number,title,isDraft,mergeStateStatus,updatedAt,url,author': [],
        'issue list --repo SCS-Tools/SCS-website --state open --json number,title,updatedAt,url,author,labels': [],
