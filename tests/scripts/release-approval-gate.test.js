...
function approvedPacketContent(overrides = {}, release = CURRENT_RELEASE) {
  ...
  return [
    `# SCS v${release} Owner Approval Packet`,
    '',
    '## Decision Register',
    '',
...
function finalLedgerContent(extra = '', release = CURRENT_RELEASE) {
  return [
    `# SCS v${release} Release URL Ledger`,
    '',
...
function manifestContent(release = CURRENT_RELEASE) {
  return [
    `# SCS v${release} Preview Pack Manifest`,
    '',
    '| Artifact | Role | Gate |',
    '| --- | --- | --- |',
...
function runTests() {
  ...
  if (test('seeded approved release passes every publication approval check', () => {
    ...
    try {
      ...
      assert.strictEqual(report.schema_version, 'scs.release-approval-gate.v1');
    ...
    try {
      ...
      seedRepo(rootDir, {
        [`${releaseDir}/release-url-ledger-2026-05-19.md`]: [
          `# SCS v${CURRENT_RELEASE} Release URL Ledger`,
          '',
          '## Approval-Gated URLs',
          '',
          '| Surface | Intended URL or command | Gate before use |',
          '| --- | --- | --- |',
          `| GitHub prerelease | https://github.com/StijnCodeSetup/StijnCodeSetup/releases/tag/v${CURRENT_RELEASE} | must return the prerelease |`,
        ].join('\n'),
      });
  ...
  if (test('announcement drafts fail on unresolved placeholders and private paths', () => {
    ...
    try {
      const releaseDir = releaseDirFor(CURRENT_RELEASE);
      seedRepo(rootDir, {
        [`${releaseDir}/x-thread.md`]: 'Ship copy with <video-url> and /Users/stijn/raw-footage.',
      });
