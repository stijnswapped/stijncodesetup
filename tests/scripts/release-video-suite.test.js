...
function runTests() {
  ...
  if (test('buildReport passes with a sanitized manifest and complete local media fixture', () => {
    ...
    try {
      ...
      assert.strictEqual(report.schema_version, 'scs.release-video-suite.v1');
  ...
  if (test('missing local roots keep the release video gate blocked', () => {
    ...
    try {
      ...
      assert.ok(report.top_actions.some(action => action.includes('SCS_VIDEO_SOURCE_ROOT')));
      assert.ok(report.top_actions.some(action => action.includes('SCS_VIDEO_RELEASE_SUITE_ROOT')));
