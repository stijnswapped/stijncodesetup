...
const SCRIPT = path.join(__dirname, '..', '..', 'scripts', 'scs.js');
...
function main() {
  ...
  const tests = [
    ['shows top-level help', () => {
      ...
      assert.match(result.stdout, /SCS selective-install CLI/);
    ...
    ['delegates lifecycle commands', () => {
      const homeDir = createTempDir('scs-cli-home-');
      const projectRoot = createTempDir('scs-cli-project-');
    ...
    ['delegates session-inspect command', () => {
      ...
      assert.strictEqual(payload.workers[0].branch, 'feat/scs-cli');
    }],
    ['delegates loop-status command', () => {
      const homeDir = createTempDir('scs-cli-home-');
      const transcriptDir = path.join(homeDir, '.claude', 'projects', '-tmp-scs');
    ...
    ['delegates security-ioc-scan command', () => {
      const projectRoot = createTempDir('scs-cli-ioc-scan-');
      fs.writeFileSync(
