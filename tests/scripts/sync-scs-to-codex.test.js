/**
 * Source-level tests for scripts/sync-scs-to-codex.sh
 */
...
const scriptPath = path.join(__dirname, '..', '..', 'scripts', 'sync-scs-to-codex.sh');
...
function runTests() {
  ...
  if (test('sync script avoids GNU-only grep -P parsing', () => {
    assert.ok(!source.includes('grep -oP'), 'sync-scs-to-codex.sh should remain portable across BSD and GNU environments');
  })) passed++; else failed++;
