export const meta = {
  name: 'orch-review',
  description:
    'SCS Review phase as a native Claude Code workflow: multi-dimension review (quality + language + conditional security) then adversarial verification of every CRITICAL/HIGH finding. Returns blocking + advisory findings for Gate 2.',
  phases: [
...
// Language → SCS reviewer agent. Mirrors the agents present in agents/.
const LANGUAGE_REVIEWER = {
  typescript: 'scs:typescript-reviewer',
  javascript: 'scs:typescript-reviewer',
  python: 'scs:python-reviewer',
  go: 'scs:go-reviewer',
  rust: 'scs:rust-reviewer',
  java: 'scs:java-reviewer',
  kotlin: 'scs:kotlin-reviewer',
  swift: 'scs:swift-reviewer',
  php: 'scs:php-reviewer',
  csharp: 'scs:csharp-reviewer',
  fsharp: 'scs:fsharp-reviewer',
  react: 'scs:react-reviewer',
  vue: 'scs:vue-reviewer',
  flutter: 'scs:flutter-reviewer',
  dart: 'scs:flutter-reviewer',
  django: 'scs:django-reviewer',
  fastapi: 'scs:fastapi-reviewer',
  cpp: 'scs:cpp-reviewer'
};
...
const dimensions = [
  { key: 'quality', label: 'correctness & quality', agentType: 'scs:code-reviewer' },
  ...(langReviewer ? [{ key: `lang:${input.language}`, label: `${input.language} idioms & pitfalls`, agentType: langReviewer }] : []),
  ...(securityNeeded ? [{ key: 'security', label: 'security (OWASP, secrets, injection)', agentType: 'scs:security-reviewer' }] : [])
];
if (securityNeeded) {
